/* Karatsuba multiplication mod M*/

inline void force_inline mul512_8_2078K_modM(__m512i out[80], __m512i a512[40], __m512i b512[40])
{

	// D0 = Al*Bl

	__m512i D0_0, D0_1;
	__m512i D0_2, D0_3;
	__m512i D0_4, D0_5;
	__m512i D0_6, D0_7;
	__m512i D0_8, D0_9;
	__m512i D0_10, D0_11;
	__m512i D0_12, D0_13;
	__m512i D0_14, D0_15;
	__m512i D0_16, D0_17;
	__m512i D0_18, D0_19;
	__m512i D0_20, D0_21;
	__m512i D0_22, D0_23;
	__m512i D0_24, D0_25;
	__m512i D0_26, D0_27;
	__m512i D0_28, D0_29;
	__m512i D0_30, D0_31;
	__m512i D0_32, D0_33;
	__m512i D0_34, D0_35;
	__m512i D0_36, D0_37;
	__m512i D0_38, D0_39;
	__m512i a5120 = _mm512_load_epi64(a512), b5120 = _mm512_load_epi64(b512);
	__m512i a5121 = _mm512_load_epi64(a512+1), b5121 = _mm512_load_epi64(b512+1);
	__m512i a5122 = _mm512_load_epi64(a512+2), b5122 = _mm512_load_epi64(b512+2);
	__m512i a5123 = _mm512_load_epi64(a512+3), b5123 = _mm512_load_epi64(b512+3);
	__m512i a5124 = _mm512_load_epi64(a512+4), b5124 = _mm512_load_epi64(b512+4);
	__m512i a5125 = _mm512_load_epi64(a512+5), b5125 = _mm512_load_epi64(b512+5);
	__m512i a5126 = _mm512_load_epi64(a512+6), b5126 = _mm512_load_epi64(b512+6);
	__m512i a5127 = _mm512_load_epi64(a512+7), b5127 = _mm512_load_epi64(b512+7);
	__m512i a5128 = _mm512_load_epi64(a512+8), b5128 = _mm512_load_epi64(b512+8);
	__m512i a5129 = _mm512_load_epi64(a512+9), b5129 = _mm512_load_epi64(b512+9);
	__m512i a51210 = _mm512_load_epi64(a512+10), b51210 = _mm512_load_epi64(b512+10);
	__m512i a51211 = _mm512_load_epi64(a512+11), b51211 = _mm512_load_epi64(b512+11);
	__m512i a51212 = _mm512_load_epi64(a512+12), b51212 = _mm512_load_epi64(b512+12);
	__m512i a51213 = _mm512_load_epi64(a512+13), b51213 = _mm512_load_epi64(b512+13);
	__m512i a51214 = _mm512_load_epi64(a512+14), b51214 = _mm512_load_epi64(b512+14);
	__m512i a51215 = _mm512_load_epi64(a512+15), b51215 = _mm512_load_epi64(b512+15);
	__m512i a51216 = _mm512_load_epi64(a512+16), b51216 = _mm512_load_epi64(b512+16);
	__m512i a51217 = _mm512_load_epi64(a512+17), b51217 = _mm512_load_epi64(b512+17);
	__m512i a51218 = _mm512_load_epi64(a512+18), b51218 = _mm512_load_epi64(b512+18);
	__m512i a51219 = _mm512_load_epi64(a512+19), b51219 = _mm512_load_epi64(b512+19);
	D0_0 = _mm512_madd52lo_epu64(zero,a5120,b5120);
	D0_1 = _mm512_madd52hi_epu64(zero,a5120,b5120);
	D0_1 = _mm512_madd52lo_epu64(D0_1,a5120,b5121);
	D0_2 = _mm512_madd52hi_epu64(zero,a5120,b5121);
	D0_2 = _mm512_madd52lo_epu64(D0_2,a5120,b5122);
	D0_3 = _mm512_madd52hi_epu64(zero,a5120,b5122);
	D0_3 = _mm512_madd52lo_epu64(D0_3,a5120,b5123);
	D0_4 = _mm512_madd52hi_epu64(zero,a5120,b5123);
	D0_4 = _mm512_madd52lo_epu64(D0_4,a5120,b5124);
	D0_5 = _mm512_madd52hi_epu64(zero,a5120,b5124);
	D0_5 = _mm512_madd52lo_epu64(D0_5,a5120,b5125);
	D0_6 = _mm512_madd52hi_epu64(zero,a5120,b5125);
	D0_6 = _mm512_madd52lo_epu64(D0_6,a5120,b5126);
	D0_7 = _mm512_madd52hi_epu64(zero,a5120,b5126);
	D0_7 = _mm512_madd52lo_epu64(D0_7,a5120,b5127);
	D0_8 = _mm512_madd52hi_epu64(zero,a5120,b5127);
	D0_8 = _mm512_madd52lo_epu64(D0_8,a5120,b5128);
	D0_9 = _mm512_madd52hi_epu64(zero,a5120,b5128);
	D0_9 = _mm512_madd52lo_epu64(D0_9,a5120,b5129);
	D0_10 = _mm512_madd52hi_epu64(zero,a5120,b5129);
	D0_10 = _mm512_madd52lo_epu64(D0_10,a5120,b51210);
	D0_11 = _mm512_madd52hi_epu64(zero,a5120,b51210);
	D0_11 = _mm512_madd52lo_epu64(D0_11,a5120,b51211);
	D0_12 = _mm512_madd52hi_epu64(zero,a5120,b51211);
	D0_12 = _mm512_madd52lo_epu64(D0_12,a5120,b51212);
	D0_13 = _mm512_madd52hi_epu64(zero,a5120,b51212);
	D0_13 = _mm512_madd52lo_epu64(D0_13,a5120,b51213);
	D0_14 = _mm512_madd52hi_epu64(zero,a5120,b51213);
	D0_14 = _mm512_madd52lo_epu64(D0_14,a5120,b51214);
	D0_15 = _mm512_madd52hi_epu64(zero,a5120,b51214);
	D0_15 = _mm512_madd52lo_epu64(D0_15,a5120,b51215);
	D0_16 = _mm512_madd52hi_epu64(zero,a5120,b51215);
	D0_16 = _mm512_madd52lo_epu64(D0_16,a5120,b51216);
	D0_17 = _mm512_madd52hi_epu64(zero,a5120,b51216);
	D0_17 = _mm512_madd52lo_epu64(D0_17,a5120,b51217);
	D0_18 = _mm512_madd52hi_epu64(zero,a5120,b51217);
	D0_18 = _mm512_madd52lo_epu64(D0_18,a5120,b51218);
	D0_19 = _mm512_madd52hi_epu64(zero,a5120,b51218);
	D0_19 = _mm512_madd52lo_epu64(D0_19,a5120,b51219);
	D0_20 = _mm512_madd52hi_epu64(zero,a5120,b51219);

	D0_1 = _mm512_madd52lo_epu64(D0_1,a5121,b5120);
	D0_2 = _mm512_madd52hi_epu64(D0_2,a5121,b5120);
	D0_2 = _mm512_madd52lo_epu64(D0_2,a5121,b5121);
	D0_3 = _mm512_madd52hi_epu64(D0_3,a5121,b5121);
	D0_3 = _mm512_madd52lo_epu64(D0_3,a5121,b5122);
	D0_4 = _mm512_madd52hi_epu64(D0_4,a5121,b5122);
	D0_4 = _mm512_madd52lo_epu64(D0_4,a5121,b5123);
	D0_5 = _mm512_madd52hi_epu64(D0_5,a5121,b5123);
	D0_5 = _mm512_madd52lo_epu64(D0_5,a5121,b5124);
	D0_6 = _mm512_madd52hi_epu64(D0_6,a5121,b5124);
	D0_6 = _mm512_madd52lo_epu64(D0_6,a5121,b5125);
	D0_7 = _mm512_madd52hi_epu64(D0_7,a5121,b5125);
	D0_7 = _mm512_madd52lo_epu64(D0_7,a5121,b5126);
	D0_8 = _mm512_madd52hi_epu64(D0_8,a5121,b5126);
	D0_8 = _mm512_madd52lo_epu64(D0_8,a5121,b5127);
	D0_9 = _mm512_madd52hi_epu64(D0_9,a5121,b5127);
	D0_9 = _mm512_madd52lo_epu64(D0_9,a5121,b5128);
	D0_10 = _mm512_madd52hi_epu64(D0_10,a5121,b5128);
	D0_10 = _mm512_madd52lo_epu64(D0_10,a5121,b5129);
	D0_11 = _mm512_madd52hi_epu64(D0_11,a5121,b5129);
	D0_11 = _mm512_madd52lo_epu64(D0_11,a5121,b51210);
	D0_12 = _mm512_madd52hi_epu64(D0_12,a5121,b51210);
	D0_12 = _mm512_madd52lo_epu64(D0_12,a5121,b51211);
	D0_13 = _mm512_madd52hi_epu64(D0_13,a5121,b51211);
	D0_13 = _mm512_madd52lo_epu64(D0_13,a5121,b51212);
	D0_14 = _mm512_madd52hi_epu64(D0_14,a5121,b51212);
	D0_14 = _mm512_madd52lo_epu64(D0_14,a5121,b51213);
	D0_15 = _mm512_madd52hi_epu64(D0_15,a5121,b51213);
	D0_15 = _mm512_madd52lo_epu64(D0_15,a5121,b51214);
	D0_16 = _mm512_madd52hi_epu64(D0_16,a5121,b51214);
	D0_16 = _mm512_madd52lo_epu64(D0_16,a5121,b51215);
	D0_17 = _mm512_madd52hi_epu64(D0_17,a5121,b51215);
	D0_17 = _mm512_madd52lo_epu64(D0_17,a5121,b51216);
	D0_18 = _mm512_madd52hi_epu64(D0_18,a5121,b51216);
	D0_18 = _mm512_madd52lo_epu64(D0_18,a5121,b51217);
	D0_19 = _mm512_madd52hi_epu64(D0_19,a5121,b51217);
	D0_19 = _mm512_madd52lo_epu64(D0_19,a5121,b51218);
	D0_20 = _mm512_madd52hi_epu64(D0_20,a5121,b51218);
	D0_20 = _mm512_madd52lo_epu64(D0_20,a5121,b51219);
	D0_21 = _mm512_madd52hi_epu64(zero,a5121,b51219);

	D0_2 = _mm512_madd52lo_epu64(D0_2,a5122,b5120);
	D0_3 = _mm512_madd52hi_epu64(D0_3,a5122,b5120);
	D0_3 = _mm512_madd52lo_epu64(D0_3,a5122,b5121);
	D0_4 = _mm512_madd52hi_epu64(D0_4,a5122,b5121);
	D0_4 = _mm512_madd52lo_epu64(D0_4,a5122,b5122);
	D0_5 = _mm512_madd52hi_epu64(D0_5,a5122,b5122);
	D0_5 = _mm512_madd52lo_epu64(D0_5,a5122,b5123);
	D0_6 = _mm512_madd52hi_epu64(D0_6,a5122,b5123);
	D0_6 = _mm512_madd52lo_epu64(D0_6,a5122,b5124);
	D0_7 = _mm512_madd52hi_epu64(D0_7,a5122,b5124);
	D0_7 = _mm512_madd52lo_epu64(D0_7,a5122,b5125);
	D0_8 = _mm512_madd52hi_epu64(D0_8,a5122,b5125);
	D0_8 = _mm512_madd52lo_epu64(D0_8,a5122,b5126);
	D0_9 = _mm512_madd52hi_epu64(D0_9,a5122,b5126);
	D0_9 = _mm512_madd52lo_epu64(D0_9,a5122,b5127);
	D0_10 = _mm512_madd52hi_epu64(D0_10,a5122,b5127);
	D0_10 = _mm512_madd52lo_epu64(D0_10,a5122,b5128);
	D0_11 = _mm512_madd52hi_epu64(D0_11,a5122,b5128);
	D0_11 = _mm512_madd52lo_epu64(D0_11,a5122,b5129);
	D0_12 = _mm512_madd52hi_epu64(D0_12,a5122,b5129);
	D0_12 = _mm512_madd52lo_epu64(D0_12,a5122,b51210);
	D0_13 = _mm512_madd52hi_epu64(D0_13,a5122,b51210);
	D0_13 = _mm512_madd52lo_epu64(D0_13,a5122,b51211);
	D0_14 = _mm512_madd52hi_epu64(D0_14,a5122,b51211);
	D0_14 = _mm512_madd52lo_epu64(D0_14,a5122,b51212);
	D0_15 = _mm512_madd52hi_epu64(D0_15,a5122,b51212);
	D0_15 = _mm512_madd52lo_epu64(D0_15,a5122,b51213);
	D0_16 = _mm512_madd52hi_epu64(D0_16,a5122,b51213);
	D0_16 = _mm512_madd52lo_epu64(D0_16,a5122,b51214);
	D0_17 = _mm512_madd52hi_epu64(D0_17,a5122,b51214);
	D0_17 = _mm512_madd52lo_epu64(D0_17,a5122,b51215);
	D0_18 = _mm512_madd52hi_epu64(D0_18,a5122,b51215);
	D0_18 = _mm512_madd52lo_epu64(D0_18,a5122,b51216);
	D0_19 = _mm512_madd52hi_epu64(D0_19,a5122,b51216);
	D0_19 = _mm512_madd52lo_epu64(D0_19,a5122,b51217);
	D0_20 = _mm512_madd52hi_epu64(D0_20,a5122,b51217);
	D0_20 = _mm512_madd52lo_epu64(D0_20,a5122,b51218);
	D0_21 = _mm512_madd52hi_epu64(D0_21,a5122,b51218);
	D0_21 = _mm512_madd52lo_epu64(D0_21,a5122,b51219);
	D0_22 = _mm512_madd52hi_epu64(zero,a5122,b51219);

	D0_3 = _mm512_madd52lo_epu64(D0_3,a5123,b5120);
	D0_4 = _mm512_madd52hi_epu64(D0_4,a5123,b5120);
	D0_4 = _mm512_madd52lo_epu64(D0_4,a5123,b5121);
	D0_5 = _mm512_madd52hi_epu64(D0_5,a5123,b5121);
	D0_5 = _mm512_madd52lo_epu64(D0_5,a5123,b5122);
	D0_6 = _mm512_madd52hi_epu64(D0_6,a5123,b5122);
	D0_6 = _mm512_madd52lo_epu64(D0_6,a5123,b5123);
	D0_7 = _mm512_madd52hi_epu64(D0_7,a5123,b5123);
	D0_7 = _mm512_madd52lo_epu64(D0_7,a5123,b5124);
	D0_8 = _mm512_madd52hi_epu64(D0_8,a5123,b5124);
	D0_8 = _mm512_madd52lo_epu64(D0_8,a5123,b5125);
	D0_9 = _mm512_madd52hi_epu64(D0_9,a5123,b5125);
	D0_9 = _mm512_madd52lo_epu64(D0_9,a5123,b5126);
	D0_10 = _mm512_madd52hi_epu64(D0_10,a5123,b5126);
	D0_10 = _mm512_madd52lo_epu64(D0_10,a5123,b5127);
	D0_11 = _mm512_madd52hi_epu64(D0_11,a5123,b5127);
	D0_11 = _mm512_madd52lo_epu64(D0_11,a5123,b5128);
	D0_12 = _mm512_madd52hi_epu64(D0_12,a5123,b5128);
	D0_12 = _mm512_madd52lo_epu64(D0_12,a5123,b5129);
	D0_13 = _mm512_madd52hi_epu64(D0_13,a5123,b5129);
	D0_13 = _mm512_madd52lo_epu64(D0_13,a5123,b51210);
	D0_14 = _mm512_madd52hi_epu64(D0_14,a5123,b51210);
	D0_14 = _mm512_madd52lo_epu64(D0_14,a5123,b51211);
	D0_15 = _mm512_madd52hi_epu64(D0_15,a5123,b51211);
	D0_15 = _mm512_madd52lo_epu64(D0_15,a5123,b51212);
	D0_16 = _mm512_madd52hi_epu64(D0_16,a5123,b51212);
	D0_16 = _mm512_madd52lo_epu64(D0_16,a5123,b51213);
	D0_17 = _mm512_madd52hi_epu64(D0_17,a5123,b51213);
	D0_17 = _mm512_madd52lo_epu64(D0_17,a5123,b51214);
	D0_18 = _mm512_madd52hi_epu64(D0_18,a5123,b51214);
	D0_18 = _mm512_madd52lo_epu64(D0_18,a5123,b51215);
	D0_19 = _mm512_madd52hi_epu64(D0_19,a5123,b51215);
	D0_19 = _mm512_madd52lo_epu64(D0_19,a5123,b51216);
	D0_20 = _mm512_madd52hi_epu64(D0_20,a5123,b51216);
	D0_20 = _mm512_madd52lo_epu64(D0_20,a5123,b51217);
	D0_21 = _mm512_madd52hi_epu64(D0_21,a5123,b51217);
	D0_21 = _mm512_madd52lo_epu64(D0_21,a5123,b51218);
	D0_22 = _mm512_madd52hi_epu64(D0_22,a5123,b51218);
	D0_22 = _mm512_madd52lo_epu64(D0_22,a5123,b51219);
	D0_23 = _mm512_madd52hi_epu64(zero,a5123,b51219);

	D0_4 = _mm512_madd52lo_epu64(D0_4,a5124,b5120);
	D0_5 = _mm512_madd52hi_epu64(D0_5,a5124,b5120);
	D0_5 = _mm512_madd52lo_epu64(D0_5,a5124,b5121);
	D0_6 = _mm512_madd52hi_epu64(D0_6,a5124,b5121);
	D0_6 = _mm512_madd52lo_epu64(D0_6,a5124,b5122);
	D0_7 = _mm512_madd52hi_epu64(D0_7,a5124,b5122);
	D0_7 = _mm512_madd52lo_epu64(D0_7,a5124,b5123);
	D0_8 = _mm512_madd52hi_epu64(D0_8,a5124,b5123);
	D0_8 = _mm512_madd52lo_epu64(D0_8,a5124,b5124);
	D0_9 = _mm512_madd52hi_epu64(D0_9,a5124,b5124);
	D0_9 = _mm512_madd52lo_epu64(D0_9,a5124,b5125);
	D0_10 = _mm512_madd52hi_epu64(D0_10,a5124,b5125);
	D0_10 = _mm512_madd52lo_epu64(D0_10,a5124,b5126);
	D0_11 = _mm512_madd52hi_epu64(D0_11,a5124,b5126);
	D0_11 = _mm512_madd52lo_epu64(D0_11,a5124,b5127);
	D0_12 = _mm512_madd52hi_epu64(D0_12,a5124,b5127);
	D0_12 = _mm512_madd52lo_epu64(D0_12,a5124,b5128);
	D0_13 = _mm512_madd52hi_epu64(D0_13,a5124,b5128);
	D0_13 = _mm512_madd52lo_epu64(D0_13,a5124,b5129);
	D0_14 = _mm512_madd52hi_epu64(D0_14,a5124,b5129);
	D0_14 = _mm512_madd52lo_epu64(D0_14,a5124,b51210);
	D0_15 = _mm512_madd52hi_epu64(D0_15,a5124,b51210);
	D0_15 = _mm512_madd52lo_epu64(D0_15,a5124,b51211);
	D0_16 = _mm512_madd52hi_epu64(D0_16,a5124,b51211);
	D0_16 = _mm512_madd52lo_epu64(D0_16,a5124,b51212);
	D0_17 = _mm512_madd52hi_epu64(D0_17,a5124,b51212);
	D0_17 = _mm512_madd52lo_epu64(D0_17,a5124,b51213);
	D0_18 = _mm512_madd52hi_epu64(D0_18,a5124,b51213);
	D0_18 = _mm512_madd52lo_epu64(D0_18,a5124,b51214);
	D0_19 = _mm512_madd52hi_epu64(D0_19,a5124,b51214);
	D0_19 = _mm512_madd52lo_epu64(D0_19,a5124,b51215);
	D0_20 = _mm512_madd52hi_epu64(D0_20,a5124,b51215);
	D0_20 = _mm512_madd52lo_epu64(D0_20,a5124,b51216);
	D0_21 = _mm512_madd52hi_epu64(D0_21,a5124,b51216);
	D0_21 = _mm512_madd52lo_epu64(D0_21,a5124,b51217);
	D0_22 = _mm512_madd52hi_epu64(D0_22,a5124,b51217);
	D0_22 = _mm512_madd52lo_epu64(D0_22,a5124,b51218);
	D0_23 = _mm512_madd52hi_epu64(D0_23,a5124,b51218);
	D0_23 = _mm512_madd52lo_epu64(D0_23,a5124,b51219);
	D0_24 = _mm512_madd52hi_epu64(zero,a5124,b51219);

	D0_5 = _mm512_madd52lo_epu64(D0_5,a5125,b5120);
	D0_6 = _mm512_madd52hi_epu64(D0_6,a5125,b5120);
	D0_6 = _mm512_madd52lo_epu64(D0_6,a5125,b5121);
	D0_7 = _mm512_madd52hi_epu64(D0_7,a5125,b5121);
	D0_7 = _mm512_madd52lo_epu64(D0_7,a5125,b5122);
	D0_8 = _mm512_madd52hi_epu64(D0_8,a5125,b5122);
	D0_8 = _mm512_madd52lo_epu64(D0_8,a5125,b5123);
	D0_9 = _mm512_madd52hi_epu64(D0_9,a5125,b5123);
	D0_9 = _mm512_madd52lo_epu64(D0_9,a5125,b5124);
	D0_10 = _mm512_madd52hi_epu64(D0_10,a5125,b5124);
	D0_10 = _mm512_madd52lo_epu64(D0_10,a5125,b5125);
	D0_11 = _mm512_madd52hi_epu64(D0_11,a5125,b5125);
	D0_11 = _mm512_madd52lo_epu64(D0_11,a5125,b5126);
	D0_12 = _mm512_madd52hi_epu64(D0_12,a5125,b5126);
	D0_12 = _mm512_madd52lo_epu64(D0_12,a5125,b5127);
	D0_13 = _mm512_madd52hi_epu64(D0_13,a5125,b5127);
	D0_13 = _mm512_madd52lo_epu64(D0_13,a5125,b5128);
	D0_14 = _mm512_madd52hi_epu64(D0_14,a5125,b5128);
	D0_14 = _mm512_madd52lo_epu64(D0_14,a5125,b5129);
	D0_15 = _mm512_madd52hi_epu64(D0_15,a5125,b5129);
	D0_15 = _mm512_madd52lo_epu64(D0_15,a5125,b51210);
	D0_16 = _mm512_madd52hi_epu64(D0_16,a5125,b51210);
	D0_16 = _mm512_madd52lo_epu64(D0_16,a5125,b51211);
	D0_17 = _mm512_madd52hi_epu64(D0_17,a5125,b51211);
	D0_17 = _mm512_madd52lo_epu64(D0_17,a5125,b51212);
	D0_18 = _mm512_madd52hi_epu64(D0_18,a5125,b51212);
	D0_18 = _mm512_madd52lo_epu64(D0_18,a5125,b51213);
	D0_19 = _mm512_madd52hi_epu64(D0_19,a5125,b51213);
	D0_19 = _mm512_madd52lo_epu64(D0_19,a5125,b51214);
	D0_20 = _mm512_madd52hi_epu64(D0_20,a5125,b51214);
	D0_20 = _mm512_madd52lo_epu64(D0_20,a5125,b51215);
	D0_21 = _mm512_madd52hi_epu64(D0_21,a5125,b51215);
	D0_21 = _mm512_madd52lo_epu64(D0_21,a5125,b51216);
	D0_22 = _mm512_madd52hi_epu64(D0_22,a5125,b51216);
	D0_22 = _mm512_madd52lo_epu64(D0_22,a5125,b51217);
	D0_23 = _mm512_madd52hi_epu64(D0_23,a5125,b51217);
	D0_23 = _mm512_madd52lo_epu64(D0_23,a5125,b51218);
	D0_24 = _mm512_madd52hi_epu64(D0_24,a5125,b51218);
	D0_24 = _mm512_madd52lo_epu64(D0_24,a5125,b51219);
	D0_25 = _mm512_madd52hi_epu64(zero,a5125,b51219);

	D0_6 = _mm512_madd52lo_epu64(D0_6,a5126,b5120);
	D0_7 = _mm512_madd52hi_epu64(D0_7,a5126,b5120);
	D0_7 = _mm512_madd52lo_epu64(D0_7,a5126,b5121);
	D0_8 = _mm512_madd52hi_epu64(D0_8,a5126,b5121);
	D0_8 = _mm512_madd52lo_epu64(D0_8,a5126,b5122);
	D0_9 = _mm512_madd52hi_epu64(D0_9,a5126,b5122);
	D0_9 = _mm512_madd52lo_epu64(D0_9,a5126,b5123);
	D0_10 = _mm512_madd52hi_epu64(D0_10,a5126,b5123);
	D0_10 = _mm512_madd52lo_epu64(D0_10,a5126,b5124);
	D0_11 = _mm512_madd52hi_epu64(D0_11,a5126,b5124);
	D0_11 = _mm512_madd52lo_epu64(D0_11,a5126,b5125);
	D0_12 = _mm512_madd52hi_epu64(D0_12,a5126,b5125);
	D0_12 = _mm512_madd52lo_epu64(D0_12,a5126,b5126);
	D0_13 = _mm512_madd52hi_epu64(D0_13,a5126,b5126);
	D0_13 = _mm512_madd52lo_epu64(D0_13,a5126,b5127);
	D0_14 = _mm512_madd52hi_epu64(D0_14,a5126,b5127);
	D0_14 = _mm512_madd52lo_epu64(D0_14,a5126,b5128);
	D0_15 = _mm512_madd52hi_epu64(D0_15,a5126,b5128);
	D0_15 = _mm512_madd52lo_epu64(D0_15,a5126,b5129);
	D0_16 = _mm512_madd52hi_epu64(D0_16,a5126,b5129);
	D0_16 = _mm512_madd52lo_epu64(D0_16,a5126,b51210);
	D0_17 = _mm512_madd52hi_epu64(D0_17,a5126,b51210);
	D0_17 = _mm512_madd52lo_epu64(D0_17,a5126,b51211);
	D0_18 = _mm512_madd52hi_epu64(D0_18,a5126,b51211);
	D0_18 = _mm512_madd52lo_epu64(D0_18,a5126,b51212);
	D0_19 = _mm512_madd52hi_epu64(D0_19,a5126,b51212);
	D0_19 = _mm512_madd52lo_epu64(D0_19,a5126,b51213);
	D0_20 = _mm512_madd52hi_epu64(D0_20,a5126,b51213);
	D0_20 = _mm512_madd52lo_epu64(D0_20,a5126,b51214);
	D0_21 = _mm512_madd52hi_epu64(D0_21,a5126,b51214);
	D0_21 = _mm512_madd52lo_epu64(D0_21,a5126,b51215);
	D0_22 = _mm512_madd52hi_epu64(D0_22,a5126,b51215);
	D0_22 = _mm512_madd52lo_epu64(D0_22,a5126,b51216);
	D0_23 = _mm512_madd52hi_epu64(D0_23,a5126,b51216);
	D0_23 = _mm512_madd52lo_epu64(D0_23,a5126,b51217);
	D0_24 = _mm512_madd52hi_epu64(D0_24,a5126,b51217);
	D0_24 = _mm512_madd52lo_epu64(D0_24,a5126,b51218);
	D0_25 = _mm512_madd52hi_epu64(D0_25,a5126,b51218);
	D0_25 = _mm512_madd52lo_epu64(D0_25,a5126,b51219);
	D0_26 = _mm512_madd52hi_epu64(zero,a5126,b51219);

	D0_7 = _mm512_madd52lo_epu64(D0_7,a5127,b5120);
	D0_8 = _mm512_madd52hi_epu64(D0_8,a5127,b5120);
	D0_8 = _mm512_madd52lo_epu64(D0_8,a5127,b5121);
	D0_9 = _mm512_madd52hi_epu64(D0_9,a5127,b5121);
	D0_9 = _mm512_madd52lo_epu64(D0_9,a5127,b5122);
	D0_10 = _mm512_madd52hi_epu64(D0_10,a5127,b5122);
	D0_10 = _mm512_madd52lo_epu64(D0_10,a5127,b5123);
	D0_11 = _mm512_madd52hi_epu64(D0_11,a5127,b5123);
	D0_11 = _mm512_madd52lo_epu64(D0_11,a5127,b5124);
	D0_12 = _mm512_madd52hi_epu64(D0_12,a5127,b5124);
	D0_12 = _mm512_madd52lo_epu64(D0_12,a5127,b5125);
	D0_13 = _mm512_madd52hi_epu64(D0_13,a5127,b5125);
	D0_13 = _mm512_madd52lo_epu64(D0_13,a5127,b5126);
	D0_14 = _mm512_madd52hi_epu64(D0_14,a5127,b5126);
	D0_14 = _mm512_madd52lo_epu64(D0_14,a5127,b5127);
	D0_15 = _mm512_madd52hi_epu64(D0_15,a5127,b5127);
	D0_15 = _mm512_madd52lo_epu64(D0_15,a5127,b5128);
	D0_16 = _mm512_madd52hi_epu64(D0_16,a5127,b5128);
	D0_16 = _mm512_madd52lo_epu64(D0_16,a5127,b5129);
	D0_17 = _mm512_madd52hi_epu64(D0_17,a5127,b5129);
	D0_17 = _mm512_madd52lo_epu64(D0_17,a5127,b51210);
	D0_18 = _mm512_madd52hi_epu64(D0_18,a5127,b51210);
	D0_18 = _mm512_madd52lo_epu64(D0_18,a5127,b51211);
	D0_19 = _mm512_madd52hi_epu64(D0_19,a5127,b51211);
	D0_19 = _mm512_madd52lo_epu64(D0_19,a5127,b51212);
	D0_20 = _mm512_madd52hi_epu64(D0_20,a5127,b51212);
	D0_20 = _mm512_madd52lo_epu64(D0_20,a5127,b51213);
	D0_21 = _mm512_madd52hi_epu64(D0_21,a5127,b51213);
	D0_21 = _mm512_madd52lo_epu64(D0_21,a5127,b51214);
	D0_22 = _mm512_madd52hi_epu64(D0_22,a5127,b51214);
	D0_22 = _mm512_madd52lo_epu64(D0_22,a5127,b51215);
	D0_23 = _mm512_madd52hi_epu64(D0_23,a5127,b51215);
	D0_23 = _mm512_madd52lo_epu64(D0_23,a5127,b51216);
	D0_24 = _mm512_madd52hi_epu64(D0_24,a5127,b51216);
	D0_24 = _mm512_madd52lo_epu64(D0_24,a5127,b51217);
	D0_25 = _mm512_madd52hi_epu64(D0_25,a5127,b51217);
	D0_25 = _mm512_madd52lo_epu64(D0_25,a5127,b51218);
	D0_26 = _mm512_madd52hi_epu64(D0_26,a5127,b51218);
	D0_26 = _mm512_madd52lo_epu64(D0_26,a5127,b51219);
	D0_27 = _mm512_madd52hi_epu64(zero,a5127,b51219);

	D0_8 = _mm512_madd52lo_epu64(D0_8,a5128,b5120);
	D0_9 = _mm512_madd52hi_epu64(D0_9,a5128,b5120);
	D0_9 = _mm512_madd52lo_epu64(D0_9,a5128,b5121);
	D0_10 = _mm512_madd52hi_epu64(D0_10,a5128,b5121);
	D0_10 = _mm512_madd52lo_epu64(D0_10,a5128,b5122);
	D0_11 = _mm512_madd52hi_epu64(D0_11,a5128,b5122);
	D0_11 = _mm512_madd52lo_epu64(D0_11,a5128,b5123);
	D0_12 = _mm512_madd52hi_epu64(D0_12,a5128,b5123);
	D0_12 = _mm512_madd52lo_epu64(D0_12,a5128,b5124);
	D0_13 = _mm512_madd52hi_epu64(D0_13,a5128,b5124);
	D0_13 = _mm512_madd52lo_epu64(D0_13,a5128,b5125);
	D0_14 = _mm512_madd52hi_epu64(D0_14,a5128,b5125);
	D0_14 = _mm512_madd52lo_epu64(D0_14,a5128,b5126);
	D0_15 = _mm512_madd52hi_epu64(D0_15,a5128,b5126);
	D0_15 = _mm512_madd52lo_epu64(D0_15,a5128,b5127);
	D0_16 = _mm512_madd52hi_epu64(D0_16,a5128,b5127);
	D0_16 = _mm512_madd52lo_epu64(D0_16,a5128,b5128);
	D0_17 = _mm512_madd52hi_epu64(D0_17,a5128,b5128);
	D0_17 = _mm512_madd52lo_epu64(D0_17,a5128,b5129);
	D0_18 = _mm512_madd52hi_epu64(D0_18,a5128,b5129);
	D0_18 = _mm512_madd52lo_epu64(D0_18,a5128,b51210);
	D0_19 = _mm512_madd52hi_epu64(D0_19,a5128,b51210);
	D0_19 = _mm512_madd52lo_epu64(D0_19,a5128,b51211);
	D0_20 = _mm512_madd52hi_epu64(D0_20,a5128,b51211);
	D0_20 = _mm512_madd52lo_epu64(D0_20,a5128,b51212);
	D0_21 = _mm512_madd52hi_epu64(D0_21,a5128,b51212);
	D0_21 = _mm512_madd52lo_epu64(D0_21,a5128,b51213);
	D0_22 = _mm512_madd52hi_epu64(D0_22,a5128,b51213);
	D0_22 = _mm512_madd52lo_epu64(D0_22,a5128,b51214);
	D0_23 = _mm512_madd52hi_epu64(D0_23,a5128,b51214);
	D0_23 = _mm512_madd52lo_epu64(D0_23,a5128,b51215);
	D0_24 = _mm512_madd52hi_epu64(D0_24,a5128,b51215);
	D0_24 = _mm512_madd52lo_epu64(D0_24,a5128,b51216);
	D0_25 = _mm512_madd52hi_epu64(D0_25,a5128,b51216);
	D0_25 = _mm512_madd52lo_epu64(D0_25,a5128,b51217);
	D0_26 = _mm512_madd52hi_epu64(D0_26,a5128,b51217);
	D0_26 = _mm512_madd52lo_epu64(D0_26,a5128,b51218);
	D0_27 = _mm512_madd52hi_epu64(D0_27,a5128,b51218);
	D0_27 = _mm512_madd52lo_epu64(D0_27,a5128,b51219);
	D0_28 = _mm512_madd52hi_epu64(zero,a5128,b51219);

	D0_9 = _mm512_madd52lo_epu64(D0_9,a5129,b5120);
	D0_10 = _mm512_madd52hi_epu64(D0_10,a5129,b5120);
	D0_10 = _mm512_madd52lo_epu64(D0_10,a5129,b5121);
	D0_11 = _mm512_madd52hi_epu64(D0_11,a5129,b5121);
	D0_11 = _mm512_madd52lo_epu64(D0_11,a5129,b5122);
	D0_12 = _mm512_madd52hi_epu64(D0_12,a5129,b5122);
	D0_12 = _mm512_madd52lo_epu64(D0_12,a5129,b5123);
	D0_13 = _mm512_madd52hi_epu64(D0_13,a5129,b5123);
	D0_13 = _mm512_madd52lo_epu64(D0_13,a5129,b5124);
	D0_14 = _mm512_madd52hi_epu64(D0_14,a5129,b5124);
	D0_14 = _mm512_madd52lo_epu64(D0_14,a5129,b5125);
	D0_15 = _mm512_madd52hi_epu64(D0_15,a5129,b5125);
	D0_15 = _mm512_madd52lo_epu64(D0_15,a5129,b5126);
	D0_16 = _mm512_madd52hi_epu64(D0_16,a5129,b5126);
	D0_16 = _mm512_madd52lo_epu64(D0_16,a5129,b5127);
	D0_17 = _mm512_madd52hi_epu64(D0_17,a5129,b5127);
	D0_17 = _mm512_madd52lo_epu64(D0_17,a5129,b5128);
	D0_18 = _mm512_madd52hi_epu64(D0_18,a5129,b5128);
	D0_18 = _mm512_madd52lo_epu64(D0_18,a5129,b5129);
	D0_19 = _mm512_madd52hi_epu64(D0_19,a5129,b5129);
	D0_19 = _mm512_madd52lo_epu64(D0_19,a5129,b51210);
	D0_20 = _mm512_madd52hi_epu64(D0_20,a5129,b51210);
	D0_20 = _mm512_madd52lo_epu64(D0_20,a5129,b51211);
	D0_21 = _mm512_madd52hi_epu64(D0_21,a5129,b51211);
	D0_21 = _mm512_madd52lo_epu64(D0_21,a5129,b51212);
	D0_22 = _mm512_madd52hi_epu64(D0_22,a5129,b51212);
	D0_22 = _mm512_madd52lo_epu64(D0_22,a5129,b51213);
	D0_23 = _mm512_madd52hi_epu64(D0_23,a5129,b51213);
	D0_23 = _mm512_madd52lo_epu64(D0_23,a5129,b51214);
	D0_24 = _mm512_madd52hi_epu64(D0_24,a5129,b51214);
	D0_24 = _mm512_madd52lo_epu64(D0_24,a5129,b51215);
	D0_25 = _mm512_madd52hi_epu64(D0_25,a5129,b51215);
	D0_25 = _mm512_madd52lo_epu64(D0_25,a5129,b51216);
	D0_26 = _mm512_madd52hi_epu64(D0_26,a5129,b51216);
	D0_26 = _mm512_madd52lo_epu64(D0_26,a5129,b51217);
	D0_27 = _mm512_madd52hi_epu64(D0_27,a5129,b51217);
	D0_27 = _mm512_madd52lo_epu64(D0_27,a5129,b51218);
	D0_28 = _mm512_madd52hi_epu64(D0_28,a5129,b51218);
	D0_28 = _mm512_madd52lo_epu64(D0_28,a5129,b51219);
	D0_29 = _mm512_madd52hi_epu64(zero,a5129,b51219);

	D0_10 = _mm512_madd52lo_epu64(D0_10,a51210,b5120);
	D0_11 = _mm512_madd52hi_epu64(D0_11,a51210,b5120);
	D0_11 = _mm512_madd52lo_epu64(D0_11,a51210,b5121);
	D0_12 = _mm512_madd52hi_epu64(D0_12,a51210,b5121);
	D0_12 = _mm512_madd52lo_epu64(D0_12,a51210,b5122);
	D0_13 = _mm512_madd52hi_epu64(D0_13,a51210,b5122);
	D0_13 = _mm512_madd52lo_epu64(D0_13,a51210,b5123);
	D0_14 = _mm512_madd52hi_epu64(D0_14,a51210,b5123);
	D0_14 = _mm512_madd52lo_epu64(D0_14,a51210,b5124);
	D0_15 = _mm512_madd52hi_epu64(D0_15,a51210,b5124);
	D0_15 = _mm512_madd52lo_epu64(D0_15,a51210,b5125);
	D0_16 = _mm512_madd52hi_epu64(D0_16,a51210,b5125);
	D0_16 = _mm512_madd52lo_epu64(D0_16,a51210,b5126);
	D0_17 = _mm512_madd52hi_epu64(D0_17,a51210,b5126);
	D0_17 = _mm512_madd52lo_epu64(D0_17,a51210,b5127);
	D0_18 = _mm512_madd52hi_epu64(D0_18,a51210,b5127);
	D0_18 = _mm512_madd52lo_epu64(D0_18,a51210,b5128);
	D0_19 = _mm512_madd52hi_epu64(D0_19,a51210,b5128);
	D0_19 = _mm512_madd52lo_epu64(D0_19,a51210,b5129);
	D0_20 = _mm512_madd52hi_epu64(D0_20,a51210,b5129);
	D0_20 = _mm512_madd52lo_epu64(D0_20,a51210,b51210);
	D0_21 = _mm512_madd52hi_epu64(D0_21,a51210,b51210);
	D0_21 = _mm512_madd52lo_epu64(D0_21,a51210,b51211);
	D0_22 = _mm512_madd52hi_epu64(D0_22,a51210,b51211);
	D0_22 = _mm512_madd52lo_epu64(D0_22,a51210,b51212);
	D0_23 = _mm512_madd52hi_epu64(D0_23,a51210,b51212);
	D0_23 = _mm512_madd52lo_epu64(D0_23,a51210,b51213);
	D0_24 = _mm512_madd52hi_epu64(D0_24,a51210,b51213);
	D0_24 = _mm512_madd52lo_epu64(D0_24,a51210,b51214);
	D0_25 = _mm512_madd52hi_epu64(D0_25,a51210,b51214);
	D0_25 = _mm512_madd52lo_epu64(D0_25,a51210,b51215);
	D0_26 = _mm512_madd52hi_epu64(D0_26,a51210,b51215);
	D0_26 = _mm512_madd52lo_epu64(D0_26,a51210,b51216);
	D0_27 = _mm512_madd52hi_epu64(D0_27,a51210,b51216);
	D0_27 = _mm512_madd52lo_epu64(D0_27,a51210,b51217);
	D0_28 = _mm512_madd52hi_epu64(D0_28,a51210,b51217);
	D0_28 = _mm512_madd52lo_epu64(D0_28,a51210,b51218);
	D0_29 = _mm512_madd52hi_epu64(D0_29,a51210,b51218);
	D0_29 = _mm512_madd52lo_epu64(D0_29,a51210,b51219);
	D0_30 = _mm512_madd52hi_epu64(zero,a51210,b51219);

	D0_11 = _mm512_madd52lo_epu64(D0_11,a51211,b5120);
	D0_12 = _mm512_madd52hi_epu64(D0_12,a51211,b5120);
	D0_12 = _mm512_madd52lo_epu64(D0_12,a51211,b5121);
	D0_13 = _mm512_madd52hi_epu64(D0_13,a51211,b5121);
	D0_13 = _mm512_madd52lo_epu64(D0_13,a51211,b5122);
	D0_14 = _mm512_madd52hi_epu64(D0_14,a51211,b5122);
	D0_14 = _mm512_madd52lo_epu64(D0_14,a51211,b5123);
	D0_15 = _mm512_madd52hi_epu64(D0_15,a51211,b5123);
	D0_15 = _mm512_madd52lo_epu64(D0_15,a51211,b5124);
	D0_16 = _mm512_madd52hi_epu64(D0_16,a51211,b5124);
	D0_16 = _mm512_madd52lo_epu64(D0_16,a51211,b5125);
	D0_17 = _mm512_madd52hi_epu64(D0_17,a51211,b5125);
	D0_17 = _mm512_madd52lo_epu64(D0_17,a51211,b5126);
	D0_18 = _mm512_madd52hi_epu64(D0_18,a51211,b5126);
	D0_18 = _mm512_madd52lo_epu64(D0_18,a51211,b5127);
	D0_19 = _mm512_madd52hi_epu64(D0_19,a51211,b5127);
	D0_19 = _mm512_madd52lo_epu64(D0_19,a51211,b5128);
	D0_20 = _mm512_madd52hi_epu64(D0_20,a51211,b5128);
	D0_20 = _mm512_madd52lo_epu64(D0_20,a51211,b5129);
	D0_21 = _mm512_madd52hi_epu64(D0_21,a51211,b5129);
	D0_21 = _mm512_madd52lo_epu64(D0_21,a51211,b51210);
	D0_22 = _mm512_madd52hi_epu64(D0_22,a51211,b51210);
	D0_22 = _mm512_madd52lo_epu64(D0_22,a51211,b51211);
	D0_23 = _mm512_madd52hi_epu64(D0_23,a51211,b51211);
	D0_23 = _mm512_madd52lo_epu64(D0_23,a51211,b51212);
	D0_24 = _mm512_madd52hi_epu64(D0_24,a51211,b51212);
	D0_24 = _mm512_madd52lo_epu64(D0_24,a51211,b51213);
	D0_25 = _mm512_madd52hi_epu64(D0_25,a51211,b51213);
	D0_25 = _mm512_madd52lo_epu64(D0_25,a51211,b51214);
	D0_26 = _mm512_madd52hi_epu64(D0_26,a51211,b51214);
	D0_26 = _mm512_madd52lo_epu64(D0_26,a51211,b51215);
	D0_27 = _mm512_madd52hi_epu64(D0_27,a51211,b51215);
	D0_27 = _mm512_madd52lo_epu64(D0_27,a51211,b51216);
	D0_28 = _mm512_madd52hi_epu64(D0_28,a51211,b51216);
	D0_28 = _mm512_madd52lo_epu64(D0_28,a51211,b51217);
	D0_29 = _mm512_madd52hi_epu64(D0_29,a51211,b51217);
	D0_29 = _mm512_madd52lo_epu64(D0_29,a51211,b51218);
	D0_30 = _mm512_madd52hi_epu64(D0_30,a51211,b51218);
	D0_30 = _mm512_madd52lo_epu64(D0_30,a51211,b51219);
	D0_31 = _mm512_madd52hi_epu64(zero,a51211,b51219);

	D0_12 = _mm512_madd52lo_epu64(D0_12,a51212,b5120);
	D0_13 = _mm512_madd52hi_epu64(D0_13,a51212,b5120);
	D0_13 = _mm512_madd52lo_epu64(D0_13,a51212,b5121);
	D0_14 = _mm512_madd52hi_epu64(D0_14,a51212,b5121);
	D0_14 = _mm512_madd52lo_epu64(D0_14,a51212,b5122);
	D0_15 = _mm512_madd52hi_epu64(D0_15,a51212,b5122);
	D0_15 = _mm512_madd52lo_epu64(D0_15,a51212,b5123);
	D0_16 = _mm512_madd52hi_epu64(D0_16,a51212,b5123);
	D0_16 = _mm512_madd52lo_epu64(D0_16,a51212,b5124);
	D0_17 = _mm512_madd52hi_epu64(D0_17,a51212,b5124);
	D0_17 = _mm512_madd52lo_epu64(D0_17,a51212,b5125);
	D0_18 = _mm512_madd52hi_epu64(D0_18,a51212,b5125);
	D0_18 = _mm512_madd52lo_epu64(D0_18,a51212,b5126);
	D0_19 = _mm512_madd52hi_epu64(D0_19,a51212,b5126);
	D0_19 = _mm512_madd52lo_epu64(D0_19,a51212,b5127);
	D0_20 = _mm512_madd52hi_epu64(D0_20,a51212,b5127);
	D0_20 = _mm512_madd52lo_epu64(D0_20,a51212,b5128);
	D0_21 = _mm512_madd52hi_epu64(D0_21,a51212,b5128);
	D0_21 = _mm512_madd52lo_epu64(D0_21,a51212,b5129);
	D0_22 = _mm512_madd52hi_epu64(D0_22,a51212,b5129);
	D0_22 = _mm512_madd52lo_epu64(D0_22,a51212,b51210);
	D0_23 = _mm512_madd52hi_epu64(D0_23,a51212,b51210);
	D0_23 = _mm512_madd52lo_epu64(D0_23,a51212,b51211);
	D0_24 = _mm512_madd52hi_epu64(D0_24,a51212,b51211);
	D0_24 = _mm512_madd52lo_epu64(D0_24,a51212,b51212);
	D0_25 = _mm512_madd52hi_epu64(D0_25,a51212,b51212);
	D0_25 = _mm512_madd52lo_epu64(D0_25,a51212,b51213);
	D0_26 = _mm512_madd52hi_epu64(D0_26,a51212,b51213);
	D0_26 = _mm512_madd52lo_epu64(D0_26,a51212,b51214);
	D0_27 = _mm512_madd52hi_epu64(D0_27,a51212,b51214);
	D0_27 = _mm512_madd52lo_epu64(D0_27,a51212,b51215);
	D0_28 = _mm512_madd52hi_epu64(D0_28,a51212,b51215);
	D0_28 = _mm512_madd52lo_epu64(D0_28,a51212,b51216);
	D0_29 = _mm512_madd52hi_epu64(D0_29,a51212,b51216);
	D0_29 = _mm512_madd52lo_epu64(D0_29,a51212,b51217);
	D0_30 = _mm512_madd52hi_epu64(D0_30,a51212,b51217);
	D0_30 = _mm512_madd52lo_epu64(D0_30,a51212,b51218);
	D0_31 = _mm512_madd52hi_epu64(D0_31,a51212,b51218);
	D0_31 = _mm512_madd52lo_epu64(D0_31,a51212,b51219);
	D0_32 = _mm512_madd52hi_epu64(zero,a51212,b51219);

	D0_13 = _mm512_madd52lo_epu64(D0_13,a51213,b5120);
	D0_14 = _mm512_madd52hi_epu64(D0_14,a51213,b5120);
	D0_14 = _mm512_madd52lo_epu64(D0_14,a51213,b5121);
	D0_15 = _mm512_madd52hi_epu64(D0_15,a51213,b5121);
	D0_15 = _mm512_madd52lo_epu64(D0_15,a51213,b5122);
	D0_16 = _mm512_madd52hi_epu64(D0_16,a51213,b5122);
	D0_16 = _mm512_madd52lo_epu64(D0_16,a51213,b5123);
	D0_17 = _mm512_madd52hi_epu64(D0_17,a51213,b5123);
	D0_17 = _mm512_madd52lo_epu64(D0_17,a51213,b5124);
	D0_18 = _mm512_madd52hi_epu64(D0_18,a51213,b5124);
	D0_18 = _mm512_madd52lo_epu64(D0_18,a51213,b5125);
	D0_19 = _mm512_madd52hi_epu64(D0_19,a51213,b5125);
	D0_19 = _mm512_madd52lo_epu64(D0_19,a51213,b5126);
	D0_20 = _mm512_madd52hi_epu64(D0_20,a51213,b5126);
	D0_20 = _mm512_madd52lo_epu64(D0_20,a51213,b5127);
	D0_21 = _mm512_madd52hi_epu64(D0_21,a51213,b5127);
	D0_21 = _mm512_madd52lo_epu64(D0_21,a51213,b5128);
	D0_22 = _mm512_madd52hi_epu64(D0_22,a51213,b5128);
	D0_22 = _mm512_madd52lo_epu64(D0_22,a51213,b5129);
	D0_23 = _mm512_madd52hi_epu64(D0_23,a51213,b5129);
	D0_23 = _mm512_madd52lo_epu64(D0_23,a51213,b51210);
	D0_24 = _mm512_madd52hi_epu64(D0_24,a51213,b51210);
	D0_24 = _mm512_madd52lo_epu64(D0_24,a51213,b51211);
	D0_25 = _mm512_madd52hi_epu64(D0_25,a51213,b51211);
	D0_25 = _mm512_madd52lo_epu64(D0_25,a51213,b51212);
	D0_26 = _mm512_madd52hi_epu64(D0_26,a51213,b51212);
	D0_26 = _mm512_madd52lo_epu64(D0_26,a51213,b51213);
	D0_27 = _mm512_madd52hi_epu64(D0_27,a51213,b51213);
	D0_27 = _mm512_madd52lo_epu64(D0_27,a51213,b51214);
	D0_28 = _mm512_madd52hi_epu64(D0_28,a51213,b51214);
	D0_28 = _mm512_madd52lo_epu64(D0_28,a51213,b51215);
	D0_29 = _mm512_madd52hi_epu64(D0_29,a51213,b51215);
	D0_29 = _mm512_madd52lo_epu64(D0_29,a51213,b51216);
	D0_30 = _mm512_madd52hi_epu64(D0_30,a51213,b51216);
	D0_30 = _mm512_madd52lo_epu64(D0_30,a51213,b51217);
	D0_31 = _mm512_madd52hi_epu64(D0_31,a51213,b51217);
	D0_31 = _mm512_madd52lo_epu64(D0_31,a51213,b51218);
	D0_32 = _mm512_madd52hi_epu64(D0_32,a51213,b51218);
	D0_32 = _mm512_madd52lo_epu64(D0_32,a51213,b51219);
	D0_33 = _mm512_madd52hi_epu64(zero,a51213,b51219);

	D0_14 = _mm512_madd52lo_epu64(D0_14,a51214,b5120);
	D0_15 = _mm512_madd52hi_epu64(D0_15,a51214,b5120);
	D0_15 = _mm512_madd52lo_epu64(D0_15,a51214,b5121);
	D0_16 = _mm512_madd52hi_epu64(D0_16,a51214,b5121);
	D0_16 = _mm512_madd52lo_epu64(D0_16,a51214,b5122);
	D0_17 = _mm512_madd52hi_epu64(D0_17,a51214,b5122);
	D0_17 = _mm512_madd52lo_epu64(D0_17,a51214,b5123);
	D0_18 = _mm512_madd52hi_epu64(D0_18,a51214,b5123);
	D0_18 = _mm512_madd52lo_epu64(D0_18,a51214,b5124);
	D0_19 = _mm512_madd52hi_epu64(D0_19,a51214,b5124);
	D0_19 = _mm512_madd52lo_epu64(D0_19,a51214,b5125);
	D0_20 = _mm512_madd52hi_epu64(D0_20,a51214,b5125);
	D0_20 = _mm512_madd52lo_epu64(D0_20,a51214,b5126);
	D0_21 = _mm512_madd52hi_epu64(D0_21,a51214,b5126);
	D0_21 = _mm512_madd52lo_epu64(D0_21,a51214,b5127);
	D0_22 = _mm512_madd52hi_epu64(D0_22,a51214,b5127);
	D0_22 = _mm512_madd52lo_epu64(D0_22,a51214,b5128);
	D0_23 = _mm512_madd52hi_epu64(D0_23,a51214,b5128);
	D0_23 = _mm512_madd52lo_epu64(D0_23,a51214,b5129);
	D0_24 = _mm512_madd52hi_epu64(D0_24,a51214,b5129);
	D0_24 = _mm512_madd52lo_epu64(D0_24,a51214,b51210);
	D0_25 = _mm512_madd52hi_epu64(D0_25,a51214,b51210);
	D0_25 = _mm512_madd52lo_epu64(D0_25,a51214,b51211);
	D0_26 = _mm512_madd52hi_epu64(D0_26,a51214,b51211);
	D0_26 = _mm512_madd52lo_epu64(D0_26,a51214,b51212);
	D0_27 = _mm512_madd52hi_epu64(D0_27,a51214,b51212);
	D0_27 = _mm512_madd52lo_epu64(D0_27,a51214,b51213);
	D0_28 = _mm512_madd52hi_epu64(D0_28,a51214,b51213);
	D0_28 = _mm512_madd52lo_epu64(D0_28,a51214,b51214);
	D0_29 = _mm512_madd52hi_epu64(D0_29,a51214,b51214);
	D0_29 = _mm512_madd52lo_epu64(D0_29,a51214,b51215);
	D0_30 = _mm512_madd52hi_epu64(D0_30,a51214,b51215);
	D0_30 = _mm512_madd52lo_epu64(D0_30,a51214,b51216);
	D0_31 = _mm512_madd52hi_epu64(D0_31,a51214,b51216);
	D0_31 = _mm512_madd52lo_epu64(D0_31,a51214,b51217);
	D0_32 = _mm512_madd52hi_epu64(D0_32,a51214,b51217);
	D0_32 = _mm512_madd52lo_epu64(D0_32,a51214,b51218);
	D0_33 = _mm512_madd52hi_epu64(D0_33,a51214,b51218);
	D0_33 = _mm512_madd52lo_epu64(D0_33,a51214,b51219);
	D0_34 = _mm512_madd52hi_epu64(zero,a51214,b51219);

	D0_15 = _mm512_madd52lo_epu64(D0_15,a51215,b5120);
	D0_16 = _mm512_madd52hi_epu64(D0_16,a51215,b5120);
	D0_16 = _mm512_madd52lo_epu64(D0_16,a51215,b5121);
	D0_17 = _mm512_madd52hi_epu64(D0_17,a51215,b5121);
	D0_17 = _mm512_madd52lo_epu64(D0_17,a51215,b5122);
	D0_18 = _mm512_madd52hi_epu64(D0_18,a51215,b5122);
	D0_18 = _mm512_madd52lo_epu64(D0_18,a51215,b5123);
	D0_19 = _mm512_madd52hi_epu64(D0_19,a51215,b5123);
	D0_19 = _mm512_madd52lo_epu64(D0_19,a51215,b5124);
	D0_20 = _mm512_madd52hi_epu64(D0_20,a51215,b5124);
	D0_20 = _mm512_madd52lo_epu64(D0_20,a51215,b5125);
	D0_21 = _mm512_madd52hi_epu64(D0_21,a51215,b5125);
	D0_21 = _mm512_madd52lo_epu64(D0_21,a51215,b5126);
	D0_22 = _mm512_madd52hi_epu64(D0_22,a51215,b5126);
	D0_22 = _mm512_madd52lo_epu64(D0_22,a51215,b5127);
	D0_23 = _mm512_madd52hi_epu64(D0_23,a51215,b5127);
	D0_23 = _mm512_madd52lo_epu64(D0_23,a51215,b5128);
	D0_24 = _mm512_madd52hi_epu64(D0_24,a51215,b5128);
	D0_24 = _mm512_madd52lo_epu64(D0_24,a51215,b5129);
	D0_25 = _mm512_madd52hi_epu64(D0_25,a51215,b5129);
	D0_25 = _mm512_madd52lo_epu64(D0_25,a51215,b51210);
	D0_26 = _mm512_madd52hi_epu64(D0_26,a51215,b51210);
	D0_26 = _mm512_madd52lo_epu64(D0_26,a51215,b51211);
	D0_27 = _mm512_madd52hi_epu64(D0_27,a51215,b51211);
	D0_27 = _mm512_madd52lo_epu64(D0_27,a51215,b51212);
	D0_28 = _mm512_madd52hi_epu64(D0_28,a51215,b51212);
	D0_28 = _mm512_madd52lo_epu64(D0_28,a51215,b51213);
	D0_29 = _mm512_madd52hi_epu64(D0_29,a51215,b51213);
	D0_29 = _mm512_madd52lo_epu64(D0_29,a51215,b51214);
	D0_30 = _mm512_madd52hi_epu64(D0_30,a51215,b51214);
	D0_30 = _mm512_madd52lo_epu64(D0_30,a51215,b51215);
	D0_31 = _mm512_madd52hi_epu64(D0_31,a51215,b51215);
	D0_31 = _mm512_madd52lo_epu64(D0_31,a51215,b51216);
	D0_32 = _mm512_madd52hi_epu64(D0_32,a51215,b51216);
	D0_32 = _mm512_madd52lo_epu64(D0_32,a51215,b51217);
	D0_33 = _mm512_madd52hi_epu64(D0_33,a51215,b51217);
	D0_33 = _mm512_madd52lo_epu64(D0_33,a51215,b51218);
	D0_34 = _mm512_madd52hi_epu64(D0_34,a51215,b51218);
	D0_34 = _mm512_madd52lo_epu64(D0_34,a51215,b51219);
	D0_35 = _mm512_madd52hi_epu64(zero,a51215,b51219);

	D0_16 = _mm512_madd52lo_epu64(D0_16,a51216,b5120);
	D0_17 = _mm512_madd52hi_epu64(D0_17,a51216,b5120);
	D0_17 = _mm512_madd52lo_epu64(D0_17,a51216,b5121);
	D0_18 = _mm512_madd52hi_epu64(D0_18,a51216,b5121);
	D0_18 = _mm512_madd52lo_epu64(D0_18,a51216,b5122);
	D0_19 = _mm512_madd52hi_epu64(D0_19,a51216,b5122);
	D0_19 = _mm512_madd52lo_epu64(D0_19,a51216,b5123);
	D0_20 = _mm512_madd52hi_epu64(D0_20,a51216,b5123);
	D0_20 = _mm512_madd52lo_epu64(D0_20,a51216,b5124);
	D0_21 = _mm512_madd52hi_epu64(D0_21,a51216,b5124);
	D0_21 = _mm512_madd52lo_epu64(D0_21,a51216,b5125);
	D0_22 = _mm512_madd52hi_epu64(D0_22,a51216,b5125);
	D0_22 = _mm512_madd52lo_epu64(D0_22,a51216,b5126);
	D0_23 = _mm512_madd52hi_epu64(D0_23,a51216,b5126);
	D0_23 = _mm512_madd52lo_epu64(D0_23,a51216,b5127);
	D0_24 = _mm512_madd52hi_epu64(D0_24,a51216,b5127);
	D0_24 = _mm512_madd52lo_epu64(D0_24,a51216,b5128);
	D0_25 = _mm512_madd52hi_epu64(D0_25,a51216,b5128);
	D0_25 = _mm512_madd52lo_epu64(D0_25,a51216,b5129);
	D0_26 = _mm512_madd52hi_epu64(D0_26,a51216,b5129);
	D0_26 = _mm512_madd52lo_epu64(D0_26,a51216,b51210);
	D0_27 = _mm512_madd52hi_epu64(D0_27,a51216,b51210);
	D0_27 = _mm512_madd52lo_epu64(D0_27,a51216,b51211);
	D0_28 = _mm512_madd52hi_epu64(D0_28,a51216,b51211);
	D0_28 = _mm512_madd52lo_epu64(D0_28,a51216,b51212);
	D0_29 = _mm512_madd52hi_epu64(D0_29,a51216,b51212);
	D0_29 = _mm512_madd52lo_epu64(D0_29,a51216,b51213);
	D0_30 = _mm512_madd52hi_epu64(D0_30,a51216,b51213);
	D0_30 = _mm512_madd52lo_epu64(D0_30,a51216,b51214);
	D0_31 = _mm512_madd52hi_epu64(D0_31,a51216,b51214);
	D0_31 = _mm512_madd52lo_epu64(D0_31,a51216,b51215);
	D0_32 = _mm512_madd52hi_epu64(D0_32,a51216,b51215);
	D0_32 = _mm512_madd52lo_epu64(D0_32,a51216,b51216);
	D0_33 = _mm512_madd52hi_epu64(D0_33,a51216,b51216);
	D0_33 = _mm512_madd52lo_epu64(D0_33,a51216,b51217);
	D0_34 = _mm512_madd52hi_epu64(D0_34,a51216,b51217);
	D0_34 = _mm512_madd52lo_epu64(D0_34,a51216,b51218);
	D0_35 = _mm512_madd52hi_epu64(D0_35,a51216,b51218);
	D0_35 = _mm512_madd52lo_epu64(D0_35,a51216,b51219);
	D0_36 = _mm512_madd52hi_epu64(zero,a51216,b51219);

	D0_17 = _mm512_madd52lo_epu64(D0_17,a51217,b5120);
	D0_18 = _mm512_madd52hi_epu64(D0_18,a51217,b5120);
	D0_18 = _mm512_madd52lo_epu64(D0_18,a51217,b5121);
	D0_19 = _mm512_madd52hi_epu64(D0_19,a51217,b5121);
	D0_19 = _mm512_madd52lo_epu64(D0_19,a51217,b5122);
	D0_20 = _mm512_madd52hi_epu64(D0_20,a51217,b5122);
	D0_20 = _mm512_madd52lo_epu64(D0_20,a51217,b5123);
	D0_21 = _mm512_madd52hi_epu64(D0_21,a51217,b5123);
	D0_21 = _mm512_madd52lo_epu64(D0_21,a51217,b5124);
	D0_22 = _mm512_madd52hi_epu64(D0_22,a51217,b5124);
	D0_22 = _mm512_madd52lo_epu64(D0_22,a51217,b5125);
	D0_23 = _mm512_madd52hi_epu64(D0_23,a51217,b5125);
	D0_23 = _mm512_madd52lo_epu64(D0_23,a51217,b5126);
	D0_24 = _mm512_madd52hi_epu64(D0_24,a51217,b5126);
	D0_24 = _mm512_madd52lo_epu64(D0_24,a51217,b5127);
	D0_25 = _mm512_madd52hi_epu64(D0_25,a51217,b5127);
	D0_25 = _mm512_madd52lo_epu64(D0_25,a51217,b5128);
	D0_26 = _mm512_madd52hi_epu64(D0_26,a51217,b5128);
	D0_26 = _mm512_madd52lo_epu64(D0_26,a51217,b5129);
	D0_27 = _mm512_madd52hi_epu64(D0_27,a51217,b5129);
	D0_27 = _mm512_madd52lo_epu64(D0_27,a51217,b51210);
	D0_28 = _mm512_madd52hi_epu64(D0_28,a51217,b51210);
	D0_28 = _mm512_madd52lo_epu64(D0_28,a51217,b51211);
	D0_29 = _mm512_madd52hi_epu64(D0_29,a51217,b51211);
	D0_29 = _mm512_madd52lo_epu64(D0_29,a51217,b51212);
	D0_30 = _mm512_madd52hi_epu64(D0_30,a51217,b51212);
	D0_30 = _mm512_madd52lo_epu64(D0_30,a51217,b51213);
	D0_31 = _mm512_madd52hi_epu64(D0_31,a51217,b51213);
	D0_31 = _mm512_madd52lo_epu64(D0_31,a51217,b51214);
	D0_32 = _mm512_madd52hi_epu64(D0_32,a51217,b51214);
	D0_32 = _mm512_madd52lo_epu64(D0_32,a51217,b51215);
	D0_33 = _mm512_madd52hi_epu64(D0_33,a51217,b51215);
	D0_33 = _mm512_madd52lo_epu64(D0_33,a51217,b51216);
	D0_34 = _mm512_madd52hi_epu64(D0_34,a51217,b51216);
	D0_34 = _mm512_madd52lo_epu64(D0_34,a51217,b51217);
	D0_35 = _mm512_madd52hi_epu64(D0_35,a51217,b51217);
	D0_35 = _mm512_madd52lo_epu64(D0_35,a51217,b51218);
	D0_36 = _mm512_madd52hi_epu64(D0_36,a51217,b51218);
	D0_36 = _mm512_madd52lo_epu64(D0_36,a51217,b51219);
	D0_37 = _mm512_madd52hi_epu64(zero,a51217,b51219);

	D0_18 = _mm512_madd52lo_epu64(D0_18,a51218,b5120);
	D0_19 = _mm512_madd52hi_epu64(D0_19,a51218,b5120);
	D0_19 = _mm512_madd52lo_epu64(D0_19,a51218,b5121);
	D0_20 = _mm512_madd52hi_epu64(D0_20,a51218,b5121);
	D0_20 = _mm512_madd52lo_epu64(D0_20,a51218,b5122);
	D0_21 = _mm512_madd52hi_epu64(D0_21,a51218,b5122);
	D0_21 = _mm512_madd52lo_epu64(D0_21,a51218,b5123);
	D0_22 = _mm512_madd52hi_epu64(D0_22,a51218,b5123);
	D0_22 = _mm512_madd52lo_epu64(D0_22,a51218,b5124);
	D0_23 = _mm512_madd52hi_epu64(D0_23,a51218,b5124);
	D0_23 = _mm512_madd52lo_epu64(D0_23,a51218,b5125);
	D0_24 = _mm512_madd52hi_epu64(D0_24,a51218,b5125);
	D0_24 = _mm512_madd52lo_epu64(D0_24,a51218,b5126);
	D0_25 = _mm512_madd52hi_epu64(D0_25,a51218,b5126);
	D0_25 = _mm512_madd52lo_epu64(D0_25,a51218,b5127);
	D0_26 = _mm512_madd52hi_epu64(D0_26,a51218,b5127);
	D0_26 = _mm512_madd52lo_epu64(D0_26,a51218,b5128);
	D0_27 = _mm512_madd52hi_epu64(D0_27,a51218,b5128);
	D0_27 = _mm512_madd52lo_epu64(D0_27,a51218,b5129);
	D0_28 = _mm512_madd52hi_epu64(D0_28,a51218,b5129);
	D0_28 = _mm512_madd52lo_epu64(D0_28,a51218,b51210);
	D0_29 = _mm512_madd52hi_epu64(D0_29,a51218,b51210);
	D0_29 = _mm512_madd52lo_epu64(D0_29,a51218,b51211);
	D0_30 = _mm512_madd52hi_epu64(D0_30,a51218,b51211);
	D0_30 = _mm512_madd52lo_epu64(D0_30,a51218,b51212);
	D0_31 = _mm512_madd52hi_epu64(D0_31,a51218,b51212);
	D0_31 = _mm512_madd52lo_epu64(D0_31,a51218,b51213);
	D0_32 = _mm512_madd52hi_epu64(D0_32,a51218,b51213);
	D0_32 = _mm512_madd52lo_epu64(D0_32,a51218,b51214);
	D0_33 = _mm512_madd52hi_epu64(D0_33,a51218,b51214);
	D0_33 = _mm512_madd52lo_epu64(D0_33,a51218,b51215);
	D0_34 = _mm512_madd52hi_epu64(D0_34,a51218,b51215);
	D0_34 = _mm512_madd52lo_epu64(D0_34,a51218,b51216);
	D0_35 = _mm512_madd52hi_epu64(D0_35,a51218,b51216);
	D0_35 = _mm512_madd52lo_epu64(D0_35,a51218,b51217);
	D0_36 = _mm512_madd52hi_epu64(D0_36,a51218,b51217);
	D0_36 = _mm512_madd52lo_epu64(D0_36,a51218,b51218);
	D0_37 = _mm512_madd52hi_epu64(D0_37,a51218,b51218);
	D0_37 = _mm512_madd52lo_epu64(D0_37,a51218,b51219);
	D0_38 = _mm512_madd52hi_epu64(zero,a51218,b51219);

	D0_19 = _mm512_madd52lo_epu64(D0_19,a51219,b5120);
	D0_20 = _mm512_madd52hi_epu64(D0_20,a51219,b5120);
	D0_20 = _mm512_madd52lo_epu64(D0_20,a51219,b5121);
	D0_21 = _mm512_madd52hi_epu64(D0_21,a51219,b5121);
	D0_21 = _mm512_madd52lo_epu64(D0_21,a51219,b5122);
	D0_22 = _mm512_madd52hi_epu64(D0_22,a51219,b5122);
	D0_22 = _mm512_madd52lo_epu64(D0_22,a51219,b5123);
	D0_23 = _mm512_madd52hi_epu64(D0_23,a51219,b5123);
	D0_23 = _mm512_madd52lo_epu64(D0_23,a51219,b5124);
	D0_24 = _mm512_madd52hi_epu64(D0_24,a51219,b5124);
	D0_24 = _mm512_madd52lo_epu64(D0_24,a51219,b5125);
	D0_25 = _mm512_madd52hi_epu64(D0_25,a51219,b5125);
	D0_25 = _mm512_madd52lo_epu64(D0_25,a51219,b5126);
	D0_26 = _mm512_madd52hi_epu64(D0_26,a51219,b5126);
	D0_26 = _mm512_madd52lo_epu64(D0_26,a51219,b5127);
	D0_27 = _mm512_madd52hi_epu64(D0_27,a51219,b5127);
	D0_27 = _mm512_madd52lo_epu64(D0_27,a51219,b5128);
	D0_28 = _mm512_madd52hi_epu64(D0_28,a51219,b5128);
	D0_28 = _mm512_madd52lo_epu64(D0_28,a51219,b5129);
	D0_29 = _mm512_madd52hi_epu64(D0_29,a51219,b5129);
	D0_29 = _mm512_madd52lo_epu64(D0_29,a51219,b51210);
	D0_30 = _mm512_madd52hi_epu64(D0_30,a51219,b51210);
	D0_30 = _mm512_madd52lo_epu64(D0_30,a51219,b51211);
	D0_31 = _mm512_madd52hi_epu64(D0_31,a51219,b51211);
	D0_31 = _mm512_madd52lo_epu64(D0_31,a51219,b51212);
	D0_32 = _mm512_madd52hi_epu64(D0_32,a51219,b51212);
	D0_32 = _mm512_madd52lo_epu64(D0_32,a51219,b51213);
	D0_33 = _mm512_madd52hi_epu64(D0_33,a51219,b51213);
	D0_33 = _mm512_madd52lo_epu64(D0_33,a51219,b51214);
	D0_34 = _mm512_madd52hi_epu64(D0_34,a51219,b51214);
	D0_34 = _mm512_madd52lo_epu64(D0_34,a51219,b51215);
	D0_35 = _mm512_madd52hi_epu64(D0_35,a51219,b51215);
	D0_35 = _mm512_madd52lo_epu64(D0_35,a51219,b51216);
	D0_36 = _mm512_madd52hi_epu64(D0_36,a51219,b51216);
	D0_36 = _mm512_madd52lo_epu64(D0_36,a51219,b51217);
	D0_37 = _mm512_madd52hi_epu64(D0_37,a51219,b51217);
	D0_37 = _mm512_madd52lo_epu64(D0_37,a51219,b51218);
	D0_38 = _mm512_madd52hi_epu64(D0_38,a51219,b51218);
	D0_38 = _mm512_madd52lo_epu64(D0_38,a51219,b51219);
	D0_39 = _mm512_madd52hi_epu64(zero,a51219,b51219);


	// Carry management

	__m512i carry  = _mm512_srli_epi64(D0_1,52);
	D0_2 = _mm512_add_epi64(D0_2,carry);
	D0_1 = _mm512_and_si512(D0_1,mask52);
	carry  = _mm512_srli_epi64(D0_2,52);
	D0_3 = _mm512_add_epi64(D0_3,carry);
	D0_2 = _mm512_and_si512(D0_2,mask52);
	carry  = _mm512_srli_epi64(D0_3,52);
	D0_4 = _mm512_add_epi64(D0_4,carry);
	D0_3 = _mm512_and_si512(D0_3,mask52);
	carry  = _mm512_srli_epi64(D0_4,52);
	D0_5 = _mm512_add_epi64(D0_5,carry);
	D0_4 = _mm512_and_si512(D0_4,mask52);
	carry  = _mm512_srli_epi64(D0_5,52);
	D0_6 = _mm512_add_epi64(D0_6,carry);
	D0_5 = _mm512_and_si512(D0_5,mask52);
	carry  = _mm512_srli_epi64(D0_6,52);
	D0_7 = _mm512_add_epi64(D0_7,carry);
	D0_6 = _mm512_and_si512(D0_6,mask52);
	carry  = _mm512_srli_epi64(D0_7,52);
	D0_8 = _mm512_add_epi64(D0_8,carry);
	D0_7 = _mm512_and_si512(D0_7,mask52);
	carry  = _mm512_srli_epi64(D0_8,52);
	D0_9 = _mm512_add_epi64(D0_9,carry);
	D0_8 = _mm512_and_si512(D0_8,mask52);
	carry  = _mm512_srli_epi64(D0_9,52);
	D0_10 = _mm512_add_epi64(D0_10,carry);
	D0_9 = _mm512_and_si512(D0_9,mask52);
	carry  = _mm512_srli_epi64(D0_10,52);
	D0_11 = _mm512_add_epi64(D0_11,carry);
	D0_10 = _mm512_and_si512(D0_10,mask52);
	carry  = _mm512_srli_epi64(D0_11,52);
	D0_12 = _mm512_add_epi64(D0_12,carry);
	D0_11 = _mm512_and_si512(D0_11,mask52);
	carry  = _mm512_srli_epi64(D0_12,52);
	D0_13 = _mm512_add_epi64(D0_13,carry);
	D0_12 = _mm512_and_si512(D0_12,mask52);
	carry  = _mm512_srli_epi64(D0_13,52);
	D0_14 = _mm512_add_epi64(D0_14,carry);
	D0_13 = _mm512_and_si512(D0_13,mask52);
	carry  = _mm512_srli_epi64(D0_14,52);
	D0_15 = _mm512_add_epi64(D0_15,carry);
	D0_14 = _mm512_and_si512(D0_14,mask52);
	carry  = _mm512_srli_epi64(D0_15,52);
	D0_16 = _mm512_add_epi64(D0_16,carry);
	D0_15 = _mm512_and_si512(D0_15,mask52);
	carry  = _mm512_srli_epi64(D0_16,52);
	D0_17 = _mm512_add_epi64(D0_17,carry);
	D0_16 = _mm512_and_si512(D0_16,mask52);
	carry  = _mm512_srli_epi64(D0_17,52);
	D0_18 = _mm512_add_epi64(D0_18,carry);
	D0_17 = _mm512_and_si512(D0_17,mask52);
	carry  = _mm512_srli_epi64(D0_18,52);
	D0_19 = _mm512_add_epi64(D0_19,carry);
	D0_18 = _mm512_and_si512(D0_18,mask52);
	/*carry  = _mm512_srli_epi64(D0_19,52);
	D0_20 = _mm512_add_epi64(D0_20,carry);
	D0_19 = _mm512_and_si512(D0_19,mask52);
	carry  = _mm512_srli_epi64(D0_20,52);
	D0_21 = _mm512_add_epi64(D0_21,carry);
	D0_20 = _mm512_and_si512(D0_20,mask52);
	carry  = _mm512_srli_epi64(D0_21,52);
	D0_22 = _mm512_add_epi64(D0_22,carry);
	D0_21 = _mm512_and_si512(D0_21,mask52);
	carry  = _mm512_srli_epi64(D0_22,52);
	D0_23 = _mm512_add_epi64(D0_23,carry);
	D0_22 = _mm512_and_si512(D0_22,mask52);
	carry  = _mm512_srli_epi64(D0_23,52);
	D0_24 = _mm512_add_epi64(D0_24,carry);
	D0_23 = _mm512_and_si512(D0_23,mask52);
	carry  = _mm512_srli_epi64(D0_24,52);
	D0_25 = _mm512_add_epi64(D0_25,carry);
	D0_24 = _mm512_and_si512(D0_24,mask52);
	carry  = _mm512_srli_epi64(D0_25,52);
	D0_26 = _mm512_add_epi64(D0_26,carry);
	D0_25 = _mm512_and_si512(D0_25,mask52);
	carry  = _mm512_srli_epi64(D0_26,52);
	D0_27 = _mm512_add_epi64(D0_27,carry);
	D0_26 = _mm512_and_si512(D0_26,mask52);
	carry  = _mm512_srli_epi64(D0_27,52);
	D0_28 = _mm512_add_epi64(D0_28,carry);
	D0_27 = _mm512_and_si512(D0_27,mask52);
	carry  = _mm512_srli_epi64(D0_28,52);
	D0_29 = _mm512_add_epi64(D0_29,carry);
	D0_28 = _mm512_and_si512(D0_28,mask52);
	carry  = _mm512_srli_epi64(D0_29,52);
	D0_30 = _mm512_add_epi64(D0_30,carry);
	D0_29 = _mm512_and_si512(D0_29,mask52);
	carry  = _mm512_srli_epi64(D0_30,52);
	D0_31 = _mm512_add_epi64(D0_31,carry);
	D0_30 = _mm512_and_si512(D0_30,mask52);
	carry  = _mm512_srli_epi64(D0_31,52);
	D0_32 = _mm512_add_epi64(D0_32,carry);
	D0_31 = _mm512_and_si512(D0_31,mask52);
	carry  = _mm512_srli_epi64(D0_32,52);
	D0_33 = _mm512_add_epi64(D0_33,carry);
	D0_32 = _mm512_and_si512(D0_32,mask52);
	carry  = _mm512_srli_epi64(D0_33,52);
	D0_34 = _mm512_add_epi64(D0_34,carry);
	D0_33 = _mm512_and_si512(D0_33,mask52);
	carry  = _mm512_srli_epi64(D0_34,52);
	D0_35 = _mm512_add_epi64(D0_35,carry);
	D0_34 = _mm512_and_si512(D0_34,mask52);
	carry  = _mm512_srli_epi64(D0_35,52);
	D0_36 = _mm512_add_epi64(D0_36,carry);
	D0_35 = _mm512_and_si512(D0_35,mask52);
	carry  = _mm512_srli_epi64(D0_36,52);
	D0_37 = _mm512_add_epi64(D0_37,carry);
	D0_36 = _mm512_and_si512(D0_36,mask52);
	carry  = _mm512_srli_epi64(D0_37,52);
	D0_38 = _mm512_add_epi64(D0_38,carry);
	D0_37 = _mm512_and_si512(D0_37,mask52);
	carry  = _mm512_srli_epi64(D0_38,52);
	D0_39 = _mm512_add_epi64(D0_39,carry);
	D0_38 = _mm512_and_si512(D0_38,mask52);//*/

	// Conversion in base 2^1039

	carry = D0_19>>51;
	D0_20 = (D0_20<<1)+carry;
	D0_19 &= mask51;
	carry = D0_20>>52;
	D0_21 = (D0_21<<1)+carry;
	D0_20 &= mask52;
	carry = D0_21>>52;
	D0_22 = (D0_22<<1)+carry;
	D0_21 &= mask52;
	carry = D0_22>>52;
	D0_23 = (D0_23<<1)+carry;
	D0_22 &= mask52;
	carry = D0_23>>52;
	D0_24 = (D0_24<<1)+carry;
	D0_23 &= mask52;
	carry = D0_24>>52;
	D0_25 = (D0_25<<1)+carry;
	D0_24 &= mask52;
	carry = D0_25>>52;
	D0_26 = (D0_26<<1)+carry;
	D0_25 &= mask52;
	carry = D0_26>>52;
	D0_27 = (D0_27<<1)+carry;
	D0_26 &= mask52;
	carry = D0_27>>52;
	D0_28 = (D0_28<<1)+carry;
	D0_27 &= mask52;
	carry = D0_28>>52;
	D0_29 = (D0_29<<1)+carry;
	D0_28 &= mask52;
	carry = D0_29>>52;
	D0_30 = (D0_30<<1)+carry;
	D0_29 &= mask52;
	carry = D0_30>>52;
	D0_31 = (D0_31<<1)+carry;
	D0_30 &= mask52;
	carry = D0_31>>52;
	D0_32 = (D0_32<<1)+carry;
	D0_31 &= mask52;
	carry = D0_32>>52;
	D0_33 = (D0_33<<1)+carry;
	D0_32 &= mask52;
	carry = D0_33>>52;
	D0_34 = (D0_34<<1)+carry;
	D0_33 &= mask52;
	carry = D0_34>>52;
	D0_35 = (D0_35<<1)+carry;
	D0_34 &= mask52;
	carry = D0_35>>52;
	D0_36 = (D0_36<<1)+carry;
	D0_35 &= mask52;
	carry = D0_36>>52;
	D0_37 = (D0_37<<1)+carry;
	D0_36 &= mask52;
	carry = D0_37>>52;
	D0_38 = (D0_38<<1)+carry;
	D0_37 &= mask52;
	carry = D0_38>>52;
	D0_39 = (D0_39<<1)+carry;
	D0_38 &= mask52;

	// D2 = Ah*Bh

	__m512i D2_0;
	__m512i D2_1;
	__m512i D2_2;
	__m512i D2_3;
	__m512i D2_4;
	__m512i D2_5;
	__m512i D2_6;
	__m512i D2_7;
	__m512i D2_8;
	__m512i D2_9;
	__m512i D2_10;
	__m512i D2_11;
	__m512i D2_12;
	__m512i D2_13;
	__m512i D2_14;
	__m512i D2_15;
	__m512i D2_16;
	__m512i D2_17;
	__m512i D2_18;
	__m512i D2_19;
	__m512i a51220 = _mm512_load_epi64(a512+20), b51220 = _mm512_load_epi64(b512+20);
	__m512i a51221 = _mm512_load_epi64(a512+21), b51221 = _mm512_load_epi64(b512+21);
	__m512i a51222 = _mm512_load_epi64(a512+22), b51222 = _mm512_load_epi64(b512+22);
	__m512i a51223 = _mm512_load_epi64(a512+23), b51223 = _mm512_load_epi64(b512+23);
	__m512i a51224 = _mm512_load_epi64(a512+24), b51224 = _mm512_load_epi64(b512+24);
	__m512i a51225 = _mm512_load_epi64(a512+25), b51225 = _mm512_load_epi64(b512+25);
	__m512i a51226 = _mm512_load_epi64(a512+26), b51226 = _mm512_load_epi64(b512+26);
	__m512i a51227 = _mm512_load_epi64(a512+27), b51227 = _mm512_load_epi64(b512+27);
	__m512i a51228 = _mm512_load_epi64(a512+28), b51228 = _mm512_load_epi64(b512+28);
	__m512i a51229 = _mm512_load_epi64(a512+29), b51229 = _mm512_load_epi64(b512+29);
	__m512i a51230 = _mm512_load_epi64(a512+30), b51230 = _mm512_load_epi64(b512+30);
	__m512i a51231 = _mm512_load_epi64(a512+31), b51231 = _mm512_load_epi64(b512+31);
	__m512i a51232 = _mm512_load_epi64(a512+32), b51232 = _mm512_load_epi64(b512+32);
	__m512i a51233 = _mm512_load_epi64(a512+33), b51233 = _mm512_load_epi64(b512+33);
	__m512i a51234 = _mm512_load_epi64(a512+34), b51234 = _mm512_load_epi64(b512+34);
	__m512i a51235 = _mm512_load_epi64(a512+35), b51235 = _mm512_load_epi64(b512+35);
	__m512i a51236 = _mm512_load_epi64(a512+36), b51236 = _mm512_load_epi64(b512+36);
	__m512i a51237 = _mm512_load_epi64(a512+37), b51237 = _mm512_load_epi64(b512+37);
	__m512i a51238 = _mm512_load_epi64(a512+38), b51238 = _mm512_load_epi64(b512+38);
	__m512i a51239 = _mm512_load_epi64(a512+39), b51239 = _mm512_load_epi64(b512+39);
	D2_0 = _mm512_madd52lo_epu64(zero,a51220,b51220);
	D2_1 = _mm512_madd52hi_epu64(zero,a51220,b51220);
	D2_1 = _mm512_madd52lo_epu64(D2_1,a51220,b51221);
	D2_2 = _mm512_madd52hi_epu64(zero,a51220,b51221);
	D2_2 = _mm512_madd52lo_epu64(D2_2,a51220,b51222);
	D2_3 = _mm512_madd52hi_epu64(zero,a51220,b51222);
	D2_3 = _mm512_madd52lo_epu64(D2_3,a51220,b51223);
	D2_4 = _mm512_madd52hi_epu64(zero,a51220,b51223);
	D2_4 = _mm512_madd52lo_epu64(D2_4,a51220,b51224);
	D2_5 = _mm512_madd52hi_epu64(zero,a51220,b51224);
	D2_5 = _mm512_madd52lo_epu64(D2_5,a51220,b51225);
	D2_6 = _mm512_madd52hi_epu64(zero,a51220,b51225);
	D2_6 = _mm512_madd52lo_epu64(D2_6,a51220,b51226);
	D2_7 = _mm512_madd52hi_epu64(zero,a51220,b51226);
	D2_7 = _mm512_madd52lo_epu64(D2_7,a51220,b51227);
	D2_8 = _mm512_madd52hi_epu64(zero,a51220,b51227);
	D2_8 = _mm512_madd52lo_epu64(D2_8,a51220,b51228);
	D2_9 = _mm512_madd52hi_epu64(zero,a51220,b51228);
	D2_9 = _mm512_madd52lo_epu64(D2_9,a51220,b51229);
	D2_10 = _mm512_madd52hi_epu64(zero,a51220,b51229);
	D2_10 = _mm512_madd52lo_epu64(D2_10,a51220,b51230);
	D2_11 = _mm512_madd52hi_epu64(zero,a51220,b51230);
	D2_11 = _mm512_madd52lo_epu64(D2_11,a51220,b51231);
	D2_12 = _mm512_madd52hi_epu64(zero,a51220,b51231);
	D2_12 = _mm512_madd52lo_epu64(D2_12,a51220,b51232);
	D2_13 = _mm512_madd52hi_epu64(zero,a51220,b51232);
	D2_13 = _mm512_madd52lo_epu64(D2_13,a51220,b51233);
	D2_14 = _mm512_madd52hi_epu64(zero,a51220,b51233);
	D2_14 = _mm512_madd52lo_epu64(D2_14,a51220,b51234);
	D2_15 = _mm512_madd52hi_epu64(zero,a51220,b51234);
	D2_15 = _mm512_madd52lo_epu64(D2_15,a51220,b51235);
	D2_16 = _mm512_madd52hi_epu64(zero,a51220,b51235);
	D2_16 = _mm512_madd52lo_epu64(D2_16,a51220,b51236);
	D2_17 = _mm512_madd52hi_epu64(zero,a51220,b51236);
	D2_17 = _mm512_madd52lo_epu64(D2_17,a51220,b51237);
	D2_18 = _mm512_madd52hi_epu64(zero,a51220,b51237);
	D2_18 = _mm512_madd52lo_epu64(D2_18,a51220,b51238);
	D2_19 = _mm512_madd52hi_epu64(zero,a51220,b51238);
	D2_19 = _mm512_madd52lo_epu64(D2_19,a51220,b51239);

	D2_1 = _mm512_madd52lo_epu64(D2_1,a51221,b51220);
	D2_2 = _mm512_madd52hi_epu64(D2_2,a51221,b51220);
	D2_2 = _mm512_madd52lo_epu64(D2_2,a51221,b51221);
	D2_3 = _mm512_madd52hi_epu64(D2_3,a51221,b51221);
	D2_3 = _mm512_madd52lo_epu64(D2_3,a51221,b51222);
	D2_4 = _mm512_madd52hi_epu64(D2_4,a51221,b51222);
	D2_4 = _mm512_madd52lo_epu64(D2_4,a51221,b51223);
	D2_5 = _mm512_madd52hi_epu64(D2_5,a51221,b51223);
	D2_5 = _mm512_madd52lo_epu64(D2_5,a51221,b51224);
	D2_6 = _mm512_madd52hi_epu64(D2_6,a51221,b51224);
	D2_6 = _mm512_madd52lo_epu64(D2_6,a51221,b51225);
	D2_7 = _mm512_madd52hi_epu64(D2_7,a51221,b51225);
	D2_7 = _mm512_madd52lo_epu64(D2_7,a51221,b51226);
	D2_8 = _mm512_madd52hi_epu64(D2_8,a51221,b51226);
	D2_8 = _mm512_madd52lo_epu64(D2_8,a51221,b51227);
	D2_9 = _mm512_madd52hi_epu64(D2_9,a51221,b51227);
	D2_9 = _mm512_madd52lo_epu64(D2_9,a51221,b51228);
	D2_10 = _mm512_madd52hi_epu64(D2_10,a51221,b51228);
	D2_10 = _mm512_madd52lo_epu64(D2_10,a51221,b51229);
	D2_11 = _mm512_madd52hi_epu64(D2_11,a51221,b51229);
	D2_11 = _mm512_madd52lo_epu64(D2_11,a51221,b51230);
	D2_12 = _mm512_madd52hi_epu64(D2_12,a51221,b51230);
	D2_12 = _mm512_madd52lo_epu64(D2_12,a51221,b51231);
	D2_13 = _mm512_madd52hi_epu64(D2_13,a51221,b51231);
	D2_13 = _mm512_madd52lo_epu64(D2_13,a51221,b51232);
	D2_14 = _mm512_madd52hi_epu64(D2_14,a51221,b51232);
	D2_14 = _mm512_madd52lo_epu64(D2_14,a51221,b51233);
	D2_15 = _mm512_madd52hi_epu64(D2_15,a51221,b51233);
	D2_15 = _mm512_madd52lo_epu64(D2_15,a51221,b51234);
	D2_16 = _mm512_madd52hi_epu64(D2_16,a51221,b51234);
	D2_16 = _mm512_madd52lo_epu64(D2_16,a51221,b51235);
	D2_17 = _mm512_madd52hi_epu64(D2_17,a51221,b51235);
	D2_17 = _mm512_madd52lo_epu64(D2_17,a51221,b51236);
	D2_18 = _mm512_madd52hi_epu64(D2_18,a51221,b51236);
	D2_18 = _mm512_madd52lo_epu64(D2_18,a51221,b51237);
	D2_19 = _mm512_madd52hi_epu64(D2_19,a51221,b51237);
	D2_19 = _mm512_madd52lo_epu64(D2_19,a51221,b51238);

	D2_2 = _mm512_madd52lo_epu64(D2_2,a51222,b51220);
	D2_3 = _mm512_madd52hi_epu64(D2_3,a51222,b51220);
	D2_3 = _mm512_madd52lo_epu64(D2_3,a51222,b51221);
	D2_4 = _mm512_madd52hi_epu64(D2_4,a51222,b51221);
	D2_4 = _mm512_madd52lo_epu64(D2_4,a51222,b51222);
	D2_5 = _mm512_madd52hi_epu64(D2_5,a51222,b51222);
	D2_5 = _mm512_madd52lo_epu64(D2_5,a51222,b51223);
	D2_6 = _mm512_madd52hi_epu64(D2_6,a51222,b51223);
	D2_6 = _mm512_madd52lo_epu64(D2_6,a51222,b51224);
	D2_7 = _mm512_madd52hi_epu64(D2_7,a51222,b51224);
	D2_7 = _mm512_madd52lo_epu64(D2_7,a51222,b51225);
	D2_8 = _mm512_madd52hi_epu64(D2_8,a51222,b51225);
	D2_8 = _mm512_madd52lo_epu64(D2_8,a51222,b51226);
	D2_9 = _mm512_madd52hi_epu64(D2_9,a51222,b51226);
	D2_9 = _mm512_madd52lo_epu64(D2_9,a51222,b51227);
	D2_10 = _mm512_madd52hi_epu64(D2_10,a51222,b51227);
	D2_10 = _mm512_madd52lo_epu64(D2_10,a51222,b51228);
	D2_11 = _mm512_madd52hi_epu64(D2_11,a51222,b51228);
	D2_11 = _mm512_madd52lo_epu64(D2_11,a51222,b51229);
	D2_12 = _mm512_madd52hi_epu64(D2_12,a51222,b51229);
	D2_12 = _mm512_madd52lo_epu64(D2_12,a51222,b51230);
	D2_13 = _mm512_madd52hi_epu64(D2_13,a51222,b51230);
	D2_13 = _mm512_madd52lo_epu64(D2_13,a51222,b51231);
	D2_14 = _mm512_madd52hi_epu64(D2_14,a51222,b51231);
	D2_14 = _mm512_madd52lo_epu64(D2_14,a51222,b51232);
	D2_15 = _mm512_madd52hi_epu64(D2_15,a51222,b51232);
	D2_15 = _mm512_madd52lo_epu64(D2_15,a51222,b51233);
	D2_16 = _mm512_madd52hi_epu64(D2_16,a51222,b51233);
	D2_16 = _mm512_madd52lo_epu64(D2_16,a51222,b51234);
	D2_17 = _mm512_madd52hi_epu64(D2_17,a51222,b51234);
	D2_17 = _mm512_madd52lo_epu64(D2_17,a51222,b51235);
	D2_18 = _mm512_madd52hi_epu64(D2_18,a51222,b51235);
	D2_18 = _mm512_madd52lo_epu64(D2_18,a51222,b51236);
	D2_19 = _mm512_madd52hi_epu64(D2_19,a51222,b51236);
	D2_19 = _mm512_madd52lo_epu64(D2_19,a51222,b51237);

	D2_3 = _mm512_madd52lo_epu64(D2_3,a51223,b51220);
	D2_4 = _mm512_madd52hi_epu64(D2_4,a51223,b51220);
	D2_4 = _mm512_madd52lo_epu64(D2_4,a51223,b51221);
	D2_5 = _mm512_madd52hi_epu64(D2_5,a51223,b51221);
	D2_5 = _mm512_madd52lo_epu64(D2_5,a51223,b51222);
	D2_6 = _mm512_madd52hi_epu64(D2_6,a51223,b51222);
	D2_6 = _mm512_madd52lo_epu64(D2_6,a51223,b51223);
	D2_7 = _mm512_madd52hi_epu64(D2_7,a51223,b51223);
	D2_7 = _mm512_madd52lo_epu64(D2_7,a51223,b51224);
	D2_8 = _mm512_madd52hi_epu64(D2_8,a51223,b51224);
	D2_8 = _mm512_madd52lo_epu64(D2_8,a51223,b51225);
	D2_9 = _mm512_madd52hi_epu64(D2_9,a51223,b51225);
	D2_9 = _mm512_madd52lo_epu64(D2_9,a51223,b51226);
	D2_10 = _mm512_madd52hi_epu64(D2_10,a51223,b51226);
	D2_10 = _mm512_madd52lo_epu64(D2_10,a51223,b51227);
	D2_11 = _mm512_madd52hi_epu64(D2_11,a51223,b51227);
	D2_11 = _mm512_madd52lo_epu64(D2_11,a51223,b51228);
	D2_12 = _mm512_madd52hi_epu64(D2_12,a51223,b51228);
	D2_12 = _mm512_madd52lo_epu64(D2_12,a51223,b51229);
	D2_13 = _mm512_madd52hi_epu64(D2_13,a51223,b51229);
	D2_13 = _mm512_madd52lo_epu64(D2_13,a51223,b51230);
	D2_14 = _mm512_madd52hi_epu64(D2_14,a51223,b51230);
	D2_14 = _mm512_madd52lo_epu64(D2_14,a51223,b51231);
	D2_15 = _mm512_madd52hi_epu64(D2_15,a51223,b51231);
	D2_15 = _mm512_madd52lo_epu64(D2_15,a51223,b51232);
	D2_16 = _mm512_madd52hi_epu64(D2_16,a51223,b51232);
	D2_16 = _mm512_madd52lo_epu64(D2_16,a51223,b51233);
	D2_17 = _mm512_madd52hi_epu64(D2_17,a51223,b51233);
	D2_17 = _mm512_madd52lo_epu64(D2_17,a51223,b51234);
	D2_18 = _mm512_madd52hi_epu64(D2_18,a51223,b51234);
	D2_18 = _mm512_madd52lo_epu64(D2_18,a51223,b51235);
	D2_19 = _mm512_madd52hi_epu64(D2_19,a51223,b51235);
	D2_19 = _mm512_madd52lo_epu64(D2_19,a51223,b51236);

	D2_4 = _mm512_madd52lo_epu64(D2_4,a51224,b51220);
	D2_5 = _mm512_madd52hi_epu64(D2_5,a51224,b51220);
	D2_5 = _mm512_madd52lo_epu64(D2_5,a51224,b51221);
	D2_6 = _mm512_madd52hi_epu64(D2_6,a51224,b51221);
	D2_6 = _mm512_madd52lo_epu64(D2_6,a51224,b51222);
	D2_7 = _mm512_madd52hi_epu64(D2_7,a51224,b51222);
	D2_7 = _mm512_madd52lo_epu64(D2_7,a51224,b51223);
	D2_8 = _mm512_madd52hi_epu64(D2_8,a51224,b51223);
	D2_8 = _mm512_madd52lo_epu64(D2_8,a51224,b51224);
	D2_9 = _mm512_madd52hi_epu64(D2_9,a51224,b51224);
	D2_9 = _mm512_madd52lo_epu64(D2_9,a51224,b51225);
	D2_10 = _mm512_madd52hi_epu64(D2_10,a51224,b51225);
	D2_10 = _mm512_madd52lo_epu64(D2_10,a51224,b51226);
	D2_11 = _mm512_madd52hi_epu64(D2_11,a51224,b51226);
	D2_11 = _mm512_madd52lo_epu64(D2_11,a51224,b51227);
	D2_12 = _mm512_madd52hi_epu64(D2_12,a51224,b51227);
	D2_12 = _mm512_madd52lo_epu64(D2_12,a51224,b51228);
	D2_13 = _mm512_madd52hi_epu64(D2_13,a51224,b51228);
	D2_13 = _mm512_madd52lo_epu64(D2_13,a51224,b51229);
	D2_14 = _mm512_madd52hi_epu64(D2_14,a51224,b51229);
	D2_14 = _mm512_madd52lo_epu64(D2_14,a51224,b51230);
	D2_15 = _mm512_madd52hi_epu64(D2_15,a51224,b51230);
	D2_15 = _mm512_madd52lo_epu64(D2_15,a51224,b51231);
	D2_16 = _mm512_madd52hi_epu64(D2_16,a51224,b51231);
	D2_16 = _mm512_madd52lo_epu64(D2_16,a51224,b51232);
	D2_17 = _mm512_madd52hi_epu64(D2_17,a51224,b51232);
	D2_17 = _mm512_madd52lo_epu64(D2_17,a51224,b51233);
	D2_18 = _mm512_madd52hi_epu64(D2_18,a51224,b51233);
	D2_18 = _mm512_madd52lo_epu64(D2_18,a51224,b51234);
	D2_19 = _mm512_madd52hi_epu64(D2_19,a51224,b51234);
	D2_19 = _mm512_madd52lo_epu64(D2_19,a51224,b51235);

	D2_5 = _mm512_madd52lo_epu64(D2_5,a51225,b51220);
	D2_6 = _mm512_madd52hi_epu64(D2_6,a51225,b51220);
	D2_6 = _mm512_madd52lo_epu64(D2_6,a51225,b51221);
	D2_7 = _mm512_madd52hi_epu64(D2_7,a51225,b51221);
	D2_7 = _mm512_madd52lo_epu64(D2_7,a51225,b51222);
	D2_8 = _mm512_madd52hi_epu64(D2_8,a51225,b51222);
	D2_8 = _mm512_madd52lo_epu64(D2_8,a51225,b51223);
	D2_9 = _mm512_madd52hi_epu64(D2_9,a51225,b51223);
	D2_9 = _mm512_madd52lo_epu64(D2_9,a51225,b51224);
	D2_10 = _mm512_madd52hi_epu64(D2_10,a51225,b51224);
	D2_10 = _mm512_madd52lo_epu64(D2_10,a51225,b51225);
	D2_11 = _mm512_madd52hi_epu64(D2_11,a51225,b51225);
	D2_11 = _mm512_madd52lo_epu64(D2_11,a51225,b51226);
	D2_12 = _mm512_madd52hi_epu64(D2_12,a51225,b51226);
	D2_12 = _mm512_madd52lo_epu64(D2_12,a51225,b51227);
	D2_13 = _mm512_madd52hi_epu64(D2_13,a51225,b51227);
	D2_13 = _mm512_madd52lo_epu64(D2_13,a51225,b51228);
	D2_14 = _mm512_madd52hi_epu64(D2_14,a51225,b51228);
	D2_14 = _mm512_madd52lo_epu64(D2_14,a51225,b51229);
	D2_15 = _mm512_madd52hi_epu64(D2_15,a51225,b51229);
	D2_15 = _mm512_madd52lo_epu64(D2_15,a51225,b51230);
	D2_16 = _mm512_madd52hi_epu64(D2_16,a51225,b51230);
	D2_16 = _mm512_madd52lo_epu64(D2_16,a51225,b51231);
	D2_17 = _mm512_madd52hi_epu64(D2_17,a51225,b51231);
	D2_17 = _mm512_madd52lo_epu64(D2_17,a51225,b51232);
	D2_18 = _mm512_madd52hi_epu64(D2_18,a51225,b51232);
	D2_18 = _mm512_madd52lo_epu64(D2_18,a51225,b51233);
	D2_19 = _mm512_madd52hi_epu64(D2_19,a51225,b51233);
	D2_19 = _mm512_madd52lo_epu64(D2_19,a51225,b51234);

	D2_6 = _mm512_madd52lo_epu64(D2_6,a51226,b51220);
	D2_7 = _mm512_madd52hi_epu64(D2_7,a51226,b51220);
	D2_7 = _mm512_madd52lo_epu64(D2_7,a51226,b51221);
	D2_8 = _mm512_madd52hi_epu64(D2_8,a51226,b51221);
	D2_8 = _mm512_madd52lo_epu64(D2_8,a51226,b51222);
	D2_9 = _mm512_madd52hi_epu64(D2_9,a51226,b51222);
	D2_9 = _mm512_madd52lo_epu64(D2_9,a51226,b51223);
	D2_10 = _mm512_madd52hi_epu64(D2_10,a51226,b51223);
	D2_10 = _mm512_madd52lo_epu64(D2_10,a51226,b51224);
	D2_11 = _mm512_madd52hi_epu64(D2_11,a51226,b51224);
	D2_11 = _mm512_madd52lo_epu64(D2_11,a51226,b51225);
	D2_12 = _mm512_madd52hi_epu64(D2_12,a51226,b51225);
	D2_12 = _mm512_madd52lo_epu64(D2_12,a51226,b51226);
	D2_13 = _mm512_madd52hi_epu64(D2_13,a51226,b51226);
	D2_13 = _mm512_madd52lo_epu64(D2_13,a51226,b51227);
	D2_14 = _mm512_madd52hi_epu64(D2_14,a51226,b51227);
	D2_14 = _mm512_madd52lo_epu64(D2_14,a51226,b51228);
	D2_15 = _mm512_madd52hi_epu64(D2_15,a51226,b51228);
	D2_15 = _mm512_madd52lo_epu64(D2_15,a51226,b51229);
	D2_16 = _mm512_madd52hi_epu64(D2_16,a51226,b51229);
	D2_16 = _mm512_madd52lo_epu64(D2_16,a51226,b51230);
	D2_17 = _mm512_madd52hi_epu64(D2_17,a51226,b51230);
	D2_17 = _mm512_madd52lo_epu64(D2_17,a51226,b51231);
	D2_18 = _mm512_madd52hi_epu64(D2_18,a51226,b51231);
	D2_18 = _mm512_madd52lo_epu64(D2_18,a51226,b51232);
	D2_19 = _mm512_madd52hi_epu64(D2_19,a51226,b51232);
	D2_19 = _mm512_madd52lo_epu64(D2_19,a51226,b51233);

	D2_7 = _mm512_madd52lo_epu64(D2_7,a51227,b51220);
	D2_8 = _mm512_madd52hi_epu64(D2_8,a51227,b51220);
	D2_8 = _mm512_madd52lo_epu64(D2_8,a51227,b51221);
	D2_9 = _mm512_madd52hi_epu64(D2_9,a51227,b51221);
	D2_9 = _mm512_madd52lo_epu64(D2_9,a51227,b51222);
	D2_10 = _mm512_madd52hi_epu64(D2_10,a51227,b51222);
	D2_10 = _mm512_madd52lo_epu64(D2_10,a51227,b51223);
	D2_11 = _mm512_madd52hi_epu64(D2_11,a51227,b51223);
	D2_11 = _mm512_madd52lo_epu64(D2_11,a51227,b51224);
	D2_12 = _mm512_madd52hi_epu64(D2_12,a51227,b51224);
	D2_12 = _mm512_madd52lo_epu64(D2_12,a51227,b51225);
	D2_13 = _mm512_madd52hi_epu64(D2_13,a51227,b51225);
	D2_13 = _mm512_madd52lo_epu64(D2_13,a51227,b51226);
	D2_14 = _mm512_madd52hi_epu64(D2_14,a51227,b51226);
	D2_14 = _mm512_madd52lo_epu64(D2_14,a51227,b51227);
	D2_15 = _mm512_madd52hi_epu64(D2_15,a51227,b51227);
	D2_15 = _mm512_madd52lo_epu64(D2_15,a51227,b51228);
	D2_16 = _mm512_madd52hi_epu64(D2_16,a51227,b51228);
	D2_16 = _mm512_madd52lo_epu64(D2_16,a51227,b51229);
	D2_17 = _mm512_madd52hi_epu64(D2_17,a51227,b51229);
	D2_17 = _mm512_madd52lo_epu64(D2_17,a51227,b51230);
	D2_18 = _mm512_madd52hi_epu64(D2_18,a51227,b51230);
	D2_18 = _mm512_madd52lo_epu64(D2_18,a51227,b51231);
	D2_19 = _mm512_madd52hi_epu64(D2_19,a51227,b51231);
	D2_19 = _mm512_madd52lo_epu64(D2_19,a51227,b51232);

	D2_8 = _mm512_madd52lo_epu64(D2_8,a51228,b51220);
	D2_9 = _mm512_madd52hi_epu64(D2_9,a51228,b51220);
	D2_9 = _mm512_madd52lo_epu64(D2_9,a51228,b51221);
	D2_10 = _mm512_madd52hi_epu64(D2_10,a51228,b51221);
	D2_10 = _mm512_madd52lo_epu64(D2_10,a51228,b51222);
	D2_11 = _mm512_madd52hi_epu64(D2_11,a51228,b51222);
	D2_11 = _mm512_madd52lo_epu64(D2_11,a51228,b51223);
	D2_12 = _mm512_madd52hi_epu64(D2_12,a51228,b51223);
	D2_12 = _mm512_madd52lo_epu64(D2_12,a51228,b51224);
	D2_13 = _mm512_madd52hi_epu64(D2_13,a51228,b51224);
	D2_13 = _mm512_madd52lo_epu64(D2_13,a51228,b51225);
	D2_14 = _mm512_madd52hi_epu64(D2_14,a51228,b51225);
	D2_14 = _mm512_madd52lo_epu64(D2_14,a51228,b51226);
	D2_15 = _mm512_madd52hi_epu64(D2_15,a51228,b51226);
	D2_15 = _mm512_madd52lo_epu64(D2_15,a51228,b51227);
	D2_16 = _mm512_madd52hi_epu64(D2_16,a51228,b51227);
	D2_16 = _mm512_madd52lo_epu64(D2_16,a51228,b51228);
	D2_17 = _mm512_madd52hi_epu64(D2_17,a51228,b51228);
	D2_17 = _mm512_madd52lo_epu64(D2_17,a51228,b51229);
	D2_18 = _mm512_madd52hi_epu64(D2_18,a51228,b51229);
	D2_18 = _mm512_madd52lo_epu64(D2_18,a51228,b51230);
	D2_19 = _mm512_madd52hi_epu64(D2_19,a51228,b51230);
	D2_19 = _mm512_madd52lo_epu64(D2_19,a51228,b51231);

	D2_9 = _mm512_madd52lo_epu64(D2_9,a51229,b51220);
	D2_10 = _mm512_madd52hi_epu64(D2_10,a51229,b51220);
	D2_10 = _mm512_madd52lo_epu64(D2_10,a51229,b51221);
	D2_11 = _mm512_madd52hi_epu64(D2_11,a51229,b51221);
	D2_11 = _mm512_madd52lo_epu64(D2_11,a51229,b51222);
	D2_12 = _mm512_madd52hi_epu64(D2_12,a51229,b51222);
	D2_12 = _mm512_madd52lo_epu64(D2_12,a51229,b51223);
	D2_13 = _mm512_madd52hi_epu64(D2_13,a51229,b51223);
	D2_13 = _mm512_madd52lo_epu64(D2_13,a51229,b51224);
	D2_14 = _mm512_madd52hi_epu64(D2_14,a51229,b51224);
	D2_14 = _mm512_madd52lo_epu64(D2_14,a51229,b51225);
	D2_15 = _mm512_madd52hi_epu64(D2_15,a51229,b51225);
	D2_15 = _mm512_madd52lo_epu64(D2_15,a51229,b51226);
	D2_16 = _mm512_madd52hi_epu64(D2_16,a51229,b51226);
	D2_16 = _mm512_madd52lo_epu64(D2_16,a51229,b51227);
	D2_17 = _mm512_madd52hi_epu64(D2_17,a51229,b51227);
	D2_17 = _mm512_madd52lo_epu64(D2_17,a51229,b51228);
	D2_18 = _mm512_madd52hi_epu64(D2_18,a51229,b51228);
	D2_18 = _mm512_madd52lo_epu64(D2_18,a51229,b51229);
	D2_19 = _mm512_madd52hi_epu64(D2_19,a51229,b51229);
	D2_19 = _mm512_madd52lo_epu64(D2_19,a51229,b51230);

	D2_10 = _mm512_madd52lo_epu64(D2_10,a51230,b51220);
	D2_11 = _mm512_madd52hi_epu64(D2_11,a51230,b51220);
	D2_11 = _mm512_madd52lo_epu64(D2_11,a51230,b51221);
	D2_12 = _mm512_madd52hi_epu64(D2_12,a51230,b51221);
	D2_12 = _mm512_madd52lo_epu64(D2_12,a51230,b51222);
	D2_13 = _mm512_madd52hi_epu64(D2_13,a51230,b51222);
	D2_13 = _mm512_madd52lo_epu64(D2_13,a51230,b51223);
	D2_14 = _mm512_madd52hi_epu64(D2_14,a51230,b51223);
	D2_14 = _mm512_madd52lo_epu64(D2_14,a51230,b51224);
	D2_15 = _mm512_madd52hi_epu64(D2_15,a51230,b51224);
	D2_15 = _mm512_madd52lo_epu64(D2_15,a51230,b51225);
	D2_16 = _mm512_madd52hi_epu64(D2_16,a51230,b51225);
	D2_16 = _mm512_madd52lo_epu64(D2_16,a51230,b51226);
	D2_17 = _mm512_madd52hi_epu64(D2_17,a51230,b51226);
	D2_17 = _mm512_madd52lo_epu64(D2_17,a51230,b51227);
	D2_18 = _mm512_madd52hi_epu64(D2_18,a51230,b51227);
	D2_18 = _mm512_madd52lo_epu64(D2_18,a51230,b51228);
	D2_19 = _mm512_madd52hi_epu64(D2_19,a51230,b51228);
	D2_19 = _mm512_madd52lo_epu64(D2_19,a51230,b51229);

	D2_11 = _mm512_madd52lo_epu64(D2_11,a51231,b51220);
	D2_12 = _mm512_madd52hi_epu64(D2_12,a51231,b51220);
	D2_12 = _mm512_madd52lo_epu64(D2_12,a51231,b51221);
	D2_13 = _mm512_madd52hi_epu64(D2_13,a51231,b51221);
	D2_13 = _mm512_madd52lo_epu64(D2_13,a51231,b51222);
	D2_14 = _mm512_madd52hi_epu64(D2_14,a51231,b51222);
	D2_14 = _mm512_madd52lo_epu64(D2_14,a51231,b51223);
	D2_15 = _mm512_madd52hi_epu64(D2_15,a51231,b51223);
	D2_15 = _mm512_madd52lo_epu64(D2_15,a51231,b51224);
	D2_16 = _mm512_madd52hi_epu64(D2_16,a51231,b51224);
	D2_16 = _mm512_madd52lo_epu64(D2_16,a51231,b51225);
	D2_17 = _mm512_madd52hi_epu64(D2_17,a51231,b51225);
	D2_17 = _mm512_madd52lo_epu64(D2_17,a51231,b51226);
	D2_18 = _mm512_madd52hi_epu64(D2_18,a51231,b51226);
	D2_18 = _mm512_madd52lo_epu64(D2_18,a51231,b51227);
	D2_19 = _mm512_madd52hi_epu64(D2_19,a51231,b51227);
	D2_19 = _mm512_madd52lo_epu64(D2_19,a51231,b51228);

	D2_12 = _mm512_madd52lo_epu64(D2_12,a51232,b51220);
	D2_13 = _mm512_madd52hi_epu64(D2_13,a51232,b51220);
	D2_13 = _mm512_madd52lo_epu64(D2_13,a51232,b51221);
	D2_14 = _mm512_madd52hi_epu64(D2_14,a51232,b51221);
	D2_14 = _mm512_madd52lo_epu64(D2_14,a51232,b51222);
	D2_15 = _mm512_madd52hi_epu64(D2_15,a51232,b51222);
	D2_15 = _mm512_madd52lo_epu64(D2_15,a51232,b51223);
	D2_16 = _mm512_madd52hi_epu64(D2_16,a51232,b51223);
	D2_16 = _mm512_madd52lo_epu64(D2_16,a51232,b51224);
	D2_17 = _mm512_madd52hi_epu64(D2_17,a51232,b51224);
	D2_17 = _mm512_madd52lo_epu64(D2_17,a51232,b51225);
	D2_18 = _mm512_madd52hi_epu64(D2_18,a51232,b51225);
	D2_18 = _mm512_madd52lo_epu64(D2_18,a51232,b51226);
	D2_19 = _mm512_madd52hi_epu64(D2_19,a51232,b51226);
	D2_19 = _mm512_madd52lo_epu64(D2_19,a51232,b51227);

	D2_13 = _mm512_madd52lo_epu64(D2_13,a51233,b51220);
	D2_14 = _mm512_madd52hi_epu64(D2_14,a51233,b51220);
	D2_14 = _mm512_madd52lo_epu64(D2_14,a51233,b51221);
	D2_15 = _mm512_madd52hi_epu64(D2_15,a51233,b51221);
	D2_15 = _mm512_madd52lo_epu64(D2_15,a51233,b51222);
	D2_16 = _mm512_madd52hi_epu64(D2_16,a51233,b51222);
	D2_16 = _mm512_madd52lo_epu64(D2_16,a51233,b51223);
	D2_17 = _mm512_madd52hi_epu64(D2_17,a51233,b51223);
	D2_17 = _mm512_madd52lo_epu64(D2_17,a51233,b51224);
	D2_18 = _mm512_madd52hi_epu64(D2_18,a51233,b51224);
	D2_18 = _mm512_madd52lo_epu64(D2_18,a51233,b51225);
	D2_19 = _mm512_madd52hi_epu64(D2_19,a51233,b51225);
	D2_19 = _mm512_madd52lo_epu64(D2_19,a51233,b51226);

	D2_14 = _mm512_madd52lo_epu64(D2_14,a51234,b51220);
	D2_15 = _mm512_madd52hi_epu64(D2_15,a51234,b51220);
	D2_15 = _mm512_madd52lo_epu64(D2_15,a51234,b51221);
	D2_16 = _mm512_madd52hi_epu64(D2_16,a51234,b51221);
	D2_16 = _mm512_madd52lo_epu64(D2_16,a51234,b51222);
	D2_17 = _mm512_madd52hi_epu64(D2_17,a51234,b51222);
	D2_17 = _mm512_madd52lo_epu64(D2_17,a51234,b51223);
	D2_18 = _mm512_madd52hi_epu64(D2_18,a51234,b51223);
	D2_18 = _mm512_madd52lo_epu64(D2_18,a51234,b51224);
	D2_19 = _mm512_madd52hi_epu64(D2_19,a51234,b51224);
	D2_19 = _mm512_madd52lo_epu64(D2_19,a51234,b51225);

	D2_15 = _mm512_madd52lo_epu64(D2_15,a51235,b51220);
	D2_16 = _mm512_madd52hi_epu64(D2_16,a51235,b51220);
	D2_16 = _mm512_madd52lo_epu64(D2_16,a51235,b51221);
	D2_17 = _mm512_madd52hi_epu64(D2_17,a51235,b51221);
	D2_17 = _mm512_madd52lo_epu64(D2_17,a51235,b51222);
	D2_18 = _mm512_madd52hi_epu64(D2_18,a51235,b51222);
	D2_18 = _mm512_madd52lo_epu64(D2_18,a51235,b51223);
	D2_19 = _mm512_madd52hi_epu64(D2_19,a51235,b51223);
	D2_19 = _mm512_madd52lo_epu64(D2_19,a51235,b51224);

	D2_16 = _mm512_madd52lo_epu64(D2_16,a51236,b51220);
	D2_17 = _mm512_madd52hi_epu64(D2_17,a51236,b51220);
	D2_17 = _mm512_madd52lo_epu64(D2_17,a51236,b51221);
	D2_18 = _mm512_madd52hi_epu64(D2_18,a51236,b51221);
	D2_18 = _mm512_madd52lo_epu64(D2_18,a51236,b51222);
	D2_19 = _mm512_madd52hi_epu64(D2_19,a51236,b51222);
	D2_19 = _mm512_madd52lo_epu64(D2_19,a51236,b51223);

	D2_17 = _mm512_madd52lo_epu64(D2_17,a51237,b51220);
	D2_18 = _mm512_madd52hi_epu64(D2_18,a51237,b51220);
	D2_18 = _mm512_madd52lo_epu64(D2_18,a51237,b51221);
	D2_19 = _mm512_madd52hi_epu64(D2_19,a51237,b51221);
	D2_19 = _mm512_madd52lo_epu64(D2_19,a51237,b51222);

	D2_18 = _mm512_madd52lo_epu64(D2_18,a51238,b51220);
	D2_19 = _mm512_madd52hi_epu64(D2_19,a51238,b51220);
	D2_19 = _mm512_madd52lo_epu64(D2_19,a51238,b51221);

	D2_19 = _mm512_madd52lo_epu64(D2_19,a51239,b51220);


	// Carry management

	carry  = _mm512_srli_epi64(D2_1,52);
	D2_2 = _mm512_add_epi64(D2_2,carry);
	D2_1 = _mm512_and_si512(D2_1,mask52);
	carry  = _mm512_srli_epi64(D2_2,52);
	D2_3 = _mm512_add_epi64(D2_3,carry);
	D2_2 = _mm512_and_si512(D2_2,mask52);
	carry  = _mm512_srli_epi64(D2_3,52);
	D2_4 = _mm512_add_epi64(D2_4,carry);
	D2_3 = _mm512_and_si512(D2_3,mask52);
	carry  = _mm512_srli_epi64(D2_4,52);
	D2_5 = _mm512_add_epi64(D2_5,carry);
	D2_4 = _mm512_and_si512(D2_4,mask52);
	carry  = _mm512_srli_epi64(D2_5,52);
	D2_6 = _mm512_add_epi64(D2_6,carry);
	D2_5 = _mm512_and_si512(D2_5,mask52);
	carry  = _mm512_srli_epi64(D2_6,52);
	D2_7 = _mm512_add_epi64(D2_7,carry);
	D2_6 = _mm512_and_si512(D2_6,mask52);
	carry  = _mm512_srli_epi64(D2_7,52);
	D2_8 = _mm512_add_epi64(D2_8,carry);
	D2_7 = _mm512_and_si512(D2_7,mask52);
	carry  = _mm512_srli_epi64(D2_8,52);
	D2_9 = _mm512_add_epi64(D2_9,carry);
	D2_8 = _mm512_and_si512(D2_8,mask52);
	carry  = _mm512_srli_epi64(D2_9,52);
	D2_10 = _mm512_add_epi64(D2_10,carry);
	D2_9 = _mm512_and_si512(D2_9,mask52);
	carry  = _mm512_srli_epi64(D2_10,52);
	D2_11 = _mm512_add_epi64(D2_11,carry);
	D2_10 = _mm512_and_si512(D2_10,mask52);
	carry  = _mm512_srli_epi64(D2_11,52);
	D2_12 = _mm512_add_epi64(D2_12,carry);
	D2_11 = _mm512_and_si512(D2_11,mask52);
	carry  = _mm512_srli_epi64(D2_12,52);
	D2_13 = _mm512_add_epi64(D2_13,carry);
	D2_12 = _mm512_and_si512(D2_12,mask52);
	carry  = _mm512_srli_epi64(D2_13,52);
	D2_14 = _mm512_add_epi64(D2_14,carry);
	D2_13 = _mm512_and_si512(D2_13,mask52);
	carry  = _mm512_srli_epi64(D2_14,52);
	D2_15 = _mm512_add_epi64(D2_15,carry);
	D2_14 = _mm512_and_si512(D2_14,mask52);
	carry  = _mm512_srli_epi64(D2_15,52);
	D2_16 = _mm512_add_epi64(D2_16,carry);
	D2_15 = _mm512_and_si512(D2_15,mask52);
	carry  = _mm512_srli_epi64(D2_16,52);
	D2_17 = _mm512_add_epi64(D2_17,carry);
	D2_16 = _mm512_and_si512(D2_16,mask52);
	carry  = _mm512_srli_epi64(D2_17,52);
	D2_18 = _mm512_add_epi64(D2_18,carry);
	D2_17 = _mm512_and_si512(D2_17,mask52);
	carry  = _mm512_srli_epi64(D2_18,52);
	D2_19 = _mm512_add_epi64(D2_19,carry);
	D2_18 = _mm512_and_si512(D2_18,mask52);

	// D1 = (Al+Ah)*(Bl+Bh)

	__m512i D1_0;
	__m512i D1_1;
	__m512i D1_2;
	__m512i D1_3;
	__m512i D1_4;
	__m512i D1_5;
	__m512i D1_6;
	__m512i D1_7;
	__m512i D1_8;
	__m512i D1_9;
	__m512i D1_10;
	__m512i D1_11;
	__m512i D1_12;
	__m512i D1_13;
	__m512i D1_14;
	__m512i D1_15;
	__m512i D1_16;
	__m512i D1_17;
	__m512i D1_18;
	__m512i D1_19;

	__m512i alpah0 = _mm512_add_epi64(a5120,a51220);
	carry = alpah0>>52;
	alpah0 &= mask52;
	__m512i alpah1 = _mm512_add_epi64(a5121,carry);
	alpah1 = _mm512_add_epi64(alpah1,a51221);
	carry = alpah1>>52;
	alpah1 &= mask52;
	__m512i alpah2 = _mm512_add_epi64(a5122,carry);
	alpah2 = _mm512_add_epi64(alpah2,a51222);
	carry = alpah2>>52;
	alpah2 &= mask52;
	__m512i alpah3 = _mm512_add_epi64(a5123,carry);
	alpah3 = _mm512_add_epi64(alpah3,a51223);
	carry = alpah3>>52;
	alpah3 &= mask52;
	__m512i alpah4 = _mm512_add_epi64(a5124,carry);
	alpah4 = _mm512_add_epi64(alpah4,a51224);
	carry = alpah4>>52;
	alpah4 &= mask52;
	__m512i alpah5 = _mm512_add_epi64(a5125,carry);
	alpah5 = _mm512_add_epi64(alpah5,a51225);
	carry = alpah5>>52;
	alpah5 &= mask52;
	__m512i alpah6 = _mm512_add_epi64(a5126,carry);
	alpah6 = _mm512_add_epi64(alpah6,a51226);
	carry = alpah6>>52;
	alpah6 &= mask52;
	__m512i alpah7 = _mm512_add_epi64(a5127,carry);
	alpah7 = _mm512_add_epi64(alpah7,a51227);
	carry = alpah7>>52;
	alpah7 &= mask52;
	__m512i alpah8 = _mm512_add_epi64(a5128,carry);
	alpah8 = _mm512_add_epi64(alpah8,a51228);
	carry = alpah8>>52;
	alpah8 &= mask52;
	__m512i alpah9 = _mm512_add_epi64(a5129,carry);
	alpah9 = _mm512_add_epi64(alpah9,a51229);
	carry = alpah9>>52;
	alpah9 &= mask52;
	__m512i alpah10 = _mm512_add_epi64(a51210,carry);
	alpah10 = _mm512_add_epi64(alpah10,a51230);
	carry = alpah10>>52;
	alpah10 &= mask52;
	__m512i alpah11 = _mm512_add_epi64(a51211,carry);
	alpah11 = _mm512_add_epi64(alpah11,a51231);
	carry = alpah11>>52;
	alpah11 &= mask52;
	__m512i alpah12 = _mm512_add_epi64(a51212,carry);
	alpah12 = _mm512_add_epi64(alpah12,a51232);
	carry = alpah12>>52;
	alpah12 &= mask52;
	__m512i alpah13 = _mm512_add_epi64(a51213,carry);
	alpah13 = _mm512_add_epi64(alpah13,a51233);
	carry = alpah13>>52;
	alpah13 &= mask52;
	__m512i alpah14 = _mm512_add_epi64(a51214,carry);
	alpah14 = _mm512_add_epi64(alpah14,a51234);
	carry = alpah14>>52;
	alpah14 &= mask52;
	__m512i alpah15 = _mm512_add_epi64(a51215,carry);
	alpah15 = _mm512_add_epi64(alpah15,a51235);
	carry = alpah15>>52;
	alpah15 &= mask52;
	__m512i alpah16 = _mm512_add_epi64(a51216,carry);
	alpah16 = _mm512_add_epi64(alpah16,a51236);
	carry = alpah16>>52;
	alpah16 &= mask52;
	__m512i alpah17 = _mm512_add_epi64(a51217,carry);
	alpah17 = _mm512_add_epi64(alpah17,a51237);
	carry = alpah17>>52;
	alpah17 &= mask52;
	__m512i alpah18 = _mm512_add_epi64(a51218,carry);
	alpah18 = _mm512_add_epi64(alpah18,a51238);
	carry = alpah18>>52;
	alpah18 &= mask52;
	__m512i alpah19 = _mm512_add_epi64(a51219,carry);
	alpah19 = _mm512_add_epi64(alpah19,a51239);
	carry = alpah19>>52;
	alpah19 &= mask52;
	__m512i blpbh0 = _mm512_add_epi64(b5120,b51220);
	carry = blpbh0>>52;
	blpbh0 &= mask52;
	__m512i blpbh1 = _mm512_add_epi64(b5121,carry);
	blpbh1 = _mm512_add_epi64(blpbh1,b51221);
	carry = blpbh1>>52;
	blpbh1 &= mask52;
	__m512i blpbh2 = _mm512_add_epi64(b5122,carry);
	blpbh2 = _mm512_add_epi64(blpbh2,b51222);
	carry = blpbh2>>52;
	blpbh2 &= mask52;
	__m512i blpbh3 = _mm512_add_epi64(b5123,carry);
	blpbh3 = _mm512_add_epi64(blpbh3,b51223);
	carry = blpbh3>>52;
	blpbh3 &= mask52;
	__m512i blpbh4 = _mm512_add_epi64(b5124,carry);
	blpbh4 = _mm512_add_epi64(blpbh4,b51224);
	carry = blpbh4>>52;
	blpbh4 &= mask52;
	__m512i blpbh5 = _mm512_add_epi64(b5125,carry);
	blpbh5 = _mm512_add_epi64(blpbh5,b51225);
	carry = blpbh5>>52;
	blpbh5 &= mask52;
	__m512i blpbh6 = _mm512_add_epi64(b5126,carry);
	blpbh6 = _mm512_add_epi64(blpbh6,b51226);
	carry = blpbh6>>52;
	blpbh6 &= mask52;
	__m512i blpbh7 = _mm512_add_epi64(b5127,carry);
	blpbh7 = _mm512_add_epi64(blpbh7,b51227);
	carry = blpbh7>>52;
	blpbh7 &= mask52;
	__m512i blpbh8 = _mm512_add_epi64(b5128,carry);
	blpbh8 = _mm512_add_epi64(blpbh8,b51228);
	carry = blpbh8>>52;
	blpbh8 &= mask52;
	__m512i blpbh9 = _mm512_add_epi64(b5129,carry);
	blpbh9 = _mm512_add_epi64(blpbh9,b51229);
	carry = blpbh9>>52;
	blpbh9 &= mask52;
	__m512i blpbh10 = _mm512_add_epi64(b51210,carry);
	blpbh10 = _mm512_add_epi64(blpbh10,b51230);
	carry = blpbh10>>52;
	blpbh10 &= mask52;
	__m512i blpbh11 = _mm512_add_epi64(b51211,carry);
	blpbh11 = _mm512_add_epi64(blpbh11,b51231);
	carry = blpbh11>>52;
	blpbh11 &= mask52;
	__m512i blpbh12 = _mm512_add_epi64(b51212,carry);
	blpbh12 = _mm512_add_epi64(blpbh12,b51232);
	carry = blpbh12>>52;
	blpbh12 &= mask52;
	__m512i blpbh13 = _mm512_add_epi64(b51213,carry);
	blpbh13 = _mm512_add_epi64(blpbh13,b51233);
	carry = blpbh13>>52;
	blpbh13 &= mask52;
	__m512i blpbh14 = _mm512_add_epi64(b51214,carry);
	blpbh14 = _mm512_add_epi64(blpbh14,b51234);
	carry = blpbh14>>52;
	blpbh14 &= mask52;
	__m512i blpbh15 = _mm512_add_epi64(b51215,carry);
	blpbh15 = _mm512_add_epi64(blpbh15,b51235);
	carry = blpbh15>>52;
	blpbh15 &= mask52;
	__m512i blpbh16 = _mm512_add_epi64(b51216,carry);
	blpbh16 = _mm512_add_epi64(blpbh16,b51236);
	carry = blpbh16>>52;
	blpbh16 &= mask52;
	__m512i blpbh17 = _mm512_add_epi64(b51217,carry);
	blpbh17 = _mm512_add_epi64(blpbh17,b51237);
	carry = blpbh17>>52;
	blpbh17 &= mask52;
	__m512i blpbh18 = _mm512_add_epi64(b51218,carry);
	blpbh18 = _mm512_add_epi64(blpbh18,b51238);
	carry = blpbh18>>52;
	blpbh18 &= mask52;
	__m512i blpbh19 = _mm512_add_epi64(b51219,carry);
	blpbh19 = _mm512_add_epi64(blpbh19,b51239);
	carry = blpbh19>>52;
	blpbh19 &= mask52;
	D1_0 = _mm512_madd52lo_epu64(D0_20,alpah0,blpbh0);
	D1_1 = _mm512_madd52hi_epu64(D0_21,alpah0,blpbh0);
	D1_1 = _mm512_madd52lo_epu64(D1_1,alpah0,blpbh1);
	D1_2 = _mm512_madd52hi_epu64(D0_22,alpah0,blpbh1);
	D1_2 = _mm512_madd52lo_epu64(D1_2,alpah0,blpbh2);
	D1_3 = _mm512_madd52hi_epu64(D0_23,alpah0,blpbh2);
	D1_3 = _mm512_madd52lo_epu64(D1_3,alpah0,blpbh3);
	D1_4 = _mm512_madd52hi_epu64(D0_24,alpah0,blpbh3);
	D1_4 = _mm512_madd52lo_epu64(D1_4,alpah0,blpbh4);
	D1_5 = _mm512_madd52hi_epu64(D0_25,alpah0,blpbh4);
	D1_5 = _mm512_madd52lo_epu64(D1_5,alpah0,blpbh5);
	D1_6 = _mm512_madd52hi_epu64(D0_26,alpah0,blpbh5);
	D1_6 = _mm512_madd52lo_epu64(D1_6,alpah0,blpbh6);
	D1_7 = _mm512_madd52hi_epu64(D0_27,alpah0,blpbh6);
	D1_7 = _mm512_madd52lo_epu64(D1_7,alpah0,blpbh7);
	D1_8 = _mm512_madd52hi_epu64(D0_28,alpah0,blpbh7);
	D1_8 = _mm512_madd52lo_epu64(D1_8,alpah0,blpbh8);
	D1_9 = _mm512_madd52hi_epu64(D0_29,alpah0,blpbh8);
	D1_9 = _mm512_madd52lo_epu64(D1_9,alpah0,blpbh9);
	D1_10 = _mm512_madd52hi_epu64(D0_30,alpah0,blpbh9);
	D1_10 = _mm512_madd52lo_epu64(D1_10,alpah0,blpbh10);
	D1_11 = _mm512_madd52hi_epu64(D0_31,alpah0,blpbh10);
	D1_11 = _mm512_madd52lo_epu64(D1_11,alpah0,blpbh11);
	D1_12 = _mm512_madd52hi_epu64(D0_32,alpah0,blpbh11);
	D1_12 = _mm512_madd52lo_epu64(D1_12,alpah0,blpbh12);
	D1_13 = _mm512_madd52hi_epu64(D0_33,alpah0,blpbh12);
	D1_13 = _mm512_madd52lo_epu64(D1_13,alpah0,blpbh13);
	D1_14 = _mm512_madd52hi_epu64(D0_34,alpah0,blpbh13);
	D1_14 = _mm512_madd52lo_epu64(D1_14,alpah0,blpbh14);
	D1_15 = _mm512_madd52hi_epu64(D0_35,alpah0,blpbh14);
	D1_15 = _mm512_madd52lo_epu64(D1_15,alpah0,blpbh15);
	D1_16 = _mm512_madd52hi_epu64(D0_36,alpah0,blpbh15);
	D1_16 = _mm512_madd52lo_epu64(D1_16,alpah0,blpbh16);
	D1_17 = _mm512_madd52hi_epu64(D0_37,alpah0,blpbh16);
	D1_17 = _mm512_madd52lo_epu64(D1_17,alpah0,blpbh17);
	D1_18 = _mm512_madd52hi_epu64(D0_38,alpah0,blpbh17);
	D1_18 = _mm512_madd52lo_epu64(D1_18,alpah0,blpbh18);
	D1_19 = _mm512_madd52hi_epu64(D0_39,alpah0,blpbh18);
	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah0,blpbh19);

	D1_1 = _mm512_madd52lo_epu64(D1_1,alpah1,blpbh0);
	D1_2 = _mm512_madd52hi_epu64(D1_2,alpah1,blpbh0);
	D1_2 = _mm512_madd52lo_epu64(D1_2,alpah1,blpbh1);
	D1_3 = _mm512_madd52hi_epu64(D1_3,alpah1,blpbh1);
	D1_3 = _mm512_madd52lo_epu64(D1_3,alpah1,blpbh2);
	D1_4 = _mm512_madd52hi_epu64(D1_4,alpah1,blpbh2);
	D1_4 = _mm512_madd52lo_epu64(D1_4,alpah1,blpbh3);
	D1_5 = _mm512_madd52hi_epu64(D1_5,alpah1,blpbh3);
	D1_5 = _mm512_madd52lo_epu64(D1_5,alpah1,blpbh4);
	D1_6 = _mm512_madd52hi_epu64(D1_6,alpah1,blpbh4);
	D1_6 = _mm512_madd52lo_epu64(D1_6,alpah1,blpbh5);
	D1_7 = _mm512_madd52hi_epu64(D1_7,alpah1,blpbh5);
	D1_7 = _mm512_madd52lo_epu64(D1_7,alpah1,blpbh6);
	D1_8 = _mm512_madd52hi_epu64(D1_8,alpah1,blpbh6);
	D1_8 = _mm512_madd52lo_epu64(D1_8,alpah1,blpbh7);
	D1_9 = _mm512_madd52hi_epu64(D1_9,alpah1,blpbh7);
	D1_9 = _mm512_madd52lo_epu64(D1_9,alpah1,blpbh8);
	D1_10 = _mm512_madd52hi_epu64(D1_10,alpah1,blpbh8);
	D1_10 = _mm512_madd52lo_epu64(D1_10,alpah1,blpbh9);
	D1_11 = _mm512_madd52hi_epu64(D1_11,alpah1,blpbh9);
	D1_11 = _mm512_madd52lo_epu64(D1_11,alpah1,blpbh10);
	D1_12 = _mm512_madd52hi_epu64(D1_12,alpah1,blpbh10);
	D1_12 = _mm512_madd52lo_epu64(D1_12,alpah1,blpbh11);
	D1_13 = _mm512_madd52hi_epu64(D1_13,alpah1,blpbh11);
	D1_13 = _mm512_madd52lo_epu64(D1_13,alpah1,blpbh12);
	D1_14 = _mm512_madd52hi_epu64(D1_14,alpah1,blpbh12);
	D1_14 = _mm512_madd52lo_epu64(D1_14,alpah1,blpbh13);
	D1_15 = _mm512_madd52hi_epu64(D1_15,alpah1,blpbh13);
	D1_15 = _mm512_madd52lo_epu64(D1_15,alpah1,blpbh14);
	D1_16 = _mm512_madd52hi_epu64(D1_16,alpah1,blpbh14);
	D1_16 = _mm512_madd52lo_epu64(D1_16,alpah1,blpbh15);
	D1_17 = _mm512_madd52hi_epu64(D1_17,alpah1,blpbh15);
	D1_17 = _mm512_madd52lo_epu64(D1_17,alpah1,blpbh16);
	D1_18 = _mm512_madd52hi_epu64(D1_18,alpah1,blpbh16);
	D1_18 = _mm512_madd52lo_epu64(D1_18,alpah1,blpbh17);
	D1_19 = _mm512_madd52hi_epu64(D1_19,alpah1,blpbh17);
	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah1,blpbh18);

	D1_2 = _mm512_madd52lo_epu64(D1_2,alpah2,blpbh0);
	D1_3 = _mm512_madd52hi_epu64(D1_3,alpah2,blpbh0);
	D1_3 = _mm512_madd52lo_epu64(D1_3,alpah2,blpbh1);
	D1_4 = _mm512_madd52hi_epu64(D1_4,alpah2,blpbh1);
	D1_4 = _mm512_madd52lo_epu64(D1_4,alpah2,blpbh2);
	D1_5 = _mm512_madd52hi_epu64(D1_5,alpah2,blpbh2);
	D1_5 = _mm512_madd52lo_epu64(D1_5,alpah2,blpbh3);
	D1_6 = _mm512_madd52hi_epu64(D1_6,alpah2,blpbh3);
	D1_6 = _mm512_madd52lo_epu64(D1_6,alpah2,blpbh4);
	D1_7 = _mm512_madd52hi_epu64(D1_7,alpah2,blpbh4);
	D1_7 = _mm512_madd52lo_epu64(D1_7,alpah2,blpbh5);
	D1_8 = _mm512_madd52hi_epu64(D1_8,alpah2,blpbh5);
	D1_8 = _mm512_madd52lo_epu64(D1_8,alpah2,blpbh6);
	D1_9 = _mm512_madd52hi_epu64(D1_9,alpah2,blpbh6);
	D1_9 = _mm512_madd52lo_epu64(D1_9,alpah2,blpbh7);
	D1_10 = _mm512_madd52hi_epu64(D1_10,alpah2,blpbh7);
	D1_10 = _mm512_madd52lo_epu64(D1_10,alpah2,blpbh8);
	D1_11 = _mm512_madd52hi_epu64(D1_11,alpah2,blpbh8);
	D1_11 = _mm512_madd52lo_epu64(D1_11,alpah2,blpbh9);
	D1_12 = _mm512_madd52hi_epu64(D1_12,alpah2,blpbh9);
	D1_12 = _mm512_madd52lo_epu64(D1_12,alpah2,blpbh10);
	D1_13 = _mm512_madd52hi_epu64(D1_13,alpah2,blpbh10);
	D1_13 = _mm512_madd52lo_epu64(D1_13,alpah2,blpbh11);
	D1_14 = _mm512_madd52hi_epu64(D1_14,alpah2,blpbh11);
	D1_14 = _mm512_madd52lo_epu64(D1_14,alpah2,blpbh12);
	D1_15 = _mm512_madd52hi_epu64(D1_15,alpah2,blpbh12);
	D1_15 = _mm512_madd52lo_epu64(D1_15,alpah2,blpbh13);
	D1_16 = _mm512_madd52hi_epu64(D1_16,alpah2,blpbh13);
	D1_16 = _mm512_madd52lo_epu64(D1_16,alpah2,blpbh14);
	D1_17 = _mm512_madd52hi_epu64(D1_17,alpah2,blpbh14);
	D1_17 = _mm512_madd52lo_epu64(D1_17,alpah2,blpbh15);
	D1_18 = _mm512_madd52hi_epu64(D1_18,alpah2,blpbh15);
	D1_18 = _mm512_madd52lo_epu64(D1_18,alpah2,blpbh16);
	D1_19 = _mm512_madd52hi_epu64(D1_19,alpah2,blpbh16);
	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah2,blpbh17);

	D1_3 = _mm512_madd52lo_epu64(D1_3,alpah3,blpbh0);
	D1_4 = _mm512_madd52hi_epu64(D1_4,alpah3,blpbh0);
	D1_4 = _mm512_madd52lo_epu64(D1_4,alpah3,blpbh1);
	D1_5 = _mm512_madd52hi_epu64(D1_5,alpah3,blpbh1);
	D1_5 = _mm512_madd52lo_epu64(D1_5,alpah3,blpbh2);
	D1_6 = _mm512_madd52hi_epu64(D1_6,alpah3,blpbh2);
	D1_6 = _mm512_madd52lo_epu64(D1_6,alpah3,blpbh3);
	D1_7 = _mm512_madd52hi_epu64(D1_7,alpah3,blpbh3);
	D1_7 = _mm512_madd52lo_epu64(D1_7,alpah3,blpbh4);
	D1_8 = _mm512_madd52hi_epu64(D1_8,alpah3,blpbh4);
	D1_8 = _mm512_madd52lo_epu64(D1_8,alpah3,blpbh5);
	D1_9 = _mm512_madd52hi_epu64(D1_9,alpah3,blpbh5);
	D1_9 = _mm512_madd52lo_epu64(D1_9,alpah3,blpbh6);
	D1_10 = _mm512_madd52hi_epu64(D1_10,alpah3,blpbh6);
	D1_10 = _mm512_madd52lo_epu64(D1_10,alpah3,blpbh7);
	D1_11 = _mm512_madd52hi_epu64(D1_11,alpah3,blpbh7);
	D1_11 = _mm512_madd52lo_epu64(D1_11,alpah3,blpbh8);
	D1_12 = _mm512_madd52hi_epu64(D1_12,alpah3,blpbh8);
	D1_12 = _mm512_madd52lo_epu64(D1_12,alpah3,blpbh9);
	D1_13 = _mm512_madd52hi_epu64(D1_13,alpah3,blpbh9);
	D1_13 = _mm512_madd52lo_epu64(D1_13,alpah3,blpbh10);
	D1_14 = _mm512_madd52hi_epu64(D1_14,alpah3,blpbh10);
	D1_14 = _mm512_madd52lo_epu64(D1_14,alpah3,blpbh11);
	D1_15 = _mm512_madd52hi_epu64(D1_15,alpah3,blpbh11);
	D1_15 = _mm512_madd52lo_epu64(D1_15,alpah3,blpbh12);
	D1_16 = _mm512_madd52hi_epu64(D1_16,alpah3,blpbh12);
	D1_16 = _mm512_madd52lo_epu64(D1_16,alpah3,blpbh13);
	D1_17 = _mm512_madd52hi_epu64(D1_17,alpah3,blpbh13);
	D1_17 = _mm512_madd52lo_epu64(D1_17,alpah3,blpbh14);
	D1_18 = _mm512_madd52hi_epu64(D1_18,alpah3,blpbh14);
	D1_18 = _mm512_madd52lo_epu64(D1_18,alpah3,blpbh15);
	D1_19 = _mm512_madd52hi_epu64(D1_19,alpah3,blpbh15);
	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah3,blpbh16);

	D1_4 = _mm512_madd52lo_epu64(D1_4,alpah4,blpbh0);
	D1_5 = _mm512_madd52hi_epu64(D1_5,alpah4,blpbh0);
	D1_5 = _mm512_madd52lo_epu64(D1_5,alpah4,blpbh1);
	D1_6 = _mm512_madd52hi_epu64(D1_6,alpah4,blpbh1);
	D1_6 = _mm512_madd52lo_epu64(D1_6,alpah4,blpbh2);
	D1_7 = _mm512_madd52hi_epu64(D1_7,alpah4,blpbh2);
	D1_7 = _mm512_madd52lo_epu64(D1_7,alpah4,blpbh3);
	D1_8 = _mm512_madd52hi_epu64(D1_8,alpah4,blpbh3);
	D1_8 = _mm512_madd52lo_epu64(D1_8,alpah4,blpbh4);
	D1_9 = _mm512_madd52hi_epu64(D1_9,alpah4,blpbh4);
	D1_9 = _mm512_madd52lo_epu64(D1_9,alpah4,blpbh5);
	D1_10 = _mm512_madd52hi_epu64(D1_10,alpah4,blpbh5);
	D1_10 = _mm512_madd52lo_epu64(D1_10,alpah4,blpbh6);
	D1_11 = _mm512_madd52hi_epu64(D1_11,alpah4,blpbh6);
	D1_11 = _mm512_madd52lo_epu64(D1_11,alpah4,blpbh7);
	D1_12 = _mm512_madd52hi_epu64(D1_12,alpah4,blpbh7);
	D1_12 = _mm512_madd52lo_epu64(D1_12,alpah4,blpbh8);
	D1_13 = _mm512_madd52hi_epu64(D1_13,alpah4,blpbh8);
	D1_13 = _mm512_madd52lo_epu64(D1_13,alpah4,blpbh9);
	D1_14 = _mm512_madd52hi_epu64(D1_14,alpah4,blpbh9);
	D1_14 = _mm512_madd52lo_epu64(D1_14,alpah4,blpbh10);
	D1_15 = _mm512_madd52hi_epu64(D1_15,alpah4,blpbh10);
	D1_15 = _mm512_madd52lo_epu64(D1_15,alpah4,blpbh11);
	D1_16 = _mm512_madd52hi_epu64(D1_16,alpah4,blpbh11);
	D1_16 = _mm512_madd52lo_epu64(D1_16,alpah4,blpbh12);
	D1_17 = _mm512_madd52hi_epu64(D1_17,alpah4,blpbh12);
	D1_17 = _mm512_madd52lo_epu64(D1_17,alpah4,blpbh13);
	D1_18 = _mm512_madd52hi_epu64(D1_18,alpah4,blpbh13);
	D1_18 = _mm512_madd52lo_epu64(D1_18,alpah4,blpbh14);
	D1_19 = _mm512_madd52hi_epu64(D1_19,alpah4,blpbh14);
	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah4,blpbh15);

	D1_5 = _mm512_madd52lo_epu64(D1_5,alpah5,blpbh0);
	D1_6 = _mm512_madd52hi_epu64(D1_6,alpah5,blpbh0);
	D1_6 = _mm512_madd52lo_epu64(D1_6,alpah5,blpbh1);
	D1_7 = _mm512_madd52hi_epu64(D1_7,alpah5,blpbh1);
	D1_7 = _mm512_madd52lo_epu64(D1_7,alpah5,blpbh2);
	D1_8 = _mm512_madd52hi_epu64(D1_8,alpah5,blpbh2);
	D1_8 = _mm512_madd52lo_epu64(D1_8,alpah5,blpbh3);
	D1_9 = _mm512_madd52hi_epu64(D1_9,alpah5,blpbh3);
	D1_9 = _mm512_madd52lo_epu64(D1_9,alpah5,blpbh4);
	D1_10 = _mm512_madd52hi_epu64(D1_10,alpah5,blpbh4);
	D1_10 = _mm512_madd52lo_epu64(D1_10,alpah5,blpbh5);
	D1_11 = _mm512_madd52hi_epu64(D1_11,alpah5,blpbh5);
	D1_11 = _mm512_madd52lo_epu64(D1_11,alpah5,blpbh6);
	D1_12 = _mm512_madd52hi_epu64(D1_12,alpah5,blpbh6);
	D1_12 = _mm512_madd52lo_epu64(D1_12,alpah5,blpbh7);
	D1_13 = _mm512_madd52hi_epu64(D1_13,alpah5,blpbh7);
	D1_13 = _mm512_madd52lo_epu64(D1_13,alpah5,blpbh8);
	D1_14 = _mm512_madd52hi_epu64(D1_14,alpah5,blpbh8);
	D1_14 = _mm512_madd52lo_epu64(D1_14,alpah5,blpbh9);
	D1_15 = _mm512_madd52hi_epu64(D1_15,alpah5,blpbh9);
	D1_15 = _mm512_madd52lo_epu64(D1_15,alpah5,blpbh10);
	D1_16 = _mm512_madd52hi_epu64(D1_16,alpah5,blpbh10);
	D1_16 = _mm512_madd52lo_epu64(D1_16,alpah5,blpbh11);
	D1_17 = _mm512_madd52hi_epu64(D1_17,alpah5,blpbh11);
	D1_17 = _mm512_madd52lo_epu64(D1_17,alpah5,blpbh12);
	D1_18 = _mm512_madd52hi_epu64(D1_18,alpah5,blpbh12);
	D1_18 = _mm512_madd52lo_epu64(D1_18,alpah5,blpbh13);
	D1_19 = _mm512_madd52hi_epu64(D1_19,alpah5,blpbh13);
	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah5,blpbh14);

	D1_6 = _mm512_madd52lo_epu64(D1_6,alpah6,blpbh0);
	D1_7 = _mm512_madd52hi_epu64(D1_7,alpah6,blpbh0);
	D1_7 = _mm512_madd52lo_epu64(D1_7,alpah6,blpbh1);
	D1_8 = _mm512_madd52hi_epu64(D1_8,alpah6,blpbh1);
	D1_8 = _mm512_madd52lo_epu64(D1_8,alpah6,blpbh2);
	D1_9 = _mm512_madd52hi_epu64(D1_9,alpah6,blpbh2);
	D1_9 = _mm512_madd52lo_epu64(D1_9,alpah6,blpbh3);
	D1_10 = _mm512_madd52hi_epu64(D1_10,alpah6,blpbh3);
	D1_10 = _mm512_madd52lo_epu64(D1_10,alpah6,blpbh4);
	D1_11 = _mm512_madd52hi_epu64(D1_11,alpah6,blpbh4);
	D1_11 = _mm512_madd52lo_epu64(D1_11,alpah6,blpbh5);
	D1_12 = _mm512_madd52hi_epu64(D1_12,alpah6,blpbh5);
	D1_12 = _mm512_madd52lo_epu64(D1_12,alpah6,blpbh6);
	D1_13 = _mm512_madd52hi_epu64(D1_13,alpah6,blpbh6);
	D1_13 = _mm512_madd52lo_epu64(D1_13,alpah6,blpbh7);
	D1_14 = _mm512_madd52hi_epu64(D1_14,alpah6,blpbh7);
	D1_14 = _mm512_madd52lo_epu64(D1_14,alpah6,blpbh8);
	D1_15 = _mm512_madd52hi_epu64(D1_15,alpah6,blpbh8);
	D1_15 = _mm512_madd52lo_epu64(D1_15,alpah6,blpbh9);
	D1_16 = _mm512_madd52hi_epu64(D1_16,alpah6,blpbh9);
	D1_16 = _mm512_madd52lo_epu64(D1_16,alpah6,blpbh10);
	D1_17 = _mm512_madd52hi_epu64(D1_17,alpah6,blpbh10);
	D1_17 = _mm512_madd52lo_epu64(D1_17,alpah6,blpbh11);
	D1_18 = _mm512_madd52hi_epu64(D1_18,alpah6,blpbh11);
	D1_18 = _mm512_madd52lo_epu64(D1_18,alpah6,blpbh12);
	D1_19 = _mm512_madd52hi_epu64(D1_19,alpah6,blpbh12);
	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah6,blpbh13);

	D1_7 = _mm512_madd52lo_epu64(D1_7,alpah7,blpbh0);
	D1_8 = _mm512_madd52hi_epu64(D1_8,alpah7,blpbh0);
	D1_8 = _mm512_madd52lo_epu64(D1_8,alpah7,blpbh1);
	D1_9 = _mm512_madd52hi_epu64(D1_9,alpah7,blpbh1);
	D1_9 = _mm512_madd52lo_epu64(D1_9,alpah7,blpbh2);
	D1_10 = _mm512_madd52hi_epu64(D1_10,alpah7,blpbh2);
	D1_10 = _mm512_madd52lo_epu64(D1_10,alpah7,blpbh3);
	D1_11 = _mm512_madd52hi_epu64(D1_11,alpah7,blpbh3);
	D1_11 = _mm512_madd52lo_epu64(D1_11,alpah7,blpbh4);
	D1_12 = _mm512_madd52hi_epu64(D1_12,alpah7,blpbh4);
	D1_12 = _mm512_madd52lo_epu64(D1_12,alpah7,blpbh5);
	D1_13 = _mm512_madd52hi_epu64(D1_13,alpah7,blpbh5);
	D1_13 = _mm512_madd52lo_epu64(D1_13,alpah7,blpbh6);
	D1_14 = _mm512_madd52hi_epu64(D1_14,alpah7,blpbh6);
	D1_14 = _mm512_madd52lo_epu64(D1_14,alpah7,blpbh7);
	D1_15 = _mm512_madd52hi_epu64(D1_15,alpah7,blpbh7);
	D1_15 = _mm512_madd52lo_epu64(D1_15,alpah7,blpbh8);
	D1_16 = _mm512_madd52hi_epu64(D1_16,alpah7,blpbh8);
	D1_16 = _mm512_madd52lo_epu64(D1_16,alpah7,blpbh9);
	D1_17 = _mm512_madd52hi_epu64(D1_17,alpah7,blpbh9);
	D1_17 = _mm512_madd52lo_epu64(D1_17,alpah7,blpbh10);
	D1_18 = _mm512_madd52hi_epu64(D1_18,alpah7,blpbh10);
	D1_18 = _mm512_madd52lo_epu64(D1_18,alpah7,blpbh11);
	D1_19 = _mm512_madd52hi_epu64(D1_19,alpah7,blpbh11);
	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah7,blpbh12);

	D1_8 = _mm512_madd52lo_epu64(D1_8,alpah8,blpbh0);
	D1_9 = _mm512_madd52hi_epu64(D1_9,alpah8,blpbh0);
	D1_9 = _mm512_madd52lo_epu64(D1_9,alpah8,blpbh1);
	D1_10 = _mm512_madd52hi_epu64(D1_10,alpah8,blpbh1);
	D1_10 = _mm512_madd52lo_epu64(D1_10,alpah8,blpbh2);
	D1_11 = _mm512_madd52hi_epu64(D1_11,alpah8,blpbh2);
	D1_11 = _mm512_madd52lo_epu64(D1_11,alpah8,blpbh3);
	D1_12 = _mm512_madd52hi_epu64(D1_12,alpah8,blpbh3);
	D1_12 = _mm512_madd52lo_epu64(D1_12,alpah8,blpbh4);
	D1_13 = _mm512_madd52hi_epu64(D1_13,alpah8,blpbh4);
	D1_13 = _mm512_madd52lo_epu64(D1_13,alpah8,blpbh5);
	D1_14 = _mm512_madd52hi_epu64(D1_14,alpah8,blpbh5);
	D1_14 = _mm512_madd52lo_epu64(D1_14,alpah8,blpbh6);
	D1_15 = _mm512_madd52hi_epu64(D1_15,alpah8,blpbh6);
	D1_15 = _mm512_madd52lo_epu64(D1_15,alpah8,blpbh7);
	D1_16 = _mm512_madd52hi_epu64(D1_16,alpah8,blpbh7);
	D1_16 = _mm512_madd52lo_epu64(D1_16,alpah8,blpbh8);
	D1_17 = _mm512_madd52hi_epu64(D1_17,alpah8,blpbh8);
	D1_17 = _mm512_madd52lo_epu64(D1_17,alpah8,blpbh9);
	D1_18 = _mm512_madd52hi_epu64(D1_18,alpah8,blpbh9);
	D1_18 = _mm512_madd52lo_epu64(D1_18,alpah8,blpbh10);
	D1_19 = _mm512_madd52hi_epu64(D1_19,alpah8,blpbh10);
	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah8,blpbh11);

	D1_9 = _mm512_madd52lo_epu64(D1_9,alpah9,blpbh0);
	D1_10 = _mm512_madd52hi_epu64(D1_10,alpah9,blpbh0);
	D1_10 = _mm512_madd52lo_epu64(D1_10,alpah9,blpbh1);
	D1_11 = _mm512_madd52hi_epu64(D1_11,alpah9,blpbh1);
	D1_11 = _mm512_madd52lo_epu64(D1_11,alpah9,blpbh2);
	D1_12 = _mm512_madd52hi_epu64(D1_12,alpah9,blpbh2);
	D1_12 = _mm512_madd52lo_epu64(D1_12,alpah9,blpbh3);
	D1_13 = _mm512_madd52hi_epu64(D1_13,alpah9,blpbh3);
	D1_13 = _mm512_madd52lo_epu64(D1_13,alpah9,blpbh4);
	D1_14 = _mm512_madd52hi_epu64(D1_14,alpah9,blpbh4);
	D1_14 = _mm512_madd52lo_epu64(D1_14,alpah9,blpbh5);
	D1_15 = _mm512_madd52hi_epu64(D1_15,alpah9,blpbh5);
	D1_15 = _mm512_madd52lo_epu64(D1_15,alpah9,blpbh6);
	D1_16 = _mm512_madd52hi_epu64(D1_16,alpah9,blpbh6);
	D1_16 = _mm512_madd52lo_epu64(D1_16,alpah9,blpbh7);
	D1_17 = _mm512_madd52hi_epu64(D1_17,alpah9,blpbh7);
	D1_17 = _mm512_madd52lo_epu64(D1_17,alpah9,blpbh8);
	D1_18 = _mm512_madd52hi_epu64(D1_18,alpah9,blpbh8);
	D1_18 = _mm512_madd52lo_epu64(D1_18,alpah9,blpbh9);
	D1_19 = _mm512_madd52hi_epu64(D1_19,alpah9,blpbh9);
	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah9,blpbh10);

	D1_10 = _mm512_madd52lo_epu64(D1_10,alpah10,blpbh0);
	D1_11 = _mm512_madd52hi_epu64(D1_11,alpah10,blpbh0);
	D1_11 = _mm512_madd52lo_epu64(D1_11,alpah10,blpbh1);
	D1_12 = _mm512_madd52hi_epu64(D1_12,alpah10,blpbh1);
	D1_12 = _mm512_madd52lo_epu64(D1_12,alpah10,blpbh2);
	D1_13 = _mm512_madd52hi_epu64(D1_13,alpah10,blpbh2);
	D1_13 = _mm512_madd52lo_epu64(D1_13,alpah10,blpbh3);
	D1_14 = _mm512_madd52hi_epu64(D1_14,alpah10,blpbh3);
	D1_14 = _mm512_madd52lo_epu64(D1_14,alpah10,blpbh4);
	D1_15 = _mm512_madd52hi_epu64(D1_15,alpah10,blpbh4);
	D1_15 = _mm512_madd52lo_epu64(D1_15,alpah10,blpbh5);
	D1_16 = _mm512_madd52hi_epu64(D1_16,alpah10,blpbh5);
	D1_16 = _mm512_madd52lo_epu64(D1_16,alpah10,blpbh6);
	D1_17 = _mm512_madd52hi_epu64(D1_17,alpah10,blpbh6);
	D1_17 = _mm512_madd52lo_epu64(D1_17,alpah10,blpbh7);
	D1_18 = _mm512_madd52hi_epu64(D1_18,alpah10,blpbh7);
	D1_18 = _mm512_madd52lo_epu64(D1_18,alpah10,blpbh8);
	D1_19 = _mm512_madd52hi_epu64(D1_19,alpah10,blpbh8);
	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah10,blpbh9);

	D1_11 = _mm512_madd52lo_epu64(D1_11,alpah11,blpbh0);
	D1_12 = _mm512_madd52hi_epu64(D1_12,alpah11,blpbh0);
	D1_12 = _mm512_madd52lo_epu64(D1_12,alpah11,blpbh1);
	D1_13 = _mm512_madd52hi_epu64(D1_13,alpah11,blpbh1);
	D1_13 = _mm512_madd52lo_epu64(D1_13,alpah11,blpbh2);
	D1_14 = _mm512_madd52hi_epu64(D1_14,alpah11,blpbh2);
	D1_14 = _mm512_madd52lo_epu64(D1_14,alpah11,blpbh3);
	D1_15 = _mm512_madd52hi_epu64(D1_15,alpah11,blpbh3);
	D1_15 = _mm512_madd52lo_epu64(D1_15,alpah11,blpbh4);
	D1_16 = _mm512_madd52hi_epu64(D1_16,alpah11,blpbh4);
	D1_16 = _mm512_madd52lo_epu64(D1_16,alpah11,blpbh5);
	D1_17 = _mm512_madd52hi_epu64(D1_17,alpah11,blpbh5);
	D1_17 = _mm512_madd52lo_epu64(D1_17,alpah11,blpbh6);
	D1_18 = _mm512_madd52hi_epu64(D1_18,alpah11,blpbh6);
	D1_18 = _mm512_madd52lo_epu64(D1_18,alpah11,blpbh7);
	D1_19 = _mm512_madd52hi_epu64(D1_19,alpah11,blpbh7);
	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah11,blpbh8);

	D1_12 = _mm512_madd52lo_epu64(D1_12,alpah12,blpbh0);
	D1_13 = _mm512_madd52hi_epu64(D1_13,alpah12,blpbh0);
	D1_13 = _mm512_madd52lo_epu64(D1_13,alpah12,blpbh1);
	D1_14 = _mm512_madd52hi_epu64(D1_14,alpah12,blpbh1);
	D1_14 = _mm512_madd52lo_epu64(D1_14,alpah12,blpbh2);
	D1_15 = _mm512_madd52hi_epu64(D1_15,alpah12,blpbh2);
	D1_15 = _mm512_madd52lo_epu64(D1_15,alpah12,blpbh3);
	D1_16 = _mm512_madd52hi_epu64(D1_16,alpah12,blpbh3);
	D1_16 = _mm512_madd52lo_epu64(D1_16,alpah12,blpbh4);
	D1_17 = _mm512_madd52hi_epu64(D1_17,alpah12,blpbh4);
	D1_17 = _mm512_madd52lo_epu64(D1_17,alpah12,blpbh5);
	D1_18 = _mm512_madd52hi_epu64(D1_18,alpah12,blpbh5);
	D1_18 = _mm512_madd52lo_epu64(D1_18,alpah12,blpbh6);
	D1_19 = _mm512_madd52hi_epu64(D1_19,alpah12,blpbh6);
	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah12,blpbh7);

	D1_13 = _mm512_madd52lo_epu64(D1_13,alpah13,blpbh0);
	D1_14 = _mm512_madd52hi_epu64(D1_14,alpah13,blpbh0);
	D1_14 = _mm512_madd52lo_epu64(D1_14,alpah13,blpbh1);
	D1_15 = _mm512_madd52hi_epu64(D1_15,alpah13,blpbh1);
	D1_15 = _mm512_madd52lo_epu64(D1_15,alpah13,blpbh2);
	D1_16 = _mm512_madd52hi_epu64(D1_16,alpah13,blpbh2);
	D1_16 = _mm512_madd52lo_epu64(D1_16,alpah13,blpbh3);
	D1_17 = _mm512_madd52hi_epu64(D1_17,alpah13,blpbh3);
	D1_17 = _mm512_madd52lo_epu64(D1_17,alpah13,blpbh4);
	D1_18 = _mm512_madd52hi_epu64(D1_18,alpah13,blpbh4);
	D1_18 = _mm512_madd52lo_epu64(D1_18,alpah13,blpbh5);
	D1_19 = _mm512_madd52hi_epu64(D1_19,alpah13,blpbh5);
	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah13,blpbh6);

	D1_14 = _mm512_madd52lo_epu64(D1_14,alpah14,blpbh0);
	D1_15 = _mm512_madd52hi_epu64(D1_15,alpah14,blpbh0);
	D1_15 = _mm512_madd52lo_epu64(D1_15,alpah14,blpbh1);
	D1_16 = _mm512_madd52hi_epu64(D1_16,alpah14,blpbh1);
	D1_16 = _mm512_madd52lo_epu64(D1_16,alpah14,blpbh2);
	D1_17 = _mm512_madd52hi_epu64(D1_17,alpah14,blpbh2);
	D1_17 = _mm512_madd52lo_epu64(D1_17,alpah14,blpbh3);
	D1_18 = _mm512_madd52hi_epu64(D1_18,alpah14,blpbh3);
	D1_18 = _mm512_madd52lo_epu64(D1_18,alpah14,blpbh4);
	D1_19 = _mm512_madd52hi_epu64(D1_19,alpah14,blpbh4);
	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah14,blpbh5);

	D1_15 = _mm512_madd52lo_epu64(D1_15,alpah15,blpbh0);
	D1_16 = _mm512_madd52hi_epu64(D1_16,alpah15,blpbh0);
	D1_16 = _mm512_madd52lo_epu64(D1_16,alpah15,blpbh1);
	D1_17 = _mm512_madd52hi_epu64(D1_17,alpah15,blpbh1);
	D1_17 = _mm512_madd52lo_epu64(D1_17,alpah15,blpbh2);
	D1_18 = _mm512_madd52hi_epu64(D1_18,alpah15,blpbh2);
	D1_18 = _mm512_madd52lo_epu64(D1_18,alpah15,blpbh3);
	D1_19 = _mm512_madd52hi_epu64(D1_19,alpah15,blpbh3);
	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah15,blpbh4);

	D1_16 = _mm512_madd52lo_epu64(D1_16,alpah16,blpbh0);
	D1_17 = _mm512_madd52hi_epu64(D1_17,alpah16,blpbh0);
	D1_17 = _mm512_madd52lo_epu64(D1_17,alpah16,blpbh1);
	D1_18 = _mm512_madd52hi_epu64(D1_18,alpah16,blpbh1);
	D1_18 = _mm512_madd52lo_epu64(D1_18,alpah16,blpbh2);
	D1_19 = _mm512_madd52hi_epu64(D1_19,alpah16,blpbh2);
	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah16,blpbh3);

	D1_17 = _mm512_madd52lo_epu64(D1_17,alpah17,blpbh0);
	D1_18 = _mm512_madd52hi_epu64(D1_18,alpah17,blpbh0);
	D1_18 = _mm512_madd52lo_epu64(D1_18,alpah17,blpbh1);
	D1_19 = _mm512_madd52hi_epu64(D1_19,alpah17,blpbh1);
	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah17,blpbh2);

	D1_18 = _mm512_madd52lo_epu64(D1_18,alpah18,blpbh0);
	D1_19 = _mm512_madd52hi_epu64(D1_19,alpah18,blpbh0);
	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah18,blpbh1);

	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah19,blpbh0);


	// Carry management

	carry  = _mm512_srli_epi64(D1_0,52);
	D1_0 = _mm512_and_si512(D1_0,mask52);
	D1_1 = _mm512_add_epi64(D1_1,carry);
	carry  = _mm512_srli_epi64(D1_1,52);
	D1_2 = _mm512_add_epi64(D1_2,carry);
	D1_1 = _mm512_and_si512(D1_1,mask52);
	carry  = _mm512_srli_epi64(D1_2,52);
	D1_3 = _mm512_add_epi64(D1_3,carry);
	D1_2 = _mm512_and_si512(D1_2,mask52);
	carry  = _mm512_srli_epi64(D1_3,52);
	D1_4 = _mm512_add_epi64(D1_4,carry);
	D1_3 = _mm512_and_si512(D1_3,mask52);
	carry  = _mm512_srli_epi64(D1_4,52);
	D1_5 = _mm512_add_epi64(D1_5,carry);
	D1_4 = _mm512_and_si512(D1_4,mask52);
	carry  = _mm512_srli_epi64(D1_5,52);
	D1_6 = _mm512_add_epi64(D1_6,carry);
	D1_5 = _mm512_and_si512(D1_5,mask52);
	carry  = _mm512_srli_epi64(D1_6,52);
	D1_7 = _mm512_add_epi64(D1_7,carry);
	D1_6 = _mm512_and_si512(D1_6,mask52);
	carry  = _mm512_srli_epi64(D1_7,52);
	D1_8 = _mm512_add_epi64(D1_8,carry);
	D1_7 = _mm512_and_si512(D1_7,mask52);
	carry  = _mm512_srli_epi64(D1_8,52);
	D1_9 = _mm512_add_epi64(D1_9,carry);
	D1_8 = _mm512_and_si512(D1_8,mask52);
	carry  = _mm512_srli_epi64(D1_9,52);
	D1_10 = _mm512_add_epi64(D1_10,carry);
	D1_9 = _mm512_and_si512(D1_9,mask52);
	carry  = _mm512_srli_epi64(D1_10,52);
	D1_11 = _mm512_add_epi64(D1_11,carry);
	D1_10 = _mm512_and_si512(D1_10,mask52);
	carry  = _mm512_srli_epi64(D1_11,52);
	D1_12 = _mm512_add_epi64(D1_12,carry);
	D1_11 = _mm512_and_si512(D1_11,mask52);
	carry  = _mm512_srli_epi64(D1_12,52);
	D1_13 = _mm512_add_epi64(D1_13,carry);
	D1_12 = _mm512_and_si512(D1_12,mask52);
	carry  = _mm512_srli_epi64(D1_13,52);
	D1_14 = _mm512_add_epi64(D1_14,carry);
	D1_13 = _mm512_and_si512(D1_13,mask52);
	carry  = _mm512_srli_epi64(D1_14,52);
	D1_15 = _mm512_add_epi64(D1_15,carry);
	D1_14 = _mm512_and_si512(D1_14,mask52);
	carry  = _mm512_srli_epi64(D1_15,52);
	D1_16 = _mm512_add_epi64(D1_16,carry);
	D1_15 = _mm512_and_si512(D1_15,mask52);
	carry  = _mm512_srli_epi64(D1_16,52);
	D1_17 = _mm512_add_epi64(D1_17,carry);
	D1_16 = _mm512_and_si512(D1_16,mask52);
	carry  = _mm512_srli_epi64(D1_17,52);
	D1_18 = _mm512_add_epi64(D1_18,carry);
	D1_17 = _mm512_and_si512(D1_17,mask52);
	carry  = _mm512_srli_epi64(D1_18,52);
	D1_19 = _mm512_add_epi64(D1_19,carry);
	D1_18 = _mm512_and_si512(D1_18,mask52);

	// Final reconstruction


	// tmp = D0+D2, radix 2^1039

	__m512i tmp0 = _mm512_add_epi64(D0_0,D2_0);
	__m512i carry_tmp  = _mm512_srli_epi64(tmp0,52);
	tmp0 = _mm512_and_si512(tmp0,mask52);

	__m512i tmp1 = _mm512_add_epi64(D0_1,carry_tmp);
	tmp1 = _mm512_add_epi64(tmp1,D2_1);
	carry_tmp  = _mm512_srli_epi64(tmp1,52);
	tmp1 = _mm512_and_si512(tmp1,mask52);

	__m512i tmp2 = _mm512_add_epi64(D0_2,carry_tmp);
	tmp2 = _mm512_add_epi64(tmp2,D2_2);
	carry_tmp  = _mm512_srli_epi64(tmp2,52);
	tmp2 = _mm512_and_si512(tmp2,mask52);

	__m512i tmp3 = _mm512_add_epi64(D0_3,carry_tmp);
	tmp3 = _mm512_add_epi64(tmp3,D2_3);
	carry_tmp  = _mm512_srli_epi64(tmp3,52);
	tmp3 = _mm512_and_si512(tmp3,mask52);

	__m512i tmp4 = _mm512_add_epi64(D0_4,carry_tmp);
	tmp4 = _mm512_add_epi64(tmp4,D2_4);
	carry_tmp  = _mm512_srli_epi64(tmp4,52);
	tmp4 = _mm512_and_si512(tmp4,mask52);

	__m512i tmp5 = _mm512_add_epi64(D0_5,carry_tmp);
	tmp5 = _mm512_add_epi64(tmp5,D2_5);
	carry_tmp  = _mm512_srli_epi64(tmp5,52);
	tmp5 = _mm512_and_si512(tmp5,mask52);

	__m512i tmp6 = _mm512_add_epi64(D0_6,carry_tmp);
	tmp6 = _mm512_add_epi64(tmp6,D2_6);
	carry_tmp  = _mm512_srli_epi64(tmp6,52);
	tmp6 = _mm512_and_si512(tmp6,mask52);

	__m512i tmp7 = _mm512_add_epi64(D0_7,carry_tmp);
	tmp7 = _mm512_add_epi64(tmp7,D2_7);
	carry_tmp  = _mm512_srli_epi64(tmp7,52);
	tmp7 = _mm512_and_si512(tmp7,mask52);

	__m512i tmp8 = _mm512_add_epi64(D0_8,carry_tmp);
	tmp8 = _mm512_add_epi64(tmp8,D2_8);
	carry_tmp  = _mm512_srli_epi64(tmp8,52);
	tmp8 = _mm512_and_si512(tmp8,mask52);

	__m512i tmp9 = _mm512_add_epi64(D0_9,carry_tmp);
	tmp9 = _mm512_add_epi64(tmp9,D2_9);
	carry_tmp  = _mm512_srli_epi64(tmp9,52);
	tmp9 = _mm512_and_si512(tmp9,mask52);

	__m512i tmp10 = _mm512_add_epi64(D0_10,carry_tmp);
	tmp10 = _mm512_add_epi64(tmp10,D2_10);
	carry_tmp  = _mm512_srli_epi64(tmp10,52);
	tmp10 = _mm512_and_si512(tmp10,mask52);

	__m512i tmp11 = _mm512_add_epi64(D0_11,carry_tmp);
	tmp11 = _mm512_add_epi64(tmp11,D2_11);
	carry_tmp  = _mm512_srli_epi64(tmp11,52);
	tmp11 = _mm512_and_si512(tmp11,mask52);

	__m512i tmp12 = _mm512_add_epi64(D0_12,carry_tmp);
	tmp12 = _mm512_add_epi64(tmp12,D2_12);
	carry_tmp  = _mm512_srli_epi64(tmp12,52);
	tmp12 = _mm512_and_si512(tmp12,mask52);

	__m512i tmp13 = _mm512_add_epi64(D0_13,carry_tmp);
	tmp13 = _mm512_add_epi64(tmp13,D2_13);
	carry_tmp  = _mm512_srli_epi64(tmp13,52);
	tmp13 = _mm512_and_si512(tmp13,mask52);

	__m512i tmp14 = _mm512_add_epi64(D0_14,carry_tmp);
	tmp14 = _mm512_add_epi64(tmp14,D2_14);
	carry_tmp  = _mm512_srli_epi64(tmp14,52);
	tmp14 = _mm512_and_si512(tmp14,mask52);

	__m512i tmp15 = _mm512_add_epi64(D0_15,carry_tmp);
	tmp15 = _mm512_add_epi64(tmp15,D2_15);
	carry_tmp  = _mm512_srli_epi64(tmp15,52);
	tmp15 = _mm512_and_si512(tmp15,mask52);

	__m512i tmp16 = _mm512_add_epi64(D0_16,carry_tmp);
	tmp16 = _mm512_add_epi64(tmp16,D2_16);
	carry_tmp  = _mm512_srli_epi64(tmp16,52);
	tmp16 = _mm512_and_si512(tmp16,mask52);

	__m512i tmp17 = _mm512_add_epi64(D0_17,carry_tmp);
	tmp17 = _mm512_add_epi64(tmp17,D2_17);
	carry_tmp  = _mm512_srli_epi64(tmp17,52);
	tmp17 = _mm512_and_si512(tmp17,mask52);

	__m512i tmp18 = _mm512_add_epi64(D0_18,carry_tmp);
	tmp18 = _mm512_add_epi64(tmp18,D2_18);
	carry_tmp  = _mm512_srli_epi64(tmp18,52);
	tmp18 = _mm512_and_si512(tmp18,mask52);

	__m512i tmp19 = _mm512_add_epi64(D0_19,carry_tmp);
	tmp19 = _mm512_add_epi64(tmp19,D2_19);
	carry_tmp  = _mm512_srli_epi64(tmp19,51);
	tmp19 = _mm512_and_si512(tmp19,mask51);// radix 2**1039


	// D1 = D1 -tmp, radix 2^1039

	D1_0 = _mm512_sub_epi64(D1_0,tmp0);
	__m512i borrow = _mm512_srli_epi64(D1_0,52)&un;
	D1_0 = _mm512_and_si512(D1_0,mask52);
	borrow = _mm512_add_epi64(tmp1,borrow);
	D1_1 = _mm512_sub_epi64(D1_1,borrow);
	borrow  = _mm512_srli_epi64(D1_1,52)&un;
	D1_1 = _mm512_and_si512(D1_1,mask52);

	borrow = _mm512_add_epi64(tmp2,borrow);
	D1_2 = _mm512_sub_epi64(D1_2,borrow);
	borrow  = _mm512_srli_epi64(D1_2,52)&un;
	D1_2 = _mm512_and_si512(D1_2,mask52);

	borrow = _mm512_add_epi64(tmp3,borrow);
	D1_3 = _mm512_sub_epi64(D1_3,borrow);
	borrow  = _mm512_srli_epi64(D1_3,52)&un;
	D1_3 = _mm512_and_si512(D1_3,mask52);

	borrow = _mm512_add_epi64(tmp4,borrow);
	D1_4 = _mm512_sub_epi64(D1_4,borrow);
	borrow  = _mm512_srli_epi64(D1_4,52)&un;
	D1_4 = _mm512_and_si512(D1_4,mask52);

	borrow = _mm512_add_epi64(tmp5,borrow);
	D1_5 = _mm512_sub_epi64(D1_5,borrow);
	borrow  = _mm512_srli_epi64(D1_5,52)&un;
	D1_5 = _mm512_and_si512(D1_5,mask52);

	borrow = _mm512_add_epi64(tmp6,borrow);
	D1_6 = _mm512_sub_epi64(D1_6,borrow);
	borrow  = _mm512_srli_epi64(D1_6,52)&un;
	D1_6 = _mm512_and_si512(D1_6,mask52);

	borrow = _mm512_add_epi64(tmp7,borrow);
	D1_7 = _mm512_sub_epi64(D1_7,borrow);
	borrow  = _mm512_srli_epi64(D1_7,52)&un;
	D1_7 = _mm512_and_si512(D1_7,mask52);

	borrow = _mm512_add_epi64(tmp8,borrow);
	D1_8 = _mm512_sub_epi64(D1_8,borrow);
	borrow  = _mm512_srli_epi64(D1_8,52)&un;
	D1_8 = _mm512_and_si512(D1_8,mask52);

	borrow = _mm512_add_epi64(tmp9,borrow);
	D1_9 = _mm512_sub_epi64(D1_9,borrow);
	borrow  = _mm512_srli_epi64(D1_9,52)&un;
	D1_9 = _mm512_and_si512(D1_9,mask52);

	borrow = _mm512_add_epi64(tmp10,borrow);
	D1_10 = _mm512_sub_epi64(D1_10,borrow);
	borrow  = _mm512_srli_epi64(D1_10,52)&un;
	D1_10 = _mm512_and_si512(D1_10,mask52);

	borrow = _mm512_add_epi64(tmp11,borrow);
	D1_11 = _mm512_sub_epi64(D1_11,borrow);
	borrow  = _mm512_srli_epi64(D1_11,52)&un;
	D1_11 = _mm512_and_si512(D1_11,mask52);

	borrow = _mm512_add_epi64(tmp12,borrow);
	D1_12 = _mm512_sub_epi64(D1_12,borrow);
	borrow  = _mm512_srli_epi64(D1_12,52)&un;
	D1_12 = _mm512_and_si512(D1_12,mask52);

	borrow = _mm512_add_epi64(tmp13,borrow);
	D1_13 = _mm512_sub_epi64(D1_13,borrow);
	borrow  = _mm512_srli_epi64(D1_13,52)&un;
	D1_13 = _mm512_and_si512(D1_13,mask52);

	borrow = _mm512_add_epi64(tmp14,borrow);
	D1_14 = _mm512_sub_epi64(D1_14,borrow);
	borrow  = _mm512_srli_epi64(D1_14,52)&un;
	D1_14 = _mm512_and_si512(D1_14,mask52);

	borrow = _mm512_add_epi64(tmp15,borrow);
	D1_15 = _mm512_sub_epi64(D1_15,borrow);
	borrow  = _mm512_srli_epi64(D1_15,52)&un;
	D1_15 = _mm512_and_si512(D1_15,mask52);

	borrow = _mm512_add_epi64(tmp16,borrow);
	D1_16 = _mm512_sub_epi64(D1_16,borrow);
	borrow  = _mm512_srli_epi64(D1_16,52)&un;
	D1_16 = _mm512_and_si512(D1_16,mask52);

	borrow = _mm512_add_epi64(tmp17,borrow);
	D1_17 = _mm512_sub_epi64(D1_17,borrow);
	borrow  = _mm512_srli_epi64(D1_17,52)&un;
	D1_17 = _mm512_and_si512(D1_17,mask52);

	borrow = _mm512_add_epi64(tmp18,borrow);
	D1_18 = _mm512_sub_epi64(D1_18,borrow);
	borrow  = _mm512_srli_epi64(D1_18,52)&un;
	D1_18 = _mm512_and_si512(D1_18,mask52);

	borrow = _mm512_add_epi64(tmp19,borrow);
	D1_19 = _mm512_sub_epi64(D1_19,borrow);
	borrow = _mm512_srli_epi64(D1_19,51)&un;
	D1_19 = _mm512_and_si512(D1_19,mask51);// radix 2**1039



	_mm512_store_epi64(out+0,D0_0);
	_mm512_store_epi64(out+1,D0_1);
	_mm512_store_epi64(out+2,D0_2);
	_mm512_store_epi64(out+3,D0_3);
	_mm512_store_epi64(out+4,D0_4);
	_mm512_store_epi64(out+5,D0_5);
	_mm512_store_epi64(out+6,D0_6);
	_mm512_store_epi64(out+7,D0_7);
	_mm512_store_epi64(out+8,D0_8);
	_mm512_store_epi64(out+9,D0_9);
	_mm512_store_epi64(out+10,D0_10);
	_mm512_store_epi64(out+11,D0_11);
	_mm512_store_epi64(out+12,D0_12);
	_mm512_store_epi64(out+13,D0_13);
	_mm512_store_epi64(out+14,D0_14);
	_mm512_store_epi64(out+15,D0_15);
	_mm512_store_epi64(out+16,D0_16);
	_mm512_store_epi64(out+17,D0_17);
	_mm512_store_epi64(out+18,D0_18);
	_mm512_store_epi64(out+19,D0_19);
	_mm512_store_epi64(out+20,D1_0);
	_mm512_store_epi64(out+21,D1_1);
	_mm512_store_epi64(out+22,D1_2);
	_mm512_store_epi64(out+23,D1_3);
	_mm512_store_epi64(out+24,D1_4);
	_mm512_store_epi64(out+25,D1_5);
	_mm512_store_epi64(out+26,D1_6);
	_mm512_store_epi64(out+27,D1_7);
	_mm512_store_epi64(out+28,D1_8);
	_mm512_store_epi64(out+29,D1_9);
	_mm512_store_epi64(out+30,D1_10);
	_mm512_store_epi64(out+31,D1_11);
	_mm512_store_epi64(out+32,D1_12);
	_mm512_store_epi64(out+33,D1_13);
	_mm512_store_epi64(out+34,D1_14);
	_mm512_store_epi64(out+35,D1_15);
	_mm512_store_epi64(out+36,D1_16);
	_mm512_store_epi64(out+37,D1_17);
	_mm512_store_epi64(out+38,D1_18);
	_mm512_store_epi64(out+39,D1_19);

}


	
/* Karatsuba multiplication */

inline void force_inline mul512_8_2078K(__m512i out[80], __m512i a512[40], __m512i b512[40])
{

	// D0 = Al*Bl

	__m512i D0_0, D0_1;
	__m512i D0_2, D0_3;
	__m512i D0_4, D0_5;
	__m512i D0_6, D0_7;
	__m512i D0_8, D0_9;
	__m512i D0_10, D0_11;
	__m512i D0_12, D0_13;
	__m512i D0_14, D0_15;
	__m512i D0_16, D0_17;
	__m512i D0_18, D0_19;
	__m512i D0_20, D0_21;
	__m512i D0_22, D0_23;
	__m512i D0_24, D0_25;
	__m512i D0_26, D0_27;
	__m512i D0_28, D0_29;
	__m512i D0_30, D0_31;
	__m512i D0_32, D0_33;
	__m512i D0_34, D0_35;
	__m512i D0_36, D0_37;
	__m512i D0_38, D0_39;
	__m512i a5120 = _mm512_load_epi64(a512), b5120 = _mm512_load_epi64(b512);
	__m512i a5121 = _mm512_load_epi64(a512+1), b5121 = _mm512_load_epi64(b512+1);
	__m512i a5122 = _mm512_load_epi64(a512+2), b5122 = _mm512_load_epi64(b512+2);
	__m512i a5123 = _mm512_load_epi64(a512+3), b5123 = _mm512_load_epi64(b512+3);
	__m512i a5124 = _mm512_load_epi64(a512+4), b5124 = _mm512_load_epi64(b512+4);
	__m512i a5125 = _mm512_load_epi64(a512+5), b5125 = _mm512_load_epi64(b512+5);
	__m512i a5126 = _mm512_load_epi64(a512+6), b5126 = _mm512_load_epi64(b512+6);
	__m512i a5127 = _mm512_load_epi64(a512+7), b5127 = _mm512_load_epi64(b512+7);
	__m512i a5128 = _mm512_load_epi64(a512+8), b5128 = _mm512_load_epi64(b512+8);
	__m512i a5129 = _mm512_load_epi64(a512+9), b5129 = _mm512_load_epi64(b512+9);
	__m512i a51210 = _mm512_load_epi64(a512+10), b51210 = _mm512_load_epi64(b512+10);
	__m512i a51211 = _mm512_load_epi64(a512+11), b51211 = _mm512_load_epi64(b512+11);
	__m512i a51212 = _mm512_load_epi64(a512+12), b51212 = _mm512_load_epi64(b512+12);
	__m512i a51213 = _mm512_load_epi64(a512+13), b51213 = _mm512_load_epi64(b512+13);
	__m512i a51214 = _mm512_load_epi64(a512+14), b51214 = _mm512_load_epi64(b512+14);
	__m512i a51215 = _mm512_load_epi64(a512+15), b51215 = _mm512_load_epi64(b512+15);
	__m512i a51216 = _mm512_load_epi64(a512+16), b51216 = _mm512_load_epi64(b512+16);
	__m512i a51217 = _mm512_load_epi64(a512+17), b51217 = _mm512_load_epi64(b512+17);
	__m512i a51218 = _mm512_load_epi64(a512+18), b51218 = _mm512_load_epi64(b512+18);
	__m512i a51219 = _mm512_load_epi64(a512+19), b51219 = _mm512_load_epi64(b512+19);
	D0_0 = _mm512_madd52lo_epu64(zero,a5120,b5120);
	D0_1 = _mm512_madd52hi_epu64(zero,a5120,b5120);
	D0_1 = _mm512_madd52lo_epu64(D0_1,a5120,b5121);
	D0_2 = _mm512_madd52hi_epu64(zero,a5120,b5121);
	D0_2 = _mm512_madd52lo_epu64(D0_2,a5120,b5122);
	D0_3 = _mm512_madd52hi_epu64(zero,a5120,b5122);
	D0_3 = _mm512_madd52lo_epu64(D0_3,a5120,b5123);
	D0_4 = _mm512_madd52hi_epu64(zero,a5120,b5123);
	D0_4 = _mm512_madd52lo_epu64(D0_4,a5120,b5124);
	D0_5 = _mm512_madd52hi_epu64(zero,a5120,b5124);
	D0_5 = _mm512_madd52lo_epu64(D0_5,a5120,b5125);
	D0_6 = _mm512_madd52hi_epu64(zero,a5120,b5125);
	D0_6 = _mm512_madd52lo_epu64(D0_6,a5120,b5126);
	D0_7 = _mm512_madd52hi_epu64(zero,a5120,b5126);
	D0_7 = _mm512_madd52lo_epu64(D0_7,a5120,b5127);
	D0_8 = _mm512_madd52hi_epu64(zero,a5120,b5127);
	D0_8 = _mm512_madd52lo_epu64(D0_8,a5120,b5128);
	D0_9 = _mm512_madd52hi_epu64(zero,a5120,b5128);
	D0_9 = _mm512_madd52lo_epu64(D0_9,a5120,b5129);
	D0_10 = _mm512_madd52hi_epu64(zero,a5120,b5129);
	D0_10 = _mm512_madd52lo_epu64(D0_10,a5120,b51210);
	D0_11 = _mm512_madd52hi_epu64(zero,a5120,b51210);
	D0_11 = _mm512_madd52lo_epu64(D0_11,a5120,b51211);
	D0_12 = _mm512_madd52hi_epu64(zero,a5120,b51211);
	D0_12 = _mm512_madd52lo_epu64(D0_12,a5120,b51212);
	D0_13 = _mm512_madd52hi_epu64(zero,a5120,b51212);
	D0_13 = _mm512_madd52lo_epu64(D0_13,a5120,b51213);
	D0_14 = _mm512_madd52hi_epu64(zero,a5120,b51213);
	D0_14 = _mm512_madd52lo_epu64(D0_14,a5120,b51214);
	D0_15 = _mm512_madd52hi_epu64(zero,a5120,b51214);
	D0_15 = _mm512_madd52lo_epu64(D0_15,a5120,b51215);
	D0_16 = _mm512_madd52hi_epu64(zero,a5120,b51215);
	D0_16 = _mm512_madd52lo_epu64(D0_16,a5120,b51216);
	D0_17 = _mm512_madd52hi_epu64(zero,a5120,b51216);
	D0_17 = _mm512_madd52lo_epu64(D0_17,a5120,b51217);
	D0_18 = _mm512_madd52hi_epu64(zero,a5120,b51217);
	D0_18 = _mm512_madd52lo_epu64(D0_18,a5120,b51218);
	D0_19 = _mm512_madd52hi_epu64(zero,a5120,b51218);
	D0_19 = _mm512_madd52lo_epu64(D0_19,a5120,b51219);
	D0_20 = _mm512_madd52hi_epu64(zero,a5120,b51219);

	D0_1 = _mm512_madd52lo_epu64(D0_1,a5121,b5120);
	D0_2 = _mm512_madd52hi_epu64(D0_2,a5121,b5120);
	D0_2 = _mm512_madd52lo_epu64(D0_2,a5121,b5121);
	D0_3 = _mm512_madd52hi_epu64(D0_3,a5121,b5121);
	D0_3 = _mm512_madd52lo_epu64(D0_3,a5121,b5122);
	D0_4 = _mm512_madd52hi_epu64(D0_4,a5121,b5122);
	D0_4 = _mm512_madd52lo_epu64(D0_4,a5121,b5123);
	D0_5 = _mm512_madd52hi_epu64(D0_5,a5121,b5123);
	D0_5 = _mm512_madd52lo_epu64(D0_5,a5121,b5124);
	D0_6 = _mm512_madd52hi_epu64(D0_6,a5121,b5124);
	D0_6 = _mm512_madd52lo_epu64(D0_6,a5121,b5125);
	D0_7 = _mm512_madd52hi_epu64(D0_7,a5121,b5125);
	D0_7 = _mm512_madd52lo_epu64(D0_7,a5121,b5126);
	D0_8 = _mm512_madd52hi_epu64(D0_8,a5121,b5126);
	D0_8 = _mm512_madd52lo_epu64(D0_8,a5121,b5127);
	D0_9 = _mm512_madd52hi_epu64(D0_9,a5121,b5127);
	D0_9 = _mm512_madd52lo_epu64(D0_9,a5121,b5128);
	D0_10 = _mm512_madd52hi_epu64(D0_10,a5121,b5128);
	D0_10 = _mm512_madd52lo_epu64(D0_10,a5121,b5129);
	D0_11 = _mm512_madd52hi_epu64(D0_11,a5121,b5129);
	D0_11 = _mm512_madd52lo_epu64(D0_11,a5121,b51210);
	D0_12 = _mm512_madd52hi_epu64(D0_12,a5121,b51210);
	D0_12 = _mm512_madd52lo_epu64(D0_12,a5121,b51211);
	D0_13 = _mm512_madd52hi_epu64(D0_13,a5121,b51211);
	D0_13 = _mm512_madd52lo_epu64(D0_13,a5121,b51212);
	D0_14 = _mm512_madd52hi_epu64(D0_14,a5121,b51212);
	D0_14 = _mm512_madd52lo_epu64(D0_14,a5121,b51213);
	D0_15 = _mm512_madd52hi_epu64(D0_15,a5121,b51213);
	D0_15 = _mm512_madd52lo_epu64(D0_15,a5121,b51214);
	D0_16 = _mm512_madd52hi_epu64(D0_16,a5121,b51214);
	D0_16 = _mm512_madd52lo_epu64(D0_16,a5121,b51215);
	D0_17 = _mm512_madd52hi_epu64(D0_17,a5121,b51215);
	D0_17 = _mm512_madd52lo_epu64(D0_17,a5121,b51216);
	D0_18 = _mm512_madd52hi_epu64(D0_18,a5121,b51216);
	D0_18 = _mm512_madd52lo_epu64(D0_18,a5121,b51217);
	D0_19 = _mm512_madd52hi_epu64(D0_19,a5121,b51217);
	D0_19 = _mm512_madd52lo_epu64(D0_19,a5121,b51218);
	D0_20 = _mm512_madd52hi_epu64(D0_20,a5121,b51218);
	D0_20 = _mm512_madd52lo_epu64(D0_20,a5121,b51219);
	D0_21 = _mm512_madd52hi_epu64(zero,a5121,b51219);

	D0_2 = _mm512_madd52lo_epu64(D0_2,a5122,b5120);
	D0_3 = _mm512_madd52hi_epu64(D0_3,a5122,b5120);
	D0_3 = _mm512_madd52lo_epu64(D0_3,a5122,b5121);
	D0_4 = _mm512_madd52hi_epu64(D0_4,a5122,b5121);
	D0_4 = _mm512_madd52lo_epu64(D0_4,a5122,b5122);
	D0_5 = _mm512_madd52hi_epu64(D0_5,a5122,b5122);
	D0_5 = _mm512_madd52lo_epu64(D0_5,a5122,b5123);
	D0_6 = _mm512_madd52hi_epu64(D0_6,a5122,b5123);
	D0_6 = _mm512_madd52lo_epu64(D0_6,a5122,b5124);
	D0_7 = _mm512_madd52hi_epu64(D0_7,a5122,b5124);
	D0_7 = _mm512_madd52lo_epu64(D0_7,a5122,b5125);
	D0_8 = _mm512_madd52hi_epu64(D0_8,a5122,b5125);
	D0_8 = _mm512_madd52lo_epu64(D0_8,a5122,b5126);
	D0_9 = _mm512_madd52hi_epu64(D0_9,a5122,b5126);
	D0_9 = _mm512_madd52lo_epu64(D0_9,a5122,b5127);
	D0_10 = _mm512_madd52hi_epu64(D0_10,a5122,b5127);
	D0_10 = _mm512_madd52lo_epu64(D0_10,a5122,b5128);
	D0_11 = _mm512_madd52hi_epu64(D0_11,a5122,b5128);
	D0_11 = _mm512_madd52lo_epu64(D0_11,a5122,b5129);
	D0_12 = _mm512_madd52hi_epu64(D0_12,a5122,b5129);
	D0_12 = _mm512_madd52lo_epu64(D0_12,a5122,b51210);
	D0_13 = _mm512_madd52hi_epu64(D0_13,a5122,b51210);
	D0_13 = _mm512_madd52lo_epu64(D0_13,a5122,b51211);
	D0_14 = _mm512_madd52hi_epu64(D0_14,a5122,b51211);
	D0_14 = _mm512_madd52lo_epu64(D0_14,a5122,b51212);
	D0_15 = _mm512_madd52hi_epu64(D0_15,a5122,b51212);
	D0_15 = _mm512_madd52lo_epu64(D0_15,a5122,b51213);
	D0_16 = _mm512_madd52hi_epu64(D0_16,a5122,b51213);
	D0_16 = _mm512_madd52lo_epu64(D0_16,a5122,b51214);
	D0_17 = _mm512_madd52hi_epu64(D0_17,a5122,b51214);
	D0_17 = _mm512_madd52lo_epu64(D0_17,a5122,b51215);
	D0_18 = _mm512_madd52hi_epu64(D0_18,a5122,b51215);
	D0_18 = _mm512_madd52lo_epu64(D0_18,a5122,b51216);
	D0_19 = _mm512_madd52hi_epu64(D0_19,a5122,b51216);
	D0_19 = _mm512_madd52lo_epu64(D0_19,a5122,b51217);
	D0_20 = _mm512_madd52hi_epu64(D0_20,a5122,b51217);
	D0_20 = _mm512_madd52lo_epu64(D0_20,a5122,b51218);
	D0_21 = _mm512_madd52hi_epu64(D0_21,a5122,b51218);
	D0_21 = _mm512_madd52lo_epu64(D0_21,a5122,b51219);
	D0_22 = _mm512_madd52hi_epu64(zero,a5122,b51219);

	D0_3 = _mm512_madd52lo_epu64(D0_3,a5123,b5120);
	D0_4 = _mm512_madd52hi_epu64(D0_4,a5123,b5120);
	D0_4 = _mm512_madd52lo_epu64(D0_4,a5123,b5121);
	D0_5 = _mm512_madd52hi_epu64(D0_5,a5123,b5121);
	D0_5 = _mm512_madd52lo_epu64(D0_5,a5123,b5122);
	D0_6 = _mm512_madd52hi_epu64(D0_6,a5123,b5122);
	D0_6 = _mm512_madd52lo_epu64(D0_6,a5123,b5123);
	D0_7 = _mm512_madd52hi_epu64(D0_7,a5123,b5123);
	D0_7 = _mm512_madd52lo_epu64(D0_7,a5123,b5124);
	D0_8 = _mm512_madd52hi_epu64(D0_8,a5123,b5124);
	D0_8 = _mm512_madd52lo_epu64(D0_8,a5123,b5125);
	D0_9 = _mm512_madd52hi_epu64(D0_9,a5123,b5125);
	D0_9 = _mm512_madd52lo_epu64(D0_9,a5123,b5126);
	D0_10 = _mm512_madd52hi_epu64(D0_10,a5123,b5126);
	D0_10 = _mm512_madd52lo_epu64(D0_10,a5123,b5127);
	D0_11 = _mm512_madd52hi_epu64(D0_11,a5123,b5127);
	D0_11 = _mm512_madd52lo_epu64(D0_11,a5123,b5128);
	D0_12 = _mm512_madd52hi_epu64(D0_12,a5123,b5128);
	D0_12 = _mm512_madd52lo_epu64(D0_12,a5123,b5129);
	D0_13 = _mm512_madd52hi_epu64(D0_13,a5123,b5129);
	D0_13 = _mm512_madd52lo_epu64(D0_13,a5123,b51210);
	D0_14 = _mm512_madd52hi_epu64(D0_14,a5123,b51210);
	D0_14 = _mm512_madd52lo_epu64(D0_14,a5123,b51211);
	D0_15 = _mm512_madd52hi_epu64(D0_15,a5123,b51211);
	D0_15 = _mm512_madd52lo_epu64(D0_15,a5123,b51212);
	D0_16 = _mm512_madd52hi_epu64(D0_16,a5123,b51212);
	D0_16 = _mm512_madd52lo_epu64(D0_16,a5123,b51213);
	D0_17 = _mm512_madd52hi_epu64(D0_17,a5123,b51213);
	D0_17 = _mm512_madd52lo_epu64(D0_17,a5123,b51214);
	D0_18 = _mm512_madd52hi_epu64(D0_18,a5123,b51214);
	D0_18 = _mm512_madd52lo_epu64(D0_18,a5123,b51215);
	D0_19 = _mm512_madd52hi_epu64(D0_19,a5123,b51215);
	D0_19 = _mm512_madd52lo_epu64(D0_19,a5123,b51216);
	D0_20 = _mm512_madd52hi_epu64(D0_20,a5123,b51216);
	D0_20 = _mm512_madd52lo_epu64(D0_20,a5123,b51217);
	D0_21 = _mm512_madd52hi_epu64(D0_21,a5123,b51217);
	D0_21 = _mm512_madd52lo_epu64(D0_21,a5123,b51218);
	D0_22 = _mm512_madd52hi_epu64(D0_22,a5123,b51218);
	D0_22 = _mm512_madd52lo_epu64(D0_22,a5123,b51219);
	D0_23 = _mm512_madd52hi_epu64(zero,a5123,b51219);

	D0_4 = _mm512_madd52lo_epu64(D0_4,a5124,b5120);
	D0_5 = _mm512_madd52hi_epu64(D0_5,a5124,b5120);
	D0_5 = _mm512_madd52lo_epu64(D0_5,a5124,b5121);
	D0_6 = _mm512_madd52hi_epu64(D0_6,a5124,b5121);
	D0_6 = _mm512_madd52lo_epu64(D0_6,a5124,b5122);
	D0_7 = _mm512_madd52hi_epu64(D0_7,a5124,b5122);
	D0_7 = _mm512_madd52lo_epu64(D0_7,a5124,b5123);
	D0_8 = _mm512_madd52hi_epu64(D0_8,a5124,b5123);
	D0_8 = _mm512_madd52lo_epu64(D0_8,a5124,b5124);
	D0_9 = _mm512_madd52hi_epu64(D0_9,a5124,b5124);
	D0_9 = _mm512_madd52lo_epu64(D0_9,a5124,b5125);
	D0_10 = _mm512_madd52hi_epu64(D0_10,a5124,b5125);
	D0_10 = _mm512_madd52lo_epu64(D0_10,a5124,b5126);
	D0_11 = _mm512_madd52hi_epu64(D0_11,a5124,b5126);
	D0_11 = _mm512_madd52lo_epu64(D0_11,a5124,b5127);
	D0_12 = _mm512_madd52hi_epu64(D0_12,a5124,b5127);
	D0_12 = _mm512_madd52lo_epu64(D0_12,a5124,b5128);
	D0_13 = _mm512_madd52hi_epu64(D0_13,a5124,b5128);
	D0_13 = _mm512_madd52lo_epu64(D0_13,a5124,b5129);
	D0_14 = _mm512_madd52hi_epu64(D0_14,a5124,b5129);
	D0_14 = _mm512_madd52lo_epu64(D0_14,a5124,b51210);
	D0_15 = _mm512_madd52hi_epu64(D0_15,a5124,b51210);
	D0_15 = _mm512_madd52lo_epu64(D0_15,a5124,b51211);
	D0_16 = _mm512_madd52hi_epu64(D0_16,a5124,b51211);
	D0_16 = _mm512_madd52lo_epu64(D0_16,a5124,b51212);
	D0_17 = _mm512_madd52hi_epu64(D0_17,a5124,b51212);
	D0_17 = _mm512_madd52lo_epu64(D0_17,a5124,b51213);
	D0_18 = _mm512_madd52hi_epu64(D0_18,a5124,b51213);
	D0_18 = _mm512_madd52lo_epu64(D0_18,a5124,b51214);
	D0_19 = _mm512_madd52hi_epu64(D0_19,a5124,b51214);
	D0_19 = _mm512_madd52lo_epu64(D0_19,a5124,b51215);
	D0_20 = _mm512_madd52hi_epu64(D0_20,a5124,b51215);
	D0_20 = _mm512_madd52lo_epu64(D0_20,a5124,b51216);
	D0_21 = _mm512_madd52hi_epu64(D0_21,a5124,b51216);
	D0_21 = _mm512_madd52lo_epu64(D0_21,a5124,b51217);
	D0_22 = _mm512_madd52hi_epu64(D0_22,a5124,b51217);
	D0_22 = _mm512_madd52lo_epu64(D0_22,a5124,b51218);
	D0_23 = _mm512_madd52hi_epu64(D0_23,a5124,b51218);
	D0_23 = _mm512_madd52lo_epu64(D0_23,a5124,b51219);
	D0_24 = _mm512_madd52hi_epu64(zero,a5124,b51219);

	D0_5 = _mm512_madd52lo_epu64(D0_5,a5125,b5120);
	D0_6 = _mm512_madd52hi_epu64(D0_6,a5125,b5120);
	D0_6 = _mm512_madd52lo_epu64(D0_6,a5125,b5121);
	D0_7 = _mm512_madd52hi_epu64(D0_7,a5125,b5121);
	D0_7 = _mm512_madd52lo_epu64(D0_7,a5125,b5122);
	D0_8 = _mm512_madd52hi_epu64(D0_8,a5125,b5122);
	D0_8 = _mm512_madd52lo_epu64(D0_8,a5125,b5123);
	D0_9 = _mm512_madd52hi_epu64(D0_9,a5125,b5123);
	D0_9 = _mm512_madd52lo_epu64(D0_9,a5125,b5124);
	D0_10 = _mm512_madd52hi_epu64(D0_10,a5125,b5124);
	D0_10 = _mm512_madd52lo_epu64(D0_10,a5125,b5125);
	D0_11 = _mm512_madd52hi_epu64(D0_11,a5125,b5125);
	D0_11 = _mm512_madd52lo_epu64(D0_11,a5125,b5126);
	D0_12 = _mm512_madd52hi_epu64(D0_12,a5125,b5126);
	D0_12 = _mm512_madd52lo_epu64(D0_12,a5125,b5127);
	D0_13 = _mm512_madd52hi_epu64(D0_13,a5125,b5127);
	D0_13 = _mm512_madd52lo_epu64(D0_13,a5125,b5128);
	D0_14 = _mm512_madd52hi_epu64(D0_14,a5125,b5128);
	D0_14 = _mm512_madd52lo_epu64(D0_14,a5125,b5129);
	D0_15 = _mm512_madd52hi_epu64(D0_15,a5125,b5129);
	D0_15 = _mm512_madd52lo_epu64(D0_15,a5125,b51210);
	D0_16 = _mm512_madd52hi_epu64(D0_16,a5125,b51210);
	D0_16 = _mm512_madd52lo_epu64(D0_16,a5125,b51211);
	D0_17 = _mm512_madd52hi_epu64(D0_17,a5125,b51211);
	D0_17 = _mm512_madd52lo_epu64(D0_17,a5125,b51212);
	D0_18 = _mm512_madd52hi_epu64(D0_18,a5125,b51212);
	D0_18 = _mm512_madd52lo_epu64(D0_18,a5125,b51213);
	D0_19 = _mm512_madd52hi_epu64(D0_19,a5125,b51213);
	D0_19 = _mm512_madd52lo_epu64(D0_19,a5125,b51214);
	D0_20 = _mm512_madd52hi_epu64(D0_20,a5125,b51214);
	D0_20 = _mm512_madd52lo_epu64(D0_20,a5125,b51215);
	D0_21 = _mm512_madd52hi_epu64(D0_21,a5125,b51215);
	D0_21 = _mm512_madd52lo_epu64(D0_21,a5125,b51216);
	D0_22 = _mm512_madd52hi_epu64(D0_22,a5125,b51216);
	D0_22 = _mm512_madd52lo_epu64(D0_22,a5125,b51217);
	D0_23 = _mm512_madd52hi_epu64(D0_23,a5125,b51217);
	D0_23 = _mm512_madd52lo_epu64(D0_23,a5125,b51218);
	D0_24 = _mm512_madd52hi_epu64(D0_24,a5125,b51218);
	D0_24 = _mm512_madd52lo_epu64(D0_24,a5125,b51219);
	D0_25 = _mm512_madd52hi_epu64(zero,a5125,b51219);

	D0_6 = _mm512_madd52lo_epu64(D0_6,a5126,b5120);
	D0_7 = _mm512_madd52hi_epu64(D0_7,a5126,b5120);
	D0_7 = _mm512_madd52lo_epu64(D0_7,a5126,b5121);
	D0_8 = _mm512_madd52hi_epu64(D0_8,a5126,b5121);
	D0_8 = _mm512_madd52lo_epu64(D0_8,a5126,b5122);
	D0_9 = _mm512_madd52hi_epu64(D0_9,a5126,b5122);
	D0_9 = _mm512_madd52lo_epu64(D0_9,a5126,b5123);
	D0_10 = _mm512_madd52hi_epu64(D0_10,a5126,b5123);
	D0_10 = _mm512_madd52lo_epu64(D0_10,a5126,b5124);
	D0_11 = _mm512_madd52hi_epu64(D0_11,a5126,b5124);
	D0_11 = _mm512_madd52lo_epu64(D0_11,a5126,b5125);
	D0_12 = _mm512_madd52hi_epu64(D0_12,a5126,b5125);
	D0_12 = _mm512_madd52lo_epu64(D0_12,a5126,b5126);
	D0_13 = _mm512_madd52hi_epu64(D0_13,a5126,b5126);
	D0_13 = _mm512_madd52lo_epu64(D0_13,a5126,b5127);
	D0_14 = _mm512_madd52hi_epu64(D0_14,a5126,b5127);
	D0_14 = _mm512_madd52lo_epu64(D0_14,a5126,b5128);
	D0_15 = _mm512_madd52hi_epu64(D0_15,a5126,b5128);
	D0_15 = _mm512_madd52lo_epu64(D0_15,a5126,b5129);
	D0_16 = _mm512_madd52hi_epu64(D0_16,a5126,b5129);
	D0_16 = _mm512_madd52lo_epu64(D0_16,a5126,b51210);
	D0_17 = _mm512_madd52hi_epu64(D0_17,a5126,b51210);
	D0_17 = _mm512_madd52lo_epu64(D0_17,a5126,b51211);
	D0_18 = _mm512_madd52hi_epu64(D0_18,a5126,b51211);
	D0_18 = _mm512_madd52lo_epu64(D0_18,a5126,b51212);
	D0_19 = _mm512_madd52hi_epu64(D0_19,a5126,b51212);
	D0_19 = _mm512_madd52lo_epu64(D0_19,a5126,b51213);
	D0_20 = _mm512_madd52hi_epu64(D0_20,a5126,b51213);
	D0_20 = _mm512_madd52lo_epu64(D0_20,a5126,b51214);
	D0_21 = _mm512_madd52hi_epu64(D0_21,a5126,b51214);
	D0_21 = _mm512_madd52lo_epu64(D0_21,a5126,b51215);
	D0_22 = _mm512_madd52hi_epu64(D0_22,a5126,b51215);
	D0_22 = _mm512_madd52lo_epu64(D0_22,a5126,b51216);
	D0_23 = _mm512_madd52hi_epu64(D0_23,a5126,b51216);
	D0_23 = _mm512_madd52lo_epu64(D0_23,a5126,b51217);
	D0_24 = _mm512_madd52hi_epu64(D0_24,a5126,b51217);
	D0_24 = _mm512_madd52lo_epu64(D0_24,a5126,b51218);
	D0_25 = _mm512_madd52hi_epu64(D0_25,a5126,b51218);
	D0_25 = _mm512_madd52lo_epu64(D0_25,a5126,b51219);
	D0_26 = _mm512_madd52hi_epu64(zero,a5126,b51219);

	D0_7 = _mm512_madd52lo_epu64(D0_7,a5127,b5120);
	D0_8 = _mm512_madd52hi_epu64(D0_8,a5127,b5120);
	D0_8 = _mm512_madd52lo_epu64(D0_8,a5127,b5121);
	D0_9 = _mm512_madd52hi_epu64(D0_9,a5127,b5121);
	D0_9 = _mm512_madd52lo_epu64(D0_9,a5127,b5122);
	D0_10 = _mm512_madd52hi_epu64(D0_10,a5127,b5122);
	D0_10 = _mm512_madd52lo_epu64(D0_10,a5127,b5123);
	D0_11 = _mm512_madd52hi_epu64(D0_11,a5127,b5123);
	D0_11 = _mm512_madd52lo_epu64(D0_11,a5127,b5124);
	D0_12 = _mm512_madd52hi_epu64(D0_12,a5127,b5124);
	D0_12 = _mm512_madd52lo_epu64(D0_12,a5127,b5125);
	D0_13 = _mm512_madd52hi_epu64(D0_13,a5127,b5125);
	D0_13 = _mm512_madd52lo_epu64(D0_13,a5127,b5126);
	D0_14 = _mm512_madd52hi_epu64(D0_14,a5127,b5126);
	D0_14 = _mm512_madd52lo_epu64(D0_14,a5127,b5127);
	D0_15 = _mm512_madd52hi_epu64(D0_15,a5127,b5127);
	D0_15 = _mm512_madd52lo_epu64(D0_15,a5127,b5128);
	D0_16 = _mm512_madd52hi_epu64(D0_16,a5127,b5128);
	D0_16 = _mm512_madd52lo_epu64(D0_16,a5127,b5129);
	D0_17 = _mm512_madd52hi_epu64(D0_17,a5127,b5129);
	D0_17 = _mm512_madd52lo_epu64(D0_17,a5127,b51210);
	D0_18 = _mm512_madd52hi_epu64(D0_18,a5127,b51210);
	D0_18 = _mm512_madd52lo_epu64(D0_18,a5127,b51211);
	D0_19 = _mm512_madd52hi_epu64(D0_19,a5127,b51211);
	D0_19 = _mm512_madd52lo_epu64(D0_19,a5127,b51212);
	D0_20 = _mm512_madd52hi_epu64(D0_20,a5127,b51212);
	D0_20 = _mm512_madd52lo_epu64(D0_20,a5127,b51213);
	D0_21 = _mm512_madd52hi_epu64(D0_21,a5127,b51213);
	D0_21 = _mm512_madd52lo_epu64(D0_21,a5127,b51214);
	D0_22 = _mm512_madd52hi_epu64(D0_22,a5127,b51214);
	D0_22 = _mm512_madd52lo_epu64(D0_22,a5127,b51215);
	D0_23 = _mm512_madd52hi_epu64(D0_23,a5127,b51215);
	D0_23 = _mm512_madd52lo_epu64(D0_23,a5127,b51216);
	D0_24 = _mm512_madd52hi_epu64(D0_24,a5127,b51216);
	D0_24 = _mm512_madd52lo_epu64(D0_24,a5127,b51217);
	D0_25 = _mm512_madd52hi_epu64(D0_25,a5127,b51217);
	D0_25 = _mm512_madd52lo_epu64(D0_25,a5127,b51218);
	D0_26 = _mm512_madd52hi_epu64(D0_26,a5127,b51218);
	D0_26 = _mm512_madd52lo_epu64(D0_26,a5127,b51219);
	D0_27 = _mm512_madd52hi_epu64(zero,a5127,b51219);

	D0_8 = _mm512_madd52lo_epu64(D0_8,a5128,b5120);
	D0_9 = _mm512_madd52hi_epu64(D0_9,a5128,b5120);
	D0_9 = _mm512_madd52lo_epu64(D0_9,a5128,b5121);
	D0_10 = _mm512_madd52hi_epu64(D0_10,a5128,b5121);
	D0_10 = _mm512_madd52lo_epu64(D0_10,a5128,b5122);
	D0_11 = _mm512_madd52hi_epu64(D0_11,a5128,b5122);
	D0_11 = _mm512_madd52lo_epu64(D0_11,a5128,b5123);
	D0_12 = _mm512_madd52hi_epu64(D0_12,a5128,b5123);
	D0_12 = _mm512_madd52lo_epu64(D0_12,a5128,b5124);
	D0_13 = _mm512_madd52hi_epu64(D0_13,a5128,b5124);
	D0_13 = _mm512_madd52lo_epu64(D0_13,a5128,b5125);
	D0_14 = _mm512_madd52hi_epu64(D0_14,a5128,b5125);
	D0_14 = _mm512_madd52lo_epu64(D0_14,a5128,b5126);
	D0_15 = _mm512_madd52hi_epu64(D0_15,a5128,b5126);
	D0_15 = _mm512_madd52lo_epu64(D0_15,a5128,b5127);
	D0_16 = _mm512_madd52hi_epu64(D0_16,a5128,b5127);
	D0_16 = _mm512_madd52lo_epu64(D0_16,a5128,b5128);
	D0_17 = _mm512_madd52hi_epu64(D0_17,a5128,b5128);
	D0_17 = _mm512_madd52lo_epu64(D0_17,a5128,b5129);
	D0_18 = _mm512_madd52hi_epu64(D0_18,a5128,b5129);
	D0_18 = _mm512_madd52lo_epu64(D0_18,a5128,b51210);
	D0_19 = _mm512_madd52hi_epu64(D0_19,a5128,b51210);
	D0_19 = _mm512_madd52lo_epu64(D0_19,a5128,b51211);
	D0_20 = _mm512_madd52hi_epu64(D0_20,a5128,b51211);
	D0_20 = _mm512_madd52lo_epu64(D0_20,a5128,b51212);
	D0_21 = _mm512_madd52hi_epu64(D0_21,a5128,b51212);
	D0_21 = _mm512_madd52lo_epu64(D0_21,a5128,b51213);
	D0_22 = _mm512_madd52hi_epu64(D0_22,a5128,b51213);
	D0_22 = _mm512_madd52lo_epu64(D0_22,a5128,b51214);
	D0_23 = _mm512_madd52hi_epu64(D0_23,a5128,b51214);
	D0_23 = _mm512_madd52lo_epu64(D0_23,a5128,b51215);
	D0_24 = _mm512_madd52hi_epu64(D0_24,a5128,b51215);
	D0_24 = _mm512_madd52lo_epu64(D0_24,a5128,b51216);
	D0_25 = _mm512_madd52hi_epu64(D0_25,a5128,b51216);
	D0_25 = _mm512_madd52lo_epu64(D0_25,a5128,b51217);
	D0_26 = _mm512_madd52hi_epu64(D0_26,a5128,b51217);
	D0_26 = _mm512_madd52lo_epu64(D0_26,a5128,b51218);
	D0_27 = _mm512_madd52hi_epu64(D0_27,a5128,b51218);
	D0_27 = _mm512_madd52lo_epu64(D0_27,a5128,b51219);
	D0_28 = _mm512_madd52hi_epu64(zero,a5128,b51219);

	D0_9 = _mm512_madd52lo_epu64(D0_9,a5129,b5120);
	D0_10 = _mm512_madd52hi_epu64(D0_10,a5129,b5120);
	D0_10 = _mm512_madd52lo_epu64(D0_10,a5129,b5121);
	D0_11 = _mm512_madd52hi_epu64(D0_11,a5129,b5121);
	D0_11 = _mm512_madd52lo_epu64(D0_11,a5129,b5122);
	D0_12 = _mm512_madd52hi_epu64(D0_12,a5129,b5122);
	D0_12 = _mm512_madd52lo_epu64(D0_12,a5129,b5123);
	D0_13 = _mm512_madd52hi_epu64(D0_13,a5129,b5123);
	D0_13 = _mm512_madd52lo_epu64(D0_13,a5129,b5124);
	D0_14 = _mm512_madd52hi_epu64(D0_14,a5129,b5124);
	D0_14 = _mm512_madd52lo_epu64(D0_14,a5129,b5125);
	D0_15 = _mm512_madd52hi_epu64(D0_15,a5129,b5125);
	D0_15 = _mm512_madd52lo_epu64(D0_15,a5129,b5126);
	D0_16 = _mm512_madd52hi_epu64(D0_16,a5129,b5126);
	D0_16 = _mm512_madd52lo_epu64(D0_16,a5129,b5127);
	D0_17 = _mm512_madd52hi_epu64(D0_17,a5129,b5127);
	D0_17 = _mm512_madd52lo_epu64(D0_17,a5129,b5128);
	D0_18 = _mm512_madd52hi_epu64(D0_18,a5129,b5128);
	D0_18 = _mm512_madd52lo_epu64(D0_18,a5129,b5129);
	D0_19 = _mm512_madd52hi_epu64(D0_19,a5129,b5129);
	D0_19 = _mm512_madd52lo_epu64(D0_19,a5129,b51210);
	D0_20 = _mm512_madd52hi_epu64(D0_20,a5129,b51210);
	D0_20 = _mm512_madd52lo_epu64(D0_20,a5129,b51211);
	D0_21 = _mm512_madd52hi_epu64(D0_21,a5129,b51211);
	D0_21 = _mm512_madd52lo_epu64(D0_21,a5129,b51212);
	D0_22 = _mm512_madd52hi_epu64(D0_22,a5129,b51212);
	D0_22 = _mm512_madd52lo_epu64(D0_22,a5129,b51213);
	D0_23 = _mm512_madd52hi_epu64(D0_23,a5129,b51213);
	D0_23 = _mm512_madd52lo_epu64(D0_23,a5129,b51214);
	D0_24 = _mm512_madd52hi_epu64(D0_24,a5129,b51214);
	D0_24 = _mm512_madd52lo_epu64(D0_24,a5129,b51215);
	D0_25 = _mm512_madd52hi_epu64(D0_25,a5129,b51215);
	D0_25 = _mm512_madd52lo_epu64(D0_25,a5129,b51216);
	D0_26 = _mm512_madd52hi_epu64(D0_26,a5129,b51216);
	D0_26 = _mm512_madd52lo_epu64(D0_26,a5129,b51217);
	D0_27 = _mm512_madd52hi_epu64(D0_27,a5129,b51217);
	D0_27 = _mm512_madd52lo_epu64(D0_27,a5129,b51218);
	D0_28 = _mm512_madd52hi_epu64(D0_28,a5129,b51218);
	D0_28 = _mm512_madd52lo_epu64(D0_28,a5129,b51219);
	D0_29 = _mm512_madd52hi_epu64(zero,a5129,b51219);

	D0_10 = _mm512_madd52lo_epu64(D0_10,a51210,b5120);
	D0_11 = _mm512_madd52hi_epu64(D0_11,a51210,b5120);
	D0_11 = _mm512_madd52lo_epu64(D0_11,a51210,b5121);
	D0_12 = _mm512_madd52hi_epu64(D0_12,a51210,b5121);
	D0_12 = _mm512_madd52lo_epu64(D0_12,a51210,b5122);
	D0_13 = _mm512_madd52hi_epu64(D0_13,a51210,b5122);
	D0_13 = _mm512_madd52lo_epu64(D0_13,a51210,b5123);
	D0_14 = _mm512_madd52hi_epu64(D0_14,a51210,b5123);
	D0_14 = _mm512_madd52lo_epu64(D0_14,a51210,b5124);
	D0_15 = _mm512_madd52hi_epu64(D0_15,a51210,b5124);
	D0_15 = _mm512_madd52lo_epu64(D0_15,a51210,b5125);
	D0_16 = _mm512_madd52hi_epu64(D0_16,a51210,b5125);
	D0_16 = _mm512_madd52lo_epu64(D0_16,a51210,b5126);
	D0_17 = _mm512_madd52hi_epu64(D0_17,a51210,b5126);
	D0_17 = _mm512_madd52lo_epu64(D0_17,a51210,b5127);
	D0_18 = _mm512_madd52hi_epu64(D0_18,a51210,b5127);
	D0_18 = _mm512_madd52lo_epu64(D0_18,a51210,b5128);
	D0_19 = _mm512_madd52hi_epu64(D0_19,a51210,b5128);
	D0_19 = _mm512_madd52lo_epu64(D0_19,a51210,b5129);
	D0_20 = _mm512_madd52hi_epu64(D0_20,a51210,b5129);
	D0_20 = _mm512_madd52lo_epu64(D0_20,a51210,b51210);
	D0_21 = _mm512_madd52hi_epu64(D0_21,a51210,b51210);
	D0_21 = _mm512_madd52lo_epu64(D0_21,a51210,b51211);
	D0_22 = _mm512_madd52hi_epu64(D0_22,a51210,b51211);
	D0_22 = _mm512_madd52lo_epu64(D0_22,a51210,b51212);
	D0_23 = _mm512_madd52hi_epu64(D0_23,a51210,b51212);
	D0_23 = _mm512_madd52lo_epu64(D0_23,a51210,b51213);
	D0_24 = _mm512_madd52hi_epu64(D0_24,a51210,b51213);
	D0_24 = _mm512_madd52lo_epu64(D0_24,a51210,b51214);
	D0_25 = _mm512_madd52hi_epu64(D0_25,a51210,b51214);
	D0_25 = _mm512_madd52lo_epu64(D0_25,a51210,b51215);
	D0_26 = _mm512_madd52hi_epu64(D0_26,a51210,b51215);
	D0_26 = _mm512_madd52lo_epu64(D0_26,a51210,b51216);
	D0_27 = _mm512_madd52hi_epu64(D0_27,a51210,b51216);
	D0_27 = _mm512_madd52lo_epu64(D0_27,a51210,b51217);
	D0_28 = _mm512_madd52hi_epu64(D0_28,a51210,b51217);
	D0_28 = _mm512_madd52lo_epu64(D0_28,a51210,b51218);
	D0_29 = _mm512_madd52hi_epu64(D0_29,a51210,b51218);
	D0_29 = _mm512_madd52lo_epu64(D0_29,a51210,b51219);
	D0_30 = _mm512_madd52hi_epu64(zero,a51210,b51219);

	D0_11 = _mm512_madd52lo_epu64(D0_11,a51211,b5120);
	D0_12 = _mm512_madd52hi_epu64(D0_12,a51211,b5120);
	D0_12 = _mm512_madd52lo_epu64(D0_12,a51211,b5121);
	D0_13 = _mm512_madd52hi_epu64(D0_13,a51211,b5121);
	D0_13 = _mm512_madd52lo_epu64(D0_13,a51211,b5122);
	D0_14 = _mm512_madd52hi_epu64(D0_14,a51211,b5122);
	D0_14 = _mm512_madd52lo_epu64(D0_14,a51211,b5123);
	D0_15 = _mm512_madd52hi_epu64(D0_15,a51211,b5123);
	D0_15 = _mm512_madd52lo_epu64(D0_15,a51211,b5124);
	D0_16 = _mm512_madd52hi_epu64(D0_16,a51211,b5124);
	D0_16 = _mm512_madd52lo_epu64(D0_16,a51211,b5125);
	D0_17 = _mm512_madd52hi_epu64(D0_17,a51211,b5125);
	D0_17 = _mm512_madd52lo_epu64(D0_17,a51211,b5126);
	D0_18 = _mm512_madd52hi_epu64(D0_18,a51211,b5126);
	D0_18 = _mm512_madd52lo_epu64(D0_18,a51211,b5127);
	D0_19 = _mm512_madd52hi_epu64(D0_19,a51211,b5127);
	D0_19 = _mm512_madd52lo_epu64(D0_19,a51211,b5128);
	D0_20 = _mm512_madd52hi_epu64(D0_20,a51211,b5128);
	D0_20 = _mm512_madd52lo_epu64(D0_20,a51211,b5129);
	D0_21 = _mm512_madd52hi_epu64(D0_21,a51211,b5129);
	D0_21 = _mm512_madd52lo_epu64(D0_21,a51211,b51210);
	D0_22 = _mm512_madd52hi_epu64(D0_22,a51211,b51210);
	D0_22 = _mm512_madd52lo_epu64(D0_22,a51211,b51211);
	D0_23 = _mm512_madd52hi_epu64(D0_23,a51211,b51211);
	D0_23 = _mm512_madd52lo_epu64(D0_23,a51211,b51212);
	D0_24 = _mm512_madd52hi_epu64(D0_24,a51211,b51212);
	D0_24 = _mm512_madd52lo_epu64(D0_24,a51211,b51213);
	D0_25 = _mm512_madd52hi_epu64(D0_25,a51211,b51213);
	D0_25 = _mm512_madd52lo_epu64(D0_25,a51211,b51214);
	D0_26 = _mm512_madd52hi_epu64(D0_26,a51211,b51214);
	D0_26 = _mm512_madd52lo_epu64(D0_26,a51211,b51215);
	D0_27 = _mm512_madd52hi_epu64(D0_27,a51211,b51215);
	D0_27 = _mm512_madd52lo_epu64(D0_27,a51211,b51216);
	D0_28 = _mm512_madd52hi_epu64(D0_28,a51211,b51216);
	D0_28 = _mm512_madd52lo_epu64(D0_28,a51211,b51217);
	D0_29 = _mm512_madd52hi_epu64(D0_29,a51211,b51217);
	D0_29 = _mm512_madd52lo_epu64(D0_29,a51211,b51218);
	D0_30 = _mm512_madd52hi_epu64(D0_30,a51211,b51218);
	D0_30 = _mm512_madd52lo_epu64(D0_30,a51211,b51219);
	D0_31 = _mm512_madd52hi_epu64(zero,a51211,b51219);

	D0_12 = _mm512_madd52lo_epu64(D0_12,a51212,b5120);
	D0_13 = _mm512_madd52hi_epu64(D0_13,a51212,b5120);
	D0_13 = _mm512_madd52lo_epu64(D0_13,a51212,b5121);
	D0_14 = _mm512_madd52hi_epu64(D0_14,a51212,b5121);
	D0_14 = _mm512_madd52lo_epu64(D0_14,a51212,b5122);
	D0_15 = _mm512_madd52hi_epu64(D0_15,a51212,b5122);
	D0_15 = _mm512_madd52lo_epu64(D0_15,a51212,b5123);
	D0_16 = _mm512_madd52hi_epu64(D0_16,a51212,b5123);
	D0_16 = _mm512_madd52lo_epu64(D0_16,a51212,b5124);
	D0_17 = _mm512_madd52hi_epu64(D0_17,a51212,b5124);
	D0_17 = _mm512_madd52lo_epu64(D0_17,a51212,b5125);
	D0_18 = _mm512_madd52hi_epu64(D0_18,a51212,b5125);
	D0_18 = _mm512_madd52lo_epu64(D0_18,a51212,b5126);
	D0_19 = _mm512_madd52hi_epu64(D0_19,a51212,b5126);
	D0_19 = _mm512_madd52lo_epu64(D0_19,a51212,b5127);
	D0_20 = _mm512_madd52hi_epu64(D0_20,a51212,b5127);
	D0_20 = _mm512_madd52lo_epu64(D0_20,a51212,b5128);
	D0_21 = _mm512_madd52hi_epu64(D0_21,a51212,b5128);
	D0_21 = _mm512_madd52lo_epu64(D0_21,a51212,b5129);
	D0_22 = _mm512_madd52hi_epu64(D0_22,a51212,b5129);
	D0_22 = _mm512_madd52lo_epu64(D0_22,a51212,b51210);
	D0_23 = _mm512_madd52hi_epu64(D0_23,a51212,b51210);
	D0_23 = _mm512_madd52lo_epu64(D0_23,a51212,b51211);
	D0_24 = _mm512_madd52hi_epu64(D0_24,a51212,b51211);
	D0_24 = _mm512_madd52lo_epu64(D0_24,a51212,b51212);
	D0_25 = _mm512_madd52hi_epu64(D0_25,a51212,b51212);
	D0_25 = _mm512_madd52lo_epu64(D0_25,a51212,b51213);
	D0_26 = _mm512_madd52hi_epu64(D0_26,a51212,b51213);
	D0_26 = _mm512_madd52lo_epu64(D0_26,a51212,b51214);
	D0_27 = _mm512_madd52hi_epu64(D0_27,a51212,b51214);
	D0_27 = _mm512_madd52lo_epu64(D0_27,a51212,b51215);
	D0_28 = _mm512_madd52hi_epu64(D0_28,a51212,b51215);
	D0_28 = _mm512_madd52lo_epu64(D0_28,a51212,b51216);
	D0_29 = _mm512_madd52hi_epu64(D0_29,a51212,b51216);
	D0_29 = _mm512_madd52lo_epu64(D0_29,a51212,b51217);
	D0_30 = _mm512_madd52hi_epu64(D0_30,a51212,b51217);
	D0_30 = _mm512_madd52lo_epu64(D0_30,a51212,b51218);
	D0_31 = _mm512_madd52hi_epu64(D0_31,a51212,b51218);
	D0_31 = _mm512_madd52lo_epu64(D0_31,a51212,b51219);
	D0_32 = _mm512_madd52hi_epu64(zero,a51212,b51219);

	D0_13 = _mm512_madd52lo_epu64(D0_13,a51213,b5120);
	D0_14 = _mm512_madd52hi_epu64(D0_14,a51213,b5120);
	D0_14 = _mm512_madd52lo_epu64(D0_14,a51213,b5121);
	D0_15 = _mm512_madd52hi_epu64(D0_15,a51213,b5121);
	D0_15 = _mm512_madd52lo_epu64(D0_15,a51213,b5122);
	D0_16 = _mm512_madd52hi_epu64(D0_16,a51213,b5122);
	D0_16 = _mm512_madd52lo_epu64(D0_16,a51213,b5123);
	D0_17 = _mm512_madd52hi_epu64(D0_17,a51213,b5123);
	D0_17 = _mm512_madd52lo_epu64(D0_17,a51213,b5124);
	D0_18 = _mm512_madd52hi_epu64(D0_18,a51213,b5124);
	D0_18 = _mm512_madd52lo_epu64(D0_18,a51213,b5125);
	D0_19 = _mm512_madd52hi_epu64(D0_19,a51213,b5125);
	D0_19 = _mm512_madd52lo_epu64(D0_19,a51213,b5126);
	D0_20 = _mm512_madd52hi_epu64(D0_20,a51213,b5126);
	D0_20 = _mm512_madd52lo_epu64(D0_20,a51213,b5127);
	D0_21 = _mm512_madd52hi_epu64(D0_21,a51213,b5127);
	D0_21 = _mm512_madd52lo_epu64(D0_21,a51213,b5128);
	D0_22 = _mm512_madd52hi_epu64(D0_22,a51213,b5128);
	D0_22 = _mm512_madd52lo_epu64(D0_22,a51213,b5129);
	D0_23 = _mm512_madd52hi_epu64(D0_23,a51213,b5129);
	D0_23 = _mm512_madd52lo_epu64(D0_23,a51213,b51210);
	D0_24 = _mm512_madd52hi_epu64(D0_24,a51213,b51210);
	D0_24 = _mm512_madd52lo_epu64(D0_24,a51213,b51211);
	D0_25 = _mm512_madd52hi_epu64(D0_25,a51213,b51211);
	D0_25 = _mm512_madd52lo_epu64(D0_25,a51213,b51212);
	D0_26 = _mm512_madd52hi_epu64(D0_26,a51213,b51212);
	D0_26 = _mm512_madd52lo_epu64(D0_26,a51213,b51213);
	D0_27 = _mm512_madd52hi_epu64(D0_27,a51213,b51213);
	D0_27 = _mm512_madd52lo_epu64(D0_27,a51213,b51214);
	D0_28 = _mm512_madd52hi_epu64(D0_28,a51213,b51214);
	D0_28 = _mm512_madd52lo_epu64(D0_28,a51213,b51215);
	D0_29 = _mm512_madd52hi_epu64(D0_29,a51213,b51215);
	D0_29 = _mm512_madd52lo_epu64(D0_29,a51213,b51216);
	D0_30 = _mm512_madd52hi_epu64(D0_30,a51213,b51216);
	D0_30 = _mm512_madd52lo_epu64(D0_30,a51213,b51217);
	D0_31 = _mm512_madd52hi_epu64(D0_31,a51213,b51217);
	D0_31 = _mm512_madd52lo_epu64(D0_31,a51213,b51218);
	D0_32 = _mm512_madd52hi_epu64(D0_32,a51213,b51218);
	D0_32 = _mm512_madd52lo_epu64(D0_32,a51213,b51219);
	D0_33 = _mm512_madd52hi_epu64(zero,a51213,b51219);

	D0_14 = _mm512_madd52lo_epu64(D0_14,a51214,b5120);
	D0_15 = _mm512_madd52hi_epu64(D0_15,a51214,b5120);
	D0_15 = _mm512_madd52lo_epu64(D0_15,a51214,b5121);
	D0_16 = _mm512_madd52hi_epu64(D0_16,a51214,b5121);
	D0_16 = _mm512_madd52lo_epu64(D0_16,a51214,b5122);
	D0_17 = _mm512_madd52hi_epu64(D0_17,a51214,b5122);
	D0_17 = _mm512_madd52lo_epu64(D0_17,a51214,b5123);
	D0_18 = _mm512_madd52hi_epu64(D0_18,a51214,b5123);
	D0_18 = _mm512_madd52lo_epu64(D0_18,a51214,b5124);
	D0_19 = _mm512_madd52hi_epu64(D0_19,a51214,b5124);
	D0_19 = _mm512_madd52lo_epu64(D0_19,a51214,b5125);
	D0_20 = _mm512_madd52hi_epu64(D0_20,a51214,b5125);
	D0_20 = _mm512_madd52lo_epu64(D0_20,a51214,b5126);
	D0_21 = _mm512_madd52hi_epu64(D0_21,a51214,b5126);
	D0_21 = _mm512_madd52lo_epu64(D0_21,a51214,b5127);
	D0_22 = _mm512_madd52hi_epu64(D0_22,a51214,b5127);
	D0_22 = _mm512_madd52lo_epu64(D0_22,a51214,b5128);
	D0_23 = _mm512_madd52hi_epu64(D0_23,a51214,b5128);
	D0_23 = _mm512_madd52lo_epu64(D0_23,a51214,b5129);
	D0_24 = _mm512_madd52hi_epu64(D0_24,a51214,b5129);
	D0_24 = _mm512_madd52lo_epu64(D0_24,a51214,b51210);
	D0_25 = _mm512_madd52hi_epu64(D0_25,a51214,b51210);
	D0_25 = _mm512_madd52lo_epu64(D0_25,a51214,b51211);
	D0_26 = _mm512_madd52hi_epu64(D0_26,a51214,b51211);
	D0_26 = _mm512_madd52lo_epu64(D0_26,a51214,b51212);
	D0_27 = _mm512_madd52hi_epu64(D0_27,a51214,b51212);
	D0_27 = _mm512_madd52lo_epu64(D0_27,a51214,b51213);
	D0_28 = _mm512_madd52hi_epu64(D0_28,a51214,b51213);
	D0_28 = _mm512_madd52lo_epu64(D0_28,a51214,b51214);
	D0_29 = _mm512_madd52hi_epu64(D0_29,a51214,b51214);
	D0_29 = _mm512_madd52lo_epu64(D0_29,a51214,b51215);
	D0_30 = _mm512_madd52hi_epu64(D0_30,a51214,b51215);
	D0_30 = _mm512_madd52lo_epu64(D0_30,a51214,b51216);
	D0_31 = _mm512_madd52hi_epu64(D0_31,a51214,b51216);
	D0_31 = _mm512_madd52lo_epu64(D0_31,a51214,b51217);
	D0_32 = _mm512_madd52hi_epu64(D0_32,a51214,b51217);
	D0_32 = _mm512_madd52lo_epu64(D0_32,a51214,b51218);
	D0_33 = _mm512_madd52hi_epu64(D0_33,a51214,b51218);
	D0_33 = _mm512_madd52lo_epu64(D0_33,a51214,b51219);
	D0_34 = _mm512_madd52hi_epu64(zero,a51214,b51219);

	D0_15 = _mm512_madd52lo_epu64(D0_15,a51215,b5120);
	D0_16 = _mm512_madd52hi_epu64(D0_16,a51215,b5120);
	D0_16 = _mm512_madd52lo_epu64(D0_16,a51215,b5121);
	D0_17 = _mm512_madd52hi_epu64(D0_17,a51215,b5121);
	D0_17 = _mm512_madd52lo_epu64(D0_17,a51215,b5122);
	D0_18 = _mm512_madd52hi_epu64(D0_18,a51215,b5122);
	D0_18 = _mm512_madd52lo_epu64(D0_18,a51215,b5123);
	D0_19 = _mm512_madd52hi_epu64(D0_19,a51215,b5123);
	D0_19 = _mm512_madd52lo_epu64(D0_19,a51215,b5124);
	D0_20 = _mm512_madd52hi_epu64(D0_20,a51215,b5124);
	D0_20 = _mm512_madd52lo_epu64(D0_20,a51215,b5125);
	D0_21 = _mm512_madd52hi_epu64(D0_21,a51215,b5125);
	D0_21 = _mm512_madd52lo_epu64(D0_21,a51215,b5126);
	D0_22 = _mm512_madd52hi_epu64(D0_22,a51215,b5126);
	D0_22 = _mm512_madd52lo_epu64(D0_22,a51215,b5127);
	D0_23 = _mm512_madd52hi_epu64(D0_23,a51215,b5127);
	D0_23 = _mm512_madd52lo_epu64(D0_23,a51215,b5128);
	D0_24 = _mm512_madd52hi_epu64(D0_24,a51215,b5128);
	D0_24 = _mm512_madd52lo_epu64(D0_24,a51215,b5129);
	D0_25 = _mm512_madd52hi_epu64(D0_25,a51215,b5129);
	D0_25 = _mm512_madd52lo_epu64(D0_25,a51215,b51210);
	D0_26 = _mm512_madd52hi_epu64(D0_26,a51215,b51210);
	D0_26 = _mm512_madd52lo_epu64(D0_26,a51215,b51211);
	D0_27 = _mm512_madd52hi_epu64(D0_27,a51215,b51211);
	D0_27 = _mm512_madd52lo_epu64(D0_27,a51215,b51212);
	D0_28 = _mm512_madd52hi_epu64(D0_28,a51215,b51212);
	D0_28 = _mm512_madd52lo_epu64(D0_28,a51215,b51213);
	D0_29 = _mm512_madd52hi_epu64(D0_29,a51215,b51213);
	D0_29 = _mm512_madd52lo_epu64(D0_29,a51215,b51214);
	D0_30 = _mm512_madd52hi_epu64(D0_30,a51215,b51214);
	D0_30 = _mm512_madd52lo_epu64(D0_30,a51215,b51215);
	D0_31 = _mm512_madd52hi_epu64(D0_31,a51215,b51215);
	D0_31 = _mm512_madd52lo_epu64(D0_31,a51215,b51216);
	D0_32 = _mm512_madd52hi_epu64(D0_32,a51215,b51216);
	D0_32 = _mm512_madd52lo_epu64(D0_32,a51215,b51217);
	D0_33 = _mm512_madd52hi_epu64(D0_33,a51215,b51217);
	D0_33 = _mm512_madd52lo_epu64(D0_33,a51215,b51218);
	D0_34 = _mm512_madd52hi_epu64(D0_34,a51215,b51218);
	D0_34 = _mm512_madd52lo_epu64(D0_34,a51215,b51219);
	D0_35 = _mm512_madd52hi_epu64(zero,a51215,b51219);

	D0_16 = _mm512_madd52lo_epu64(D0_16,a51216,b5120);
	D0_17 = _mm512_madd52hi_epu64(D0_17,a51216,b5120);
	D0_17 = _mm512_madd52lo_epu64(D0_17,a51216,b5121);
	D0_18 = _mm512_madd52hi_epu64(D0_18,a51216,b5121);
	D0_18 = _mm512_madd52lo_epu64(D0_18,a51216,b5122);
	D0_19 = _mm512_madd52hi_epu64(D0_19,a51216,b5122);
	D0_19 = _mm512_madd52lo_epu64(D0_19,a51216,b5123);
	D0_20 = _mm512_madd52hi_epu64(D0_20,a51216,b5123);
	D0_20 = _mm512_madd52lo_epu64(D0_20,a51216,b5124);
	D0_21 = _mm512_madd52hi_epu64(D0_21,a51216,b5124);
	D0_21 = _mm512_madd52lo_epu64(D0_21,a51216,b5125);
	D0_22 = _mm512_madd52hi_epu64(D0_22,a51216,b5125);
	D0_22 = _mm512_madd52lo_epu64(D0_22,a51216,b5126);
	D0_23 = _mm512_madd52hi_epu64(D0_23,a51216,b5126);
	D0_23 = _mm512_madd52lo_epu64(D0_23,a51216,b5127);
	D0_24 = _mm512_madd52hi_epu64(D0_24,a51216,b5127);
	D0_24 = _mm512_madd52lo_epu64(D0_24,a51216,b5128);
	D0_25 = _mm512_madd52hi_epu64(D0_25,a51216,b5128);
	D0_25 = _mm512_madd52lo_epu64(D0_25,a51216,b5129);
	D0_26 = _mm512_madd52hi_epu64(D0_26,a51216,b5129);
	D0_26 = _mm512_madd52lo_epu64(D0_26,a51216,b51210);
	D0_27 = _mm512_madd52hi_epu64(D0_27,a51216,b51210);
	D0_27 = _mm512_madd52lo_epu64(D0_27,a51216,b51211);
	D0_28 = _mm512_madd52hi_epu64(D0_28,a51216,b51211);
	D0_28 = _mm512_madd52lo_epu64(D0_28,a51216,b51212);
	D0_29 = _mm512_madd52hi_epu64(D0_29,a51216,b51212);
	D0_29 = _mm512_madd52lo_epu64(D0_29,a51216,b51213);
	D0_30 = _mm512_madd52hi_epu64(D0_30,a51216,b51213);
	D0_30 = _mm512_madd52lo_epu64(D0_30,a51216,b51214);
	D0_31 = _mm512_madd52hi_epu64(D0_31,a51216,b51214);
	D0_31 = _mm512_madd52lo_epu64(D0_31,a51216,b51215);
	D0_32 = _mm512_madd52hi_epu64(D0_32,a51216,b51215);
	D0_32 = _mm512_madd52lo_epu64(D0_32,a51216,b51216);
	D0_33 = _mm512_madd52hi_epu64(D0_33,a51216,b51216);
	D0_33 = _mm512_madd52lo_epu64(D0_33,a51216,b51217);
	D0_34 = _mm512_madd52hi_epu64(D0_34,a51216,b51217);
	D0_34 = _mm512_madd52lo_epu64(D0_34,a51216,b51218);
	D0_35 = _mm512_madd52hi_epu64(D0_35,a51216,b51218);
	D0_35 = _mm512_madd52lo_epu64(D0_35,a51216,b51219);
	D0_36 = _mm512_madd52hi_epu64(zero,a51216,b51219);

	D0_17 = _mm512_madd52lo_epu64(D0_17,a51217,b5120);
	D0_18 = _mm512_madd52hi_epu64(D0_18,a51217,b5120);
	D0_18 = _mm512_madd52lo_epu64(D0_18,a51217,b5121);
	D0_19 = _mm512_madd52hi_epu64(D0_19,a51217,b5121);
	D0_19 = _mm512_madd52lo_epu64(D0_19,a51217,b5122);
	D0_20 = _mm512_madd52hi_epu64(D0_20,a51217,b5122);
	D0_20 = _mm512_madd52lo_epu64(D0_20,a51217,b5123);
	D0_21 = _mm512_madd52hi_epu64(D0_21,a51217,b5123);
	D0_21 = _mm512_madd52lo_epu64(D0_21,a51217,b5124);
	D0_22 = _mm512_madd52hi_epu64(D0_22,a51217,b5124);
	D0_22 = _mm512_madd52lo_epu64(D0_22,a51217,b5125);
	D0_23 = _mm512_madd52hi_epu64(D0_23,a51217,b5125);
	D0_23 = _mm512_madd52lo_epu64(D0_23,a51217,b5126);
	D0_24 = _mm512_madd52hi_epu64(D0_24,a51217,b5126);
	D0_24 = _mm512_madd52lo_epu64(D0_24,a51217,b5127);
	D0_25 = _mm512_madd52hi_epu64(D0_25,a51217,b5127);
	D0_25 = _mm512_madd52lo_epu64(D0_25,a51217,b5128);
	D0_26 = _mm512_madd52hi_epu64(D0_26,a51217,b5128);
	D0_26 = _mm512_madd52lo_epu64(D0_26,a51217,b5129);
	D0_27 = _mm512_madd52hi_epu64(D0_27,a51217,b5129);
	D0_27 = _mm512_madd52lo_epu64(D0_27,a51217,b51210);
	D0_28 = _mm512_madd52hi_epu64(D0_28,a51217,b51210);
	D0_28 = _mm512_madd52lo_epu64(D0_28,a51217,b51211);
	D0_29 = _mm512_madd52hi_epu64(D0_29,a51217,b51211);
	D0_29 = _mm512_madd52lo_epu64(D0_29,a51217,b51212);
	D0_30 = _mm512_madd52hi_epu64(D0_30,a51217,b51212);
	D0_30 = _mm512_madd52lo_epu64(D0_30,a51217,b51213);
	D0_31 = _mm512_madd52hi_epu64(D0_31,a51217,b51213);
	D0_31 = _mm512_madd52lo_epu64(D0_31,a51217,b51214);
	D0_32 = _mm512_madd52hi_epu64(D0_32,a51217,b51214);
	D0_32 = _mm512_madd52lo_epu64(D0_32,a51217,b51215);
	D0_33 = _mm512_madd52hi_epu64(D0_33,a51217,b51215);
	D0_33 = _mm512_madd52lo_epu64(D0_33,a51217,b51216);
	D0_34 = _mm512_madd52hi_epu64(D0_34,a51217,b51216);
	D0_34 = _mm512_madd52lo_epu64(D0_34,a51217,b51217);
	D0_35 = _mm512_madd52hi_epu64(D0_35,a51217,b51217);
	D0_35 = _mm512_madd52lo_epu64(D0_35,a51217,b51218);
	D0_36 = _mm512_madd52hi_epu64(D0_36,a51217,b51218);
	D0_36 = _mm512_madd52lo_epu64(D0_36,a51217,b51219);
	D0_37 = _mm512_madd52hi_epu64(zero,a51217,b51219);

	D0_18 = _mm512_madd52lo_epu64(D0_18,a51218,b5120);
	D0_19 = _mm512_madd52hi_epu64(D0_19,a51218,b5120);
	D0_19 = _mm512_madd52lo_epu64(D0_19,a51218,b5121);
	D0_20 = _mm512_madd52hi_epu64(D0_20,a51218,b5121);
	D0_20 = _mm512_madd52lo_epu64(D0_20,a51218,b5122);
	D0_21 = _mm512_madd52hi_epu64(D0_21,a51218,b5122);
	D0_21 = _mm512_madd52lo_epu64(D0_21,a51218,b5123);
	D0_22 = _mm512_madd52hi_epu64(D0_22,a51218,b5123);
	D0_22 = _mm512_madd52lo_epu64(D0_22,a51218,b5124);
	D0_23 = _mm512_madd52hi_epu64(D0_23,a51218,b5124);
	D0_23 = _mm512_madd52lo_epu64(D0_23,a51218,b5125);
	D0_24 = _mm512_madd52hi_epu64(D0_24,a51218,b5125);
	D0_24 = _mm512_madd52lo_epu64(D0_24,a51218,b5126);
	D0_25 = _mm512_madd52hi_epu64(D0_25,a51218,b5126);
	D0_25 = _mm512_madd52lo_epu64(D0_25,a51218,b5127);
	D0_26 = _mm512_madd52hi_epu64(D0_26,a51218,b5127);
	D0_26 = _mm512_madd52lo_epu64(D0_26,a51218,b5128);
	D0_27 = _mm512_madd52hi_epu64(D0_27,a51218,b5128);
	D0_27 = _mm512_madd52lo_epu64(D0_27,a51218,b5129);
	D0_28 = _mm512_madd52hi_epu64(D0_28,a51218,b5129);
	D0_28 = _mm512_madd52lo_epu64(D0_28,a51218,b51210);
	D0_29 = _mm512_madd52hi_epu64(D0_29,a51218,b51210);
	D0_29 = _mm512_madd52lo_epu64(D0_29,a51218,b51211);
	D0_30 = _mm512_madd52hi_epu64(D0_30,a51218,b51211);
	D0_30 = _mm512_madd52lo_epu64(D0_30,a51218,b51212);
	D0_31 = _mm512_madd52hi_epu64(D0_31,a51218,b51212);
	D0_31 = _mm512_madd52lo_epu64(D0_31,a51218,b51213);
	D0_32 = _mm512_madd52hi_epu64(D0_32,a51218,b51213);
	D0_32 = _mm512_madd52lo_epu64(D0_32,a51218,b51214);
	D0_33 = _mm512_madd52hi_epu64(D0_33,a51218,b51214);
	D0_33 = _mm512_madd52lo_epu64(D0_33,a51218,b51215);
	D0_34 = _mm512_madd52hi_epu64(D0_34,a51218,b51215);
	D0_34 = _mm512_madd52lo_epu64(D0_34,a51218,b51216);
	D0_35 = _mm512_madd52hi_epu64(D0_35,a51218,b51216);
	D0_35 = _mm512_madd52lo_epu64(D0_35,a51218,b51217);
	D0_36 = _mm512_madd52hi_epu64(D0_36,a51218,b51217);
	D0_36 = _mm512_madd52lo_epu64(D0_36,a51218,b51218);
	D0_37 = _mm512_madd52hi_epu64(D0_37,a51218,b51218);
	D0_37 = _mm512_madd52lo_epu64(D0_37,a51218,b51219);
	D0_38 = _mm512_madd52hi_epu64(zero,a51218,b51219);

	D0_19 = _mm512_madd52lo_epu64(D0_19,a51219,b5120);
	D0_20 = _mm512_madd52hi_epu64(D0_20,a51219,b5120);
	D0_20 = _mm512_madd52lo_epu64(D0_20,a51219,b5121);
	D0_21 = _mm512_madd52hi_epu64(D0_21,a51219,b5121);
	D0_21 = _mm512_madd52lo_epu64(D0_21,a51219,b5122);
	D0_22 = _mm512_madd52hi_epu64(D0_22,a51219,b5122);
	D0_22 = _mm512_madd52lo_epu64(D0_22,a51219,b5123);
	D0_23 = _mm512_madd52hi_epu64(D0_23,a51219,b5123);
	D0_23 = _mm512_madd52lo_epu64(D0_23,a51219,b5124);
	D0_24 = _mm512_madd52hi_epu64(D0_24,a51219,b5124);
	D0_24 = _mm512_madd52lo_epu64(D0_24,a51219,b5125);
	D0_25 = _mm512_madd52hi_epu64(D0_25,a51219,b5125);
	D0_25 = _mm512_madd52lo_epu64(D0_25,a51219,b5126);
	D0_26 = _mm512_madd52hi_epu64(D0_26,a51219,b5126);
	D0_26 = _mm512_madd52lo_epu64(D0_26,a51219,b5127);
	D0_27 = _mm512_madd52hi_epu64(D0_27,a51219,b5127);
	D0_27 = _mm512_madd52lo_epu64(D0_27,a51219,b5128);
	D0_28 = _mm512_madd52hi_epu64(D0_28,a51219,b5128);
	D0_28 = _mm512_madd52lo_epu64(D0_28,a51219,b5129);
	D0_29 = _mm512_madd52hi_epu64(D0_29,a51219,b5129);
	D0_29 = _mm512_madd52lo_epu64(D0_29,a51219,b51210);
	D0_30 = _mm512_madd52hi_epu64(D0_30,a51219,b51210);
	D0_30 = _mm512_madd52lo_epu64(D0_30,a51219,b51211);
	D0_31 = _mm512_madd52hi_epu64(D0_31,a51219,b51211);
	D0_31 = _mm512_madd52lo_epu64(D0_31,a51219,b51212);
	D0_32 = _mm512_madd52hi_epu64(D0_32,a51219,b51212);
	D0_32 = _mm512_madd52lo_epu64(D0_32,a51219,b51213);
	D0_33 = _mm512_madd52hi_epu64(D0_33,a51219,b51213);
	D0_33 = _mm512_madd52lo_epu64(D0_33,a51219,b51214);
	D0_34 = _mm512_madd52hi_epu64(D0_34,a51219,b51214);
	D0_34 = _mm512_madd52lo_epu64(D0_34,a51219,b51215);
	D0_35 = _mm512_madd52hi_epu64(D0_35,a51219,b51215);
	D0_35 = _mm512_madd52lo_epu64(D0_35,a51219,b51216);
	D0_36 = _mm512_madd52hi_epu64(D0_36,a51219,b51216);
	D0_36 = _mm512_madd52lo_epu64(D0_36,a51219,b51217);
	D0_37 = _mm512_madd52hi_epu64(D0_37,a51219,b51217);
	D0_37 = _mm512_madd52lo_epu64(D0_37,a51219,b51218);
	D0_38 = _mm512_madd52hi_epu64(D0_38,a51219,b51218);
	D0_38 = _mm512_madd52lo_epu64(D0_38,a51219,b51219);
	D0_39 = _mm512_madd52hi_epu64(zero,a51219,b51219);


	// Carry management

	__m512i carry  = _mm512_srli_epi64(D0_1,52);
	D0_2 = _mm512_add_epi64(D0_2,carry);
	D0_1 = _mm512_and_si512(D0_1,mask52);
	carry  = _mm512_srli_epi64(D0_2,52);
	D0_3 = _mm512_add_epi64(D0_3,carry);
	D0_2 = _mm512_and_si512(D0_2,mask52);
	carry  = _mm512_srli_epi64(D0_3,52);
	D0_4 = _mm512_add_epi64(D0_4,carry);
	D0_3 = _mm512_and_si512(D0_3,mask52);
	carry  = _mm512_srli_epi64(D0_4,52);
	D0_5 = _mm512_add_epi64(D0_5,carry);
	D0_4 = _mm512_and_si512(D0_4,mask52);
	carry  = _mm512_srli_epi64(D0_5,52);
	D0_6 = _mm512_add_epi64(D0_6,carry);
	D0_5 = _mm512_and_si512(D0_5,mask52);
	carry  = _mm512_srli_epi64(D0_6,52);
	D0_7 = _mm512_add_epi64(D0_7,carry);
	D0_6 = _mm512_and_si512(D0_6,mask52);
	carry  = _mm512_srli_epi64(D0_7,52);
	D0_8 = _mm512_add_epi64(D0_8,carry);
	D0_7 = _mm512_and_si512(D0_7,mask52);
	carry  = _mm512_srli_epi64(D0_8,52);
	D0_9 = _mm512_add_epi64(D0_9,carry);
	D0_8 = _mm512_and_si512(D0_8,mask52);
	carry  = _mm512_srli_epi64(D0_9,52);
	D0_10 = _mm512_add_epi64(D0_10,carry);
	D0_9 = _mm512_and_si512(D0_9,mask52);
	carry  = _mm512_srli_epi64(D0_10,52);
	D0_11 = _mm512_add_epi64(D0_11,carry);
	D0_10 = _mm512_and_si512(D0_10,mask52);
	carry  = _mm512_srli_epi64(D0_11,52);
	D0_12 = _mm512_add_epi64(D0_12,carry);
	D0_11 = _mm512_and_si512(D0_11,mask52);
	carry  = _mm512_srli_epi64(D0_12,52);
	D0_13 = _mm512_add_epi64(D0_13,carry);
	D0_12 = _mm512_and_si512(D0_12,mask52);
	carry  = _mm512_srli_epi64(D0_13,52);
	D0_14 = _mm512_add_epi64(D0_14,carry);
	D0_13 = _mm512_and_si512(D0_13,mask52);
	carry  = _mm512_srli_epi64(D0_14,52);
	D0_15 = _mm512_add_epi64(D0_15,carry);
	D0_14 = _mm512_and_si512(D0_14,mask52);
	carry  = _mm512_srli_epi64(D0_15,52);
	D0_16 = _mm512_add_epi64(D0_16,carry);
	D0_15 = _mm512_and_si512(D0_15,mask52);
	carry  = _mm512_srli_epi64(D0_16,52);
	D0_17 = _mm512_add_epi64(D0_17,carry);
	D0_16 = _mm512_and_si512(D0_16,mask52);
	carry  = _mm512_srli_epi64(D0_17,52);
	D0_18 = _mm512_add_epi64(D0_18,carry);
	D0_17 = _mm512_and_si512(D0_17,mask52);
	carry  = _mm512_srli_epi64(D0_18,52);
	D0_19 = _mm512_add_epi64(D0_19,carry);
	D0_18 = _mm512_and_si512(D0_18,mask52);

	// Conversion in base 2^1039

	carry = D0_19>>51;
	D0_20 = (D0_20<<1)+carry;
	D0_19 &= mask51;
	carry = D0_20>>52;
	D0_21 = (D0_21<<1)+carry;
	D0_20 &= mask52;
	carry = D0_21>>52;
	D0_22 = (D0_22<<1)+carry;
	D0_21 &= mask52;
	carry = D0_22>>52;
	D0_23 = (D0_23<<1)+carry;
	D0_22 &= mask52;
	carry = D0_23>>52;
	D0_24 = (D0_24<<1)+carry;
	D0_23 &= mask52;
	carry = D0_24>>52;
	D0_25 = (D0_25<<1)+carry;
	D0_24 &= mask52;
	carry = D0_25>>52;
	D0_26 = (D0_26<<1)+carry;
	D0_25 &= mask52;
	carry = D0_26>>52;
	D0_27 = (D0_27<<1)+carry;
	D0_26 &= mask52;
	carry = D0_27>>52;
	D0_28 = (D0_28<<1)+carry;
	D0_27 &= mask52;
	carry = D0_28>>52;
	D0_29 = (D0_29<<1)+carry;
	D0_28 &= mask52;
	carry = D0_29>>52;
	D0_30 = (D0_30<<1)+carry;
	D0_29 &= mask52;
	carry = D0_30>>52;
	D0_31 = (D0_31<<1)+carry;
	D0_30 &= mask52;
	carry = D0_31>>52;
	D0_32 = (D0_32<<1)+carry;
	D0_31 &= mask52;
	carry = D0_32>>52;
	D0_33 = (D0_33<<1)+carry;
	D0_32 &= mask52;
	carry = D0_33>>52;
	D0_34 = (D0_34<<1)+carry;
	D0_33 &= mask52;
	carry = D0_34>>52;
	D0_35 = (D0_35<<1)+carry;
	D0_34 &= mask52;
	carry = D0_35>>52;
	D0_36 = (D0_36<<1)+carry;
	D0_35 &= mask52;
	carry = D0_36>>52;
	D0_37 = (D0_37<<1)+carry;
	D0_36 &= mask52;
	carry = D0_37>>52;
	D0_38 = (D0_38<<1)+carry;
	D0_37 &= mask52;
	carry = D0_38>>52;
	D0_39 = (D0_39<<1)+carry;
	D0_38 &= mask52;

	// D2 = Ah*Bh

	__m512i D2_0, D2_1;
	__m512i D2_2, D2_3;
	__m512i D2_4, D2_5;
	__m512i D2_6, D2_7;
	__m512i D2_8, D2_9;
	__m512i D2_10, D2_11;
	__m512i D2_12, D2_13;
	__m512i D2_14, D2_15;
	__m512i D2_16, D2_17;
	__m512i D2_18, D2_19;
	__m512i D2_20, D2_21;
	__m512i D2_22, D2_23;
	__m512i D2_24, D2_25;
	__m512i D2_26, D2_27;
	__m512i D2_28, D2_29;
	__m512i D2_30, D2_31;
	__m512i D2_32, D2_33;
	__m512i D2_34, D2_35;
	__m512i D2_36, D2_37;
	__m512i D2_38, D2_39;
	__m512i a51220 = _mm512_load_epi64(a512+20), b51220 = _mm512_load_epi64(b512+20);
	__m512i a51221 = _mm512_load_epi64(a512+21), b51221 = _mm512_load_epi64(b512+21);
	__m512i a51222 = _mm512_load_epi64(a512+22), b51222 = _mm512_load_epi64(b512+22);
	__m512i a51223 = _mm512_load_epi64(a512+23), b51223 = _mm512_load_epi64(b512+23);
	__m512i a51224 = _mm512_load_epi64(a512+24), b51224 = _mm512_load_epi64(b512+24);
	__m512i a51225 = _mm512_load_epi64(a512+25), b51225 = _mm512_load_epi64(b512+25);
	__m512i a51226 = _mm512_load_epi64(a512+26), b51226 = _mm512_load_epi64(b512+26);
	__m512i a51227 = _mm512_load_epi64(a512+27), b51227 = _mm512_load_epi64(b512+27);
	__m512i a51228 = _mm512_load_epi64(a512+28), b51228 = _mm512_load_epi64(b512+28);
	__m512i a51229 = _mm512_load_epi64(a512+29), b51229 = _mm512_load_epi64(b512+29);
	__m512i a51230 = _mm512_load_epi64(a512+30), b51230 = _mm512_load_epi64(b512+30);
	__m512i a51231 = _mm512_load_epi64(a512+31), b51231 = _mm512_load_epi64(b512+31);
	__m512i a51232 = _mm512_load_epi64(a512+32), b51232 = _mm512_load_epi64(b512+32);
	__m512i a51233 = _mm512_load_epi64(a512+33), b51233 = _mm512_load_epi64(b512+33);
	__m512i a51234 = _mm512_load_epi64(a512+34), b51234 = _mm512_load_epi64(b512+34);
	__m512i a51235 = _mm512_load_epi64(a512+35), b51235 = _mm512_load_epi64(b512+35);
	__m512i a51236 = _mm512_load_epi64(a512+36), b51236 = _mm512_load_epi64(b512+36);
	__m512i a51237 = _mm512_load_epi64(a512+37), b51237 = _mm512_load_epi64(b512+37);
	__m512i a51238 = _mm512_load_epi64(a512+38), b51238 = _mm512_load_epi64(b512+38);
	__m512i a51239 = _mm512_load_epi64(a512+39), b51239 = _mm512_load_epi64(b512+39);
	D2_0 = _mm512_madd52lo_epu64(zero,a51220,b51220);
	D2_1 = _mm512_madd52hi_epu64(zero,a51220,b51220);
	D2_1 = _mm512_madd52lo_epu64(D2_1,a51220,b51221);
	D2_2 = _mm512_madd52hi_epu64(zero,a51220,b51221);
	D2_2 = _mm512_madd52lo_epu64(D2_2,a51220,b51222);
	D2_3 = _mm512_madd52hi_epu64(zero,a51220,b51222);
	D2_3 = _mm512_madd52lo_epu64(D2_3,a51220,b51223);
	D2_4 = _mm512_madd52hi_epu64(zero,a51220,b51223);
	D2_4 = _mm512_madd52lo_epu64(D2_4,a51220,b51224);
	D2_5 = _mm512_madd52hi_epu64(zero,a51220,b51224);
	D2_5 = _mm512_madd52lo_epu64(D2_5,a51220,b51225);
	D2_6 = _mm512_madd52hi_epu64(zero,a51220,b51225);
	D2_6 = _mm512_madd52lo_epu64(D2_6,a51220,b51226);
	D2_7 = _mm512_madd52hi_epu64(zero,a51220,b51226);
	D2_7 = _mm512_madd52lo_epu64(D2_7,a51220,b51227);
	D2_8 = _mm512_madd52hi_epu64(zero,a51220,b51227);
	D2_8 = _mm512_madd52lo_epu64(D2_8,a51220,b51228);
	D2_9 = _mm512_madd52hi_epu64(zero,a51220,b51228);
	D2_9 = _mm512_madd52lo_epu64(D2_9,a51220,b51229);
	D2_10 = _mm512_madd52hi_epu64(zero,a51220,b51229);
	D2_10 = _mm512_madd52lo_epu64(D2_10,a51220,b51230);
	D2_11 = _mm512_madd52hi_epu64(zero,a51220,b51230);
	D2_11 = _mm512_madd52lo_epu64(D2_11,a51220,b51231);
	D2_12 = _mm512_madd52hi_epu64(zero,a51220,b51231);
	D2_12 = _mm512_madd52lo_epu64(D2_12,a51220,b51232);
	D2_13 = _mm512_madd52hi_epu64(zero,a51220,b51232);
	D2_13 = _mm512_madd52lo_epu64(D2_13,a51220,b51233);
	D2_14 = _mm512_madd52hi_epu64(zero,a51220,b51233);
	D2_14 = _mm512_madd52lo_epu64(D2_14,a51220,b51234);
	D2_15 = _mm512_madd52hi_epu64(zero,a51220,b51234);
	D2_15 = _mm512_madd52lo_epu64(D2_15,a51220,b51235);
	D2_16 = _mm512_madd52hi_epu64(zero,a51220,b51235);
	D2_16 = _mm512_madd52lo_epu64(D2_16,a51220,b51236);
	D2_17 = _mm512_madd52hi_epu64(zero,a51220,b51236);
	D2_17 = _mm512_madd52lo_epu64(D2_17,a51220,b51237);
	D2_18 = _mm512_madd52hi_epu64(zero,a51220,b51237);
	D2_18 = _mm512_madd52lo_epu64(D2_18,a51220,b51238);
	D2_19 = _mm512_madd52hi_epu64(zero,a51220,b51238);
	D2_19 = _mm512_madd52lo_epu64(D2_19,a51220,b51239);
	D2_20 = _mm512_madd52hi_epu64(zero,a51220,b51239);

	D2_1 = _mm512_madd52lo_epu64(D2_1,a51221,b51220);
	D2_2 = _mm512_madd52hi_epu64(D2_2,a51221,b51220);
	D2_2 = _mm512_madd52lo_epu64(D2_2,a51221,b51221);
	D2_3 = _mm512_madd52hi_epu64(D2_3,a51221,b51221);
	D2_3 = _mm512_madd52lo_epu64(D2_3,a51221,b51222);
	D2_4 = _mm512_madd52hi_epu64(D2_4,a51221,b51222);
	D2_4 = _mm512_madd52lo_epu64(D2_4,a51221,b51223);
	D2_5 = _mm512_madd52hi_epu64(D2_5,a51221,b51223);
	D2_5 = _mm512_madd52lo_epu64(D2_5,a51221,b51224);
	D2_6 = _mm512_madd52hi_epu64(D2_6,a51221,b51224);
	D2_6 = _mm512_madd52lo_epu64(D2_6,a51221,b51225);
	D2_7 = _mm512_madd52hi_epu64(D2_7,a51221,b51225);
	D2_7 = _mm512_madd52lo_epu64(D2_7,a51221,b51226);
	D2_8 = _mm512_madd52hi_epu64(D2_8,a51221,b51226);
	D2_8 = _mm512_madd52lo_epu64(D2_8,a51221,b51227);
	D2_9 = _mm512_madd52hi_epu64(D2_9,a51221,b51227);
	D2_9 = _mm512_madd52lo_epu64(D2_9,a51221,b51228);
	D2_10 = _mm512_madd52hi_epu64(D2_10,a51221,b51228);
	D2_10 = _mm512_madd52lo_epu64(D2_10,a51221,b51229);
	D2_11 = _mm512_madd52hi_epu64(D2_11,a51221,b51229);
	D2_11 = _mm512_madd52lo_epu64(D2_11,a51221,b51230);
	D2_12 = _mm512_madd52hi_epu64(D2_12,a51221,b51230);
	D2_12 = _mm512_madd52lo_epu64(D2_12,a51221,b51231);
	D2_13 = _mm512_madd52hi_epu64(D2_13,a51221,b51231);
	D2_13 = _mm512_madd52lo_epu64(D2_13,a51221,b51232);
	D2_14 = _mm512_madd52hi_epu64(D2_14,a51221,b51232);
	D2_14 = _mm512_madd52lo_epu64(D2_14,a51221,b51233);
	D2_15 = _mm512_madd52hi_epu64(D2_15,a51221,b51233);
	D2_15 = _mm512_madd52lo_epu64(D2_15,a51221,b51234);
	D2_16 = _mm512_madd52hi_epu64(D2_16,a51221,b51234);
	D2_16 = _mm512_madd52lo_epu64(D2_16,a51221,b51235);
	D2_17 = _mm512_madd52hi_epu64(D2_17,a51221,b51235);
	D2_17 = _mm512_madd52lo_epu64(D2_17,a51221,b51236);
	D2_18 = _mm512_madd52hi_epu64(D2_18,a51221,b51236);
	D2_18 = _mm512_madd52lo_epu64(D2_18,a51221,b51237);
	D2_19 = _mm512_madd52hi_epu64(D2_19,a51221,b51237);
	D2_19 = _mm512_madd52lo_epu64(D2_19,a51221,b51238);
	D2_20 = _mm512_madd52hi_epu64(D2_20,a51221,b51238);
	D2_20 = _mm512_madd52lo_epu64(D2_20,a51221,b51239);
	D2_21 = _mm512_madd52hi_epu64(zero,a51221,b51239);

	D2_2 = _mm512_madd52lo_epu64(D2_2,a51222,b51220);
	D2_3 = _mm512_madd52hi_epu64(D2_3,a51222,b51220);
	D2_3 = _mm512_madd52lo_epu64(D2_3,a51222,b51221);
	D2_4 = _mm512_madd52hi_epu64(D2_4,a51222,b51221);
	D2_4 = _mm512_madd52lo_epu64(D2_4,a51222,b51222);
	D2_5 = _mm512_madd52hi_epu64(D2_5,a51222,b51222);
	D2_5 = _mm512_madd52lo_epu64(D2_5,a51222,b51223);
	D2_6 = _mm512_madd52hi_epu64(D2_6,a51222,b51223);
	D2_6 = _mm512_madd52lo_epu64(D2_6,a51222,b51224);
	D2_7 = _mm512_madd52hi_epu64(D2_7,a51222,b51224);
	D2_7 = _mm512_madd52lo_epu64(D2_7,a51222,b51225);
	D2_8 = _mm512_madd52hi_epu64(D2_8,a51222,b51225);
	D2_8 = _mm512_madd52lo_epu64(D2_8,a51222,b51226);
	D2_9 = _mm512_madd52hi_epu64(D2_9,a51222,b51226);
	D2_9 = _mm512_madd52lo_epu64(D2_9,a51222,b51227);
	D2_10 = _mm512_madd52hi_epu64(D2_10,a51222,b51227);
	D2_10 = _mm512_madd52lo_epu64(D2_10,a51222,b51228);
	D2_11 = _mm512_madd52hi_epu64(D2_11,a51222,b51228);
	D2_11 = _mm512_madd52lo_epu64(D2_11,a51222,b51229);
	D2_12 = _mm512_madd52hi_epu64(D2_12,a51222,b51229);
	D2_12 = _mm512_madd52lo_epu64(D2_12,a51222,b51230);
	D2_13 = _mm512_madd52hi_epu64(D2_13,a51222,b51230);
	D2_13 = _mm512_madd52lo_epu64(D2_13,a51222,b51231);
	D2_14 = _mm512_madd52hi_epu64(D2_14,a51222,b51231);
	D2_14 = _mm512_madd52lo_epu64(D2_14,a51222,b51232);
	D2_15 = _mm512_madd52hi_epu64(D2_15,a51222,b51232);
	D2_15 = _mm512_madd52lo_epu64(D2_15,a51222,b51233);
	D2_16 = _mm512_madd52hi_epu64(D2_16,a51222,b51233);
	D2_16 = _mm512_madd52lo_epu64(D2_16,a51222,b51234);
	D2_17 = _mm512_madd52hi_epu64(D2_17,a51222,b51234);
	D2_17 = _mm512_madd52lo_epu64(D2_17,a51222,b51235);
	D2_18 = _mm512_madd52hi_epu64(D2_18,a51222,b51235);
	D2_18 = _mm512_madd52lo_epu64(D2_18,a51222,b51236);
	D2_19 = _mm512_madd52hi_epu64(D2_19,a51222,b51236);
	D2_19 = _mm512_madd52lo_epu64(D2_19,a51222,b51237);
	D2_20 = _mm512_madd52hi_epu64(D2_20,a51222,b51237);
	D2_20 = _mm512_madd52lo_epu64(D2_20,a51222,b51238);
	D2_21 = _mm512_madd52hi_epu64(D2_21,a51222,b51238);
	D2_21 = _mm512_madd52lo_epu64(D2_21,a51222,b51239);
	D2_22 = _mm512_madd52hi_epu64(zero,a51222,b51239);

	D2_3 = _mm512_madd52lo_epu64(D2_3,a51223,b51220);
	D2_4 = _mm512_madd52hi_epu64(D2_4,a51223,b51220);
	D2_4 = _mm512_madd52lo_epu64(D2_4,a51223,b51221);
	D2_5 = _mm512_madd52hi_epu64(D2_5,a51223,b51221);
	D2_5 = _mm512_madd52lo_epu64(D2_5,a51223,b51222);
	D2_6 = _mm512_madd52hi_epu64(D2_6,a51223,b51222);
	D2_6 = _mm512_madd52lo_epu64(D2_6,a51223,b51223);
	D2_7 = _mm512_madd52hi_epu64(D2_7,a51223,b51223);
	D2_7 = _mm512_madd52lo_epu64(D2_7,a51223,b51224);
	D2_8 = _mm512_madd52hi_epu64(D2_8,a51223,b51224);
	D2_8 = _mm512_madd52lo_epu64(D2_8,a51223,b51225);
	D2_9 = _mm512_madd52hi_epu64(D2_9,a51223,b51225);
	D2_9 = _mm512_madd52lo_epu64(D2_9,a51223,b51226);
	D2_10 = _mm512_madd52hi_epu64(D2_10,a51223,b51226);
	D2_10 = _mm512_madd52lo_epu64(D2_10,a51223,b51227);
	D2_11 = _mm512_madd52hi_epu64(D2_11,a51223,b51227);
	D2_11 = _mm512_madd52lo_epu64(D2_11,a51223,b51228);
	D2_12 = _mm512_madd52hi_epu64(D2_12,a51223,b51228);
	D2_12 = _mm512_madd52lo_epu64(D2_12,a51223,b51229);
	D2_13 = _mm512_madd52hi_epu64(D2_13,a51223,b51229);
	D2_13 = _mm512_madd52lo_epu64(D2_13,a51223,b51230);
	D2_14 = _mm512_madd52hi_epu64(D2_14,a51223,b51230);
	D2_14 = _mm512_madd52lo_epu64(D2_14,a51223,b51231);
	D2_15 = _mm512_madd52hi_epu64(D2_15,a51223,b51231);
	D2_15 = _mm512_madd52lo_epu64(D2_15,a51223,b51232);
	D2_16 = _mm512_madd52hi_epu64(D2_16,a51223,b51232);
	D2_16 = _mm512_madd52lo_epu64(D2_16,a51223,b51233);
	D2_17 = _mm512_madd52hi_epu64(D2_17,a51223,b51233);
	D2_17 = _mm512_madd52lo_epu64(D2_17,a51223,b51234);
	D2_18 = _mm512_madd52hi_epu64(D2_18,a51223,b51234);
	D2_18 = _mm512_madd52lo_epu64(D2_18,a51223,b51235);
	D2_19 = _mm512_madd52hi_epu64(D2_19,a51223,b51235);
	D2_19 = _mm512_madd52lo_epu64(D2_19,a51223,b51236);
	D2_20 = _mm512_madd52hi_epu64(D2_20,a51223,b51236);
	D2_20 = _mm512_madd52lo_epu64(D2_20,a51223,b51237);
	D2_21 = _mm512_madd52hi_epu64(D2_21,a51223,b51237);
	D2_21 = _mm512_madd52lo_epu64(D2_21,a51223,b51238);
	D2_22 = _mm512_madd52hi_epu64(D2_22,a51223,b51238);
	D2_22 = _mm512_madd52lo_epu64(D2_22,a51223,b51239);
	D2_23 = _mm512_madd52hi_epu64(zero,a51223,b51239);

	D2_4 = _mm512_madd52lo_epu64(D2_4,a51224,b51220);
	D2_5 = _mm512_madd52hi_epu64(D2_5,a51224,b51220);
	D2_5 = _mm512_madd52lo_epu64(D2_5,a51224,b51221);
	D2_6 = _mm512_madd52hi_epu64(D2_6,a51224,b51221);
	D2_6 = _mm512_madd52lo_epu64(D2_6,a51224,b51222);
	D2_7 = _mm512_madd52hi_epu64(D2_7,a51224,b51222);
	D2_7 = _mm512_madd52lo_epu64(D2_7,a51224,b51223);
	D2_8 = _mm512_madd52hi_epu64(D2_8,a51224,b51223);
	D2_8 = _mm512_madd52lo_epu64(D2_8,a51224,b51224);
	D2_9 = _mm512_madd52hi_epu64(D2_9,a51224,b51224);
	D2_9 = _mm512_madd52lo_epu64(D2_9,a51224,b51225);
	D2_10 = _mm512_madd52hi_epu64(D2_10,a51224,b51225);
	D2_10 = _mm512_madd52lo_epu64(D2_10,a51224,b51226);
	D2_11 = _mm512_madd52hi_epu64(D2_11,a51224,b51226);
	D2_11 = _mm512_madd52lo_epu64(D2_11,a51224,b51227);
	D2_12 = _mm512_madd52hi_epu64(D2_12,a51224,b51227);
	D2_12 = _mm512_madd52lo_epu64(D2_12,a51224,b51228);
	D2_13 = _mm512_madd52hi_epu64(D2_13,a51224,b51228);
	D2_13 = _mm512_madd52lo_epu64(D2_13,a51224,b51229);
	D2_14 = _mm512_madd52hi_epu64(D2_14,a51224,b51229);
	D2_14 = _mm512_madd52lo_epu64(D2_14,a51224,b51230);
	D2_15 = _mm512_madd52hi_epu64(D2_15,a51224,b51230);
	D2_15 = _mm512_madd52lo_epu64(D2_15,a51224,b51231);
	D2_16 = _mm512_madd52hi_epu64(D2_16,a51224,b51231);
	D2_16 = _mm512_madd52lo_epu64(D2_16,a51224,b51232);
	D2_17 = _mm512_madd52hi_epu64(D2_17,a51224,b51232);
	D2_17 = _mm512_madd52lo_epu64(D2_17,a51224,b51233);
	D2_18 = _mm512_madd52hi_epu64(D2_18,a51224,b51233);
	D2_18 = _mm512_madd52lo_epu64(D2_18,a51224,b51234);
	D2_19 = _mm512_madd52hi_epu64(D2_19,a51224,b51234);
	D2_19 = _mm512_madd52lo_epu64(D2_19,a51224,b51235);
	D2_20 = _mm512_madd52hi_epu64(D2_20,a51224,b51235);
	D2_20 = _mm512_madd52lo_epu64(D2_20,a51224,b51236);
	D2_21 = _mm512_madd52hi_epu64(D2_21,a51224,b51236);
	D2_21 = _mm512_madd52lo_epu64(D2_21,a51224,b51237);
	D2_22 = _mm512_madd52hi_epu64(D2_22,a51224,b51237);
	D2_22 = _mm512_madd52lo_epu64(D2_22,a51224,b51238);
	D2_23 = _mm512_madd52hi_epu64(D2_23,a51224,b51238);
	D2_23 = _mm512_madd52lo_epu64(D2_23,a51224,b51239);
	D2_24 = _mm512_madd52hi_epu64(zero,a51224,b51239);

	D2_5 = _mm512_madd52lo_epu64(D2_5,a51225,b51220);
	D2_6 = _mm512_madd52hi_epu64(D2_6,a51225,b51220);
	D2_6 = _mm512_madd52lo_epu64(D2_6,a51225,b51221);
	D2_7 = _mm512_madd52hi_epu64(D2_7,a51225,b51221);
	D2_7 = _mm512_madd52lo_epu64(D2_7,a51225,b51222);
	D2_8 = _mm512_madd52hi_epu64(D2_8,a51225,b51222);
	D2_8 = _mm512_madd52lo_epu64(D2_8,a51225,b51223);
	D2_9 = _mm512_madd52hi_epu64(D2_9,a51225,b51223);
	D2_9 = _mm512_madd52lo_epu64(D2_9,a51225,b51224);
	D2_10 = _mm512_madd52hi_epu64(D2_10,a51225,b51224);
	D2_10 = _mm512_madd52lo_epu64(D2_10,a51225,b51225);
	D2_11 = _mm512_madd52hi_epu64(D2_11,a51225,b51225);
	D2_11 = _mm512_madd52lo_epu64(D2_11,a51225,b51226);
	D2_12 = _mm512_madd52hi_epu64(D2_12,a51225,b51226);
	D2_12 = _mm512_madd52lo_epu64(D2_12,a51225,b51227);
	D2_13 = _mm512_madd52hi_epu64(D2_13,a51225,b51227);
	D2_13 = _mm512_madd52lo_epu64(D2_13,a51225,b51228);
	D2_14 = _mm512_madd52hi_epu64(D2_14,a51225,b51228);
	D2_14 = _mm512_madd52lo_epu64(D2_14,a51225,b51229);
	D2_15 = _mm512_madd52hi_epu64(D2_15,a51225,b51229);
	D2_15 = _mm512_madd52lo_epu64(D2_15,a51225,b51230);
	D2_16 = _mm512_madd52hi_epu64(D2_16,a51225,b51230);
	D2_16 = _mm512_madd52lo_epu64(D2_16,a51225,b51231);
	D2_17 = _mm512_madd52hi_epu64(D2_17,a51225,b51231);
	D2_17 = _mm512_madd52lo_epu64(D2_17,a51225,b51232);
	D2_18 = _mm512_madd52hi_epu64(D2_18,a51225,b51232);
	D2_18 = _mm512_madd52lo_epu64(D2_18,a51225,b51233);
	D2_19 = _mm512_madd52hi_epu64(D2_19,a51225,b51233);
	D2_19 = _mm512_madd52lo_epu64(D2_19,a51225,b51234);
	D2_20 = _mm512_madd52hi_epu64(D2_20,a51225,b51234);
	D2_20 = _mm512_madd52lo_epu64(D2_20,a51225,b51235);
	D2_21 = _mm512_madd52hi_epu64(D2_21,a51225,b51235);
	D2_21 = _mm512_madd52lo_epu64(D2_21,a51225,b51236);
	D2_22 = _mm512_madd52hi_epu64(D2_22,a51225,b51236);
	D2_22 = _mm512_madd52lo_epu64(D2_22,a51225,b51237);
	D2_23 = _mm512_madd52hi_epu64(D2_23,a51225,b51237);
	D2_23 = _mm512_madd52lo_epu64(D2_23,a51225,b51238);
	D2_24 = _mm512_madd52hi_epu64(D2_24,a51225,b51238);
	D2_24 = _mm512_madd52lo_epu64(D2_24,a51225,b51239);
	D2_25 = _mm512_madd52hi_epu64(zero,a51225,b51239);

	D2_6 = _mm512_madd52lo_epu64(D2_6,a51226,b51220);
	D2_7 = _mm512_madd52hi_epu64(D2_7,a51226,b51220);
	D2_7 = _mm512_madd52lo_epu64(D2_7,a51226,b51221);
	D2_8 = _mm512_madd52hi_epu64(D2_8,a51226,b51221);
	D2_8 = _mm512_madd52lo_epu64(D2_8,a51226,b51222);
	D2_9 = _mm512_madd52hi_epu64(D2_9,a51226,b51222);
	D2_9 = _mm512_madd52lo_epu64(D2_9,a51226,b51223);
	D2_10 = _mm512_madd52hi_epu64(D2_10,a51226,b51223);
	D2_10 = _mm512_madd52lo_epu64(D2_10,a51226,b51224);
	D2_11 = _mm512_madd52hi_epu64(D2_11,a51226,b51224);
	D2_11 = _mm512_madd52lo_epu64(D2_11,a51226,b51225);
	D2_12 = _mm512_madd52hi_epu64(D2_12,a51226,b51225);
	D2_12 = _mm512_madd52lo_epu64(D2_12,a51226,b51226);
	D2_13 = _mm512_madd52hi_epu64(D2_13,a51226,b51226);
	D2_13 = _mm512_madd52lo_epu64(D2_13,a51226,b51227);
	D2_14 = _mm512_madd52hi_epu64(D2_14,a51226,b51227);
	D2_14 = _mm512_madd52lo_epu64(D2_14,a51226,b51228);
	D2_15 = _mm512_madd52hi_epu64(D2_15,a51226,b51228);
	D2_15 = _mm512_madd52lo_epu64(D2_15,a51226,b51229);
	D2_16 = _mm512_madd52hi_epu64(D2_16,a51226,b51229);
	D2_16 = _mm512_madd52lo_epu64(D2_16,a51226,b51230);
	D2_17 = _mm512_madd52hi_epu64(D2_17,a51226,b51230);
	D2_17 = _mm512_madd52lo_epu64(D2_17,a51226,b51231);
	D2_18 = _mm512_madd52hi_epu64(D2_18,a51226,b51231);
	D2_18 = _mm512_madd52lo_epu64(D2_18,a51226,b51232);
	D2_19 = _mm512_madd52hi_epu64(D2_19,a51226,b51232);
	D2_19 = _mm512_madd52lo_epu64(D2_19,a51226,b51233);
	D2_20 = _mm512_madd52hi_epu64(D2_20,a51226,b51233);
	D2_20 = _mm512_madd52lo_epu64(D2_20,a51226,b51234);
	D2_21 = _mm512_madd52hi_epu64(D2_21,a51226,b51234);
	D2_21 = _mm512_madd52lo_epu64(D2_21,a51226,b51235);
	D2_22 = _mm512_madd52hi_epu64(D2_22,a51226,b51235);
	D2_22 = _mm512_madd52lo_epu64(D2_22,a51226,b51236);
	D2_23 = _mm512_madd52hi_epu64(D2_23,a51226,b51236);
	D2_23 = _mm512_madd52lo_epu64(D2_23,a51226,b51237);
	D2_24 = _mm512_madd52hi_epu64(D2_24,a51226,b51237);
	D2_24 = _mm512_madd52lo_epu64(D2_24,a51226,b51238);
	D2_25 = _mm512_madd52hi_epu64(D2_25,a51226,b51238);
	D2_25 = _mm512_madd52lo_epu64(D2_25,a51226,b51239);
	D2_26 = _mm512_madd52hi_epu64(zero,a51226,b51239);

	D2_7 = _mm512_madd52lo_epu64(D2_7,a51227,b51220);
	D2_8 = _mm512_madd52hi_epu64(D2_8,a51227,b51220);
	D2_8 = _mm512_madd52lo_epu64(D2_8,a51227,b51221);
	D2_9 = _mm512_madd52hi_epu64(D2_9,a51227,b51221);
	D2_9 = _mm512_madd52lo_epu64(D2_9,a51227,b51222);
	D2_10 = _mm512_madd52hi_epu64(D2_10,a51227,b51222);
	D2_10 = _mm512_madd52lo_epu64(D2_10,a51227,b51223);
	D2_11 = _mm512_madd52hi_epu64(D2_11,a51227,b51223);
	D2_11 = _mm512_madd52lo_epu64(D2_11,a51227,b51224);
	D2_12 = _mm512_madd52hi_epu64(D2_12,a51227,b51224);
	D2_12 = _mm512_madd52lo_epu64(D2_12,a51227,b51225);
	D2_13 = _mm512_madd52hi_epu64(D2_13,a51227,b51225);
	D2_13 = _mm512_madd52lo_epu64(D2_13,a51227,b51226);
	D2_14 = _mm512_madd52hi_epu64(D2_14,a51227,b51226);
	D2_14 = _mm512_madd52lo_epu64(D2_14,a51227,b51227);
	D2_15 = _mm512_madd52hi_epu64(D2_15,a51227,b51227);
	D2_15 = _mm512_madd52lo_epu64(D2_15,a51227,b51228);
	D2_16 = _mm512_madd52hi_epu64(D2_16,a51227,b51228);
	D2_16 = _mm512_madd52lo_epu64(D2_16,a51227,b51229);
	D2_17 = _mm512_madd52hi_epu64(D2_17,a51227,b51229);
	D2_17 = _mm512_madd52lo_epu64(D2_17,a51227,b51230);
	D2_18 = _mm512_madd52hi_epu64(D2_18,a51227,b51230);
	D2_18 = _mm512_madd52lo_epu64(D2_18,a51227,b51231);
	D2_19 = _mm512_madd52hi_epu64(D2_19,a51227,b51231);
	D2_19 = _mm512_madd52lo_epu64(D2_19,a51227,b51232);
	D2_20 = _mm512_madd52hi_epu64(D2_20,a51227,b51232);
	D2_20 = _mm512_madd52lo_epu64(D2_20,a51227,b51233);
	D2_21 = _mm512_madd52hi_epu64(D2_21,a51227,b51233);
	D2_21 = _mm512_madd52lo_epu64(D2_21,a51227,b51234);
	D2_22 = _mm512_madd52hi_epu64(D2_22,a51227,b51234);
	D2_22 = _mm512_madd52lo_epu64(D2_22,a51227,b51235);
	D2_23 = _mm512_madd52hi_epu64(D2_23,a51227,b51235);
	D2_23 = _mm512_madd52lo_epu64(D2_23,a51227,b51236);
	D2_24 = _mm512_madd52hi_epu64(D2_24,a51227,b51236);
	D2_24 = _mm512_madd52lo_epu64(D2_24,a51227,b51237);
	D2_25 = _mm512_madd52hi_epu64(D2_25,a51227,b51237);
	D2_25 = _mm512_madd52lo_epu64(D2_25,a51227,b51238);
	D2_26 = _mm512_madd52hi_epu64(D2_26,a51227,b51238);
	D2_26 = _mm512_madd52lo_epu64(D2_26,a51227,b51239);
	D2_27 = _mm512_madd52hi_epu64(zero,a51227,b51239);

	D2_8 = _mm512_madd52lo_epu64(D2_8,a51228,b51220);
	D2_9 = _mm512_madd52hi_epu64(D2_9,a51228,b51220);
	D2_9 = _mm512_madd52lo_epu64(D2_9,a51228,b51221);
	D2_10 = _mm512_madd52hi_epu64(D2_10,a51228,b51221);
	D2_10 = _mm512_madd52lo_epu64(D2_10,a51228,b51222);
	D2_11 = _mm512_madd52hi_epu64(D2_11,a51228,b51222);
	D2_11 = _mm512_madd52lo_epu64(D2_11,a51228,b51223);
	D2_12 = _mm512_madd52hi_epu64(D2_12,a51228,b51223);
	D2_12 = _mm512_madd52lo_epu64(D2_12,a51228,b51224);
	D2_13 = _mm512_madd52hi_epu64(D2_13,a51228,b51224);
	D2_13 = _mm512_madd52lo_epu64(D2_13,a51228,b51225);
	D2_14 = _mm512_madd52hi_epu64(D2_14,a51228,b51225);
	D2_14 = _mm512_madd52lo_epu64(D2_14,a51228,b51226);
	D2_15 = _mm512_madd52hi_epu64(D2_15,a51228,b51226);
	D2_15 = _mm512_madd52lo_epu64(D2_15,a51228,b51227);
	D2_16 = _mm512_madd52hi_epu64(D2_16,a51228,b51227);
	D2_16 = _mm512_madd52lo_epu64(D2_16,a51228,b51228);
	D2_17 = _mm512_madd52hi_epu64(D2_17,a51228,b51228);
	D2_17 = _mm512_madd52lo_epu64(D2_17,a51228,b51229);
	D2_18 = _mm512_madd52hi_epu64(D2_18,a51228,b51229);
	D2_18 = _mm512_madd52lo_epu64(D2_18,a51228,b51230);
	D2_19 = _mm512_madd52hi_epu64(D2_19,a51228,b51230);
	D2_19 = _mm512_madd52lo_epu64(D2_19,a51228,b51231);
	D2_20 = _mm512_madd52hi_epu64(D2_20,a51228,b51231);
	D2_20 = _mm512_madd52lo_epu64(D2_20,a51228,b51232);
	D2_21 = _mm512_madd52hi_epu64(D2_21,a51228,b51232);
	D2_21 = _mm512_madd52lo_epu64(D2_21,a51228,b51233);
	D2_22 = _mm512_madd52hi_epu64(D2_22,a51228,b51233);
	D2_22 = _mm512_madd52lo_epu64(D2_22,a51228,b51234);
	D2_23 = _mm512_madd52hi_epu64(D2_23,a51228,b51234);
	D2_23 = _mm512_madd52lo_epu64(D2_23,a51228,b51235);
	D2_24 = _mm512_madd52hi_epu64(D2_24,a51228,b51235);
	D2_24 = _mm512_madd52lo_epu64(D2_24,a51228,b51236);
	D2_25 = _mm512_madd52hi_epu64(D2_25,a51228,b51236);
	D2_25 = _mm512_madd52lo_epu64(D2_25,a51228,b51237);
	D2_26 = _mm512_madd52hi_epu64(D2_26,a51228,b51237);
	D2_26 = _mm512_madd52lo_epu64(D2_26,a51228,b51238);
	D2_27 = _mm512_madd52hi_epu64(D2_27,a51228,b51238);
	D2_27 = _mm512_madd52lo_epu64(D2_27,a51228,b51239);
	D2_28 = _mm512_madd52hi_epu64(zero,a51228,b51239);

	D2_9 = _mm512_madd52lo_epu64(D2_9,a51229,b51220);
	D2_10 = _mm512_madd52hi_epu64(D2_10,a51229,b51220);
	D2_10 = _mm512_madd52lo_epu64(D2_10,a51229,b51221);
	D2_11 = _mm512_madd52hi_epu64(D2_11,a51229,b51221);
	D2_11 = _mm512_madd52lo_epu64(D2_11,a51229,b51222);
	D2_12 = _mm512_madd52hi_epu64(D2_12,a51229,b51222);
	D2_12 = _mm512_madd52lo_epu64(D2_12,a51229,b51223);
	D2_13 = _mm512_madd52hi_epu64(D2_13,a51229,b51223);
	D2_13 = _mm512_madd52lo_epu64(D2_13,a51229,b51224);
	D2_14 = _mm512_madd52hi_epu64(D2_14,a51229,b51224);
	D2_14 = _mm512_madd52lo_epu64(D2_14,a51229,b51225);
	D2_15 = _mm512_madd52hi_epu64(D2_15,a51229,b51225);
	D2_15 = _mm512_madd52lo_epu64(D2_15,a51229,b51226);
	D2_16 = _mm512_madd52hi_epu64(D2_16,a51229,b51226);
	D2_16 = _mm512_madd52lo_epu64(D2_16,a51229,b51227);
	D2_17 = _mm512_madd52hi_epu64(D2_17,a51229,b51227);
	D2_17 = _mm512_madd52lo_epu64(D2_17,a51229,b51228);
	D2_18 = _mm512_madd52hi_epu64(D2_18,a51229,b51228);
	D2_18 = _mm512_madd52lo_epu64(D2_18,a51229,b51229);
	D2_19 = _mm512_madd52hi_epu64(D2_19,a51229,b51229);
	D2_19 = _mm512_madd52lo_epu64(D2_19,a51229,b51230);
	D2_20 = _mm512_madd52hi_epu64(D2_20,a51229,b51230);
	D2_20 = _mm512_madd52lo_epu64(D2_20,a51229,b51231);
	D2_21 = _mm512_madd52hi_epu64(D2_21,a51229,b51231);
	D2_21 = _mm512_madd52lo_epu64(D2_21,a51229,b51232);
	D2_22 = _mm512_madd52hi_epu64(D2_22,a51229,b51232);
	D2_22 = _mm512_madd52lo_epu64(D2_22,a51229,b51233);
	D2_23 = _mm512_madd52hi_epu64(D2_23,a51229,b51233);
	D2_23 = _mm512_madd52lo_epu64(D2_23,a51229,b51234);
	D2_24 = _mm512_madd52hi_epu64(D2_24,a51229,b51234);
	D2_24 = _mm512_madd52lo_epu64(D2_24,a51229,b51235);
	D2_25 = _mm512_madd52hi_epu64(D2_25,a51229,b51235);
	D2_25 = _mm512_madd52lo_epu64(D2_25,a51229,b51236);
	D2_26 = _mm512_madd52hi_epu64(D2_26,a51229,b51236);
	D2_26 = _mm512_madd52lo_epu64(D2_26,a51229,b51237);
	D2_27 = _mm512_madd52hi_epu64(D2_27,a51229,b51237);
	D2_27 = _mm512_madd52lo_epu64(D2_27,a51229,b51238);
	D2_28 = _mm512_madd52hi_epu64(D2_28,a51229,b51238);
	D2_28 = _mm512_madd52lo_epu64(D2_28,a51229,b51239);
	D2_29 = _mm512_madd52hi_epu64(zero,a51229,b51239);

	D2_10 = _mm512_madd52lo_epu64(D2_10,a51230,b51220);
	D2_11 = _mm512_madd52hi_epu64(D2_11,a51230,b51220);
	D2_11 = _mm512_madd52lo_epu64(D2_11,a51230,b51221);
	D2_12 = _mm512_madd52hi_epu64(D2_12,a51230,b51221);
	D2_12 = _mm512_madd52lo_epu64(D2_12,a51230,b51222);
	D2_13 = _mm512_madd52hi_epu64(D2_13,a51230,b51222);
	D2_13 = _mm512_madd52lo_epu64(D2_13,a51230,b51223);
	D2_14 = _mm512_madd52hi_epu64(D2_14,a51230,b51223);
	D2_14 = _mm512_madd52lo_epu64(D2_14,a51230,b51224);
	D2_15 = _mm512_madd52hi_epu64(D2_15,a51230,b51224);
	D2_15 = _mm512_madd52lo_epu64(D2_15,a51230,b51225);
	D2_16 = _mm512_madd52hi_epu64(D2_16,a51230,b51225);
	D2_16 = _mm512_madd52lo_epu64(D2_16,a51230,b51226);
	D2_17 = _mm512_madd52hi_epu64(D2_17,a51230,b51226);
	D2_17 = _mm512_madd52lo_epu64(D2_17,a51230,b51227);
	D2_18 = _mm512_madd52hi_epu64(D2_18,a51230,b51227);
	D2_18 = _mm512_madd52lo_epu64(D2_18,a51230,b51228);
	D2_19 = _mm512_madd52hi_epu64(D2_19,a51230,b51228);
	D2_19 = _mm512_madd52lo_epu64(D2_19,a51230,b51229);
	D2_20 = _mm512_madd52hi_epu64(D2_20,a51230,b51229);
	D2_20 = _mm512_madd52lo_epu64(D2_20,a51230,b51230);
	D2_21 = _mm512_madd52hi_epu64(D2_21,a51230,b51230);
	D2_21 = _mm512_madd52lo_epu64(D2_21,a51230,b51231);
	D2_22 = _mm512_madd52hi_epu64(D2_22,a51230,b51231);
	D2_22 = _mm512_madd52lo_epu64(D2_22,a51230,b51232);
	D2_23 = _mm512_madd52hi_epu64(D2_23,a51230,b51232);
	D2_23 = _mm512_madd52lo_epu64(D2_23,a51230,b51233);
	D2_24 = _mm512_madd52hi_epu64(D2_24,a51230,b51233);
	D2_24 = _mm512_madd52lo_epu64(D2_24,a51230,b51234);
	D2_25 = _mm512_madd52hi_epu64(D2_25,a51230,b51234);
	D2_25 = _mm512_madd52lo_epu64(D2_25,a51230,b51235);
	D2_26 = _mm512_madd52hi_epu64(D2_26,a51230,b51235);
	D2_26 = _mm512_madd52lo_epu64(D2_26,a51230,b51236);
	D2_27 = _mm512_madd52hi_epu64(D2_27,a51230,b51236);
	D2_27 = _mm512_madd52lo_epu64(D2_27,a51230,b51237);
	D2_28 = _mm512_madd52hi_epu64(D2_28,a51230,b51237);
	D2_28 = _mm512_madd52lo_epu64(D2_28,a51230,b51238);
	D2_29 = _mm512_madd52hi_epu64(D2_29,a51230,b51238);
	D2_29 = _mm512_madd52lo_epu64(D2_29,a51230,b51239);
	D2_30 = _mm512_madd52hi_epu64(zero,a51230,b51239);

	D2_11 = _mm512_madd52lo_epu64(D2_11,a51231,b51220);
	D2_12 = _mm512_madd52hi_epu64(D2_12,a51231,b51220);
	D2_12 = _mm512_madd52lo_epu64(D2_12,a51231,b51221);
	D2_13 = _mm512_madd52hi_epu64(D2_13,a51231,b51221);
	D2_13 = _mm512_madd52lo_epu64(D2_13,a51231,b51222);
	D2_14 = _mm512_madd52hi_epu64(D2_14,a51231,b51222);
	D2_14 = _mm512_madd52lo_epu64(D2_14,a51231,b51223);
	D2_15 = _mm512_madd52hi_epu64(D2_15,a51231,b51223);
	D2_15 = _mm512_madd52lo_epu64(D2_15,a51231,b51224);
	D2_16 = _mm512_madd52hi_epu64(D2_16,a51231,b51224);
	D2_16 = _mm512_madd52lo_epu64(D2_16,a51231,b51225);
	D2_17 = _mm512_madd52hi_epu64(D2_17,a51231,b51225);
	D2_17 = _mm512_madd52lo_epu64(D2_17,a51231,b51226);
	D2_18 = _mm512_madd52hi_epu64(D2_18,a51231,b51226);
	D2_18 = _mm512_madd52lo_epu64(D2_18,a51231,b51227);
	D2_19 = _mm512_madd52hi_epu64(D2_19,a51231,b51227);
	D2_19 = _mm512_madd52lo_epu64(D2_19,a51231,b51228);
	D2_20 = _mm512_madd52hi_epu64(D2_20,a51231,b51228);
	D2_20 = _mm512_madd52lo_epu64(D2_20,a51231,b51229);
	D2_21 = _mm512_madd52hi_epu64(D2_21,a51231,b51229);
	D2_21 = _mm512_madd52lo_epu64(D2_21,a51231,b51230);
	D2_22 = _mm512_madd52hi_epu64(D2_22,a51231,b51230);
	D2_22 = _mm512_madd52lo_epu64(D2_22,a51231,b51231);
	D2_23 = _mm512_madd52hi_epu64(D2_23,a51231,b51231);
	D2_23 = _mm512_madd52lo_epu64(D2_23,a51231,b51232);
	D2_24 = _mm512_madd52hi_epu64(D2_24,a51231,b51232);
	D2_24 = _mm512_madd52lo_epu64(D2_24,a51231,b51233);
	D2_25 = _mm512_madd52hi_epu64(D2_25,a51231,b51233);
	D2_25 = _mm512_madd52lo_epu64(D2_25,a51231,b51234);
	D2_26 = _mm512_madd52hi_epu64(D2_26,a51231,b51234);
	D2_26 = _mm512_madd52lo_epu64(D2_26,a51231,b51235);
	D2_27 = _mm512_madd52hi_epu64(D2_27,a51231,b51235);
	D2_27 = _mm512_madd52lo_epu64(D2_27,a51231,b51236);
	D2_28 = _mm512_madd52hi_epu64(D2_28,a51231,b51236);
	D2_28 = _mm512_madd52lo_epu64(D2_28,a51231,b51237);
	D2_29 = _mm512_madd52hi_epu64(D2_29,a51231,b51237);
	D2_29 = _mm512_madd52lo_epu64(D2_29,a51231,b51238);
	D2_30 = _mm512_madd52hi_epu64(D2_30,a51231,b51238);
	D2_30 = _mm512_madd52lo_epu64(D2_30,a51231,b51239);
	D2_31 = _mm512_madd52hi_epu64(zero,a51231,b51239);

	D2_12 = _mm512_madd52lo_epu64(D2_12,a51232,b51220);
	D2_13 = _mm512_madd52hi_epu64(D2_13,a51232,b51220);
	D2_13 = _mm512_madd52lo_epu64(D2_13,a51232,b51221);
	D2_14 = _mm512_madd52hi_epu64(D2_14,a51232,b51221);
	D2_14 = _mm512_madd52lo_epu64(D2_14,a51232,b51222);
	D2_15 = _mm512_madd52hi_epu64(D2_15,a51232,b51222);
	D2_15 = _mm512_madd52lo_epu64(D2_15,a51232,b51223);
	D2_16 = _mm512_madd52hi_epu64(D2_16,a51232,b51223);
	D2_16 = _mm512_madd52lo_epu64(D2_16,a51232,b51224);
	D2_17 = _mm512_madd52hi_epu64(D2_17,a51232,b51224);
	D2_17 = _mm512_madd52lo_epu64(D2_17,a51232,b51225);
	D2_18 = _mm512_madd52hi_epu64(D2_18,a51232,b51225);
	D2_18 = _mm512_madd52lo_epu64(D2_18,a51232,b51226);
	D2_19 = _mm512_madd52hi_epu64(D2_19,a51232,b51226);
	D2_19 = _mm512_madd52lo_epu64(D2_19,a51232,b51227);
	D2_20 = _mm512_madd52hi_epu64(D2_20,a51232,b51227);
	D2_20 = _mm512_madd52lo_epu64(D2_20,a51232,b51228);
	D2_21 = _mm512_madd52hi_epu64(D2_21,a51232,b51228);
	D2_21 = _mm512_madd52lo_epu64(D2_21,a51232,b51229);
	D2_22 = _mm512_madd52hi_epu64(D2_22,a51232,b51229);
	D2_22 = _mm512_madd52lo_epu64(D2_22,a51232,b51230);
	D2_23 = _mm512_madd52hi_epu64(D2_23,a51232,b51230);
	D2_23 = _mm512_madd52lo_epu64(D2_23,a51232,b51231);
	D2_24 = _mm512_madd52hi_epu64(D2_24,a51232,b51231);
	D2_24 = _mm512_madd52lo_epu64(D2_24,a51232,b51232);
	D2_25 = _mm512_madd52hi_epu64(D2_25,a51232,b51232);
	D2_25 = _mm512_madd52lo_epu64(D2_25,a51232,b51233);
	D2_26 = _mm512_madd52hi_epu64(D2_26,a51232,b51233);
	D2_26 = _mm512_madd52lo_epu64(D2_26,a51232,b51234);
	D2_27 = _mm512_madd52hi_epu64(D2_27,a51232,b51234);
	D2_27 = _mm512_madd52lo_epu64(D2_27,a51232,b51235);
	D2_28 = _mm512_madd52hi_epu64(D2_28,a51232,b51235);
	D2_28 = _mm512_madd52lo_epu64(D2_28,a51232,b51236);
	D2_29 = _mm512_madd52hi_epu64(D2_29,a51232,b51236);
	D2_29 = _mm512_madd52lo_epu64(D2_29,a51232,b51237);
	D2_30 = _mm512_madd52hi_epu64(D2_30,a51232,b51237);
	D2_30 = _mm512_madd52lo_epu64(D2_30,a51232,b51238);
	D2_31 = _mm512_madd52hi_epu64(D2_31,a51232,b51238);
	D2_31 = _mm512_madd52lo_epu64(D2_31,a51232,b51239);
	D2_32 = _mm512_madd52hi_epu64(zero,a51232,b51239);

	D2_13 = _mm512_madd52lo_epu64(D2_13,a51233,b51220);
	D2_14 = _mm512_madd52hi_epu64(D2_14,a51233,b51220);
	D2_14 = _mm512_madd52lo_epu64(D2_14,a51233,b51221);
	D2_15 = _mm512_madd52hi_epu64(D2_15,a51233,b51221);
	D2_15 = _mm512_madd52lo_epu64(D2_15,a51233,b51222);
	D2_16 = _mm512_madd52hi_epu64(D2_16,a51233,b51222);
	D2_16 = _mm512_madd52lo_epu64(D2_16,a51233,b51223);
	D2_17 = _mm512_madd52hi_epu64(D2_17,a51233,b51223);
	D2_17 = _mm512_madd52lo_epu64(D2_17,a51233,b51224);
	D2_18 = _mm512_madd52hi_epu64(D2_18,a51233,b51224);
	D2_18 = _mm512_madd52lo_epu64(D2_18,a51233,b51225);
	D2_19 = _mm512_madd52hi_epu64(D2_19,a51233,b51225);
	D2_19 = _mm512_madd52lo_epu64(D2_19,a51233,b51226);
	D2_20 = _mm512_madd52hi_epu64(D2_20,a51233,b51226);
	D2_20 = _mm512_madd52lo_epu64(D2_20,a51233,b51227);
	D2_21 = _mm512_madd52hi_epu64(D2_21,a51233,b51227);
	D2_21 = _mm512_madd52lo_epu64(D2_21,a51233,b51228);
	D2_22 = _mm512_madd52hi_epu64(D2_22,a51233,b51228);
	D2_22 = _mm512_madd52lo_epu64(D2_22,a51233,b51229);
	D2_23 = _mm512_madd52hi_epu64(D2_23,a51233,b51229);
	D2_23 = _mm512_madd52lo_epu64(D2_23,a51233,b51230);
	D2_24 = _mm512_madd52hi_epu64(D2_24,a51233,b51230);
	D2_24 = _mm512_madd52lo_epu64(D2_24,a51233,b51231);
	D2_25 = _mm512_madd52hi_epu64(D2_25,a51233,b51231);
	D2_25 = _mm512_madd52lo_epu64(D2_25,a51233,b51232);
	D2_26 = _mm512_madd52hi_epu64(D2_26,a51233,b51232);
	D2_26 = _mm512_madd52lo_epu64(D2_26,a51233,b51233);
	D2_27 = _mm512_madd52hi_epu64(D2_27,a51233,b51233);
	D2_27 = _mm512_madd52lo_epu64(D2_27,a51233,b51234);
	D2_28 = _mm512_madd52hi_epu64(D2_28,a51233,b51234);
	D2_28 = _mm512_madd52lo_epu64(D2_28,a51233,b51235);
	D2_29 = _mm512_madd52hi_epu64(D2_29,a51233,b51235);
	D2_29 = _mm512_madd52lo_epu64(D2_29,a51233,b51236);
	D2_30 = _mm512_madd52hi_epu64(D2_30,a51233,b51236);
	D2_30 = _mm512_madd52lo_epu64(D2_30,a51233,b51237);
	D2_31 = _mm512_madd52hi_epu64(D2_31,a51233,b51237);
	D2_31 = _mm512_madd52lo_epu64(D2_31,a51233,b51238);
	D2_32 = _mm512_madd52hi_epu64(D2_32,a51233,b51238);
	D2_32 = _mm512_madd52lo_epu64(D2_32,a51233,b51239);
	D2_33 = _mm512_madd52hi_epu64(zero,a51233,b51239);

	D2_14 = _mm512_madd52lo_epu64(D2_14,a51234,b51220);
	D2_15 = _mm512_madd52hi_epu64(D2_15,a51234,b51220);
	D2_15 = _mm512_madd52lo_epu64(D2_15,a51234,b51221);
	D2_16 = _mm512_madd52hi_epu64(D2_16,a51234,b51221);
	D2_16 = _mm512_madd52lo_epu64(D2_16,a51234,b51222);
	D2_17 = _mm512_madd52hi_epu64(D2_17,a51234,b51222);
	D2_17 = _mm512_madd52lo_epu64(D2_17,a51234,b51223);
	D2_18 = _mm512_madd52hi_epu64(D2_18,a51234,b51223);
	D2_18 = _mm512_madd52lo_epu64(D2_18,a51234,b51224);
	D2_19 = _mm512_madd52hi_epu64(D2_19,a51234,b51224);
	D2_19 = _mm512_madd52lo_epu64(D2_19,a51234,b51225);
	D2_20 = _mm512_madd52hi_epu64(D2_20,a51234,b51225);
	D2_20 = _mm512_madd52lo_epu64(D2_20,a51234,b51226);
	D2_21 = _mm512_madd52hi_epu64(D2_21,a51234,b51226);
	D2_21 = _mm512_madd52lo_epu64(D2_21,a51234,b51227);
	D2_22 = _mm512_madd52hi_epu64(D2_22,a51234,b51227);
	D2_22 = _mm512_madd52lo_epu64(D2_22,a51234,b51228);
	D2_23 = _mm512_madd52hi_epu64(D2_23,a51234,b51228);
	D2_23 = _mm512_madd52lo_epu64(D2_23,a51234,b51229);
	D2_24 = _mm512_madd52hi_epu64(D2_24,a51234,b51229);
	D2_24 = _mm512_madd52lo_epu64(D2_24,a51234,b51230);
	D2_25 = _mm512_madd52hi_epu64(D2_25,a51234,b51230);
	D2_25 = _mm512_madd52lo_epu64(D2_25,a51234,b51231);
	D2_26 = _mm512_madd52hi_epu64(D2_26,a51234,b51231);
	D2_26 = _mm512_madd52lo_epu64(D2_26,a51234,b51232);
	D2_27 = _mm512_madd52hi_epu64(D2_27,a51234,b51232);
	D2_27 = _mm512_madd52lo_epu64(D2_27,a51234,b51233);
	D2_28 = _mm512_madd52hi_epu64(D2_28,a51234,b51233);
	D2_28 = _mm512_madd52lo_epu64(D2_28,a51234,b51234);
	D2_29 = _mm512_madd52hi_epu64(D2_29,a51234,b51234);
	D2_29 = _mm512_madd52lo_epu64(D2_29,a51234,b51235);
	D2_30 = _mm512_madd52hi_epu64(D2_30,a51234,b51235);
	D2_30 = _mm512_madd52lo_epu64(D2_30,a51234,b51236);
	D2_31 = _mm512_madd52hi_epu64(D2_31,a51234,b51236);
	D2_31 = _mm512_madd52lo_epu64(D2_31,a51234,b51237);
	D2_32 = _mm512_madd52hi_epu64(D2_32,a51234,b51237);
	D2_32 = _mm512_madd52lo_epu64(D2_32,a51234,b51238);
	D2_33 = _mm512_madd52hi_epu64(D2_33,a51234,b51238);
	D2_33 = _mm512_madd52lo_epu64(D2_33,a51234,b51239);
	D2_34 = _mm512_madd52hi_epu64(zero,a51234,b51239);

	D2_15 = _mm512_madd52lo_epu64(D2_15,a51235,b51220);
	D2_16 = _mm512_madd52hi_epu64(D2_16,a51235,b51220);
	D2_16 = _mm512_madd52lo_epu64(D2_16,a51235,b51221);
	D2_17 = _mm512_madd52hi_epu64(D2_17,a51235,b51221);
	D2_17 = _mm512_madd52lo_epu64(D2_17,a51235,b51222);
	D2_18 = _mm512_madd52hi_epu64(D2_18,a51235,b51222);
	D2_18 = _mm512_madd52lo_epu64(D2_18,a51235,b51223);
	D2_19 = _mm512_madd52hi_epu64(D2_19,a51235,b51223);
	D2_19 = _mm512_madd52lo_epu64(D2_19,a51235,b51224);
	D2_20 = _mm512_madd52hi_epu64(D2_20,a51235,b51224);
	D2_20 = _mm512_madd52lo_epu64(D2_20,a51235,b51225);
	D2_21 = _mm512_madd52hi_epu64(D2_21,a51235,b51225);
	D2_21 = _mm512_madd52lo_epu64(D2_21,a51235,b51226);
	D2_22 = _mm512_madd52hi_epu64(D2_22,a51235,b51226);
	D2_22 = _mm512_madd52lo_epu64(D2_22,a51235,b51227);
	D2_23 = _mm512_madd52hi_epu64(D2_23,a51235,b51227);
	D2_23 = _mm512_madd52lo_epu64(D2_23,a51235,b51228);
	D2_24 = _mm512_madd52hi_epu64(D2_24,a51235,b51228);
	D2_24 = _mm512_madd52lo_epu64(D2_24,a51235,b51229);
	D2_25 = _mm512_madd52hi_epu64(D2_25,a51235,b51229);
	D2_25 = _mm512_madd52lo_epu64(D2_25,a51235,b51230);
	D2_26 = _mm512_madd52hi_epu64(D2_26,a51235,b51230);
	D2_26 = _mm512_madd52lo_epu64(D2_26,a51235,b51231);
	D2_27 = _mm512_madd52hi_epu64(D2_27,a51235,b51231);
	D2_27 = _mm512_madd52lo_epu64(D2_27,a51235,b51232);
	D2_28 = _mm512_madd52hi_epu64(D2_28,a51235,b51232);
	D2_28 = _mm512_madd52lo_epu64(D2_28,a51235,b51233);
	D2_29 = _mm512_madd52hi_epu64(D2_29,a51235,b51233);
	D2_29 = _mm512_madd52lo_epu64(D2_29,a51235,b51234);
	D2_30 = _mm512_madd52hi_epu64(D2_30,a51235,b51234);
	D2_30 = _mm512_madd52lo_epu64(D2_30,a51235,b51235);
	D2_31 = _mm512_madd52hi_epu64(D2_31,a51235,b51235);
	D2_31 = _mm512_madd52lo_epu64(D2_31,a51235,b51236);
	D2_32 = _mm512_madd52hi_epu64(D2_32,a51235,b51236);
	D2_32 = _mm512_madd52lo_epu64(D2_32,a51235,b51237);
	D2_33 = _mm512_madd52hi_epu64(D2_33,a51235,b51237);
	D2_33 = _mm512_madd52lo_epu64(D2_33,a51235,b51238);
	D2_34 = _mm512_madd52hi_epu64(D2_34,a51235,b51238);
	D2_34 = _mm512_madd52lo_epu64(D2_34,a51235,b51239);
	D2_35 = _mm512_madd52hi_epu64(zero,a51235,b51239);

	D2_16 = _mm512_madd52lo_epu64(D2_16,a51236,b51220);
	D2_17 = _mm512_madd52hi_epu64(D2_17,a51236,b51220);
	D2_17 = _mm512_madd52lo_epu64(D2_17,a51236,b51221);
	D2_18 = _mm512_madd52hi_epu64(D2_18,a51236,b51221);
	D2_18 = _mm512_madd52lo_epu64(D2_18,a51236,b51222);
	D2_19 = _mm512_madd52hi_epu64(D2_19,a51236,b51222);
	D2_19 = _mm512_madd52lo_epu64(D2_19,a51236,b51223);
	D2_20 = _mm512_madd52hi_epu64(D2_20,a51236,b51223);
	D2_20 = _mm512_madd52lo_epu64(D2_20,a51236,b51224);
	D2_21 = _mm512_madd52hi_epu64(D2_21,a51236,b51224);
	D2_21 = _mm512_madd52lo_epu64(D2_21,a51236,b51225);
	D2_22 = _mm512_madd52hi_epu64(D2_22,a51236,b51225);
	D2_22 = _mm512_madd52lo_epu64(D2_22,a51236,b51226);
	D2_23 = _mm512_madd52hi_epu64(D2_23,a51236,b51226);
	D2_23 = _mm512_madd52lo_epu64(D2_23,a51236,b51227);
	D2_24 = _mm512_madd52hi_epu64(D2_24,a51236,b51227);
	D2_24 = _mm512_madd52lo_epu64(D2_24,a51236,b51228);
	D2_25 = _mm512_madd52hi_epu64(D2_25,a51236,b51228);
	D2_25 = _mm512_madd52lo_epu64(D2_25,a51236,b51229);
	D2_26 = _mm512_madd52hi_epu64(D2_26,a51236,b51229);
	D2_26 = _mm512_madd52lo_epu64(D2_26,a51236,b51230);
	D2_27 = _mm512_madd52hi_epu64(D2_27,a51236,b51230);
	D2_27 = _mm512_madd52lo_epu64(D2_27,a51236,b51231);
	D2_28 = _mm512_madd52hi_epu64(D2_28,a51236,b51231);
	D2_28 = _mm512_madd52lo_epu64(D2_28,a51236,b51232);
	D2_29 = _mm512_madd52hi_epu64(D2_29,a51236,b51232);
	D2_29 = _mm512_madd52lo_epu64(D2_29,a51236,b51233);
	D2_30 = _mm512_madd52hi_epu64(D2_30,a51236,b51233);
	D2_30 = _mm512_madd52lo_epu64(D2_30,a51236,b51234);
	D2_31 = _mm512_madd52hi_epu64(D2_31,a51236,b51234);
	D2_31 = _mm512_madd52lo_epu64(D2_31,a51236,b51235);
	D2_32 = _mm512_madd52hi_epu64(D2_32,a51236,b51235);
	D2_32 = _mm512_madd52lo_epu64(D2_32,a51236,b51236);
	D2_33 = _mm512_madd52hi_epu64(D2_33,a51236,b51236);
	D2_33 = _mm512_madd52lo_epu64(D2_33,a51236,b51237);
	D2_34 = _mm512_madd52hi_epu64(D2_34,a51236,b51237);
	D2_34 = _mm512_madd52lo_epu64(D2_34,a51236,b51238);
	D2_35 = _mm512_madd52hi_epu64(D2_35,a51236,b51238);
	D2_35 = _mm512_madd52lo_epu64(D2_35,a51236,b51239);
	D2_36 = _mm512_madd52hi_epu64(zero,a51236,b51239);

	D2_17 = _mm512_madd52lo_epu64(D2_17,a51237,b51220);
	D2_18 = _mm512_madd52hi_epu64(D2_18,a51237,b51220);
	D2_18 = _mm512_madd52lo_epu64(D2_18,a51237,b51221);
	D2_19 = _mm512_madd52hi_epu64(D2_19,a51237,b51221);
	D2_19 = _mm512_madd52lo_epu64(D2_19,a51237,b51222);
	D2_20 = _mm512_madd52hi_epu64(D2_20,a51237,b51222);
	D2_20 = _mm512_madd52lo_epu64(D2_20,a51237,b51223);
	D2_21 = _mm512_madd52hi_epu64(D2_21,a51237,b51223);
	D2_21 = _mm512_madd52lo_epu64(D2_21,a51237,b51224);
	D2_22 = _mm512_madd52hi_epu64(D2_22,a51237,b51224);
	D2_22 = _mm512_madd52lo_epu64(D2_22,a51237,b51225);
	D2_23 = _mm512_madd52hi_epu64(D2_23,a51237,b51225);
	D2_23 = _mm512_madd52lo_epu64(D2_23,a51237,b51226);
	D2_24 = _mm512_madd52hi_epu64(D2_24,a51237,b51226);
	D2_24 = _mm512_madd52lo_epu64(D2_24,a51237,b51227);
	D2_25 = _mm512_madd52hi_epu64(D2_25,a51237,b51227);
	D2_25 = _mm512_madd52lo_epu64(D2_25,a51237,b51228);
	D2_26 = _mm512_madd52hi_epu64(D2_26,a51237,b51228);
	D2_26 = _mm512_madd52lo_epu64(D2_26,a51237,b51229);
	D2_27 = _mm512_madd52hi_epu64(D2_27,a51237,b51229);
	D2_27 = _mm512_madd52lo_epu64(D2_27,a51237,b51230);
	D2_28 = _mm512_madd52hi_epu64(D2_28,a51237,b51230);
	D2_28 = _mm512_madd52lo_epu64(D2_28,a51237,b51231);
	D2_29 = _mm512_madd52hi_epu64(D2_29,a51237,b51231);
	D2_29 = _mm512_madd52lo_epu64(D2_29,a51237,b51232);
	D2_30 = _mm512_madd52hi_epu64(D2_30,a51237,b51232);
	D2_30 = _mm512_madd52lo_epu64(D2_30,a51237,b51233);
	D2_31 = _mm512_madd52hi_epu64(D2_31,a51237,b51233);
	D2_31 = _mm512_madd52lo_epu64(D2_31,a51237,b51234);
	D2_32 = _mm512_madd52hi_epu64(D2_32,a51237,b51234);
	D2_32 = _mm512_madd52lo_epu64(D2_32,a51237,b51235);
	D2_33 = _mm512_madd52hi_epu64(D2_33,a51237,b51235);
	D2_33 = _mm512_madd52lo_epu64(D2_33,a51237,b51236);
	D2_34 = _mm512_madd52hi_epu64(D2_34,a51237,b51236);
	D2_34 = _mm512_madd52lo_epu64(D2_34,a51237,b51237);
	D2_35 = _mm512_madd52hi_epu64(D2_35,a51237,b51237);
	D2_35 = _mm512_madd52lo_epu64(D2_35,a51237,b51238);
	D2_36 = _mm512_madd52hi_epu64(D2_36,a51237,b51238);
	D2_36 = _mm512_madd52lo_epu64(D2_36,a51237,b51239);
	D2_37 = _mm512_madd52hi_epu64(zero,a51237,b51239);

	D2_18 = _mm512_madd52lo_epu64(D2_18,a51238,b51220);
	D2_19 = _mm512_madd52hi_epu64(D2_19,a51238,b51220);
	D2_19 = _mm512_madd52lo_epu64(D2_19,a51238,b51221);
	D2_20 = _mm512_madd52hi_epu64(D2_20,a51238,b51221);
	D2_20 = _mm512_madd52lo_epu64(D2_20,a51238,b51222);
	D2_21 = _mm512_madd52hi_epu64(D2_21,a51238,b51222);
	D2_21 = _mm512_madd52lo_epu64(D2_21,a51238,b51223);
	D2_22 = _mm512_madd52hi_epu64(D2_22,a51238,b51223);
	D2_22 = _mm512_madd52lo_epu64(D2_22,a51238,b51224);
	D2_23 = _mm512_madd52hi_epu64(D2_23,a51238,b51224);
	D2_23 = _mm512_madd52lo_epu64(D2_23,a51238,b51225);
	D2_24 = _mm512_madd52hi_epu64(D2_24,a51238,b51225);
	D2_24 = _mm512_madd52lo_epu64(D2_24,a51238,b51226);
	D2_25 = _mm512_madd52hi_epu64(D2_25,a51238,b51226);
	D2_25 = _mm512_madd52lo_epu64(D2_25,a51238,b51227);
	D2_26 = _mm512_madd52hi_epu64(D2_26,a51238,b51227);
	D2_26 = _mm512_madd52lo_epu64(D2_26,a51238,b51228);
	D2_27 = _mm512_madd52hi_epu64(D2_27,a51238,b51228);
	D2_27 = _mm512_madd52lo_epu64(D2_27,a51238,b51229);
	D2_28 = _mm512_madd52hi_epu64(D2_28,a51238,b51229);
	D2_28 = _mm512_madd52lo_epu64(D2_28,a51238,b51230);
	D2_29 = _mm512_madd52hi_epu64(D2_29,a51238,b51230);
	D2_29 = _mm512_madd52lo_epu64(D2_29,a51238,b51231);
	D2_30 = _mm512_madd52hi_epu64(D2_30,a51238,b51231);
	D2_30 = _mm512_madd52lo_epu64(D2_30,a51238,b51232);
	D2_31 = _mm512_madd52hi_epu64(D2_31,a51238,b51232);
	D2_31 = _mm512_madd52lo_epu64(D2_31,a51238,b51233);
	D2_32 = _mm512_madd52hi_epu64(D2_32,a51238,b51233);
	D2_32 = _mm512_madd52lo_epu64(D2_32,a51238,b51234);
	D2_33 = _mm512_madd52hi_epu64(D2_33,a51238,b51234);
	D2_33 = _mm512_madd52lo_epu64(D2_33,a51238,b51235);
	D2_34 = _mm512_madd52hi_epu64(D2_34,a51238,b51235);
	D2_34 = _mm512_madd52lo_epu64(D2_34,a51238,b51236);
	D2_35 = _mm512_madd52hi_epu64(D2_35,a51238,b51236);
	D2_35 = _mm512_madd52lo_epu64(D2_35,a51238,b51237);
	D2_36 = _mm512_madd52hi_epu64(D2_36,a51238,b51237);
	D2_36 = _mm512_madd52lo_epu64(D2_36,a51238,b51238);
	D2_37 = _mm512_madd52hi_epu64(D2_37,a51238,b51238);
	D2_37 = _mm512_madd52lo_epu64(D2_37,a51238,b51239);
	D2_38 = _mm512_madd52hi_epu64(zero,a51238,b51239);

	D2_19 = _mm512_madd52lo_epu64(D2_19,a51239,b51220);
	D2_20 = _mm512_madd52hi_epu64(D2_20,a51239,b51220);
	D2_20 = _mm512_madd52lo_epu64(D2_20,a51239,b51221);
	D2_21 = _mm512_madd52hi_epu64(D2_21,a51239,b51221);
	D2_21 = _mm512_madd52lo_epu64(D2_21,a51239,b51222);
	D2_22 = _mm512_madd52hi_epu64(D2_22,a51239,b51222);
	D2_22 = _mm512_madd52lo_epu64(D2_22,a51239,b51223);
	D2_23 = _mm512_madd52hi_epu64(D2_23,a51239,b51223);
	D2_23 = _mm512_madd52lo_epu64(D2_23,a51239,b51224);
	D2_24 = _mm512_madd52hi_epu64(D2_24,a51239,b51224);
	D2_24 = _mm512_madd52lo_epu64(D2_24,a51239,b51225);
	D2_25 = _mm512_madd52hi_epu64(D2_25,a51239,b51225);
	D2_25 = _mm512_madd52lo_epu64(D2_25,a51239,b51226);
	D2_26 = _mm512_madd52hi_epu64(D2_26,a51239,b51226);
	D2_26 = _mm512_madd52lo_epu64(D2_26,a51239,b51227);
	D2_27 = _mm512_madd52hi_epu64(D2_27,a51239,b51227);
	D2_27 = _mm512_madd52lo_epu64(D2_27,a51239,b51228);
	D2_28 = _mm512_madd52hi_epu64(D2_28,a51239,b51228);
	D2_28 = _mm512_madd52lo_epu64(D2_28,a51239,b51229);
	D2_29 = _mm512_madd52hi_epu64(D2_29,a51239,b51229);
	D2_29 = _mm512_madd52lo_epu64(D2_29,a51239,b51230);
	D2_30 = _mm512_madd52hi_epu64(D2_30,a51239,b51230);
	D2_30 = _mm512_madd52lo_epu64(D2_30,a51239,b51231);
	D2_31 = _mm512_madd52hi_epu64(D2_31,a51239,b51231);
	D2_31 = _mm512_madd52lo_epu64(D2_31,a51239,b51232);
	D2_32 = _mm512_madd52hi_epu64(D2_32,a51239,b51232);
	D2_32 = _mm512_madd52lo_epu64(D2_32,a51239,b51233);
	D2_33 = _mm512_madd52hi_epu64(D2_33,a51239,b51233);
	D2_33 = _mm512_madd52lo_epu64(D2_33,a51239,b51234);
	D2_34 = _mm512_madd52hi_epu64(D2_34,a51239,b51234);
	D2_34 = _mm512_madd52lo_epu64(D2_34,a51239,b51235);
	D2_35 = _mm512_madd52hi_epu64(D2_35,a51239,b51235);
	D2_35 = _mm512_madd52lo_epu64(D2_35,a51239,b51236);
	D2_36 = _mm512_madd52hi_epu64(D2_36,a51239,b51236);
	D2_36 = _mm512_madd52lo_epu64(D2_36,a51239,b51237);
	D2_37 = _mm512_madd52hi_epu64(D2_37,a51239,b51237);
	D2_37 = _mm512_madd52lo_epu64(D2_37,a51239,b51238);
	D2_38 = _mm512_madd52hi_epu64(D2_38,a51239,b51238);
	D2_38 = _mm512_madd52lo_epu64(D2_38,a51239,b51239);
	D2_39 = _mm512_madd52hi_epu64(zero,a51239,b51239);


	// Carry management

	carry  = _mm512_srli_epi64(D2_1,52);
	D2_2 = _mm512_add_epi64(D2_2,carry);
	D2_1 = _mm512_and_si512(D2_1,mask52);
	carry  = _mm512_srli_epi64(D2_2,52);
	D2_3 = _mm512_add_epi64(D2_3,carry);
	D2_2 = _mm512_and_si512(D2_2,mask52);
	carry  = _mm512_srli_epi64(D2_3,52);
	D2_4 = _mm512_add_epi64(D2_4,carry);
	D2_3 = _mm512_and_si512(D2_3,mask52);
	carry  = _mm512_srli_epi64(D2_4,52);
	D2_5 = _mm512_add_epi64(D2_5,carry);
	D2_4 = _mm512_and_si512(D2_4,mask52);
	carry  = _mm512_srli_epi64(D2_5,52);
	D2_6 = _mm512_add_epi64(D2_6,carry);
	D2_5 = _mm512_and_si512(D2_5,mask52);
	carry  = _mm512_srli_epi64(D2_6,52);
	D2_7 = _mm512_add_epi64(D2_7,carry);
	D2_6 = _mm512_and_si512(D2_6,mask52);
	carry  = _mm512_srli_epi64(D2_7,52);
	D2_8 = _mm512_add_epi64(D2_8,carry);
	D2_7 = _mm512_and_si512(D2_7,mask52);
	carry  = _mm512_srli_epi64(D2_8,52);
	D2_9 = _mm512_add_epi64(D2_9,carry);
	D2_8 = _mm512_and_si512(D2_8,mask52);
	carry  = _mm512_srli_epi64(D2_9,52);
	D2_10 = _mm512_add_epi64(D2_10,carry);
	D2_9 = _mm512_and_si512(D2_9,mask52);
	carry  = _mm512_srli_epi64(D2_10,52);
	D2_11 = _mm512_add_epi64(D2_11,carry);
	D2_10 = _mm512_and_si512(D2_10,mask52);
	carry  = _mm512_srli_epi64(D2_11,52);
	D2_12 = _mm512_add_epi64(D2_12,carry);
	D2_11 = _mm512_and_si512(D2_11,mask52);
	carry  = _mm512_srli_epi64(D2_12,52);
	D2_13 = _mm512_add_epi64(D2_13,carry);
	D2_12 = _mm512_and_si512(D2_12,mask52);
	carry  = _mm512_srli_epi64(D2_13,52);
	D2_14 = _mm512_add_epi64(D2_14,carry);
	D2_13 = _mm512_and_si512(D2_13,mask52);
	carry  = _mm512_srli_epi64(D2_14,52);
	D2_15 = _mm512_add_epi64(D2_15,carry);
	D2_14 = _mm512_and_si512(D2_14,mask52);
	carry  = _mm512_srli_epi64(D2_15,52);
	D2_16 = _mm512_add_epi64(D2_16,carry);
	D2_15 = _mm512_and_si512(D2_15,mask52);
	carry  = _mm512_srli_epi64(D2_16,52);
	D2_17 = _mm512_add_epi64(D2_17,carry);
	D2_16 = _mm512_and_si512(D2_16,mask52);
	carry  = _mm512_srli_epi64(D2_17,52);
	D2_18 = _mm512_add_epi64(D2_18,carry);
	D2_17 = _mm512_and_si512(D2_17,mask52);
	carry  = _mm512_srli_epi64(D2_18,52);
	D2_19 = _mm512_add_epi64(D2_19,carry);
	D2_18 = _mm512_and_si512(D2_18,mask52);

	// Conversion in base 2^1039

	carry = D2_19>>51;
	D2_20 = (D2_20<<1)+carry;
	D2_19 &= mask51;
	carry = D2_20>>52;
	D2_21 = (D2_21<<1)+carry;
	D2_20 &= mask52;
	carry = D2_21>>52;
	D2_22 = (D2_22<<1)+carry;
	D2_21 &= mask52;
	carry = D2_22>>52;
	D2_23 = (D2_23<<1)+carry;
	D2_22 &= mask52;
	carry = D2_23>>52;
	D2_24 = (D2_24<<1)+carry;
	D2_23 &= mask52;
	carry = D2_24>>52;
	D2_25 = (D2_25<<1)+carry;
	D2_24 &= mask52;
	carry = D2_25>>52;
	D2_26 = (D2_26<<1)+carry;
	D2_25 &= mask52;
	carry = D2_26>>52;
	D2_27 = (D2_27<<1)+carry;
	D2_26 &= mask52;
	carry = D2_27>>52;
	D2_28 = (D2_28<<1)+carry;
	D2_27 &= mask52;
	carry = D2_28>>52;
	D2_29 = (D2_29<<1)+carry;
	D2_28 &= mask52;
	carry = D2_29>>52;
	D2_30 = (D2_30<<1)+carry;
	D2_29 &= mask52;
	carry = D2_30>>52;
	D2_31 = (D2_31<<1)+carry;
	D2_30 &= mask52;
	carry = D2_31>>52;
	D2_32 = (D2_32<<1)+carry;
	D2_31 &= mask52;
	carry = D2_32>>52;
	D2_33 = (D2_33<<1)+carry;
	D2_32 &= mask52;
	carry = D2_33>>52;
	D2_34 = (D2_34<<1)+carry;
	D2_33 &= mask52;
	carry = D2_34>>52;
	D2_35 = (D2_35<<1)+carry;
	D2_34 &= mask52;
	carry = D2_35>>52;
	D2_36 = (D2_36<<1)+carry;
	D2_35 &= mask52;
	carry = D2_36>>52;
	D2_37 = (D2_37<<1)+carry;
	D2_36 &= mask52;
	carry = D2_37>>52;
	D2_38 = (D2_38<<1)+carry;
	D2_37 &= mask52;
	carry = D2_38>>52;
	D2_39 = (D2_39<<1)+carry;
	D2_38 &= mask52;

	// D1 = (Al+Ah)*(Bl+Bh)

	__m512i D1_0, D1_1;
	__m512i D1_2, D1_3;
	__m512i D1_4, D1_5;
	__m512i D1_6, D1_7;
	__m512i D1_8, D1_9;
	__m512i D1_10, D1_11;
	__m512i D1_12, D1_13;
	__m512i D1_14, D1_15;
	__m512i D1_16, D1_17;
	__m512i D1_18, D1_19;
	__m512i D1_20, D1_21;
	__m512i D1_22, D1_23;
	__m512i D1_24, D1_25;
	__m512i D1_26, D1_27;
	__m512i D1_28, D1_29;
	__m512i D1_30, D1_31;
	__m512i D1_32, D1_33;
	__m512i D1_34, D1_35;
	__m512i D1_36, D1_37;
	__m512i D1_38, D1_39;

	__m512i alpah0 = _mm512_add_epi64(a5120,a51220);
	carry = alpah0>>52;
	alpah0 &= mask52;
	__m512i alpah1 = _mm512_add_epi64(a5121,carry);
	alpah1 = _mm512_add_epi64(alpah1,a51221);
	carry = alpah1>>52;
	alpah1 &= mask52;
	__m512i alpah2 = _mm512_add_epi64(a5122,carry);
	alpah2 = _mm512_add_epi64(alpah2,a51222);
	carry = alpah2>>52;
	alpah2 &= mask52;
	__m512i alpah3 = _mm512_add_epi64(a5123,carry);
	alpah3 = _mm512_add_epi64(alpah3,a51223);
	carry = alpah3>>52;
	alpah3 &= mask52;
	__m512i alpah4 = _mm512_add_epi64(a5124,carry);
	alpah4 = _mm512_add_epi64(alpah4,a51224);
	carry = alpah4>>52;
	alpah4 &= mask52;
	__m512i alpah5 = _mm512_add_epi64(a5125,carry);
	alpah5 = _mm512_add_epi64(alpah5,a51225);
	carry = alpah5>>52;
	alpah5 &= mask52;
	__m512i alpah6 = _mm512_add_epi64(a5126,carry);
	alpah6 = _mm512_add_epi64(alpah6,a51226);
	carry = alpah6>>52;
	alpah6 &= mask52;
	__m512i alpah7 = _mm512_add_epi64(a5127,carry);
	alpah7 = _mm512_add_epi64(alpah7,a51227);
	carry = alpah7>>52;
	alpah7 &= mask52;
	__m512i alpah8 = _mm512_add_epi64(a5128,carry);
	alpah8 = _mm512_add_epi64(alpah8,a51228);
	carry = alpah8>>52;
	alpah8 &= mask52;
	__m512i alpah9 = _mm512_add_epi64(a5129,carry);
	alpah9 = _mm512_add_epi64(alpah9,a51229);
	carry = alpah9>>52;
	alpah9 &= mask52;
	__m512i alpah10 = _mm512_add_epi64(a51210,carry);
	alpah10 = _mm512_add_epi64(alpah10,a51230);
	carry = alpah10>>52;
	alpah10 &= mask52;
	__m512i alpah11 = _mm512_add_epi64(a51211,carry);
	alpah11 = _mm512_add_epi64(alpah11,a51231);
	carry = alpah11>>52;
	alpah11 &= mask52;
	__m512i alpah12 = _mm512_add_epi64(a51212,carry);
	alpah12 = _mm512_add_epi64(alpah12,a51232);
	carry = alpah12>>52;
	alpah12 &= mask52;
	__m512i alpah13 = _mm512_add_epi64(a51213,carry);
	alpah13 = _mm512_add_epi64(alpah13,a51233);
	carry = alpah13>>52;
	alpah13 &= mask52;
	__m512i alpah14 = _mm512_add_epi64(a51214,carry);
	alpah14 = _mm512_add_epi64(alpah14,a51234);
	carry = alpah14>>52;
	alpah14 &= mask52;
	__m512i alpah15 = _mm512_add_epi64(a51215,carry);
	alpah15 = _mm512_add_epi64(alpah15,a51235);
	carry = alpah15>>52;
	alpah15 &= mask52;
	__m512i alpah16 = _mm512_add_epi64(a51216,carry);
	alpah16 = _mm512_add_epi64(alpah16,a51236);
	carry = alpah16>>52;
	alpah16 &= mask52;
	__m512i alpah17 = _mm512_add_epi64(a51217,carry);
	alpah17 = _mm512_add_epi64(alpah17,a51237);
	carry = alpah17>>52;
	alpah17 &= mask52;
	__m512i alpah18 = _mm512_add_epi64(a51218,carry);
	alpah18 = _mm512_add_epi64(alpah18,a51238);
	carry = alpah18>>52;
	alpah18 &= mask52;
	__m512i alpah19 = _mm512_add_epi64(a51219,carry);
	alpah19 = _mm512_add_epi64(alpah19,a51239);
	carry = alpah19>>52;
	alpah19 &= mask52;
	__m512i blpbh0 = _mm512_add_epi64(b5120,b51220);
	carry = blpbh0>>52;
	blpbh0 &= mask52;
	__m512i blpbh1 = _mm512_add_epi64(b5121,carry);
	blpbh1 = _mm512_add_epi64(blpbh1,b51221);
	carry = blpbh1>>52;
	blpbh1 &= mask52;
	__m512i blpbh2 = _mm512_add_epi64(b5122,carry);
	blpbh2 = _mm512_add_epi64(blpbh2,b51222);
	carry = blpbh2>>52;
	blpbh2 &= mask52;
	__m512i blpbh3 = _mm512_add_epi64(b5123,carry);
	blpbh3 = _mm512_add_epi64(blpbh3,b51223);
	carry = blpbh3>>52;
	blpbh3 &= mask52;
	__m512i blpbh4 = _mm512_add_epi64(b5124,carry);
	blpbh4 = _mm512_add_epi64(blpbh4,b51224);
	carry = blpbh4>>52;
	blpbh4 &= mask52;
	__m512i blpbh5 = _mm512_add_epi64(b5125,carry);
	blpbh5 = _mm512_add_epi64(blpbh5,b51225);
	carry = blpbh5>>52;
	blpbh5 &= mask52;
	__m512i blpbh6 = _mm512_add_epi64(b5126,carry);
	blpbh6 = _mm512_add_epi64(blpbh6,b51226);
	carry = blpbh6>>52;
	blpbh6 &= mask52;
	__m512i blpbh7 = _mm512_add_epi64(b5127,carry);
	blpbh7 = _mm512_add_epi64(blpbh7,b51227);
	carry = blpbh7>>52;
	blpbh7 &= mask52;
	__m512i blpbh8 = _mm512_add_epi64(b5128,carry);
	blpbh8 = _mm512_add_epi64(blpbh8,b51228);
	carry = blpbh8>>52;
	blpbh8 &= mask52;
	__m512i blpbh9 = _mm512_add_epi64(b5129,carry);
	blpbh9 = _mm512_add_epi64(blpbh9,b51229);
	carry = blpbh9>>52;
	blpbh9 &= mask52;
	__m512i blpbh10 = _mm512_add_epi64(b51210,carry);
	blpbh10 = _mm512_add_epi64(blpbh10,b51230);
	carry = blpbh10>>52;
	blpbh10 &= mask52;
	__m512i blpbh11 = _mm512_add_epi64(b51211,carry);
	blpbh11 = _mm512_add_epi64(blpbh11,b51231);
	carry = blpbh11>>52;
	blpbh11 &= mask52;
	__m512i blpbh12 = _mm512_add_epi64(b51212,carry);
	blpbh12 = _mm512_add_epi64(blpbh12,b51232);
	carry = blpbh12>>52;
	blpbh12 &= mask52;
	__m512i blpbh13 = _mm512_add_epi64(b51213,carry);
	blpbh13 = _mm512_add_epi64(blpbh13,b51233);
	carry = blpbh13>>52;
	blpbh13 &= mask52;
	__m512i blpbh14 = _mm512_add_epi64(b51214,carry);
	blpbh14 = _mm512_add_epi64(blpbh14,b51234);
	carry = blpbh14>>52;
	blpbh14 &= mask52;
	__m512i blpbh15 = _mm512_add_epi64(b51215,carry);
	blpbh15 = _mm512_add_epi64(blpbh15,b51235);
	carry = blpbh15>>52;
	blpbh15 &= mask52;
	__m512i blpbh16 = _mm512_add_epi64(b51216,carry);
	blpbh16 = _mm512_add_epi64(blpbh16,b51236);
	carry = blpbh16>>52;
	blpbh16 &= mask52;
	__m512i blpbh17 = _mm512_add_epi64(b51217,carry);
	blpbh17 = _mm512_add_epi64(blpbh17,b51237);
	carry = blpbh17>>52;
	blpbh17 &= mask52;
	__m512i blpbh18 = _mm512_add_epi64(b51218,carry);
	blpbh18 = _mm512_add_epi64(blpbh18,b51238);
	carry = blpbh18>>52;
	blpbh18 &= mask52;
	__m512i blpbh19 = _mm512_add_epi64(b51219,carry);
	blpbh19 = _mm512_add_epi64(blpbh19,b51239);
	carry = blpbh19>>52;
	blpbh19 &= mask52;
	D1_0 = _mm512_madd52lo_epu64(D0_20,alpah0,blpbh0);
	D1_1 = _mm512_madd52hi_epu64(D0_21,alpah0,blpbh0);
	D1_1 = _mm512_madd52lo_epu64(D1_1,alpah0,blpbh1);
	D1_2 = _mm512_madd52hi_epu64(D0_22,alpah0,blpbh1);
	D1_2 = _mm512_madd52lo_epu64(D1_2,alpah0,blpbh2);
	D1_3 = _mm512_madd52hi_epu64(D0_23,alpah0,blpbh2);
	D1_3 = _mm512_madd52lo_epu64(D1_3,alpah0,blpbh3);
	D1_4 = _mm512_madd52hi_epu64(D0_24,alpah0,blpbh3);
	D1_4 = _mm512_madd52lo_epu64(D1_4,alpah0,blpbh4);
	D1_5 = _mm512_madd52hi_epu64(D0_25,alpah0,blpbh4);
	D1_5 = _mm512_madd52lo_epu64(D1_5,alpah0,blpbh5);
	D1_6 = _mm512_madd52hi_epu64(D0_26,alpah0,blpbh5);
	D1_6 = _mm512_madd52lo_epu64(D1_6,alpah0,blpbh6);
	D1_7 = _mm512_madd52hi_epu64(D0_27,alpah0,blpbh6);
	D1_7 = _mm512_madd52lo_epu64(D1_7,alpah0,blpbh7);
	D1_8 = _mm512_madd52hi_epu64(D0_28,alpah0,blpbh7);
	D1_8 = _mm512_madd52lo_epu64(D1_8,alpah0,blpbh8);
	D1_9 = _mm512_madd52hi_epu64(D0_29,alpah0,blpbh8);
	D1_9 = _mm512_madd52lo_epu64(D1_9,alpah0,blpbh9);
	D1_10 = _mm512_madd52hi_epu64(D0_30,alpah0,blpbh9);
	D1_10 = _mm512_madd52lo_epu64(D1_10,alpah0,blpbh10);
	D1_11 = _mm512_madd52hi_epu64(D0_31,alpah0,blpbh10);
	D1_11 = _mm512_madd52lo_epu64(D1_11,alpah0,blpbh11);
	D1_12 = _mm512_madd52hi_epu64(D0_32,alpah0,blpbh11);
	D1_12 = _mm512_madd52lo_epu64(D1_12,alpah0,blpbh12);
	D1_13 = _mm512_madd52hi_epu64(D0_33,alpah0,blpbh12);
	D1_13 = _mm512_madd52lo_epu64(D1_13,alpah0,blpbh13);
	D1_14 = _mm512_madd52hi_epu64(D0_34,alpah0,blpbh13);
	D1_14 = _mm512_madd52lo_epu64(D1_14,alpah0,blpbh14);
	D1_15 = _mm512_madd52hi_epu64(D0_35,alpah0,blpbh14);
	D1_15 = _mm512_madd52lo_epu64(D1_15,alpah0,blpbh15);
	D1_16 = _mm512_madd52hi_epu64(D0_36,alpah0,blpbh15);
	D1_16 = _mm512_madd52lo_epu64(D1_16,alpah0,blpbh16);
	D1_17 = _mm512_madd52hi_epu64(D0_37,alpah0,blpbh16);
	D1_17 = _mm512_madd52lo_epu64(D1_17,alpah0,blpbh17);
	D1_18 = _mm512_madd52hi_epu64(D0_38,alpah0,blpbh17);
	D1_18 = _mm512_madd52lo_epu64(D1_18,alpah0,blpbh18);
	D1_19 = _mm512_madd52hi_epu64(D0_39,alpah0,blpbh18);
	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah0,blpbh19);
	D1_20 = _mm512_madd52hi_epu64(zero,alpah0,blpbh19);

	D1_1 = _mm512_madd52lo_epu64(D1_1,alpah1,blpbh0);
	D1_2 = _mm512_madd52hi_epu64(D1_2,alpah1,blpbh0);
	D1_2 = _mm512_madd52lo_epu64(D1_2,alpah1,blpbh1);
	D1_3 = _mm512_madd52hi_epu64(D1_3,alpah1,blpbh1);
	D1_3 = _mm512_madd52lo_epu64(D1_3,alpah1,blpbh2);
	D1_4 = _mm512_madd52hi_epu64(D1_4,alpah1,blpbh2);
	D1_4 = _mm512_madd52lo_epu64(D1_4,alpah1,blpbh3);
	D1_5 = _mm512_madd52hi_epu64(D1_5,alpah1,blpbh3);
	D1_5 = _mm512_madd52lo_epu64(D1_5,alpah1,blpbh4);
	D1_6 = _mm512_madd52hi_epu64(D1_6,alpah1,blpbh4);
	D1_6 = _mm512_madd52lo_epu64(D1_6,alpah1,blpbh5);
	D1_7 = _mm512_madd52hi_epu64(D1_7,alpah1,blpbh5);
	D1_7 = _mm512_madd52lo_epu64(D1_7,alpah1,blpbh6);
	D1_8 = _mm512_madd52hi_epu64(D1_8,alpah1,blpbh6);
	D1_8 = _mm512_madd52lo_epu64(D1_8,alpah1,blpbh7);
	D1_9 = _mm512_madd52hi_epu64(D1_9,alpah1,blpbh7);
	D1_9 = _mm512_madd52lo_epu64(D1_9,alpah1,blpbh8);
	D1_10 = _mm512_madd52hi_epu64(D1_10,alpah1,blpbh8);
	D1_10 = _mm512_madd52lo_epu64(D1_10,alpah1,blpbh9);
	D1_11 = _mm512_madd52hi_epu64(D1_11,alpah1,blpbh9);
	D1_11 = _mm512_madd52lo_epu64(D1_11,alpah1,blpbh10);
	D1_12 = _mm512_madd52hi_epu64(D1_12,alpah1,blpbh10);
	D1_12 = _mm512_madd52lo_epu64(D1_12,alpah1,blpbh11);
	D1_13 = _mm512_madd52hi_epu64(D1_13,alpah1,blpbh11);
	D1_13 = _mm512_madd52lo_epu64(D1_13,alpah1,blpbh12);
	D1_14 = _mm512_madd52hi_epu64(D1_14,alpah1,blpbh12);
	D1_14 = _mm512_madd52lo_epu64(D1_14,alpah1,blpbh13);
	D1_15 = _mm512_madd52hi_epu64(D1_15,alpah1,blpbh13);
	D1_15 = _mm512_madd52lo_epu64(D1_15,alpah1,blpbh14);
	D1_16 = _mm512_madd52hi_epu64(D1_16,alpah1,blpbh14);
	D1_16 = _mm512_madd52lo_epu64(D1_16,alpah1,blpbh15);
	D1_17 = _mm512_madd52hi_epu64(D1_17,alpah1,blpbh15);
	D1_17 = _mm512_madd52lo_epu64(D1_17,alpah1,blpbh16);
	D1_18 = _mm512_madd52hi_epu64(D1_18,alpah1,blpbh16);
	D1_18 = _mm512_madd52lo_epu64(D1_18,alpah1,blpbh17);
	D1_19 = _mm512_madd52hi_epu64(D1_19,alpah1,blpbh17);
	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah1,blpbh18);
	D1_20 = _mm512_madd52hi_epu64(D1_20,alpah1,blpbh18);
	D1_20 = _mm512_madd52lo_epu64(D1_20,alpah1,blpbh19);
	D1_21 = _mm512_madd52hi_epu64(zero,alpah1,blpbh19);

	D1_2 = _mm512_madd52lo_epu64(D1_2,alpah2,blpbh0);
	D1_3 = _mm512_madd52hi_epu64(D1_3,alpah2,blpbh0);
	D1_3 = _mm512_madd52lo_epu64(D1_3,alpah2,blpbh1);
	D1_4 = _mm512_madd52hi_epu64(D1_4,alpah2,blpbh1);
	D1_4 = _mm512_madd52lo_epu64(D1_4,alpah2,blpbh2);
	D1_5 = _mm512_madd52hi_epu64(D1_5,alpah2,blpbh2);
	D1_5 = _mm512_madd52lo_epu64(D1_5,alpah2,blpbh3);
	D1_6 = _mm512_madd52hi_epu64(D1_6,alpah2,blpbh3);
	D1_6 = _mm512_madd52lo_epu64(D1_6,alpah2,blpbh4);
	D1_7 = _mm512_madd52hi_epu64(D1_7,alpah2,blpbh4);
	D1_7 = _mm512_madd52lo_epu64(D1_7,alpah2,blpbh5);
	D1_8 = _mm512_madd52hi_epu64(D1_8,alpah2,blpbh5);
	D1_8 = _mm512_madd52lo_epu64(D1_8,alpah2,blpbh6);
	D1_9 = _mm512_madd52hi_epu64(D1_9,alpah2,blpbh6);
	D1_9 = _mm512_madd52lo_epu64(D1_9,alpah2,blpbh7);
	D1_10 = _mm512_madd52hi_epu64(D1_10,alpah2,blpbh7);
	D1_10 = _mm512_madd52lo_epu64(D1_10,alpah2,blpbh8);
	D1_11 = _mm512_madd52hi_epu64(D1_11,alpah2,blpbh8);
	D1_11 = _mm512_madd52lo_epu64(D1_11,alpah2,blpbh9);
	D1_12 = _mm512_madd52hi_epu64(D1_12,alpah2,blpbh9);
	D1_12 = _mm512_madd52lo_epu64(D1_12,alpah2,blpbh10);
	D1_13 = _mm512_madd52hi_epu64(D1_13,alpah2,blpbh10);
	D1_13 = _mm512_madd52lo_epu64(D1_13,alpah2,blpbh11);
	D1_14 = _mm512_madd52hi_epu64(D1_14,alpah2,blpbh11);
	D1_14 = _mm512_madd52lo_epu64(D1_14,alpah2,blpbh12);
	D1_15 = _mm512_madd52hi_epu64(D1_15,alpah2,blpbh12);
	D1_15 = _mm512_madd52lo_epu64(D1_15,alpah2,blpbh13);
	D1_16 = _mm512_madd52hi_epu64(D1_16,alpah2,blpbh13);
	D1_16 = _mm512_madd52lo_epu64(D1_16,alpah2,blpbh14);
	D1_17 = _mm512_madd52hi_epu64(D1_17,alpah2,blpbh14);
	D1_17 = _mm512_madd52lo_epu64(D1_17,alpah2,blpbh15);
	D1_18 = _mm512_madd52hi_epu64(D1_18,alpah2,blpbh15);
	D1_18 = _mm512_madd52lo_epu64(D1_18,alpah2,blpbh16);
	D1_19 = _mm512_madd52hi_epu64(D1_19,alpah2,blpbh16);
	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah2,blpbh17);
	D1_20 = _mm512_madd52hi_epu64(D1_20,alpah2,blpbh17);
	D1_20 = _mm512_madd52lo_epu64(D1_20,alpah2,blpbh18);
	D1_21 = _mm512_madd52hi_epu64(D1_21,alpah2,blpbh18);
	D1_21 = _mm512_madd52lo_epu64(D1_21,alpah2,blpbh19);
	D1_22 = _mm512_madd52hi_epu64(zero,alpah2,blpbh19);

	D1_3 = _mm512_madd52lo_epu64(D1_3,alpah3,blpbh0);
	D1_4 = _mm512_madd52hi_epu64(D1_4,alpah3,blpbh0);
	D1_4 = _mm512_madd52lo_epu64(D1_4,alpah3,blpbh1);
	D1_5 = _mm512_madd52hi_epu64(D1_5,alpah3,blpbh1);
	D1_5 = _mm512_madd52lo_epu64(D1_5,alpah3,blpbh2);
	D1_6 = _mm512_madd52hi_epu64(D1_6,alpah3,blpbh2);
	D1_6 = _mm512_madd52lo_epu64(D1_6,alpah3,blpbh3);
	D1_7 = _mm512_madd52hi_epu64(D1_7,alpah3,blpbh3);
	D1_7 = _mm512_madd52lo_epu64(D1_7,alpah3,blpbh4);
	D1_8 = _mm512_madd52hi_epu64(D1_8,alpah3,blpbh4);
	D1_8 = _mm512_madd52lo_epu64(D1_8,alpah3,blpbh5);
	D1_9 = _mm512_madd52hi_epu64(D1_9,alpah3,blpbh5);
	D1_9 = _mm512_madd52lo_epu64(D1_9,alpah3,blpbh6);
	D1_10 = _mm512_madd52hi_epu64(D1_10,alpah3,blpbh6);
	D1_10 = _mm512_madd52lo_epu64(D1_10,alpah3,blpbh7);
	D1_11 = _mm512_madd52hi_epu64(D1_11,alpah3,blpbh7);
	D1_11 = _mm512_madd52lo_epu64(D1_11,alpah3,blpbh8);
	D1_12 = _mm512_madd52hi_epu64(D1_12,alpah3,blpbh8);
	D1_12 = _mm512_madd52lo_epu64(D1_12,alpah3,blpbh9);
	D1_13 = _mm512_madd52hi_epu64(D1_13,alpah3,blpbh9);
	D1_13 = _mm512_madd52lo_epu64(D1_13,alpah3,blpbh10);
	D1_14 = _mm512_madd52hi_epu64(D1_14,alpah3,blpbh10);
	D1_14 = _mm512_madd52lo_epu64(D1_14,alpah3,blpbh11);
	D1_15 = _mm512_madd52hi_epu64(D1_15,alpah3,blpbh11);
	D1_15 = _mm512_madd52lo_epu64(D1_15,alpah3,blpbh12);
	D1_16 = _mm512_madd52hi_epu64(D1_16,alpah3,blpbh12);
	D1_16 = _mm512_madd52lo_epu64(D1_16,alpah3,blpbh13);
	D1_17 = _mm512_madd52hi_epu64(D1_17,alpah3,blpbh13);
	D1_17 = _mm512_madd52lo_epu64(D1_17,alpah3,blpbh14);
	D1_18 = _mm512_madd52hi_epu64(D1_18,alpah3,blpbh14);
	D1_18 = _mm512_madd52lo_epu64(D1_18,alpah3,blpbh15);
	D1_19 = _mm512_madd52hi_epu64(D1_19,alpah3,blpbh15);
	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah3,blpbh16);
	D1_20 = _mm512_madd52hi_epu64(D1_20,alpah3,blpbh16);
	D1_20 = _mm512_madd52lo_epu64(D1_20,alpah3,blpbh17);
	D1_21 = _mm512_madd52hi_epu64(D1_21,alpah3,blpbh17);
	D1_21 = _mm512_madd52lo_epu64(D1_21,alpah3,blpbh18);
	D1_22 = _mm512_madd52hi_epu64(D1_22,alpah3,blpbh18);
	D1_22 = _mm512_madd52lo_epu64(D1_22,alpah3,blpbh19);
	D1_23 = _mm512_madd52hi_epu64(zero,alpah3,blpbh19);

	D1_4 = _mm512_madd52lo_epu64(D1_4,alpah4,blpbh0);
	D1_5 = _mm512_madd52hi_epu64(D1_5,alpah4,blpbh0);
	D1_5 = _mm512_madd52lo_epu64(D1_5,alpah4,blpbh1);
	D1_6 = _mm512_madd52hi_epu64(D1_6,alpah4,blpbh1);
	D1_6 = _mm512_madd52lo_epu64(D1_6,alpah4,blpbh2);
	D1_7 = _mm512_madd52hi_epu64(D1_7,alpah4,blpbh2);
	D1_7 = _mm512_madd52lo_epu64(D1_7,alpah4,blpbh3);
	D1_8 = _mm512_madd52hi_epu64(D1_8,alpah4,blpbh3);
	D1_8 = _mm512_madd52lo_epu64(D1_8,alpah4,blpbh4);
	D1_9 = _mm512_madd52hi_epu64(D1_9,alpah4,blpbh4);
	D1_9 = _mm512_madd52lo_epu64(D1_9,alpah4,blpbh5);
	D1_10 = _mm512_madd52hi_epu64(D1_10,alpah4,blpbh5);
	D1_10 = _mm512_madd52lo_epu64(D1_10,alpah4,blpbh6);
	D1_11 = _mm512_madd52hi_epu64(D1_11,alpah4,blpbh6);
	D1_11 = _mm512_madd52lo_epu64(D1_11,alpah4,blpbh7);
	D1_12 = _mm512_madd52hi_epu64(D1_12,alpah4,blpbh7);
	D1_12 = _mm512_madd52lo_epu64(D1_12,alpah4,blpbh8);
	D1_13 = _mm512_madd52hi_epu64(D1_13,alpah4,blpbh8);
	D1_13 = _mm512_madd52lo_epu64(D1_13,alpah4,blpbh9);
	D1_14 = _mm512_madd52hi_epu64(D1_14,alpah4,blpbh9);
	D1_14 = _mm512_madd52lo_epu64(D1_14,alpah4,blpbh10);
	D1_15 = _mm512_madd52hi_epu64(D1_15,alpah4,blpbh10);
	D1_15 = _mm512_madd52lo_epu64(D1_15,alpah4,blpbh11);
	D1_16 = _mm512_madd52hi_epu64(D1_16,alpah4,blpbh11);
	D1_16 = _mm512_madd52lo_epu64(D1_16,alpah4,blpbh12);
	D1_17 = _mm512_madd52hi_epu64(D1_17,alpah4,blpbh12);
	D1_17 = _mm512_madd52lo_epu64(D1_17,alpah4,blpbh13);
	D1_18 = _mm512_madd52hi_epu64(D1_18,alpah4,blpbh13);
	D1_18 = _mm512_madd52lo_epu64(D1_18,alpah4,blpbh14);
	D1_19 = _mm512_madd52hi_epu64(D1_19,alpah4,blpbh14);
	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah4,blpbh15);
	D1_20 = _mm512_madd52hi_epu64(D1_20,alpah4,blpbh15);
	D1_20 = _mm512_madd52lo_epu64(D1_20,alpah4,blpbh16);
	D1_21 = _mm512_madd52hi_epu64(D1_21,alpah4,blpbh16);
	D1_21 = _mm512_madd52lo_epu64(D1_21,alpah4,blpbh17);
	D1_22 = _mm512_madd52hi_epu64(D1_22,alpah4,blpbh17);
	D1_22 = _mm512_madd52lo_epu64(D1_22,alpah4,blpbh18);
	D1_23 = _mm512_madd52hi_epu64(D1_23,alpah4,blpbh18);
	D1_23 = _mm512_madd52lo_epu64(D1_23,alpah4,blpbh19);
	D1_24 = _mm512_madd52hi_epu64(zero,alpah4,blpbh19);

	D1_5 = _mm512_madd52lo_epu64(D1_5,alpah5,blpbh0);
	D1_6 = _mm512_madd52hi_epu64(D1_6,alpah5,blpbh0);
	D1_6 = _mm512_madd52lo_epu64(D1_6,alpah5,blpbh1);
	D1_7 = _mm512_madd52hi_epu64(D1_7,alpah5,blpbh1);
	D1_7 = _mm512_madd52lo_epu64(D1_7,alpah5,blpbh2);
	D1_8 = _mm512_madd52hi_epu64(D1_8,alpah5,blpbh2);
	D1_8 = _mm512_madd52lo_epu64(D1_8,alpah5,blpbh3);
	D1_9 = _mm512_madd52hi_epu64(D1_9,alpah5,blpbh3);
	D1_9 = _mm512_madd52lo_epu64(D1_9,alpah5,blpbh4);
	D1_10 = _mm512_madd52hi_epu64(D1_10,alpah5,blpbh4);
	D1_10 = _mm512_madd52lo_epu64(D1_10,alpah5,blpbh5);
	D1_11 = _mm512_madd52hi_epu64(D1_11,alpah5,blpbh5);
	D1_11 = _mm512_madd52lo_epu64(D1_11,alpah5,blpbh6);
	D1_12 = _mm512_madd52hi_epu64(D1_12,alpah5,blpbh6);
	D1_12 = _mm512_madd52lo_epu64(D1_12,alpah5,blpbh7);
	D1_13 = _mm512_madd52hi_epu64(D1_13,alpah5,blpbh7);
	D1_13 = _mm512_madd52lo_epu64(D1_13,alpah5,blpbh8);
	D1_14 = _mm512_madd52hi_epu64(D1_14,alpah5,blpbh8);
	D1_14 = _mm512_madd52lo_epu64(D1_14,alpah5,blpbh9);
	D1_15 = _mm512_madd52hi_epu64(D1_15,alpah5,blpbh9);
	D1_15 = _mm512_madd52lo_epu64(D1_15,alpah5,blpbh10);
	D1_16 = _mm512_madd52hi_epu64(D1_16,alpah5,blpbh10);
	D1_16 = _mm512_madd52lo_epu64(D1_16,alpah5,blpbh11);
	D1_17 = _mm512_madd52hi_epu64(D1_17,alpah5,blpbh11);
	D1_17 = _mm512_madd52lo_epu64(D1_17,alpah5,blpbh12);
	D1_18 = _mm512_madd52hi_epu64(D1_18,alpah5,blpbh12);
	D1_18 = _mm512_madd52lo_epu64(D1_18,alpah5,blpbh13);
	D1_19 = _mm512_madd52hi_epu64(D1_19,alpah5,blpbh13);
	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah5,blpbh14);
	D1_20 = _mm512_madd52hi_epu64(D1_20,alpah5,blpbh14);
	D1_20 = _mm512_madd52lo_epu64(D1_20,alpah5,blpbh15);
	D1_21 = _mm512_madd52hi_epu64(D1_21,alpah5,blpbh15);
	D1_21 = _mm512_madd52lo_epu64(D1_21,alpah5,blpbh16);
	D1_22 = _mm512_madd52hi_epu64(D1_22,alpah5,blpbh16);
	D1_22 = _mm512_madd52lo_epu64(D1_22,alpah5,blpbh17);
	D1_23 = _mm512_madd52hi_epu64(D1_23,alpah5,blpbh17);
	D1_23 = _mm512_madd52lo_epu64(D1_23,alpah5,blpbh18);
	D1_24 = _mm512_madd52hi_epu64(D1_24,alpah5,blpbh18);
	D1_24 = _mm512_madd52lo_epu64(D1_24,alpah5,blpbh19);
	D1_25 = _mm512_madd52hi_epu64(zero,alpah5,blpbh19);

	D1_6 = _mm512_madd52lo_epu64(D1_6,alpah6,blpbh0);
	D1_7 = _mm512_madd52hi_epu64(D1_7,alpah6,blpbh0);
	D1_7 = _mm512_madd52lo_epu64(D1_7,alpah6,blpbh1);
	D1_8 = _mm512_madd52hi_epu64(D1_8,alpah6,blpbh1);
	D1_8 = _mm512_madd52lo_epu64(D1_8,alpah6,blpbh2);
	D1_9 = _mm512_madd52hi_epu64(D1_9,alpah6,blpbh2);
	D1_9 = _mm512_madd52lo_epu64(D1_9,alpah6,blpbh3);
	D1_10 = _mm512_madd52hi_epu64(D1_10,alpah6,blpbh3);
	D1_10 = _mm512_madd52lo_epu64(D1_10,alpah6,blpbh4);
	D1_11 = _mm512_madd52hi_epu64(D1_11,alpah6,blpbh4);
	D1_11 = _mm512_madd52lo_epu64(D1_11,alpah6,blpbh5);
	D1_12 = _mm512_madd52hi_epu64(D1_12,alpah6,blpbh5);
	D1_12 = _mm512_madd52lo_epu64(D1_12,alpah6,blpbh6);
	D1_13 = _mm512_madd52hi_epu64(D1_13,alpah6,blpbh6);
	D1_13 = _mm512_madd52lo_epu64(D1_13,alpah6,blpbh7);
	D1_14 = _mm512_madd52hi_epu64(D1_14,alpah6,blpbh7);
	D1_14 = _mm512_madd52lo_epu64(D1_14,alpah6,blpbh8);
	D1_15 = _mm512_madd52hi_epu64(D1_15,alpah6,blpbh8);
	D1_15 = _mm512_madd52lo_epu64(D1_15,alpah6,blpbh9);
	D1_16 = _mm512_madd52hi_epu64(D1_16,alpah6,blpbh9);
	D1_16 = _mm512_madd52lo_epu64(D1_16,alpah6,blpbh10);
	D1_17 = _mm512_madd52hi_epu64(D1_17,alpah6,blpbh10);
	D1_17 = _mm512_madd52lo_epu64(D1_17,alpah6,blpbh11);
	D1_18 = _mm512_madd52hi_epu64(D1_18,alpah6,blpbh11);
	D1_18 = _mm512_madd52lo_epu64(D1_18,alpah6,blpbh12);
	D1_19 = _mm512_madd52hi_epu64(D1_19,alpah6,blpbh12);
	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah6,blpbh13);
	D1_20 = _mm512_madd52hi_epu64(D1_20,alpah6,blpbh13);
	D1_20 = _mm512_madd52lo_epu64(D1_20,alpah6,blpbh14);
	D1_21 = _mm512_madd52hi_epu64(D1_21,alpah6,blpbh14);
	D1_21 = _mm512_madd52lo_epu64(D1_21,alpah6,blpbh15);
	D1_22 = _mm512_madd52hi_epu64(D1_22,alpah6,blpbh15);
	D1_22 = _mm512_madd52lo_epu64(D1_22,alpah6,blpbh16);
	D1_23 = _mm512_madd52hi_epu64(D1_23,alpah6,blpbh16);
	D1_23 = _mm512_madd52lo_epu64(D1_23,alpah6,blpbh17);
	D1_24 = _mm512_madd52hi_epu64(D1_24,alpah6,blpbh17);
	D1_24 = _mm512_madd52lo_epu64(D1_24,alpah6,blpbh18);
	D1_25 = _mm512_madd52hi_epu64(D1_25,alpah6,blpbh18);
	D1_25 = _mm512_madd52lo_epu64(D1_25,alpah6,blpbh19);
	D1_26 = _mm512_madd52hi_epu64(zero,alpah6,blpbh19);

	D1_7 = _mm512_madd52lo_epu64(D1_7,alpah7,blpbh0);
	D1_8 = _mm512_madd52hi_epu64(D1_8,alpah7,blpbh0);
	D1_8 = _mm512_madd52lo_epu64(D1_8,alpah7,blpbh1);
	D1_9 = _mm512_madd52hi_epu64(D1_9,alpah7,blpbh1);
	D1_9 = _mm512_madd52lo_epu64(D1_9,alpah7,blpbh2);
	D1_10 = _mm512_madd52hi_epu64(D1_10,alpah7,blpbh2);
	D1_10 = _mm512_madd52lo_epu64(D1_10,alpah7,blpbh3);
	D1_11 = _mm512_madd52hi_epu64(D1_11,alpah7,blpbh3);
	D1_11 = _mm512_madd52lo_epu64(D1_11,alpah7,blpbh4);
	D1_12 = _mm512_madd52hi_epu64(D1_12,alpah7,blpbh4);
	D1_12 = _mm512_madd52lo_epu64(D1_12,alpah7,blpbh5);
	D1_13 = _mm512_madd52hi_epu64(D1_13,alpah7,blpbh5);
	D1_13 = _mm512_madd52lo_epu64(D1_13,alpah7,blpbh6);
	D1_14 = _mm512_madd52hi_epu64(D1_14,alpah7,blpbh6);
	D1_14 = _mm512_madd52lo_epu64(D1_14,alpah7,blpbh7);
	D1_15 = _mm512_madd52hi_epu64(D1_15,alpah7,blpbh7);
	D1_15 = _mm512_madd52lo_epu64(D1_15,alpah7,blpbh8);
	D1_16 = _mm512_madd52hi_epu64(D1_16,alpah7,blpbh8);
	D1_16 = _mm512_madd52lo_epu64(D1_16,alpah7,blpbh9);
	D1_17 = _mm512_madd52hi_epu64(D1_17,alpah7,blpbh9);
	D1_17 = _mm512_madd52lo_epu64(D1_17,alpah7,blpbh10);
	D1_18 = _mm512_madd52hi_epu64(D1_18,alpah7,blpbh10);
	D1_18 = _mm512_madd52lo_epu64(D1_18,alpah7,blpbh11);
	D1_19 = _mm512_madd52hi_epu64(D1_19,alpah7,blpbh11);
	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah7,blpbh12);
	D1_20 = _mm512_madd52hi_epu64(D1_20,alpah7,blpbh12);
	D1_20 = _mm512_madd52lo_epu64(D1_20,alpah7,blpbh13);
	D1_21 = _mm512_madd52hi_epu64(D1_21,alpah7,blpbh13);
	D1_21 = _mm512_madd52lo_epu64(D1_21,alpah7,blpbh14);
	D1_22 = _mm512_madd52hi_epu64(D1_22,alpah7,blpbh14);
	D1_22 = _mm512_madd52lo_epu64(D1_22,alpah7,blpbh15);
	D1_23 = _mm512_madd52hi_epu64(D1_23,alpah7,blpbh15);
	D1_23 = _mm512_madd52lo_epu64(D1_23,alpah7,blpbh16);
	D1_24 = _mm512_madd52hi_epu64(D1_24,alpah7,blpbh16);
	D1_24 = _mm512_madd52lo_epu64(D1_24,alpah7,blpbh17);
	D1_25 = _mm512_madd52hi_epu64(D1_25,alpah7,blpbh17);
	D1_25 = _mm512_madd52lo_epu64(D1_25,alpah7,blpbh18);
	D1_26 = _mm512_madd52hi_epu64(D1_26,alpah7,blpbh18);
	D1_26 = _mm512_madd52lo_epu64(D1_26,alpah7,blpbh19);
	D1_27 = _mm512_madd52hi_epu64(zero,alpah7,blpbh19);

	D1_8 = _mm512_madd52lo_epu64(D1_8,alpah8,blpbh0);
	D1_9 = _mm512_madd52hi_epu64(D1_9,alpah8,blpbh0);
	D1_9 = _mm512_madd52lo_epu64(D1_9,alpah8,blpbh1);
	D1_10 = _mm512_madd52hi_epu64(D1_10,alpah8,blpbh1);
	D1_10 = _mm512_madd52lo_epu64(D1_10,alpah8,blpbh2);
	D1_11 = _mm512_madd52hi_epu64(D1_11,alpah8,blpbh2);
	D1_11 = _mm512_madd52lo_epu64(D1_11,alpah8,blpbh3);
	D1_12 = _mm512_madd52hi_epu64(D1_12,alpah8,blpbh3);
	D1_12 = _mm512_madd52lo_epu64(D1_12,alpah8,blpbh4);
	D1_13 = _mm512_madd52hi_epu64(D1_13,alpah8,blpbh4);
	D1_13 = _mm512_madd52lo_epu64(D1_13,alpah8,blpbh5);
	D1_14 = _mm512_madd52hi_epu64(D1_14,alpah8,blpbh5);
	D1_14 = _mm512_madd52lo_epu64(D1_14,alpah8,blpbh6);
	D1_15 = _mm512_madd52hi_epu64(D1_15,alpah8,blpbh6);
	D1_15 = _mm512_madd52lo_epu64(D1_15,alpah8,blpbh7);
	D1_16 = _mm512_madd52hi_epu64(D1_16,alpah8,blpbh7);
	D1_16 = _mm512_madd52lo_epu64(D1_16,alpah8,blpbh8);
	D1_17 = _mm512_madd52hi_epu64(D1_17,alpah8,blpbh8);
	D1_17 = _mm512_madd52lo_epu64(D1_17,alpah8,blpbh9);
	D1_18 = _mm512_madd52hi_epu64(D1_18,alpah8,blpbh9);
	D1_18 = _mm512_madd52lo_epu64(D1_18,alpah8,blpbh10);
	D1_19 = _mm512_madd52hi_epu64(D1_19,alpah8,blpbh10);
	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah8,blpbh11);
	D1_20 = _mm512_madd52hi_epu64(D1_20,alpah8,blpbh11);
	D1_20 = _mm512_madd52lo_epu64(D1_20,alpah8,blpbh12);
	D1_21 = _mm512_madd52hi_epu64(D1_21,alpah8,blpbh12);
	D1_21 = _mm512_madd52lo_epu64(D1_21,alpah8,blpbh13);
	D1_22 = _mm512_madd52hi_epu64(D1_22,alpah8,blpbh13);
	D1_22 = _mm512_madd52lo_epu64(D1_22,alpah8,blpbh14);
	D1_23 = _mm512_madd52hi_epu64(D1_23,alpah8,blpbh14);
	D1_23 = _mm512_madd52lo_epu64(D1_23,alpah8,blpbh15);
	D1_24 = _mm512_madd52hi_epu64(D1_24,alpah8,blpbh15);
	D1_24 = _mm512_madd52lo_epu64(D1_24,alpah8,blpbh16);
	D1_25 = _mm512_madd52hi_epu64(D1_25,alpah8,blpbh16);
	D1_25 = _mm512_madd52lo_epu64(D1_25,alpah8,blpbh17);
	D1_26 = _mm512_madd52hi_epu64(D1_26,alpah8,blpbh17);
	D1_26 = _mm512_madd52lo_epu64(D1_26,alpah8,blpbh18);
	D1_27 = _mm512_madd52hi_epu64(D1_27,alpah8,blpbh18);
	D1_27 = _mm512_madd52lo_epu64(D1_27,alpah8,blpbh19);
	D1_28 = _mm512_madd52hi_epu64(zero,alpah8,blpbh19);

	D1_9 = _mm512_madd52lo_epu64(D1_9,alpah9,blpbh0);
	D1_10 = _mm512_madd52hi_epu64(D1_10,alpah9,blpbh0);
	D1_10 = _mm512_madd52lo_epu64(D1_10,alpah9,blpbh1);
	D1_11 = _mm512_madd52hi_epu64(D1_11,alpah9,blpbh1);
	D1_11 = _mm512_madd52lo_epu64(D1_11,alpah9,blpbh2);
	D1_12 = _mm512_madd52hi_epu64(D1_12,alpah9,blpbh2);
	D1_12 = _mm512_madd52lo_epu64(D1_12,alpah9,blpbh3);
	D1_13 = _mm512_madd52hi_epu64(D1_13,alpah9,blpbh3);
	D1_13 = _mm512_madd52lo_epu64(D1_13,alpah9,blpbh4);
	D1_14 = _mm512_madd52hi_epu64(D1_14,alpah9,blpbh4);
	D1_14 = _mm512_madd52lo_epu64(D1_14,alpah9,blpbh5);
	D1_15 = _mm512_madd52hi_epu64(D1_15,alpah9,blpbh5);
	D1_15 = _mm512_madd52lo_epu64(D1_15,alpah9,blpbh6);
	D1_16 = _mm512_madd52hi_epu64(D1_16,alpah9,blpbh6);
	D1_16 = _mm512_madd52lo_epu64(D1_16,alpah9,blpbh7);
	D1_17 = _mm512_madd52hi_epu64(D1_17,alpah9,blpbh7);
	D1_17 = _mm512_madd52lo_epu64(D1_17,alpah9,blpbh8);
	D1_18 = _mm512_madd52hi_epu64(D1_18,alpah9,blpbh8);
	D1_18 = _mm512_madd52lo_epu64(D1_18,alpah9,blpbh9);
	D1_19 = _mm512_madd52hi_epu64(D1_19,alpah9,blpbh9);
	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah9,blpbh10);
	D1_20 = _mm512_madd52hi_epu64(D1_20,alpah9,blpbh10);
	D1_20 = _mm512_madd52lo_epu64(D1_20,alpah9,blpbh11);
	D1_21 = _mm512_madd52hi_epu64(D1_21,alpah9,blpbh11);
	D1_21 = _mm512_madd52lo_epu64(D1_21,alpah9,blpbh12);
	D1_22 = _mm512_madd52hi_epu64(D1_22,alpah9,blpbh12);
	D1_22 = _mm512_madd52lo_epu64(D1_22,alpah9,blpbh13);
	D1_23 = _mm512_madd52hi_epu64(D1_23,alpah9,blpbh13);
	D1_23 = _mm512_madd52lo_epu64(D1_23,alpah9,blpbh14);
	D1_24 = _mm512_madd52hi_epu64(D1_24,alpah9,blpbh14);
	D1_24 = _mm512_madd52lo_epu64(D1_24,alpah9,blpbh15);
	D1_25 = _mm512_madd52hi_epu64(D1_25,alpah9,blpbh15);
	D1_25 = _mm512_madd52lo_epu64(D1_25,alpah9,blpbh16);
	D1_26 = _mm512_madd52hi_epu64(D1_26,alpah9,blpbh16);
	D1_26 = _mm512_madd52lo_epu64(D1_26,alpah9,blpbh17);
	D1_27 = _mm512_madd52hi_epu64(D1_27,alpah9,blpbh17);
	D1_27 = _mm512_madd52lo_epu64(D1_27,alpah9,blpbh18);
	D1_28 = _mm512_madd52hi_epu64(D1_28,alpah9,blpbh18);
	D1_28 = _mm512_madd52lo_epu64(D1_28,alpah9,blpbh19);
	D1_29 = _mm512_madd52hi_epu64(zero,alpah9,blpbh19);

	D1_10 = _mm512_madd52lo_epu64(D1_10,alpah10,blpbh0);
	D1_11 = _mm512_madd52hi_epu64(D1_11,alpah10,blpbh0);
	D1_11 = _mm512_madd52lo_epu64(D1_11,alpah10,blpbh1);
	D1_12 = _mm512_madd52hi_epu64(D1_12,alpah10,blpbh1);
	D1_12 = _mm512_madd52lo_epu64(D1_12,alpah10,blpbh2);
	D1_13 = _mm512_madd52hi_epu64(D1_13,alpah10,blpbh2);
	D1_13 = _mm512_madd52lo_epu64(D1_13,alpah10,blpbh3);
	D1_14 = _mm512_madd52hi_epu64(D1_14,alpah10,blpbh3);
	D1_14 = _mm512_madd52lo_epu64(D1_14,alpah10,blpbh4);
	D1_15 = _mm512_madd52hi_epu64(D1_15,alpah10,blpbh4);
	D1_15 = _mm512_madd52lo_epu64(D1_15,alpah10,blpbh5);
	D1_16 = _mm512_madd52hi_epu64(D1_16,alpah10,blpbh5);
	D1_16 = _mm512_madd52lo_epu64(D1_16,alpah10,blpbh6);
	D1_17 = _mm512_madd52hi_epu64(D1_17,alpah10,blpbh6);
	D1_17 = _mm512_madd52lo_epu64(D1_17,alpah10,blpbh7);
	D1_18 = _mm512_madd52hi_epu64(D1_18,alpah10,blpbh7);
	D1_18 = _mm512_madd52lo_epu64(D1_18,alpah10,blpbh8);
	D1_19 = _mm512_madd52hi_epu64(D1_19,alpah10,blpbh8);
	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah10,blpbh9);
	D1_20 = _mm512_madd52hi_epu64(D1_20,alpah10,blpbh9);
	D1_20 = _mm512_madd52lo_epu64(D1_20,alpah10,blpbh10);
	D1_21 = _mm512_madd52hi_epu64(D1_21,alpah10,blpbh10);
	D1_21 = _mm512_madd52lo_epu64(D1_21,alpah10,blpbh11);
	D1_22 = _mm512_madd52hi_epu64(D1_22,alpah10,blpbh11);
	D1_22 = _mm512_madd52lo_epu64(D1_22,alpah10,blpbh12);
	D1_23 = _mm512_madd52hi_epu64(D1_23,alpah10,blpbh12);
	D1_23 = _mm512_madd52lo_epu64(D1_23,alpah10,blpbh13);
	D1_24 = _mm512_madd52hi_epu64(D1_24,alpah10,blpbh13);
	D1_24 = _mm512_madd52lo_epu64(D1_24,alpah10,blpbh14);
	D1_25 = _mm512_madd52hi_epu64(D1_25,alpah10,blpbh14);
	D1_25 = _mm512_madd52lo_epu64(D1_25,alpah10,blpbh15);
	D1_26 = _mm512_madd52hi_epu64(D1_26,alpah10,blpbh15);
	D1_26 = _mm512_madd52lo_epu64(D1_26,alpah10,blpbh16);
	D1_27 = _mm512_madd52hi_epu64(D1_27,alpah10,blpbh16);
	D1_27 = _mm512_madd52lo_epu64(D1_27,alpah10,blpbh17);
	D1_28 = _mm512_madd52hi_epu64(D1_28,alpah10,blpbh17);
	D1_28 = _mm512_madd52lo_epu64(D1_28,alpah10,blpbh18);
	D1_29 = _mm512_madd52hi_epu64(D1_29,alpah10,blpbh18);
	D1_29 = _mm512_madd52lo_epu64(D1_29,alpah10,blpbh19);
	D1_30 = _mm512_madd52hi_epu64(zero,alpah10,blpbh19);

	D1_11 = _mm512_madd52lo_epu64(D1_11,alpah11,blpbh0);
	D1_12 = _mm512_madd52hi_epu64(D1_12,alpah11,blpbh0);
	D1_12 = _mm512_madd52lo_epu64(D1_12,alpah11,blpbh1);
	D1_13 = _mm512_madd52hi_epu64(D1_13,alpah11,blpbh1);
	D1_13 = _mm512_madd52lo_epu64(D1_13,alpah11,blpbh2);
	D1_14 = _mm512_madd52hi_epu64(D1_14,alpah11,blpbh2);
	D1_14 = _mm512_madd52lo_epu64(D1_14,alpah11,blpbh3);
	D1_15 = _mm512_madd52hi_epu64(D1_15,alpah11,blpbh3);
	D1_15 = _mm512_madd52lo_epu64(D1_15,alpah11,blpbh4);
	D1_16 = _mm512_madd52hi_epu64(D1_16,alpah11,blpbh4);
	D1_16 = _mm512_madd52lo_epu64(D1_16,alpah11,blpbh5);
	D1_17 = _mm512_madd52hi_epu64(D1_17,alpah11,blpbh5);
	D1_17 = _mm512_madd52lo_epu64(D1_17,alpah11,blpbh6);
	D1_18 = _mm512_madd52hi_epu64(D1_18,alpah11,blpbh6);
	D1_18 = _mm512_madd52lo_epu64(D1_18,alpah11,blpbh7);
	D1_19 = _mm512_madd52hi_epu64(D1_19,alpah11,blpbh7);
	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah11,blpbh8);
	D1_20 = _mm512_madd52hi_epu64(D1_20,alpah11,blpbh8);
	D1_20 = _mm512_madd52lo_epu64(D1_20,alpah11,blpbh9);
	D1_21 = _mm512_madd52hi_epu64(D1_21,alpah11,blpbh9);
	D1_21 = _mm512_madd52lo_epu64(D1_21,alpah11,blpbh10);
	D1_22 = _mm512_madd52hi_epu64(D1_22,alpah11,blpbh10);
	D1_22 = _mm512_madd52lo_epu64(D1_22,alpah11,blpbh11);
	D1_23 = _mm512_madd52hi_epu64(D1_23,alpah11,blpbh11);
	D1_23 = _mm512_madd52lo_epu64(D1_23,alpah11,blpbh12);
	D1_24 = _mm512_madd52hi_epu64(D1_24,alpah11,blpbh12);
	D1_24 = _mm512_madd52lo_epu64(D1_24,alpah11,blpbh13);
	D1_25 = _mm512_madd52hi_epu64(D1_25,alpah11,blpbh13);
	D1_25 = _mm512_madd52lo_epu64(D1_25,alpah11,blpbh14);
	D1_26 = _mm512_madd52hi_epu64(D1_26,alpah11,blpbh14);
	D1_26 = _mm512_madd52lo_epu64(D1_26,alpah11,blpbh15);
	D1_27 = _mm512_madd52hi_epu64(D1_27,alpah11,blpbh15);
	D1_27 = _mm512_madd52lo_epu64(D1_27,alpah11,blpbh16);
	D1_28 = _mm512_madd52hi_epu64(D1_28,alpah11,blpbh16);
	D1_28 = _mm512_madd52lo_epu64(D1_28,alpah11,blpbh17);
	D1_29 = _mm512_madd52hi_epu64(D1_29,alpah11,blpbh17);
	D1_29 = _mm512_madd52lo_epu64(D1_29,alpah11,blpbh18);
	D1_30 = _mm512_madd52hi_epu64(D1_30,alpah11,blpbh18);
	D1_30 = _mm512_madd52lo_epu64(D1_30,alpah11,blpbh19);
	D1_31 = _mm512_madd52hi_epu64(zero,alpah11,blpbh19);

	D1_12 = _mm512_madd52lo_epu64(D1_12,alpah12,blpbh0);
	D1_13 = _mm512_madd52hi_epu64(D1_13,alpah12,blpbh0);
	D1_13 = _mm512_madd52lo_epu64(D1_13,alpah12,blpbh1);
	D1_14 = _mm512_madd52hi_epu64(D1_14,alpah12,blpbh1);
	D1_14 = _mm512_madd52lo_epu64(D1_14,alpah12,blpbh2);
	D1_15 = _mm512_madd52hi_epu64(D1_15,alpah12,blpbh2);
	D1_15 = _mm512_madd52lo_epu64(D1_15,alpah12,blpbh3);
	D1_16 = _mm512_madd52hi_epu64(D1_16,alpah12,blpbh3);
	D1_16 = _mm512_madd52lo_epu64(D1_16,alpah12,blpbh4);
	D1_17 = _mm512_madd52hi_epu64(D1_17,alpah12,blpbh4);
	D1_17 = _mm512_madd52lo_epu64(D1_17,alpah12,blpbh5);
	D1_18 = _mm512_madd52hi_epu64(D1_18,alpah12,blpbh5);
	D1_18 = _mm512_madd52lo_epu64(D1_18,alpah12,blpbh6);
	D1_19 = _mm512_madd52hi_epu64(D1_19,alpah12,blpbh6);
	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah12,blpbh7);
	D1_20 = _mm512_madd52hi_epu64(D1_20,alpah12,blpbh7);
	D1_20 = _mm512_madd52lo_epu64(D1_20,alpah12,blpbh8);
	D1_21 = _mm512_madd52hi_epu64(D1_21,alpah12,blpbh8);
	D1_21 = _mm512_madd52lo_epu64(D1_21,alpah12,blpbh9);
	D1_22 = _mm512_madd52hi_epu64(D1_22,alpah12,blpbh9);
	D1_22 = _mm512_madd52lo_epu64(D1_22,alpah12,blpbh10);
	D1_23 = _mm512_madd52hi_epu64(D1_23,alpah12,blpbh10);
	D1_23 = _mm512_madd52lo_epu64(D1_23,alpah12,blpbh11);
	D1_24 = _mm512_madd52hi_epu64(D1_24,alpah12,blpbh11);
	D1_24 = _mm512_madd52lo_epu64(D1_24,alpah12,blpbh12);
	D1_25 = _mm512_madd52hi_epu64(D1_25,alpah12,blpbh12);
	D1_25 = _mm512_madd52lo_epu64(D1_25,alpah12,blpbh13);
	D1_26 = _mm512_madd52hi_epu64(D1_26,alpah12,blpbh13);
	D1_26 = _mm512_madd52lo_epu64(D1_26,alpah12,blpbh14);
	D1_27 = _mm512_madd52hi_epu64(D1_27,alpah12,blpbh14);
	D1_27 = _mm512_madd52lo_epu64(D1_27,alpah12,blpbh15);
	D1_28 = _mm512_madd52hi_epu64(D1_28,alpah12,blpbh15);
	D1_28 = _mm512_madd52lo_epu64(D1_28,alpah12,blpbh16);
	D1_29 = _mm512_madd52hi_epu64(D1_29,alpah12,blpbh16);
	D1_29 = _mm512_madd52lo_epu64(D1_29,alpah12,blpbh17);
	D1_30 = _mm512_madd52hi_epu64(D1_30,alpah12,blpbh17);
	D1_30 = _mm512_madd52lo_epu64(D1_30,alpah12,blpbh18);
	D1_31 = _mm512_madd52hi_epu64(D1_31,alpah12,blpbh18);
	D1_31 = _mm512_madd52lo_epu64(D1_31,alpah12,blpbh19);
	D1_32 = _mm512_madd52hi_epu64(zero,alpah12,blpbh19);

	D1_13 = _mm512_madd52lo_epu64(D1_13,alpah13,blpbh0);
	D1_14 = _mm512_madd52hi_epu64(D1_14,alpah13,blpbh0);
	D1_14 = _mm512_madd52lo_epu64(D1_14,alpah13,blpbh1);
	D1_15 = _mm512_madd52hi_epu64(D1_15,alpah13,blpbh1);
	D1_15 = _mm512_madd52lo_epu64(D1_15,alpah13,blpbh2);
	D1_16 = _mm512_madd52hi_epu64(D1_16,alpah13,blpbh2);
	D1_16 = _mm512_madd52lo_epu64(D1_16,alpah13,blpbh3);
	D1_17 = _mm512_madd52hi_epu64(D1_17,alpah13,blpbh3);
	D1_17 = _mm512_madd52lo_epu64(D1_17,alpah13,blpbh4);
	D1_18 = _mm512_madd52hi_epu64(D1_18,alpah13,blpbh4);
	D1_18 = _mm512_madd52lo_epu64(D1_18,alpah13,blpbh5);
	D1_19 = _mm512_madd52hi_epu64(D1_19,alpah13,blpbh5);
	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah13,blpbh6);
	D1_20 = _mm512_madd52hi_epu64(D1_20,alpah13,blpbh6);
	D1_20 = _mm512_madd52lo_epu64(D1_20,alpah13,blpbh7);
	D1_21 = _mm512_madd52hi_epu64(D1_21,alpah13,blpbh7);
	D1_21 = _mm512_madd52lo_epu64(D1_21,alpah13,blpbh8);
	D1_22 = _mm512_madd52hi_epu64(D1_22,alpah13,blpbh8);
	D1_22 = _mm512_madd52lo_epu64(D1_22,alpah13,blpbh9);
	D1_23 = _mm512_madd52hi_epu64(D1_23,alpah13,blpbh9);
	D1_23 = _mm512_madd52lo_epu64(D1_23,alpah13,blpbh10);
	D1_24 = _mm512_madd52hi_epu64(D1_24,alpah13,blpbh10);
	D1_24 = _mm512_madd52lo_epu64(D1_24,alpah13,blpbh11);
	D1_25 = _mm512_madd52hi_epu64(D1_25,alpah13,blpbh11);
	D1_25 = _mm512_madd52lo_epu64(D1_25,alpah13,blpbh12);
	D1_26 = _mm512_madd52hi_epu64(D1_26,alpah13,blpbh12);
	D1_26 = _mm512_madd52lo_epu64(D1_26,alpah13,blpbh13);
	D1_27 = _mm512_madd52hi_epu64(D1_27,alpah13,blpbh13);
	D1_27 = _mm512_madd52lo_epu64(D1_27,alpah13,blpbh14);
	D1_28 = _mm512_madd52hi_epu64(D1_28,alpah13,blpbh14);
	D1_28 = _mm512_madd52lo_epu64(D1_28,alpah13,blpbh15);
	D1_29 = _mm512_madd52hi_epu64(D1_29,alpah13,blpbh15);
	D1_29 = _mm512_madd52lo_epu64(D1_29,alpah13,blpbh16);
	D1_30 = _mm512_madd52hi_epu64(D1_30,alpah13,blpbh16);
	D1_30 = _mm512_madd52lo_epu64(D1_30,alpah13,blpbh17);
	D1_31 = _mm512_madd52hi_epu64(D1_31,alpah13,blpbh17);
	D1_31 = _mm512_madd52lo_epu64(D1_31,alpah13,blpbh18);
	D1_32 = _mm512_madd52hi_epu64(D1_32,alpah13,blpbh18);
	D1_32 = _mm512_madd52lo_epu64(D1_32,alpah13,blpbh19);
	D1_33 = _mm512_madd52hi_epu64(zero,alpah13,blpbh19);

	D1_14 = _mm512_madd52lo_epu64(D1_14,alpah14,blpbh0);
	D1_15 = _mm512_madd52hi_epu64(D1_15,alpah14,blpbh0);
	D1_15 = _mm512_madd52lo_epu64(D1_15,alpah14,blpbh1);
	D1_16 = _mm512_madd52hi_epu64(D1_16,alpah14,blpbh1);
	D1_16 = _mm512_madd52lo_epu64(D1_16,alpah14,blpbh2);
	D1_17 = _mm512_madd52hi_epu64(D1_17,alpah14,blpbh2);
	D1_17 = _mm512_madd52lo_epu64(D1_17,alpah14,blpbh3);
	D1_18 = _mm512_madd52hi_epu64(D1_18,alpah14,blpbh3);
	D1_18 = _mm512_madd52lo_epu64(D1_18,alpah14,blpbh4);
	D1_19 = _mm512_madd52hi_epu64(D1_19,alpah14,blpbh4);
	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah14,blpbh5);
	D1_20 = _mm512_madd52hi_epu64(D1_20,alpah14,blpbh5);
	D1_20 = _mm512_madd52lo_epu64(D1_20,alpah14,blpbh6);
	D1_21 = _mm512_madd52hi_epu64(D1_21,alpah14,blpbh6);
	D1_21 = _mm512_madd52lo_epu64(D1_21,alpah14,blpbh7);
	D1_22 = _mm512_madd52hi_epu64(D1_22,alpah14,blpbh7);
	D1_22 = _mm512_madd52lo_epu64(D1_22,alpah14,blpbh8);
	D1_23 = _mm512_madd52hi_epu64(D1_23,alpah14,blpbh8);
	D1_23 = _mm512_madd52lo_epu64(D1_23,alpah14,blpbh9);
	D1_24 = _mm512_madd52hi_epu64(D1_24,alpah14,blpbh9);
	D1_24 = _mm512_madd52lo_epu64(D1_24,alpah14,blpbh10);
	D1_25 = _mm512_madd52hi_epu64(D1_25,alpah14,blpbh10);
	D1_25 = _mm512_madd52lo_epu64(D1_25,alpah14,blpbh11);
	D1_26 = _mm512_madd52hi_epu64(D1_26,alpah14,blpbh11);
	D1_26 = _mm512_madd52lo_epu64(D1_26,alpah14,blpbh12);
	D1_27 = _mm512_madd52hi_epu64(D1_27,alpah14,blpbh12);
	D1_27 = _mm512_madd52lo_epu64(D1_27,alpah14,blpbh13);
	D1_28 = _mm512_madd52hi_epu64(D1_28,alpah14,blpbh13);
	D1_28 = _mm512_madd52lo_epu64(D1_28,alpah14,blpbh14);
	D1_29 = _mm512_madd52hi_epu64(D1_29,alpah14,blpbh14);
	D1_29 = _mm512_madd52lo_epu64(D1_29,alpah14,blpbh15);
	D1_30 = _mm512_madd52hi_epu64(D1_30,alpah14,blpbh15);
	D1_30 = _mm512_madd52lo_epu64(D1_30,alpah14,blpbh16);
	D1_31 = _mm512_madd52hi_epu64(D1_31,alpah14,blpbh16);
	D1_31 = _mm512_madd52lo_epu64(D1_31,alpah14,blpbh17);
	D1_32 = _mm512_madd52hi_epu64(D1_32,alpah14,blpbh17);
	D1_32 = _mm512_madd52lo_epu64(D1_32,alpah14,blpbh18);
	D1_33 = _mm512_madd52hi_epu64(D1_33,alpah14,blpbh18);
	D1_33 = _mm512_madd52lo_epu64(D1_33,alpah14,blpbh19);
	D1_34 = _mm512_madd52hi_epu64(zero,alpah14,blpbh19);

	D1_15 = _mm512_madd52lo_epu64(D1_15,alpah15,blpbh0);
	D1_16 = _mm512_madd52hi_epu64(D1_16,alpah15,blpbh0);
	D1_16 = _mm512_madd52lo_epu64(D1_16,alpah15,blpbh1);
	D1_17 = _mm512_madd52hi_epu64(D1_17,alpah15,blpbh1);
	D1_17 = _mm512_madd52lo_epu64(D1_17,alpah15,blpbh2);
	D1_18 = _mm512_madd52hi_epu64(D1_18,alpah15,blpbh2);
	D1_18 = _mm512_madd52lo_epu64(D1_18,alpah15,blpbh3);
	D1_19 = _mm512_madd52hi_epu64(D1_19,alpah15,blpbh3);
	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah15,blpbh4);
	D1_20 = _mm512_madd52hi_epu64(D1_20,alpah15,blpbh4);
	D1_20 = _mm512_madd52lo_epu64(D1_20,alpah15,blpbh5);
	D1_21 = _mm512_madd52hi_epu64(D1_21,alpah15,blpbh5);
	D1_21 = _mm512_madd52lo_epu64(D1_21,alpah15,blpbh6);
	D1_22 = _mm512_madd52hi_epu64(D1_22,alpah15,blpbh6);
	D1_22 = _mm512_madd52lo_epu64(D1_22,alpah15,blpbh7);
	D1_23 = _mm512_madd52hi_epu64(D1_23,alpah15,blpbh7);
	D1_23 = _mm512_madd52lo_epu64(D1_23,alpah15,blpbh8);
	D1_24 = _mm512_madd52hi_epu64(D1_24,alpah15,blpbh8);
	D1_24 = _mm512_madd52lo_epu64(D1_24,alpah15,blpbh9);
	D1_25 = _mm512_madd52hi_epu64(D1_25,alpah15,blpbh9);
	D1_25 = _mm512_madd52lo_epu64(D1_25,alpah15,blpbh10);
	D1_26 = _mm512_madd52hi_epu64(D1_26,alpah15,blpbh10);
	D1_26 = _mm512_madd52lo_epu64(D1_26,alpah15,blpbh11);
	D1_27 = _mm512_madd52hi_epu64(D1_27,alpah15,blpbh11);
	D1_27 = _mm512_madd52lo_epu64(D1_27,alpah15,blpbh12);
	D1_28 = _mm512_madd52hi_epu64(D1_28,alpah15,blpbh12);
	D1_28 = _mm512_madd52lo_epu64(D1_28,alpah15,blpbh13);
	D1_29 = _mm512_madd52hi_epu64(D1_29,alpah15,blpbh13);
	D1_29 = _mm512_madd52lo_epu64(D1_29,alpah15,blpbh14);
	D1_30 = _mm512_madd52hi_epu64(D1_30,alpah15,blpbh14);
	D1_30 = _mm512_madd52lo_epu64(D1_30,alpah15,blpbh15);
	D1_31 = _mm512_madd52hi_epu64(D1_31,alpah15,blpbh15);
	D1_31 = _mm512_madd52lo_epu64(D1_31,alpah15,blpbh16);
	D1_32 = _mm512_madd52hi_epu64(D1_32,alpah15,blpbh16);
	D1_32 = _mm512_madd52lo_epu64(D1_32,alpah15,blpbh17);
	D1_33 = _mm512_madd52hi_epu64(D1_33,alpah15,blpbh17);
	D1_33 = _mm512_madd52lo_epu64(D1_33,alpah15,blpbh18);
	D1_34 = _mm512_madd52hi_epu64(D1_34,alpah15,blpbh18);
	D1_34 = _mm512_madd52lo_epu64(D1_34,alpah15,blpbh19);
	D1_35 = _mm512_madd52hi_epu64(zero,alpah15,blpbh19);

	D1_16 = _mm512_madd52lo_epu64(D1_16,alpah16,blpbh0);
	D1_17 = _mm512_madd52hi_epu64(D1_17,alpah16,blpbh0);
	D1_17 = _mm512_madd52lo_epu64(D1_17,alpah16,blpbh1);
	D1_18 = _mm512_madd52hi_epu64(D1_18,alpah16,blpbh1);
	D1_18 = _mm512_madd52lo_epu64(D1_18,alpah16,blpbh2);
	D1_19 = _mm512_madd52hi_epu64(D1_19,alpah16,blpbh2);
	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah16,blpbh3);
	D1_20 = _mm512_madd52hi_epu64(D1_20,alpah16,blpbh3);
	D1_20 = _mm512_madd52lo_epu64(D1_20,alpah16,blpbh4);
	D1_21 = _mm512_madd52hi_epu64(D1_21,alpah16,blpbh4);
	D1_21 = _mm512_madd52lo_epu64(D1_21,alpah16,blpbh5);
	D1_22 = _mm512_madd52hi_epu64(D1_22,alpah16,blpbh5);
	D1_22 = _mm512_madd52lo_epu64(D1_22,alpah16,blpbh6);
	D1_23 = _mm512_madd52hi_epu64(D1_23,alpah16,blpbh6);
	D1_23 = _mm512_madd52lo_epu64(D1_23,alpah16,blpbh7);
	D1_24 = _mm512_madd52hi_epu64(D1_24,alpah16,blpbh7);
	D1_24 = _mm512_madd52lo_epu64(D1_24,alpah16,blpbh8);
	D1_25 = _mm512_madd52hi_epu64(D1_25,alpah16,blpbh8);
	D1_25 = _mm512_madd52lo_epu64(D1_25,alpah16,blpbh9);
	D1_26 = _mm512_madd52hi_epu64(D1_26,alpah16,blpbh9);
	D1_26 = _mm512_madd52lo_epu64(D1_26,alpah16,blpbh10);
	D1_27 = _mm512_madd52hi_epu64(D1_27,alpah16,blpbh10);
	D1_27 = _mm512_madd52lo_epu64(D1_27,alpah16,blpbh11);
	D1_28 = _mm512_madd52hi_epu64(D1_28,alpah16,blpbh11);
	D1_28 = _mm512_madd52lo_epu64(D1_28,alpah16,blpbh12);
	D1_29 = _mm512_madd52hi_epu64(D1_29,alpah16,blpbh12);
	D1_29 = _mm512_madd52lo_epu64(D1_29,alpah16,blpbh13);
	D1_30 = _mm512_madd52hi_epu64(D1_30,alpah16,blpbh13);
	D1_30 = _mm512_madd52lo_epu64(D1_30,alpah16,blpbh14);
	D1_31 = _mm512_madd52hi_epu64(D1_31,alpah16,blpbh14);
	D1_31 = _mm512_madd52lo_epu64(D1_31,alpah16,blpbh15);
	D1_32 = _mm512_madd52hi_epu64(D1_32,alpah16,blpbh15);
	D1_32 = _mm512_madd52lo_epu64(D1_32,alpah16,blpbh16);
	D1_33 = _mm512_madd52hi_epu64(D1_33,alpah16,blpbh16);
	D1_33 = _mm512_madd52lo_epu64(D1_33,alpah16,blpbh17);
	D1_34 = _mm512_madd52hi_epu64(D1_34,alpah16,blpbh17);
	D1_34 = _mm512_madd52lo_epu64(D1_34,alpah16,blpbh18);
	D1_35 = _mm512_madd52hi_epu64(D1_35,alpah16,blpbh18);
	D1_35 = _mm512_madd52lo_epu64(D1_35,alpah16,blpbh19);
	D1_36 = _mm512_madd52hi_epu64(zero,alpah16,blpbh19);

	D1_17 = _mm512_madd52lo_epu64(D1_17,alpah17,blpbh0);
	D1_18 = _mm512_madd52hi_epu64(D1_18,alpah17,blpbh0);
	D1_18 = _mm512_madd52lo_epu64(D1_18,alpah17,blpbh1);
	D1_19 = _mm512_madd52hi_epu64(D1_19,alpah17,blpbh1);
	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah17,blpbh2);
	D1_20 = _mm512_madd52hi_epu64(D1_20,alpah17,blpbh2);
	D1_20 = _mm512_madd52lo_epu64(D1_20,alpah17,blpbh3);
	D1_21 = _mm512_madd52hi_epu64(D1_21,alpah17,blpbh3);
	D1_21 = _mm512_madd52lo_epu64(D1_21,alpah17,blpbh4);
	D1_22 = _mm512_madd52hi_epu64(D1_22,alpah17,blpbh4);
	D1_22 = _mm512_madd52lo_epu64(D1_22,alpah17,blpbh5);
	D1_23 = _mm512_madd52hi_epu64(D1_23,alpah17,blpbh5);
	D1_23 = _mm512_madd52lo_epu64(D1_23,alpah17,blpbh6);
	D1_24 = _mm512_madd52hi_epu64(D1_24,alpah17,blpbh6);
	D1_24 = _mm512_madd52lo_epu64(D1_24,alpah17,blpbh7);
	D1_25 = _mm512_madd52hi_epu64(D1_25,alpah17,blpbh7);
	D1_25 = _mm512_madd52lo_epu64(D1_25,alpah17,blpbh8);
	D1_26 = _mm512_madd52hi_epu64(D1_26,alpah17,blpbh8);
	D1_26 = _mm512_madd52lo_epu64(D1_26,alpah17,blpbh9);
	D1_27 = _mm512_madd52hi_epu64(D1_27,alpah17,blpbh9);
	D1_27 = _mm512_madd52lo_epu64(D1_27,alpah17,blpbh10);
	D1_28 = _mm512_madd52hi_epu64(D1_28,alpah17,blpbh10);
	D1_28 = _mm512_madd52lo_epu64(D1_28,alpah17,blpbh11);
	D1_29 = _mm512_madd52hi_epu64(D1_29,alpah17,blpbh11);
	D1_29 = _mm512_madd52lo_epu64(D1_29,alpah17,blpbh12);
	D1_30 = _mm512_madd52hi_epu64(D1_30,alpah17,blpbh12);
	D1_30 = _mm512_madd52lo_epu64(D1_30,alpah17,blpbh13);
	D1_31 = _mm512_madd52hi_epu64(D1_31,alpah17,blpbh13);
	D1_31 = _mm512_madd52lo_epu64(D1_31,alpah17,blpbh14);
	D1_32 = _mm512_madd52hi_epu64(D1_32,alpah17,blpbh14);
	D1_32 = _mm512_madd52lo_epu64(D1_32,alpah17,blpbh15);
	D1_33 = _mm512_madd52hi_epu64(D1_33,alpah17,blpbh15);
	D1_33 = _mm512_madd52lo_epu64(D1_33,alpah17,blpbh16);
	D1_34 = _mm512_madd52hi_epu64(D1_34,alpah17,blpbh16);
	D1_34 = _mm512_madd52lo_epu64(D1_34,alpah17,blpbh17);
	D1_35 = _mm512_madd52hi_epu64(D1_35,alpah17,blpbh17);
	D1_35 = _mm512_madd52lo_epu64(D1_35,alpah17,blpbh18);
	D1_36 = _mm512_madd52hi_epu64(D1_36,alpah17,blpbh18);
	D1_36 = _mm512_madd52lo_epu64(D1_36,alpah17,blpbh19);
	D1_37 = _mm512_madd52hi_epu64(zero,alpah17,blpbh19);

	D1_18 = _mm512_madd52lo_epu64(D1_18,alpah18,blpbh0);
	D1_19 = _mm512_madd52hi_epu64(D1_19,alpah18,blpbh0);
	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah18,blpbh1);
	D1_20 = _mm512_madd52hi_epu64(D1_20,alpah18,blpbh1);
	D1_20 = _mm512_madd52lo_epu64(D1_20,alpah18,blpbh2);
	D1_21 = _mm512_madd52hi_epu64(D1_21,alpah18,blpbh2);
	D1_21 = _mm512_madd52lo_epu64(D1_21,alpah18,blpbh3);
	D1_22 = _mm512_madd52hi_epu64(D1_22,alpah18,blpbh3);
	D1_22 = _mm512_madd52lo_epu64(D1_22,alpah18,blpbh4);
	D1_23 = _mm512_madd52hi_epu64(D1_23,alpah18,blpbh4);
	D1_23 = _mm512_madd52lo_epu64(D1_23,alpah18,blpbh5);
	D1_24 = _mm512_madd52hi_epu64(D1_24,alpah18,blpbh5);
	D1_24 = _mm512_madd52lo_epu64(D1_24,alpah18,blpbh6);
	D1_25 = _mm512_madd52hi_epu64(D1_25,alpah18,blpbh6);
	D1_25 = _mm512_madd52lo_epu64(D1_25,alpah18,blpbh7);
	D1_26 = _mm512_madd52hi_epu64(D1_26,alpah18,blpbh7);
	D1_26 = _mm512_madd52lo_epu64(D1_26,alpah18,blpbh8);
	D1_27 = _mm512_madd52hi_epu64(D1_27,alpah18,blpbh8);
	D1_27 = _mm512_madd52lo_epu64(D1_27,alpah18,blpbh9);
	D1_28 = _mm512_madd52hi_epu64(D1_28,alpah18,blpbh9);
	D1_28 = _mm512_madd52lo_epu64(D1_28,alpah18,blpbh10);
	D1_29 = _mm512_madd52hi_epu64(D1_29,alpah18,blpbh10);
	D1_29 = _mm512_madd52lo_epu64(D1_29,alpah18,blpbh11);
	D1_30 = _mm512_madd52hi_epu64(D1_30,alpah18,blpbh11);
	D1_30 = _mm512_madd52lo_epu64(D1_30,alpah18,blpbh12);
	D1_31 = _mm512_madd52hi_epu64(D1_31,alpah18,blpbh12);
	D1_31 = _mm512_madd52lo_epu64(D1_31,alpah18,blpbh13);
	D1_32 = _mm512_madd52hi_epu64(D1_32,alpah18,blpbh13);
	D1_32 = _mm512_madd52lo_epu64(D1_32,alpah18,blpbh14);
	D1_33 = _mm512_madd52hi_epu64(D1_33,alpah18,blpbh14);
	D1_33 = _mm512_madd52lo_epu64(D1_33,alpah18,blpbh15);
	D1_34 = _mm512_madd52hi_epu64(D1_34,alpah18,blpbh15);
	D1_34 = _mm512_madd52lo_epu64(D1_34,alpah18,blpbh16);
	D1_35 = _mm512_madd52hi_epu64(D1_35,alpah18,blpbh16);
	D1_35 = _mm512_madd52lo_epu64(D1_35,alpah18,blpbh17);
	D1_36 = _mm512_madd52hi_epu64(D1_36,alpah18,blpbh17);
	D1_36 = _mm512_madd52lo_epu64(D1_36,alpah18,blpbh18);
	D1_37 = _mm512_madd52hi_epu64(D1_37,alpah18,blpbh18);
	D1_37 = _mm512_madd52lo_epu64(D1_37,alpah18,blpbh19);
	D1_38 = _mm512_madd52hi_epu64(zero,alpah18,blpbh19);

	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah19,blpbh0);
	D1_20 = _mm512_madd52hi_epu64(D1_20,alpah19,blpbh0);
	D1_20 = _mm512_madd52lo_epu64(D1_20,alpah19,blpbh1);
	D1_21 = _mm512_madd52hi_epu64(D1_21,alpah19,blpbh1);
	D1_21 = _mm512_madd52lo_epu64(D1_21,alpah19,blpbh2);
	D1_22 = _mm512_madd52hi_epu64(D1_22,alpah19,blpbh2);
	D1_22 = _mm512_madd52lo_epu64(D1_22,alpah19,blpbh3);
	D1_23 = _mm512_madd52hi_epu64(D1_23,alpah19,blpbh3);
	D1_23 = _mm512_madd52lo_epu64(D1_23,alpah19,blpbh4);
	D1_24 = _mm512_madd52hi_epu64(D1_24,alpah19,blpbh4);
	D1_24 = _mm512_madd52lo_epu64(D1_24,alpah19,blpbh5);
	D1_25 = _mm512_madd52hi_epu64(D1_25,alpah19,blpbh5);
	D1_25 = _mm512_madd52lo_epu64(D1_25,alpah19,blpbh6);
	D1_26 = _mm512_madd52hi_epu64(D1_26,alpah19,blpbh6);
	D1_26 = _mm512_madd52lo_epu64(D1_26,alpah19,blpbh7);
	D1_27 = _mm512_madd52hi_epu64(D1_27,alpah19,blpbh7);
	D1_27 = _mm512_madd52lo_epu64(D1_27,alpah19,blpbh8);
	D1_28 = _mm512_madd52hi_epu64(D1_28,alpah19,blpbh8);
	D1_28 = _mm512_madd52lo_epu64(D1_28,alpah19,blpbh9);
	D1_29 = _mm512_madd52hi_epu64(D1_29,alpah19,blpbh9);
	D1_29 = _mm512_madd52lo_epu64(D1_29,alpah19,blpbh10);
	D1_30 = _mm512_madd52hi_epu64(D1_30,alpah19,blpbh10);
	D1_30 = _mm512_madd52lo_epu64(D1_30,alpah19,blpbh11);
	D1_31 = _mm512_madd52hi_epu64(D1_31,alpah19,blpbh11);
	D1_31 = _mm512_madd52lo_epu64(D1_31,alpah19,blpbh12);
	D1_32 = _mm512_madd52hi_epu64(D1_32,alpah19,blpbh12);
	D1_32 = _mm512_madd52lo_epu64(D1_32,alpah19,blpbh13);
	D1_33 = _mm512_madd52hi_epu64(D1_33,alpah19,blpbh13);
	D1_33 = _mm512_madd52lo_epu64(D1_33,alpah19,blpbh14);
	D1_34 = _mm512_madd52hi_epu64(D1_34,alpah19,blpbh14);
	D1_34 = _mm512_madd52lo_epu64(D1_34,alpah19,blpbh15);
	D1_35 = _mm512_madd52hi_epu64(D1_35,alpah19,blpbh15);
	D1_35 = _mm512_madd52lo_epu64(D1_35,alpah19,blpbh16);
	D1_36 = _mm512_madd52hi_epu64(D1_36,alpah19,blpbh16);
	D1_36 = _mm512_madd52lo_epu64(D1_36,alpah19,blpbh17);
	D1_37 = _mm512_madd52hi_epu64(D1_37,alpah19,blpbh17);
	D1_37 = _mm512_madd52lo_epu64(D1_37,alpah19,blpbh18);
	D1_38 = _mm512_madd52hi_epu64(D1_38,alpah19,blpbh18);
	D1_38 = _mm512_madd52lo_epu64(D1_38,alpah19,blpbh19);
	D1_39 = _mm512_madd52hi_epu64(zero,alpah19,blpbh19);


	// Carry management

	carry  = _mm512_srli_epi64(D1_0,52);
	D1_0 = _mm512_and_si512(D1_0,mask52);
	D1_1 = _mm512_add_epi64(D1_1,carry);
	carry  = _mm512_srli_epi64(D1_1,52);
	D1_2 = _mm512_add_epi64(D1_2,carry);
	D1_1 = _mm512_and_si512(D1_1,mask52);
	carry  = _mm512_srli_epi64(D1_2,52);
	D1_3 = _mm512_add_epi64(D1_3,carry);
	D1_2 = _mm512_and_si512(D1_2,mask52);
	carry  = _mm512_srli_epi64(D1_3,52);
	D1_4 = _mm512_add_epi64(D1_4,carry);
	D1_3 = _mm512_and_si512(D1_3,mask52);
	carry  = _mm512_srli_epi64(D1_4,52);
	D1_5 = _mm512_add_epi64(D1_5,carry);
	D1_4 = _mm512_and_si512(D1_4,mask52);
	carry  = _mm512_srli_epi64(D1_5,52);
	D1_6 = _mm512_add_epi64(D1_6,carry);
	D1_5 = _mm512_and_si512(D1_5,mask52);
	carry  = _mm512_srli_epi64(D1_6,52);
	D1_7 = _mm512_add_epi64(D1_7,carry);
	D1_6 = _mm512_and_si512(D1_6,mask52);
	carry  = _mm512_srli_epi64(D1_7,52);
	D1_8 = _mm512_add_epi64(D1_8,carry);
	D1_7 = _mm512_and_si512(D1_7,mask52);
	carry  = _mm512_srli_epi64(D1_8,52);
	D1_9 = _mm512_add_epi64(D1_9,carry);
	D1_8 = _mm512_and_si512(D1_8,mask52);
	carry  = _mm512_srli_epi64(D1_9,52);
	D1_10 = _mm512_add_epi64(D1_10,carry);
	D1_9 = _mm512_and_si512(D1_9,mask52);
	carry  = _mm512_srli_epi64(D1_10,52);
	D1_11 = _mm512_add_epi64(D1_11,carry);
	D1_10 = _mm512_and_si512(D1_10,mask52);
	carry  = _mm512_srli_epi64(D1_11,52);
	D1_12 = _mm512_add_epi64(D1_12,carry);
	D1_11 = _mm512_and_si512(D1_11,mask52);
	carry  = _mm512_srli_epi64(D1_12,52);
	D1_13 = _mm512_add_epi64(D1_13,carry);
	D1_12 = _mm512_and_si512(D1_12,mask52);
	carry  = _mm512_srli_epi64(D1_13,52);
	D1_14 = _mm512_add_epi64(D1_14,carry);
	D1_13 = _mm512_and_si512(D1_13,mask52);
	carry  = _mm512_srli_epi64(D1_14,52);
	D1_15 = _mm512_add_epi64(D1_15,carry);
	D1_14 = _mm512_and_si512(D1_14,mask52);
	carry  = _mm512_srli_epi64(D1_15,52);
	D1_16 = _mm512_add_epi64(D1_16,carry);
	D1_15 = _mm512_and_si512(D1_15,mask52);
	carry  = _mm512_srli_epi64(D1_16,52);
	D1_17 = _mm512_add_epi64(D1_17,carry);
	D1_16 = _mm512_and_si512(D1_16,mask52);
	carry  = _mm512_srli_epi64(D1_17,52);
	D1_18 = _mm512_add_epi64(D1_18,carry);
	D1_17 = _mm512_and_si512(D1_17,mask52);
	carry  = _mm512_srli_epi64(D1_18,52);
	D1_19 = _mm512_add_epi64(D1_19,carry);
	D1_18 = _mm512_and_si512(D1_18,mask52);

	// Conversion in base 2^1039

	carry = D1_19>>51;
	D1_20 = (D1_20<<1)+carry;
	D1_19 &= mask51;
	D1_20 = _mm512_add_epi64(D1_20,D2_0);	carry = D1_20>>52;
	D1_21 = (D1_21<<1)+carry;
	D1_20 &= mask52;
	D1_21 = _mm512_add_epi64(D1_21,D2_1);
	carry = D1_21>>52;
	D1_22 = (D1_22<<1)+carry;
	D1_21 &= mask52;
	D1_22 = _mm512_add_epi64(D1_22,D2_2);
	carry = D1_22>>52;
	D1_23 = (D1_23<<1)+carry;
	D1_22 &= mask52;
	D1_23 = _mm512_add_epi64(D1_23,D2_3);
	carry = D1_23>>52;
	D1_24 = (D1_24<<1)+carry;
	D1_23 &= mask52;
	D1_24 = _mm512_add_epi64(D1_24,D2_4);
	carry = D1_24>>52;
	D1_25 = (D1_25<<1)+carry;
	D1_24 &= mask52;
	D1_25 = _mm512_add_epi64(D1_25,D2_5);
	carry = D1_25>>52;
	D1_26 = (D1_26<<1)+carry;
	D1_25 &= mask52;
	D1_26 = _mm512_add_epi64(D1_26,D2_6);
	carry = D1_26>>52;
	D1_27 = (D1_27<<1)+carry;
	D1_26 &= mask52;
	D1_27 = _mm512_add_epi64(D1_27,D2_7);
	carry = D1_27>>52;
	D1_28 = (D1_28<<1)+carry;
	D1_27 &= mask52;
	D1_28 = _mm512_add_epi64(D1_28,D2_8);
	carry = D1_28>>52;
	D1_29 = (D1_29<<1)+carry;
	D1_28 &= mask52;
	D1_29 = _mm512_add_epi64(D1_29,D2_9);
	carry = D1_29>>52;
	D1_30 = (D1_30<<1)+carry;
	D1_29 &= mask52;
	D1_30 = _mm512_add_epi64(D1_30,D2_10);
	carry = D1_30>>52;
	D1_31 = (D1_31<<1)+carry;
	D1_30 &= mask52;
	D1_31 = _mm512_add_epi64(D1_31,D2_11);
	carry = D1_31>>52;
	D1_32 = (D1_32<<1)+carry;
	D1_31 &= mask52;
	D1_32 = _mm512_add_epi64(D1_32,D2_12);
	carry = D1_32>>52;
	D1_33 = (D1_33<<1)+carry;
	D1_32 &= mask52;
	D1_33 = _mm512_add_epi64(D1_33,D2_13);
	carry = D1_33>>52;
	D1_34 = (D1_34<<1)+carry;
	D1_33 &= mask52;
	D1_34 = _mm512_add_epi64(D1_34,D2_14);
	carry = D1_34>>52;
	D1_35 = (D1_35<<1)+carry;
	D1_34 &= mask52;
	D1_35 = _mm512_add_epi64(D1_35,D2_15);
	carry = D1_35>>52;
	D1_36 = (D1_36<<1)+carry;
	D1_35 &= mask52;
	D1_36 = _mm512_add_epi64(D1_36,D2_16);
	carry = D1_36>>52;
	D1_37 = (D1_37<<1)+carry;
	D1_36 &= mask52;
	D1_37 = _mm512_add_epi64(D1_37,D2_17);
	carry = D1_37>>52;
	D1_38 = (D1_38<<1)+carry;
	D1_37 &= mask52;
	D1_38 = _mm512_add_epi64(D1_38,D2_18);
	carry = D1_38>>52;
	D1_39 = (D1_39<<1)+carry;
	D1_38 &= mask52;
	D1_39 = _mm512_add_epi64(D1_39,D2_19);
	carry = D1_39>>51;
	D1_39 &= mask51;

	// Final reconstruction


	// tmp = D0+D2, radix 2^1039

	__m512i tmp0 = _mm512_add_epi64(D0_0,D2_0);
	__m512i carry_tmp  = _mm512_srli_epi64(tmp0,52);
	tmp0 = _mm512_and_si512(tmp0,mask52);

	__m512i tmp1 = _mm512_add_epi64(D0_1,carry_tmp);
	tmp1 = _mm512_add_epi64(tmp1,D2_1);
	carry_tmp  = _mm512_srli_epi64(tmp1,52);
	tmp1 = _mm512_and_si512(tmp1,mask52);

	__m512i tmp2 = _mm512_add_epi64(D0_2,carry_tmp);
	tmp2 = _mm512_add_epi64(tmp2,D2_2);
	carry_tmp  = _mm512_srli_epi64(tmp2,52);
	tmp2 = _mm512_and_si512(tmp2,mask52);

	__m512i tmp3 = _mm512_add_epi64(D0_3,carry_tmp);
	tmp3 = _mm512_add_epi64(tmp3,D2_3);
	carry_tmp  = _mm512_srli_epi64(tmp3,52);
	tmp3 = _mm512_and_si512(tmp3,mask52);

	__m512i tmp4 = _mm512_add_epi64(D0_4,carry_tmp);
	tmp4 = _mm512_add_epi64(tmp4,D2_4);
	carry_tmp  = _mm512_srli_epi64(tmp4,52);
	tmp4 = _mm512_and_si512(tmp4,mask52);

	__m512i tmp5 = _mm512_add_epi64(D0_5,carry_tmp);
	tmp5 = _mm512_add_epi64(tmp5,D2_5);
	carry_tmp  = _mm512_srli_epi64(tmp5,52);
	tmp5 = _mm512_and_si512(tmp5,mask52);

	__m512i tmp6 = _mm512_add_epi64(D0_6,carry_tmp);
	tmp6 = _mm512_add_epi64(tmp6,D2_6);
	carry_tmp  = _mm512_srli_epi64(tmp6,52);
	tmp6 = _mm512_and_si512(tmp6,mask52);

	__m512i tmp7 = _mm512_add_epi64(D0_7,carry_tmp);
	tmp7 = _mm512_add_epi64(tmp7,D2_7);
	carry_tmp  = _mm512_srli_epi64(tmp7,52);
	tmp7 = _mm512_and_si512(tmp7,mask52);

	__m512i tmp8 = _mm512_add_epi64(D0_8,carry_tmp);
	tmp8 = _mm512_add_epi64(tmp8,D2_8);
	carry_tmp  = _mm512_srli_epi64(tmp8,52);
	tmp8 = _mm512_and_si512(tmp8,mask52);

	__m512i tmp9 = _mm512_add_epi64(D0_9,carry_tmp);
	tmp9 = _mm512_add_epi64(tmp9,D2_9);
	carry_tmp  = _mm512_srli_epi64(tmp9,52);
	tmp9 = _mm512_and_si512(tmp9,mask52);

	__m512i tmp10 = _mm512_add_epi64(D0_10,carry_tmp);
	tmp10 = _mm512_add_epi64(tmp10,D2_10);
	carry_tmp  = _mm512_srli_epi64(tmp10,52);
	tmp10 = _mm512_and_si512(tmp10,mask52);

	__m512i tmp11 = _mm512_add_epi64(D0_11,carry_tmp);
	tmp11 = _mm512_add_epi64(tmp11,D2_11);
	carry_tmp  = _mm512_srli_epi64(tmp11,52);
	tmp11 = _mm512_and_si512(tmp11,mask52);

	__m512i tmp12 = _mm512_add_epi64(D0_12,carry_tmp);
	tmp12 = _mm512_add_epi64(tmp12,D2_12);
	carry_tmp  = _mm512_srli_epi64(tmp12,52);
	tmp12 = _mm512_and_si512(tmp12,mask52);

	__m512i tmp13 = _mm512_add_epi64(D0_13,carry_tmp);
	tmp13 = _mm512_add_epi64(tmp13,D2_13);
	carry_tmp  = _mm512_srli_epi64(tmp13,52);
	tmp13 = _mm512_and_si512(tmp13,mask52);

	__m512i tmp14 = _mm512_add_epi64(D0_14,carry_tmp);
	tmp14 = _mm512_add_epi64(tmp14,D2_14);
	carry_tmp  = _mm512_srli_epi64(tmp14,52);
	tmp14 = _mm512_and_si512(tmp14,mask52);

	__m512i tmp15 = _mm512_add_epi64(D0_15,carry_tmp);
	tmp15 = _mm512_add_epi64(tmp15,D2_15);
	carry_tmp  = _mm512_srli_epi64(tmp15,52);
	tmp15 = _mm512_and_si512(tmp15,mask52);

	__m512i tmp16 = _mm512_add_epi64(D0_16,carry_tmp);
	tmp16 = _mm512_add_epi64(tmp16,D2_16);
	carry_tmp  = _mm512_srli_epi64(tmp16,52);
	tmp16 = _mm512_and_si512(tmp16,mask52);

	__m512i tmp17 = _mm512_add_epi64(D0_17,carry_tmp);
	tmp17 = _mm512_add_epi64(tmp17,D2_17);
	carry_tmp  = _mm512_srli_epi64(tmp17,52);
	tmp17 = _mm512_and_si512(tmp17,mask52);

	__m512i tmp18 = _mm512_add_epi64(D0_18,carry_tmp);
	tmp18 = _mm512_add_epi64(tmp18,D2_18);
	carry_tmp  = _mm512_srli_epi64(tmp18,52);
	tmp18 = _mm512_and_si512(tmp18,mask52);

	__m512i tmp19 = _mm512_add_epi64(D0_19,carry_tmp);
	tmp19 = _mm512_add_epi64(tmp19,D2_19);
	carry_tmp  = _mm512_srli_epi64(tmp19,51);
	tmp19 = _mm512_and_si512(tmp19,mask51);// radix 2**1039

	__m512i tmp20 = _mm512_add_epi64(D0_20,carry_tmp);
	tmp20 = _mm512_add_epi64(tmp20,D2_20);
	carry_tmp  = _mm512_srli_epi64(tmp20,52);
	tmp20 = _mm512_and_si512(tmp20,mask52);

	__m512i tmp21 = _mm512_add_epi64(D0_21,carry_tmp);
	tmp21 = _mm512_add_epi64(tmp21,D2_21);
	carry_tmp  = _mm512_srli_epi64(tmp21,52);
	tmp21 = _mm512_and_si512(tmp21,mask52);

	__m512i tmp22 = _mm512_add_epi64(D0_22,carry_tmp);
	tmp22 = _mm512_add_epi64(tmp22,D2_22);
	carry_tmp  = _mm512_srli_epi64(tmp22,52);
	tmp22 = _mm512_and_si512(tmp22,mask52);

	__m512i tmp23 = _mm512_add_epi64(D0_23,carry_tmp);
	tmp23 = _mm512_add_epi64(tmp23,D2_23);
	carry_tmp  = _mm512_srli_epi64(tmp23,52);
	tmp23 = _mm512_and_si512(tmp23,mask52);

	__m512i tmp24 = _mm512_add_epi64(D0_24,carry_tmp);
	tmp24 = _mm512_add_epi64(tmp24,D2_24);
	carry_tmp  = _mm512_srli_epi64(tmp24,52);
	tmp24 = _mm512_and_si512(tmp24,mask52);

	__m512i tmp25 = _mm512_add_epi64(D0_25,carry_tmp);
	tmp25 = _mm512_add_epi64(tmp25,D2_25);
	carry_tmp  = _mm512_srli_epi64(tmp25,52);
	tmp25 = _mm512_and_si512(tmp25,mask52);

	__m512i tmp26 = _mm512_add_epi64(D0_26,carry_tmp);
	tmp26 = _mm512_add_epi64(tmp26,D2_26);
	carry_tmp  = _mm512_srli_epi64(tmp26,52);
	tmp26 = _mm512_and_si512(tmp26,mask52);

	__m512i tmp27 = _mm512_add_epi64(D0_27,carry_tmp);
	tmp27 = _mm512_add_epi64(tmp27,D2_27);
	carry_tmp  = _mm512_srli_epi64(tmp27,52);
	tmp27 = _mm512_and_si512(tmp27,mask52);

	__m512i tmp28 = _mm512_add_epi64(D0_28,carry_tmp);
	tmp28 = _mm512_add_epi64(tmp28,D2_28);
	carry_tmp  = _mm512_srli_epi64(tmp28,52);
	tmp28 = _mm512_and_si512(tmp28,mask52);

	__m512i tmp29 = _mm512_add_epi64(D0_29,carry_tmp);
	tmp29 = _mm512_add_epi64(tmp29,D2_29);
	carry_tmp  = _mm512_srli_epi64(tmp29,52);
	tmp29 = _mm512_and_si512(tmp29,mask52);

	__m512i tmp30 = _mm512_add_epi64(D0_30,carry_tmp);
	tmp30 = _mm512_add_epi64(tmp30,D2_30);
	carry_tmp  = _mm512_srli_epi64(tmp30,52);
	tmp30 = _mm512_and_si512(tmp30,mask52);

	__m512i tmp31 = _mm512_add_epi64(D0_31,carry_tmp);
	tmp31 = _mm512_add_epi64(tmp31,D2_31);
	carry_tmp  = _mm512_srli_epi64(tmp31,52);
	tmp31 = _mm512_and_si512(tmp31,mask52);

	__m512i tmp32 = _mm512_add_epi64(D0_32,carry_tmp);
	tmp32 = _mm512_add_epi64(tmp32,D2_32);
	carry_tmp  = _mm512_srli_epi64(tmp32,52);
	tmp32 = _mm512_and_si512(tmp32,mask52);

	__m512i tmp33 = _mm512_add_epi64(D0_33,carry_tmp);
	tmp33 = _mm512_add_epi64(tmp33,D2_33);
	carry_tmp  = _mm512_srli_epi64(tmp33,52);
	tmp33 = _mm512_and_si512(tmp33,mask52);

	__m512i tmp34 = _mm512_add_epi64(D0_34,carry_tmp);
	tmp34 = _mm512_add_epi64(tmp34,D2_34);
	carry_tmp  = _mm512_srli_epi64(tmp34,52);
	tmp34 = _mm512_and_si512(tmp34,mask52);

	__m512i tmp35 = _mm512_add_epi64(D0_35,carry_tmp);
	tmp35 = _mm512_add_epi64(tmp35,D2_35);
	carry_tmp  = _mm512_srli_epi64(tmp35,52);
	tmp35 = _mm512_and_si512(tmp35,mask52);

	__m512i tmp36 = _mm512_add_epi64(D0_36,carry_tmp);
	tmp36 = _mm512_add_epi64(tmp36,D2_36);
	carry_tmp  = _mm512_srli_epi64(tmp36,52);
	tmp36 = _mm512_and_si512(tmp36,mask52);

	__m512i tmp37 = _mm512_add_epi64(D0_37,carry_tmp);
	tmp37 = _mm512_add_epi64(tmp37,D2_37);
	carry_tmp  = _mm512_srli_epi64(tmp37,52);
	tmp37 = _mm512_and_si512(tmp37,mask52);

	__m512i tmp38 = _mm512_add_epi64(D0_38,carry_tmp);
	tmp38 = _mm512_add_epi64(tmp38,D2_38);
	carry_tmp  = _mm512_srli_epi64(tmp38,52);
	tmp38 = _mm512_and_si512(tmp38,mask52);

	__m512i tmp39 = _mm512_add_epi64(D0_39,carry_tmp);
	tmp39 = _mm512_add_epi64(tmp39,D2_39);
	carry_tmp  = _mm512_srli_epi64(tmp39,51);
	tmp39 = _mm512_and_si512(tmp39,mask51);// radix 2**1039




	// D1 = D1 -tmp, radix 2^1039

	D1_0 = _mm512_sub_epi64(D1_0,tmp0);
	__m512i borrow = _mm512_srli_epi64(D1_0,52)&un;
	D1_0 = _mm512_and_si512(D1_0,mask52);
	borrow = _mm512_add_epi64(tmp1,borrow);
	D1_1 = _mm512_sub_epi64(D1_1,borrow);
	borrow  = _mm512_srli_epi64(D1_1,52)&un;
	D1_1 = _mm512_and_si512(D1_1,mask52);

	borrow = _mm512_add_epi64(tmp2,borrow);
	D1_2 = _mm512_sub_epi64(D1_2,borrow);
	borrow  = _mm512_srli_epi64(D1_2,52)&un;
	D1_2 = _mm512_and_si512(D1_2,mask52);

	borrow = _mm512_add_epi64(tmp3,borrow);
	D1_3 = _mm512_sub_epi64(D1_3,borrow);
	borrow  = _mm512_srli_epi64(D1_3,52)&un;
	D1_3 = _mm512_and_si512(D1_3,mask52);

	borrow = _mm512_add_epi64(tmp4,borrow);
	D1_4 = _mm512_sub_epi64(D1_4,borrow);
	borrow  = _mm512_srli_epi64(D1_4,52)&un;
	D1_4 = _mm512_and_si512(D1_4,mask52);

	borrow = _mm512_add_epi64(tmp5,borrow);
	D1_5 = _mm512_sub_epi64(D1_5,borrow);
	borrow  = _mm512_srli_epi64(D1_5,52)&un;
	D1_5 = _mm512_and_si512(D1_5,mask52);

	borrow = _mm512_add_epi64(tmp6,borrow);
	D1_6 = _mm512_sub_epi64(D1_6,borrow);
	borrow  = _mm512_srli_epi64(D1_6,52)&un;
	D1_6 = _mm512_and_si512(D1_6,mask52);

	borrow = _mm512_add_epi64(tmp7,borrow);
	D1_7 = _mm512_sub_epi64(D1_7,borrow);
	borrow  = _mm512_srli_epi64(D1_7,52)&un;
	D1_7 = _mm512_and_si512(D1_7,mask52);

	borrow = _mm512_add_epi64(tmp8,borrow);
	D1_8 = _mm512_sub_epi64(D1_8,borrow);
	borrow  = _mm512_srli_epi64(D1_8,52)&un;
	D1_8 = _mm512_and_si512(D1_8,mask52);

	borrow = _mm512_add_epi64(tmp9,borrow);
	D1_9 = _mm512_sub_epi64(D1_9,borrow);
	borrow  = _mm512_srli_epi64(D1_9,52)&un;
	D1_9 = _mm512_and_si512(D1_9,mask52);

	borrow = _mm512_add_epi64(tmp10,borrow);
	D1_10 = _mm512_sub_epi64(D1_10,borrow);
	borrow  = _mm512_srli_epi64(D1_10,52)&un;
	D1_10 = _mm512_and_si512(D1_10,mask52);

	borrow = _mm512_add_epi64(tmp11,borrow);
	D1_11 = _mm512_sub_epi64(D1_11,borrow);
	borrow  = _mm512_srli_epi64(D1_11,52)&un;
	D1_11 = _mm512_and_si512(D1_11,mask52);

	borrow = _mm512_add_epi64(tmp12,borrow);
	D1_12 = _mm512_sub_epi64(D1_12,borrow);
	borrow  = _mm512_srli_epi64(D1_12,52)&un;
	D1_12 = _mm512_and_si512(D1_12,mask52);

	borrow = _mm512_add_epi64(tmp13,borrow);
	D1_13 = _mm512_sub_epi64(D1_13,borrow);
	borrow  = _mm512_srli_epi64(D1_13,52)&un;
	D1_13 = _mm512_and_si512(D1_13,mask52);

	borrow = _mm512_add_epi64(tmp14,borrow);
	D1_14 = _mm512_sub_epi64(D1_14,borrow);
	borrow  = _mm512_srli_epi64(D1_14,52)&un;
	D1_14 = _mm512_and_si512(D1_14,mask52);

	borrow = _mm512_add_epi64(tmp15,borrow);
	D1_15 = _mm512_sub_epi64(D1_15,borrow);
	borrow  = _mm512_srli_epi64(D1_15,52)&un;
	D1_15 = _mm512_and_si512(D1_15,mask52);

	borrow = _mm512_add_epi64(tmp16,borrow);
	D1_16 = _mm512_sub_epi64(D1_16,borrow);
	borrow  = _mm512_srli_epi64(D1_16,52)&un;
	D1_16 = _mm512_and_si512(D1_16,mask52);

	borrow = _mm512_add_epi64(tmp17,borrow);
	D1_17 = _mm512_sub_epi64(D1_17,borrow);
	borrow  = _mm512_srli_epi64(D1_17,52)&un;
	D1_17 = _mm512_and_si512(D1_17,mask52);

	borrow = _mm512_add_epi64(tmp18,borrow);
	D1_18 = _mm512_sub_epi64(D1_18,borrow);
	borrow  = _mm512_srli_epi64(D1_18,52)&un;
	D1_18 = _mm512_and_si512(D1_18,mask52);

	borrow = _mm512_add_epi64(tmp19,borrow);
	D1_19 = _mm512_sub_epi64(D1_19,borrow);
	borrow = _mm512_srli_epi64(D1_19,51)&un;
	D1_19 = _mm512_and_si512(D1_19,mask51);// radix 2**1039

	borrow = _mm512_add_epi64(tmp20,borrow);
	D1_20 = _mm512_sub_epi64(D1_20,borrow);
	borrow  = _mm512_srli_epi64(D1_20,52)&un;
	D1_20 = _mm512_and_si512(D1_20,mask52);

	borrow = _mm512_add_epi64(tmp21,borrow);
	D1_21 = _mm512_sub_epi64(D1_21,borrow);
	borrow  = _mm512_srli_epi64(D1_21,52)&un;
	D1_21 = _mm512_and_si512(D1_21,mask52);

	borrow = _mm512_add_epi64(tmp22,borrow);
	D1_22 = _mm512_sub_epi64(D1_22,borrow);
	borrow  = _mm512_srli_epi64(D1_22,52)&un;
	D1_22 = _mm512_and_si512(D1_22,mask52);

	borrow = _mm512_add_epi64(tmp23,borrow);
	D1_23 = _mm512_sub_epi64(D1_23,borrow);
	borrow  = _mm512_srli_epi64(D1_23,52)&un;
	D1_23 = _mm512_and_si512(D1_23,mask52);

	borrow = _mm512_add_epi64(tmp24,borrow);
	D1_24 = _mm512_sub_epi64(D1_24,borrow);
	borrow  = _mm512_srli_epi64(D1_24,52)&un;
	D1_24 = _mm512_and_si512(D1_24,mask52);

	borrow = _mm512_add_epi64(tmp25,borrow);
	D1_25 = _mm512_sub_epi64(D1_25,borrow);
	borrow  = _mm512_srli_epi64(D1_25,52)&un;
	D1_25 = _mm512_and_si512(D1_25,mask52);

	borrow = _mm512_add_epi64(tmp26,borrow);
	D1_26 = _mm512_sub_epi64(D1_26,borrow);
	borrow  = _mm512_srli_epi64(D1_26,52)&un;
	D1_26 = _mm512_and_si512(D1_26,mask52);

	borrow = _mm512_add_epi64(tmp27,borrow);
	D1_27 = _mm512_sub_epi64(D1_27,borrow);
	borrow  = _mm512_srli_epi64(D1_27,52)&un;
	D1_27 = _mm512_and_si512(D1_27,mask52);

	borrow = _mm512_add_epi64(tmp28,borrow);
	D1_28 = _mm512_sub_epi64(D1_28,borrow);
	borrow  = _mm512_srli_epi64(D1_28,52)&un;
	D1_28 = _mm512_and_si512(D1_28,mask52);

	borrow = _mm512_add_epi64(tmp29,borrow);
	D1_29 = _mm512_sub_epi64(D1_29,borrow);
	borrow  = _mm512_srli_epi64(D1_29,52)&un;
	D1_29 = _mm512_and_si512(D1_29,mask52);

	borrow = _mm512_add_epi64(tmp30,borrow);
	D1_30 = _mm512_sub_epi64(D1_30,borrow);
	borrow  = _mm512_srli_epi64(D1_30,52)&un;
	D1_30 = _mm512_and_si512(D1_30,mask52);

	borrow = _mm512_add_epi64(tmp31,borrow);
	D1_31 = _mm512_sub_epi64(D1_31,borrow);
	borrow  = _mm512_srli_epi64(D1_31,52)&un;
	D1_31 = _mm512_and_si512(D1_31,mask52);

	borrow = _mm512_add_epi64(tmp32,borrow);
	D1_32 = _mm512_sub_epi64(D1_32,borrow);
	borrow  = _mm512_srli_epi64(D1_32,52)&un;
	D1_32 = _mm512_and_si512(D1_32,mask52);

	borrow = _mm512_add_epi64(tmp33,borrow);
	D1_33 = _mm512_sub_epi64(D1_33,borrow);
	borrow  = _mm512_srli_epi64(D1_33,52)&un;
	D1_33 = _mm512_and_si512(D1_33,mask52);

	borrow = _mm512_add_epi64(tmp34,borrow);
	D1_34 = _mm512_sub_epi64(D1_34,borrow);
	borrow  = _mm512_srli_epi64(D1_34,52)&un;
	D1_34 = _mm512_and_si512(D1_34,mask52);

	borrow = _mm512_add_epi64(tmp35,borrow);
	D1_35 = _mm512_sub_epi64(D1_35,borrow);
	borrow  = _mm512_srli_epi64(D1_35,52)&un;
	D1_35 = _mm512_and_si512(D1_35,mask52);

	borrow = _mm512_add_epi64(tmp36,borrow);
	D1_36 = _mm512_sub_epi64(D1_36,borrow);
	borrow  = _mm512_srli_epi64(D1_36,52)&un;
	D1_36 = _mm512_and_si512(D1_36,mask52);

	borrow = _mm512_add_epi64(tmp37,borrow);
	D1_37 = _mm512_sub_epi64(D1_37,borrow);
	borrow  = _mm512_srli_epi64(D1_37,52)&un;
	D1_37 = _mm512_and_si512(D1_37,mask52);

	borrow = _mm512_add_epi64(tmp38,borrow);
	D1_38 = _mm512_sub_epi64(D1_38,borrow);
	borrow  = _mm512_srli_epi64(D1_38,52)&un;
	D1_38 = _mm512_and_si512(D1_38,mask52);

	borrow = _mm512_add_epi64(tmp39,borrow);
	D1_39 = _mm512_sub_epi64(D1_39,borrow);
	borrow = _mm512_srli_epi64(D1_39,51)&un;
	D1_39 = _mm512_and_si512(D1_39,mask51);// radix 2**1039


	// Final borrows

	borrow = _mm512_add_epi64(carry_tmp,borrow);
	D2_20 = _mm512_add_epi64(D2_20,carry);
	D2_20 = _mm512_sub_epi64(D2_20,borrow);
	borrow  = _mm512_srli_epi64(D2_20,52)&un;
	D2_20 = _mm512_and_si512(D2_20,mask52);

	D2_21 = _mm512_sub_epi64(D2_21,borrow);
	borrow  = _mm512_srli_epi64(D2_21,52)&un;
	D2_21 = _mm512_and_si512(D2_21,mask52);

	D2_22 = _mm512_sub_epi64(D2_22,borrow);
	borrow  = _mm512_srli_epi64(D2_22,52)&un;
	D2_22 = _mm512_and_si512(D2_22,mask52);

	D2_23 = _mm512_sub_epi64(D2_23,borrow);
	borrow  = _mm512_srli_epi64(D2_23,52)&un;
	D2_23 = _mm512_and_si512(D2_23,mask52);

	D2_24 = _mm512_sub_epi64(D2_24,borrow);
	borrow  = _mm512_srli_epi64(D2_24,52)&un;
	D2_24 = _mm512_and_si512(D2_24,mask52);

	D2_25 = _mm512_sub_epi64(D2_25,borrow);
	borrow  = _mm512_srli_epi64(D2_25,52)&un;
	D2_25 = _mm512_and_si512(D2_25,mask52);

	D2_26 = _mm512_sub_epi64(D2_26,borrow);
	borrow  = _mm512_srli_epi64(D2_26,52)&un;
	D2_26 = _mm512_and_si512(D2_26,mask52);

	D2_27 = _mm512_sub_epi64(D2_27,borrow);
	borrow  = _mm512_srli_epi64(D2_27,52)&un;
	D2_27 = _mm512_and_si512(D2_27,mask52);

	D2_28 = _mm512_sub_epi64(D2_28,borrow);
	borrow  = _mm512_srli_epi64(D2_28,52)&un;
	D2_28 = _mm512_and_si512(D2_28,mask52);

	D2_29 = _mm512_sub_epi64(D2_29,borrow);
	borrow  = _mm512_srli_epi64(D2_29,52)&un;
	D2_29 = _mm512_and_si512(D2_29,mask52);

	D2_30 = _mm512_sub_epi64(D2_30,borrow);
	borrow  = _mm512_srli_epi64(D2_30,52)&un;
	D2_30 = _mm512_and_si512(D2_30,mask52);

	D2_31 = _mm512_sub_epi64(D2_31,borrow);
	borrow  = _mm512_srli_epi64(D2_31,52)&un;
	D2_31 = _mm512_and_si512(D2_31,mask52);

	D2_32 = _mm512_sub_epi64(D2_32,borrow);
	borrow  = _mm512_srli_epi64(D2_32,52)&un;
	D2_32 = _mm512_and_si512(D2_32,mask52);

	D2_33 = _mm512_sub_epi64(D2_33,borrow);
	borrow  = _mm512_srli_epi64(D2_33,52)&un;
	D2_33 = _mm512_and_si512(D2_33,mask52);

	D2_34 = _mm512_sub_epi64(D2_34,borrow);
	borrow  = _mm512_srli_epi64(D2_34,52)&un;
	D2_34 = _mm512_and_si512(D2_34,mask52);

	D2_35 = _mm512_sub_epi64(D2_35,borrow);
	borrow  = _mm512_srli_epi64(D2_35,52)&un;
	D2_35 = _mm512_and_si512(D2_35,mask52);

	D2_36 = _mm512_sub_epi64(D2_36,borrow);
	borrow  = _mm512_srli_epi64(D2_36,52)&un;
	D2_36 = _mm512_and_si512(D2_36,mask52);

	D2_37 = _mm512_sub_epi64(D2_37,borrow);
	borrow  = _mm512_srli_epi64(D2_37,52)&un;
	D2_37 = _mm512_and_si512(D2_37,mask52);

	D2_38 = _mm512_sub_epi64(D2_38,borrow);
	borrow  = _mm512_srli_epi64(D2_38,52)&un;
	D2_38 = _mm512_and_si512(D2_38,mask52);

	D2_39 = _mm512_sub_epi64(D2_39,borrow);


	_mm512_store_epi64(out+0,D0_0);
	_mm512_store_epi64(out+1,D0_1);
	_mm512_store_epi64(out+2,D0_2);
	_mm512_store_epi64(out+3,D0_3);
	_mm512_store_epi64(out+4,D0_4);
	_mm512_store_epi64(out+5,D0_5);
	_mm512_store_epi64(out+6,D0_6);
	_mm512_store_epi64(out+7,D0_7);
	_mm512_store_epi64(out+8,D0_8);
	_mm512_store_epi64(out+9,D0_9);
	_mm512_store_epi64(out+10,D0_10);
	_mm512_store_epi64(out+11,D0_11);
	_mm512_store_epi64(out+12,D0_12);
	_mm512_store_epi64(out+13,D0_13);
	_mm512_store_epi64(out+14,D0_14);
	_mm512_store_epi64(out+15,D0_15);
	_mm512_store_epi64(out+16,D0_16);
	_mm512_store_epi64(out+17,D0_17);
	_mm512_store_epi64(out+18,D0_18);
	_mm512_store_epi64(out+19,D0_19);
	_mm512_store_epi64(out+20,D1_0);
	_mm512_store_epi64(out+21,D1_1);
	_mm512_store_epi64(out+22,D1_2);
	_mm512_store_epi64(out+23,D1_3);
	_mm512_store_epi64(out+24,D1_4);
	_mm512_store_epi64(out+25,D1_5);
	_mm512_store_epi64(out+26,D1_6);
	_mm512_store_epi64(out+27,D1_7);
	_mm512_store_epi64(out+28,D1_8);
	_mm512_store_epi64(out+29,D1_9);
	_mm512_store_epi64(out+30,D1_10);
	_mm512_store_epi64(out+31,D1_11);
	_mm512_store_epi64(out+32,D1_12);
	_mm512_store_epi64(out+33,D1_13);
	_mm512_store_epi64(out+34,D1_14);
	_mm512_store_epi64(out+35,D1_15);
	_mm512_store_epi64(out+36,D1_16);
	_mm512_store_epi64(out+37,D1_17);
	_mm512_store_epi64(out+38,D1_18);
	_mm512_store_epi64(out+39,D1_19);
	_mm512_store_epi64(out+40,D1_20);
	_mm512_store_epi64(out+41,D1_21);
	_mm512_store_epi64(out+42,D1_22);
	_mm512_store_epi64(out+43,D1_23);
	_mm512_store_epi64(out+44,D1_24);
	_mm512_store_epi64(out+45,D1_25);
	_mm512_store_epi64(out+46,D1_26);
	_mm512_store_epi64(out+47,D1_27);
	_mm512_store_epi64(out+48,D1_28);
	_mm512_store_epi64(out+49,D1_29);
	_mm512_store_epi64(out+50,D1_30);
	_mm512_store_epi64(out+51,D1_31);
	_mm512_store_epi64(out+52,D1_32);
	_mm512_store_epi64(out+53,D1_33);
	_mm512_store_epi64(out+54,D1_34);
	_mm512_store_epi64(out+55,D1_35);
	_mm512_store_epi64(out+56,D1_36);
	_mm512_store_epi64(out+57,D1_37);
	_mm512_store_epi64(out+58,D1_38);
	_mm512_store_epi64(out+59,D1_39);
	_mm512_store_epi64(out+60,D2_20);
	_mm512_store_epi64(out+61,D2_21);
	_mm512_store_epi64(out+62,D2_22);
	_mm512_store_epi64(out+63,D2_23);
	_mm512_store_epi64(out+64,D2_24);
	_mm512_store_epi64(out+65,D2_25);
	_mm512_store_epi64(out+66,D2_26);
	_mm512_store_epi64(out+67,D2_27);
	_mm512_store_epi64(out+68,D2_28);
	_mm512_store_epi64(out+69,D2_29);
	_mm512_store_epi64(out+70,D2_30);
	_mm512_store_epi64(out+71,D2_31);
	_mm512_store_epi64(out+72,D2_32);
	_mm512_store_epi64(out+73,D2_33);
	_mm512_store_epi64(out+74,D2_34);
	_mm512_store_epi64(out+75,D2_35);
	_mm512_store_epi64(out+76,D2_36);
	_mm512_store_epi64(out+77,D2_37);
	_mm512_store_epi64(out+78,D2_38);
	_mm512_store_epi64(out+79,D2_39);

}

/* Karatsuba squaring */

inline void force_inline square512_8_2078K(__m512i out[80], __m512i a512[40])
{

	// D0 = Al^2

	__m512i D0_0, D0_1;
	__m512i D0_2, D0_3;
	__m512i D0_4, D0_5;
	__m512i D0_6, D0_7;
	__m512i D0_8, D0_9;
	__m512i D0_10, D0_11;
	__m512i D0_12, D0_13;
	__m512i D0_14, D0_15;
	__m512i D0_16, D0_17;
	__m512i D0_18, D0_19;
	__m512i D0_20, D0_21;
	__m512i D0_22, D0_23;
	__m512i D0_24, D0_25;
	__m512i D0_26, D0_27;
	__m512i D0_28, D0_29;
	__m512i D0_30, D0_31;
	__m512i D0_32, D0_33;
	__m512i D0_34, D0_35;
	__m512i D0_36, D0_37;
	__m512i D0_38, D0_39;
	__m512i a5120 = _mm512_load_epi64(a512);
	__m512i a5121 = _mm512_load_epi64(a512+1);
	__m512i a5122 = _mm512_load_epi64(a512+2);
	__m512i a5123 = _mm512_load_epi64(a512+3);
	__m512i a5124 = _mm512_load_epi64(a512+4);
	__m512i a5125 = _mm512_load_epi64(a512+5);
	__m512i a5126 = _mm512_load_epi64(a512+6);
	__m512i a5127 = _mm512_load_epi64(a512+7);
	__m512i a5128 = _mm512_load_epi64(a512+8);
	__m512i a5129 = _mm512_load_epi64(a512+9);
	__m512i a51210 = _mm512_load_epi64(a512+10);
	__m512i a51211 = _mm512_load_epi64(a512+11);
	__m512i a51212 = _mm512_load_epi64(a512+12);
	__m512i a51213 = _mm512_load_epi64(a512+13);
	__m512i a51214 = _mm512_load_epi64(a512+14);
	__m512i a51215 = _mm512_load_epi64(a512+15);
	__m512i a51216 = _mm512_load_epi64(a512+16);
	__m512i a51217 = _mm512_load_epi64(a512+17);
	__m512i a51218 = _mm512_load_epi64(a512+18);
	__m512i a51219 = _mm512_load_epi64(a512+19);

	D0_0 = _mm512_madd52lo_epu64(zero,a5120,a5120);

	D0_1 = _mm512_madd52lo_epu64(zero,a5120,a5121);
	D0_1 = _mm512_slli_epi64(D0_1,1);
	D0_1 = _mm512_madd52hi_epu64(D0_1,a5120,a5120);

	D0_2 = _mm512_madd52hi_epu64(zero,a5120,a5121);
	D0_2 = _mm512_madd52lo_epu64(D0_2,a5120,a5122);
	D0_2 = _mm512_slli_epi64(D0_2,1);
	D0_2 = _mm512_madd52lo_epu64(D0_2,a5121,a5121);

	D0_3 = _mm512_madd52hi_epu64(zero,a5120,a5122);
	D0_3 = _mm512_madd52lo_epu64(D0_3,a5120,a5123);
	D0_3 = _mm512_madd52lo_epu64(D0_3,a5121,a5122);
	D0_3 = _mm512_slli_epi64(D0_3,1);
	D0_3 = _mm512_madd52hi_epu64(D0_3,a5121,a5121);

	D0_4 = _mm512_madd52hi_epu64(zero,a5120,a5123);
	D0_4 = _mm512_madd52hi_epu64(D0_4,a5121,a5122);
	D0_4 = _mm512_madd52lo_epu64(D0_4,a5120,a5124);
	D0_4 = _mm512_madd52lo_epu64(D0_4,a5121,a5123);
	D0_4 = _mm512_slli_epi64(D0_4,1);
	D0_4 = _mm512_madd52lo_epu64(D0_4,a5122,a5122);

	D0_5 = _mm512_madd52hi_epu64(zero,a5120,a5124);
	D0_5 = _mm512_madd52hi_epu64(D0_5,a5121,a5123);
	D0_5 = _mm512_madd52lo_epu64(D0_5,a5120,a5125);
	D0_5 = _mm512_madd52lo_epu64(D0_5,a5121,a5124);
	D0_5 = _mm512_madd52lo_epu64(D0_5,a5122,a5123);
	D0_5 = _mm512_slli_epi64(D0_5,1);
	D0_5 = _mm512_madd52hi_epu64(D0_5,a5122,a5122);

	D0_6 = _mm512_madd52hi_epu64(zero,a5120,a5125);
	D0_6 = _mm512_madd52hi_epu64(D0_6,a5121,a5124);
	D0_6 = _mm512_madd52hi_epu64(D0_6,a5122,a5123);
	D0_6 = _mm512_madd52lo_epu64(D0_6,a5120,a5126);
	D0_6 = _mm512_madd52lo_epu64(D0_6,a5121,a5125);
	D0_6 = _mm512_madd52lo_epu64(D0_6,a5122,a5124);
	D0_6 = _mm512_slli_epi64(D0_6,1);
	D0_6 = _mm512_madd52lo_epu64(D0_6,a5123,a5123);

	D0_7 = _mm512_madd52hi_epu64(zero,a5120,a5126);
	D0_7 = _mm512_madd52hi_epu64(D0_7,a5121,a5125);
	D0_7 = _mm512_madd52hi_epu64(D0_7,a5122,a5124);
	D0_7 = _mm512_madd52lo_epu64(D0_7,a5120,a5127);
	D0_7 = _mm512_madd52lo_epu64(D0_7,a5121,a5126);
	D0_7 = _mm512_madd52lo_epu64(D0_7,a5122,a5125);
	D0_7 = _mm512_madd52lo_epu64(D0_7,a5123,a5124);
	D0_7 = _mm512_slli_epi64(D0_7,1);
	D0_7 = _mm512_madd52hi_epu64(D0_7,a5123,a5123);

	D0_8 = _mm512_madd52hi_epu64(zero,a5120,a5127);
	D0_8 = _mm512_madd52hi_epu64(D0_8,a5121,a5126);
	D0_8 = _mm512_madd52hi_epu64(D0_8,a5122,a5125);
	D0_8 = _mm512_madd52hi_epu64(D0_8,a5123,a5124);
	D0_8 = _mm512_madd52lo_epu64(D0_8,a5120,a5128);
	D0_8 = _mm512_madd52lo_epu64(D0_8,a5121,a5127);
	D0_8 = _mm512_madd52lo_epu64(D0_8,a5122,a5126);
	D0_8 = _mm512_madd52lo_epu64(D0_8,a5123,a5125);
	D0_8 = _mm512_slli_epi64(D0_8,1);
	D0_8 = _mm512_madd52lo_epu64(D0_8,a5124,a5124);

	D0_9 = _mm512_madd52hi_epu64(zero,a5120,a5128);
	D0_9 = _mm512_madd52hi_epu64(D0_9,a5121,a5127);
	D0_9 = _mm512_madd52hi_epu64(D0_9,a5122,a5126);
	D0_9 = _mm512_madd52hi_epu64(D0_9,a5123,a5125);
	D0_9 = _mm512_madd52lo_epu64(D0_9,a5120,a5129);
	D0_9 = _mm512_madd52lo_epu64(D0_9,a5121,a5128);
	D0_9 = _mm512_madd52lo_epu64(D0_9,a5122,a5127);
	D0_9 = _mm512_madd52lo_epu64(D0_9,a5123,a5126);
	D0_9 = _mm512_madd52lo_epu64(D0_9,a5124,a5125);
	D0_9 = _mm512_slli_epi64(D0_9,1);
	D0_9 = _mm512_madd52hi_epu64(D0_9,a5124,a5124);

	D0_10 = _mm512_madd52hi_epu64(zero,a5120,a5129);
	D0_10 = _mm512_madd52hi_epu64(D0_10,a5121,a5128);
	D0_10 = _mm512_madd52hi_epu64(D0_10,a5122,a5127);
	D0_10 = _mm512_madd52hi_epu64(D0_10,a5123,a5126);
	D0_10 = _mm512_madd52hi_epu64(D0_10,a5124,a5125);
	D0_10 = _mm512_madd52lo_epu64(D0_10,a5120,a51210);
	D0_10 = _mm512_madd52lo_epu64(D0_10,a5121,a5129);
	D0_10 = _mm512_madd52lo_epu64(D0_10,a5122,a5128);
	D0_10 = _mm512_madd52lo_epu64(D0_10,a5123,a5127);
	D0_10 = _mm512_madd52lo_epu64(D0_10,a5124,a5126);
	D0_10 = _mm512_slli_epi64(D0_10,1);
	D0_10 = _mm512_madd52lo_epu64(D0_10,a5125,a5125);

	D0_11 = _mm512_madd52hi_epu64(zero,a5120,a51210);
	D0_11 = _mm512_madd52hi_epu64(D0_11,a5121,a5129);
	D0_11 = _mm512_madd52hi_epu64(D0_11,a5122,a5128);
	D0_11 = _mm512_madd52hi_epu64(D0_11,a5123,a5127);
	D0_11 = _mm512_madd52hi_epu64(D0_11,a5124,a5126);
	D0_11 = _mm512_madd52lo_epu64(D0_11,a5120,a51211);
	D0_11 = _mm512_madd52lo_epu64(D0_11,a5121,a51210);
	D0_11 = _mm512_madd52lo_epu64(D0_11,a5122,a5129);
	D0_11 = _mm512_madd52lo_epu64(D0_11,a5123,a5128);
	D0_11 = _mm512_madd52lo_epu64(D0_11,a5124,a5127);
	D0_11 = _mm512_madd52lo_epu64(D0_11,a5125,a5126);
	D0_11 = _mm512_slli_epi64(D0_11,1);
	D0_11 = _mm512_madd52hi_epu64(D0_11,a5125,a5125);

	D0_12 = _mm512_madd52hi_epu64(zero,a5120,a51211);
	D0_12 = _mm512_madd52hi_epu64(D0_12,a5121,a51210);
	D0_12 = _mm512_madd52hi_epu64(D0_12,a5122,a5129);
	D0_12 = _mm512_madd52hi_epu64(D0_12,a5123,a5128);
	D0_12 = _mm512_madd52hi_epu64(D0_12,a5124,a5127);
	D0_12 = _mm512_madd52hi_epu64(D0_12,a5125,a5126);
	D0_12 = _mm512_madd52lo_epu64(D0_12,a5120,a51212);
	D0_12 = _mm512_madd52lo_epu64(D0_12,a5121,a51211);
	D0_12 = _mm512_madd52lo_epu64(D0_12,a5122,a51210);
	D0_12 = _mm512_madd52lo_epu64(D0_12,a5123,a5129);
	D0_12 = _mm512_madd52lo_epu64(D0_12,a5124,a5128);
	D0_12 = _mm512_madd52lo_epu64(D0_12,a5125,a5127);
	D0_12 = _mm512_slli_epi64(D0_12,1);
	D0_12 = _mm512_madd52lo_epu64(D0_12,a5126,a5126);

	D0_13 = _mm512_madd52hi_epu64(zero,a5120,a51212);
	D0_13 = _mm512_madd52hi_epu64(D0_13,a5121,a51211);
	D0_13 = _mm512_madd52hi_epu64(D0_13,a5122,a51210);
	D0_13 = _mm512_madd52hi_epu64(D0_13,a5123,a5129);
	D0_13 = _mm512_madd52hi_epu64(D0_13,a5124,a5128);
	D0_13 = _mm512_madd52hi_epu64(D0_13,a5125,a5127);
	D0_13 = _mm512_madd52lo_epu64(D0_13,a5120,a51213);
	D0_13 = _mm512_madd52lo_epu64(D0_13,a5121,a51212);
	D0_13 = _mm512_madd52lo_epu64(D0_13,a5122,a51211);
	D0_13 = _mm512_madd52lo_epu64(D0_13,a5123,a51210);
	D0_13 = _mm512_madd52lo_epu64(D0_13,a5124,a5129);
	D0_13 = _mm512_madd52lo_epu64(D0_13,a5125,a5128);
	D0_13 = _mm512_madd52lo_epu64(D0_13,a5126,a5127);
	D0_13 = _mm512_slli_epi64(D0_13,1);
	D0_13 = _mm512_madd52hi_epu64(D0_13,a5126,a5126);

	D0_14 = _mm512_madd52hi_epu64(zero,a5120,a51213);
	D0_14 = _mm512_madd52hi_epu64(D0_14,a5121,a51212);
	D0_14 = _mm512_madd52hi_epu64(D0_14,a5122,a51211);
	D0_14 = _mm512_madd52hi_epu64(D0_14,a5123,a51210);
	D0_14 = _mm512_madd52hi_epu64(D0_14,a5124,a5129);
	D0_14 = _mm512_madd52hi_epu64(D0_14,a5125,a5128);
	D0_14 = _mm512_madd52hi_epu64(D0_14,a5126,a5127);
	D0_14 = _mm512_madd52lo_epu64(D0_14,a5120,a51214);
	D0_14 = _mm512_madd52lo_epu64(D0_14,a5121,a51213);
	D0_14 = _mm512_madd52lo_epu64(D0_14,a5122,a51212);
	D0_14 = _mm512_madd52lo_epu64(D0_14,a5123,a51211);
	D0_14 = _mm512_madd52lo_epu64(D0_14,a5124,a51210);
	D0_14 = _mm512_madd52lo_epu64(D0_14,a5125,a5129);
	D0_14 = _mm512_madd52lo_epu64(D0_14,a5126,a5128);
	D0_14 = _mm512_slli_epi64(D0_14,1);
	D0_14 = _mm512_madd52lo_epu64(D0_14,a5127,a5127);

	D0_15 = _mm512_madd52hi_epu64(zero,a5120,a51214);
	D0_15 = _mm512_madd52hi_epu64(D0_15,a5121,a51213);
	D0_15 = _mm512_madd52hi_epu64(D0_15,a5122,a51212);
	D0_15 = _mm512_madd52hi_epu64(D0_15,a5123,a51211);
	D0_15 = _mm512_madd52hi_epu64(D0_15,a5124,a51210);
	D0_15 = _mm512_madd52hi_epu64(D0_15,a5125,a5129);
	D0_15 = _mm512_madd52hi_epu64(D0_15,a5126,a5128);
	D0_15 = _mm512_madd52lo_epu64(D0_15,a5120,a51215);
	D0_15 = _mm512_madd52lo_epu64(D0_15,a5121,a51214);
	D0_15 = _mm512_madd52lo_epu64(D0_15,a5122,a51213);
	D0_15 = _mm512_madd52lo_epu64(D0_15,a5123,a51212);
	D0_15 = _mm512_madd52lo_epu64(D0_15,a5124,a51211);
	D0_15 = _mm512_madd52lo_epu64(D0_15,a5125,a51210);
	D0_15 = _mm512_madd52lo_epu64(D0_15,a5126,a5129);
	D0_15 = _mm512_madd52lo_epu64(D0_15,a5127,a5128);
	D0_15 = _mm512_slli_epi64(D0_15,1);
	D0_15 = _mm512_madd52hi_epu64(D0_15,a5127,a5127);

	D0_16 = _mm512_madd52hi_epu64(zero,a5120,a51215);
	D0_16 = _mm512_madd52hi_epu64(D0_16,a5121,a51214);
	D0_16 = _mm512_madd52hi_epu64(D0_16,a5122,a51213);
	D0_16 = _mm512_madd52hi_epu64(D0_16,a5123,a51212);
	D0_16 = _mm512_madd52hi_epu64(D0_16,a5124,a51211);
	D0_16 = _mm512_madd52hi_epu64(D0_16,a5125,a51210);
	D0_16 = _mm512_madd52hi_epu64(D0_16,a5126,a5129);
	D0_16 = _mm512_madd52hi_epu64(D0_16,a5127,a5128);
	D0_16 = _mm512_madd52lo_epu64(D0_16,a5120,a51216);
	D0_16 = _mm512_madd52lo_epu64(D0_16,a5121,a51215);
	D0_16 = _mm512_madd52lo_epu64(D0_16,a5122,a51214);
	D0_16 = _mm512_madd52lo_epu64(D0_16,a5123,a51213);
	D0_16 = _mm512_madd52lo_epu64(D0_16,a5124,a51212);
	D0_16 = _mm512_madd52lo_epu64(D0_16,a5125,a51211);
	D0_16 = _mm512_madd52lo_epu64(D0_16,a5126,a51210);
	D0_16 = _mm512_madd52lo_epu64(D0_16,a5127,a5129);
	D0_16 = _mm512_slli_epi64(D0_16,1);
	D0_16 = _mm512_madd52lo_epu64(D0_16,a5128,a5128);

	D0_17 = _mm512_madd52hi_epu64(zero,a5120,a51216);
	D0_17 = _mm512_madd52hi_epu64(D0_17,a5121,a51215);
	D0_17 = _mm512_madd52hi_epu64(D0_17,a5122,a51214);
	D0_17 = _mm512_madd52hi_epu64(D0_17,a5123,a51213);
	D0_17 = _mm512_madd52hi_epu64(D0_17,a5124,a51212);
	D0_17 = _mm512_madd52hi_epu64(D0_17,a5125,a51211);
	D0_17 = _mm512_madd52hi_epu64(D0_17,a5126,a51210);
	D0_17 = _mm512_madd52hi_epu64(D0_17,a5127,a5129);
	D0_17 = _mm512_madd52lo_epu64(D0_17,a5120,a51217);
	D0_17 = _mm512_madd52lo_epu64(D0_17,a5121,a51216);
	D0_17 = _mm512_madd52lo_epu64(D0_17,a5122,a51215);
	D0_17 = _mm512_madd52lo_epu64(D0_17,a5123,a51214);
	D0_17 = _mm512_madd52lo_epu64(D0_17,a5124,a51213);
	D0_17 = _mm512_madd52lo_epu64(D0_17,a5125,a51212);
	D0_17 = _mm512_madd52lo_epu64(D0_17,a5126,a51211);
	D0_17 = _mm512_madd52lo_epu64(D0_17,a5127,a51210);
	D0_17 = _mm512_madd52lo_epu64(D0_17,a5128,a5129);
	D0_17 = _mm512_slli_epi64(D0_17,1);
	D0_17 = _mm512_madd52hi_epu64(D0_17,a5128,a5128);

	D0_18 = _mm512_madd52hi_epu64(zero,a5120,a51217);
	D0_18 = _mm512_madd52hi_epu64(D0_18,a5121,a51216);
	D0_18 = _mm512_madd52hi_epu64(D0_18,a5122,a51215);
	D0_18 = _mm512_madd52hi_epu64(D0_18,a5123,a51214);
	D0_18 = _mm512_madd52hi_epu64(D0_18,a5124,a51213);
	D0_18 = _mm512_madd52hi_epu64(D0_18,a5125,a51212);
	D0_18 = _mm512_madd52hi_epu64(D0_18,a5126,a51211);
	D0_18 = _mm512_madd52hi_epu64(D0_18,a5127,a51210);
	D0_18 = _mm512_madd52hi_epu64(D0_18,a5128,a5129);
	D0_18 = _mm512_madd52lo_epu64(D0_18,a5120,a51218);
	D0_18 = _mm512_madd52lo_epu64(D0_18,a5121,a51217);
	D0_18 = _mm512_madd52lo_epu64(D0_18,a5122,a51216);
	D0_18 = _mm512_madd52lo_epu64(D0_18,a5123,a51215);
	D0_18 = _mm512_madd52lo_epu64(D0_18,a5124,a51214);
	D0_18 = _mm512_madd52lo_epu64(D0_18,a5125,a51213);
	D0_18 = _mm512_madd52lo_epu64(D0_18,a5126,a51212);
	D0_18 = _mm512_madd52lo_epu64(D0_18,a5127,a51211);
	D0_18 = _mm512_madd52lo_epu64(D0_18,a5128,a51210);
	D0_18 = _mm512_slli_epi64(D0_18,1);
	D0_18 = _mm512_madd52lo_epu64(D0_18,a5129,a5129);

	D0_19 = _mm512_madd52hi_epu64(zero,a5120,a51218);
	D0_19 = _mm512_madd52hi_epu64(D0_19,a5121,a51217);
	D0_19 = _mm512_madd52hi_epu64(D0_19,a5122,a51216);
	D0_19 = _mm512_madd52hi_epu64(D0_19,a5123,a51215);
	D0_19 = _mm512_madd52hi_epu64(D0_19,a5124,a51214);
	D0_19 = _mm512_madd52hi_epu64(D0_19,a5125,a51213);
	D0_19 = _mm512_madd52hi_epu64(D0_19,a5126,a51212);
	D0_19 = _mm512_madd52hi_epu64(D0_19,a5127,a51211);
	D0_19 = _mm512_madd52hi_epu64(D0_19,a5128,a51210);
	D0_19 = _mm512_madd52lo_epu64(D0_19,a5120,a51219);
	D0_19 = _mm512_madd52lo_epu64(D0_19,a5121,a51218);
	D0_19 = _mm512_madd52lo_epu64(D0_19,a5122,a51217);
	D0_19 = _mm512_madd52lo_epu64(D0_19,a5123,a51216);
	D0_19 = _mm512_madd52lo_epu64(D0_19,a5124,a51215);
	D0_19 = _mm512_madd52lo_epu64(D0_19,a5125,a51214);
	D0_19 = _mm512_madd52lo_epu64(D0_19,a5126,a51213);
	D0_19 = _mm512_madd52lo_epu64(D0_19,a5127,a51212);
	D0_19 = _mm512_madd52lo_epu64(D0_19,a5128,a51211);
	D0_19 = _mm512_madd52lo_epu64(D0_19,a5129,a51210);
	D0_19 = _mm512_slli_epi64(D0_19,1);
	D0_19 = _mm512_madd52hi_epu64(D0_19,a5129,a5129);

	D0_20 = _mm512_madd52hi_epu64(zero,a5120,a51219);
	D0_20 = _mm512_madd52hi_epu64(D0_20,a5121,a51218);
	D0_20 = _mm512_madd52hi_epu64(D0_20,a5122,a51217);
	D0_20 = _mm512_madd52hi_epu64(D0_20,a5123,a51216);
	D0_20 = _mm512_madd52hi_epu64(D0_20,a5124,a51215);
	D0_20 = _mm512_madd52hi_epu64(D0_20,a5125,a51214);
	D0_20 = _mm512_madd52hi_epu64(D0_20,a5126,a51213);
	D0_20 = _mm512_madd52hi_epu64(D0_20,a5127,a51212);
	D0_20 = _mm512_madd52hi_epu64(D0_20,a5128,a51211);
	D0_20 = _mm512_madd52hi_epu64(D0_20,a5129,a51210);
	D0_20 = _mm512_madd52lo_epu64(D0_20,a5121,a51219);
	D0_20 = _mm512_madd52lo_epu64(D0_20,a5122,a51218);
	D0_20 = _mm512_madd52lo_epu64(D0_20,a5123,a51217);
	D0_20 = _mm512_madd52lo_epu64(D0_20,a5124,a51216);
	D0_20 = _mm512_madd52lo_epu64(D0_20,a5125,a51215);
	D0_20 = _mm512_madd52lo_epu64(D0_20,a5126,a51214);
	D0_20 = _mm512_madd52lo_epu64(D0_20,a5127,a51213);
	D0_20 = _mm512_madd52lo_epu64(D0_20,a5128,a51212);
	D0_20 = _mm512_madd52lo_epu64(D0_20,a5129,a51211);
	D0_20 = _mm512_slli_epi64(D0_20,1);
	D0_20 = _mm512_madd52lo_epu64(D0_20,a51210,a51210);

	D0_21 = _mm512_madd52hi_epu64(zero,a5121,a51219);
	D0_21 = _mm512_madd52hi_epu64(D0_21,a5122,a51218);
	D0_21 = _mm512_madd52hi_epu64(D0_21,a5123,a51217);
	D0_21 = _mm512_madd52hi_epu64(D0_21,a5124,a51216);
	D0_21 = _mm512_madd52hi_epu64(D0_21,a5125,a51215);
	D0_21 = _mm512_madd52hi_epu64(D0_21,a5126,a51214);
	D0_21 = _mm512_madd52hi_epu64(D0_21,a5127,a51213);
	D0_21 = _mm512_madd52hi_epu64(D0_21,a5128,a51212);
	D0_21 = _mm512_madd52hi_epu64(D0_21,a5129,a51211);
	D0_21 = _mm512_madd52lo_epu64(D0_21,a5122,a51219);
	D0_21 = _mm512_madd52lo_epu64(D0_21,a5123,a51218);
	D0_21 = _mm512_madd52lo_epu64(D0_21,a5124,a51217);
	D0_21 = _mm512_madd52lo_epu64(D0_21,a5125,a51216);
	D0_21 = _mm512_madd52lo_epu64(D0_21,a5126,a51215);
	D0_21 = _mm512_madd52lo_epu64(D0_21,a5127,a51214);
	D0_21 = _mm512_madd52lo_epu64(D0_21,a5128,a51213);
	D0_21 = _mm512_madd52lo_epu64(D0_21,a5129,a51212);
	D0_21 = _mm512_madd52lo_epu64(D0_21,a51210,a51211);
	D0_21 = _mm512_slli_epi64(D0_21,1);
	D0_21 = _mm512_madd52hi_epu64(D0_21,a51210,a51210);

	D0_22 = _mm512_madd52hi_epu64(zero,a5122,a51219);
	D0_22 = _mm512_madd52hi_epu64(D0_22,a5123,a51218);
	D0_22 = _mm512_madd52hi_epu64(D0_22,a5124,a51217);
	D0_22 = _mm512_madd52hi_epu64(D0_22,a5125,a51216);
	D0_22 = _mm512_madd52hi_epu64(D0_22,a5126,a51215);
	D0_22 = _mm512_madd52hi_epu64(D0_22,a5127,a51214);
	D0_22 = _mm512_madd52hi_epu64(D0_22,a5128,a51213);
	D0_22 = _mm512_madd52hi_epu64(D0_22,a5129,a51212);
	D0_22 = _mm512_madd52hi_epu64(D0_22,a51210,a51211);
	D0_22 = _mm512_madd52lo_epu64(D0_22,a5123,a51219);
	D0_22 = _mm512_madd52lo_epu64(D0_22,a5124,a51218);
	D0_22 = _mm512_madd52lo_epu64(D0_22,a5125,a51217);
	D0_22 = _mm512_madd52lo_epu64(D0_22,a5126,a51216);
	D0_22 = _mm512_madd52lo_epu64(D0_22,a5127,a51215);
	D0_22 = _mm512_madd52lo_epu64(D0_22,a5128,a51214);
	D0_22 = _mm512_madd52lo_epu64(D0_22,a5129,a51213);
	D0_22 = _mm512_madd52lo_epu64(D0_22,a51210,a51212);
	D0_22 = _mm512_slli_epi64(D0_22,1);
	D0_22 = _mm512_madd52lo_epu64(D0_22,a51211,a51211);

	D0_23 = _mm512_madd52hi_epu64(zero,a5123,a51219);
	D0_23 = _mm512_madd52hi_epu64(D0_23,a5124,a51218);
	D0_23 = _mm512_madd52hi_epu64(D0_23,a5125,a51217);
	D0_23 = _mm512_madd52hi_epu64(D0_23,a5126,a51216);
	D0_23 = _mm512_madd52hi_epu64(D0_23,a5127,a51215);
	D0_23 = _mm512_madd52hi_epu64(D0_23,a5128,a51214);
	D0_23 = _mm512_madd52hi_epu64(D0_23,a5129,a51213);
	D0_23 = _mm512_madd52hi_epu64(D0_23,a51210,a51212);
	D0_23 = _mm512_madd52lo_epu64(D0_23,a5124,a51219);
	D0_23 = _mm512_madd52lo_epu64(D0_23,a5125,a51218);
	D0_23 = _mm512_madd52lo_epu64(D0_23,a5126,a51217);
	D0_23 = _mm512_madd52lo_epu64(D0_23,a5127,a51216);
	D0_23 = _mm512_madd52lo_epu64(D0_23,a5128,a51215);
	D0_23 = _mm512_madd52lo_epu64(D0_23,a5129,a51214);
	D0_23 = _mm512_madd52lo_epu64(D0_23,a51210,a51213);
	D0_23 = _mm512_madd52lo_epu64(D0_23,a51211,a51212);
	D0_23 = _mm512_slli_epi64(D0_23,1);
	D0_23 = _mm512_madd52hi_epu64(D0_23,a51211,a51211);

	D0_24 = _mm512_madd52hi_epu64(zero,a5124,a51219);
	D0_24 = _mm512_madd52hi_epu64(D0_24,a5125,a51218);
	D0_24 = _mm512_madd52hi_epu64(D0_24,a5126,a51217);
	D0_24 = _mm512_madd52hi_epu64(D0_24,a5127,a51216);
	D0_24 = _mm512_madd52hi_epu64(D0_24,a5128,a51215);
	D0_24 = _mm512_madd52hi_epu64(D0_24,a5129,a51214);
	D0_24 = _mm512_madd52hi_epu64(D0_24,a51210,a51213);
	D0_24 = _mm512_madd52hi_epu64(D0_24,a51211,a51212);
	D0_24 = _mm512_madd52lo_epu64(D0_24,a5125,a51219);
	D0_24 = _mm512_madd52lo_epu64(D0_24,a5126,a51218);
	D0_24 = _mm512_madd52lo_epu64(D0_24,a5127,a51217);
	D0_24 = _mm512_madd52lo_epu64(D0_24,a5128,a51216);
	D0_24 = _mm512_madd52lo_epu64(D0_24,a5129,a51215);
	D0_24 = _mm512_madd52lo_epu64(D0_24,a51210,a51214);
	D0_24 = _mm512_madd52lo_epu64(D0_24,a51211,a51213);
	D0_24 = _mm512_slli_epi64(D0_24,1);
	D0_24 = _mm512_madd52lo_epu64(D0_24,a51212,a51212);

	D0_25 = _mm512_madd52hi_epu64(zero,a5125,a51219);
	D0_25 = _mm512_madd52hi_epu64(D0_25,a5126,a51218);
	D0_25 = _mm512_madd52hi_epu64(D0_25,a5127,a51217);
	D0_25 = _mm512_madd52hi_epu64(D0_25,a5128,a51216);
	D0_25 = _mm512_madd52hi_epu64(D0_25,a5129,a51215);
	D0_25 = _mm512_madd52hi_epu64(D0_25,a51210,a51214);
	D0_25 = _mm512_madd52hi_epu64(D0_25,a51211,a51213);
	D0_25 = _mm512_madd52lo_epu64(D0_25,a5126,a51219);
	D0_25 = _mm512_madd52lo_epu64(D0_25,a5127,a51218);
	D0_25 = _mm512_madd52lo_epu64(D0_25,a5128,a51217);
	D0_25 = _mm512_madd52lo_epu64(D0_25,a5129,a51216);
	D0_25 = _mm512_madd52lo_epu64(D0_25,a51210,a51215);
	D0_25 = _mm512_madd52lo_epu64(D0_25,a51211,a51214);
	D0_25 = _mm512_madd52lo_epu64(D0_25,a51212,a51213);
	D0_25 = _mm512_slli_epi64(D0_25,1);
	D0_25 = _mm512_madd52hi_epu64(D0_25,a51212,a51212);

	D0_26 = _mm512_madd52hi_epu64(zero,a5126,a51219);
	D0_26 = _mm512_madd52hi_epu64(D0_26,a5127,a51218);
	D0_26 = _mm512_madd52hi_epu64(D0_26,a5128,a51217);
	D0_26 = _mm512_madd52hi_epu64(D0_26,a5129,a51216);
	D0_26 = _mm512_madd52hi_epu64(D0_26,a51210,a51215);
	D0_26 = _mm512_madd52hi_epu64(D0_26,a51211,a51214);
	D0_26 = _mm512_madd52hi_epu64(D0_26,a51212,a51213);
	D0_26 = _mm512_madd52lo_epu64(D0_26,a5127,a51219);
	D0_26 = _mm512_madd52lo_epu64(D0_26,a5128,a51218);
	D0_26 = _mm512_madd52lo_epu64(D0_26,a5129,a51217);
	D0_26 = _mm512_madd52lo_epu64(D0_26,a51210,a51216);
	D0_26 = _mm512_madd52lo_epu64(D0_26,a51211,a51215);
	D0_26 = _mm512_madd52lo_epu64(D0_26,a51212,a51214);
	D0_26 = _mm512_slli_epi64(D0_26,1);
	D0_26 = _mm512_madd52lo_epu64(D0_26,a51213,a51213);

	D0_27 = _mm512_madd52hi_epu64(zero,a5127,a51219);
	D0_27 = _mm512_madd52hi_epu64(D0_27,a5128,a51218);
	D0_27 = _mm512_madd52hi_epu64(D0_27,a5129,a51217);
	D0_27 = _mm512_madd52hi_epu64(D0_27,a51210,a51216);
	D0_27 = _mm512_madd52hi_epu64(D0_27,a51211,a51215);
	D0_27 = _mm512_madd52hi_epu64(D0_27,a51212,a51214);
	D0_27 = _mm512_madd52lo_epu64(D0_27,a5128,a51219);
	D0_27 = _mm512_madd52lo_epu64(D0_27,a5129,a51218);
	D0_27 = _mm512_madd52lo_epu64(D0_27,a51210,a51217);
	D0_27 = _mm512_madd52lo_epu64(D0_27,a51211,a51216);
	D0_27 = _mm512_madd52lo_epu64(D0_27,a51212,a51215);
	D0_27 = _mm512_madd52lo_epu64(D0_27,a51213,a51214);
	D0_27 = _mm512_slli_epi64(D0_27,1);
	D0_27 = _mm512_madd52hi_epu64(D0_27,a51213,a51213);

	D0_28 = _mm512_madd52hi_epu64(zero,a5128,a51219);
	D0_28 = _mm512_madd52hi_epu64(D0_28,a5129,a51218);
	D0_28 = _mm512_madd52hi_epu64(D0_28,a51210,a51217);
	D0_28 = _mm512_madd52hi_epu64(D0_28,a51211,a51216);
	D0_28 = _mm512_madd52hi_epu64(D0_28,a51212,a51215);
	D0_28 = _mm512_madd52hi_epu64(D0_28,a51213,a51214);
	D0_28 = _mm512_madd52lo_epu64(D0_28,a5129,a51219);
	D0_28 = _mm512_madd52lo_epu64(D0_28,a51210,a51218);
	D0_28 = _mm512_madd52lo_epu64(D0_28,a51211,a51217);
	D0_28 = _mm512_madd52lo_epu64(D0_28,a51212,a51216);
	D0_28 = _mm512_madd52lo_epu64(D0_28,a51213,a51215);
	D0_28 = _mm512_slli_epi64(D0_28,1);
	D0_28 = _mm512_madd52lo_epu64(D0_28,a51214,a51214);

	D0_29 = _mm512_madd52hi_epu64(zero,a5129,a51219);
	D0_29 = _mm512_madd52hi_epu64(D0_29,a51210,a51218);
	D0_29 = _mm512_madd52hi_epu64(D0_29,a51211,a51217);
	D0_29 = _mm512_madd52hi_epu64(D0_29,a51212,a51216);
	D0_29 = _mm512_madd52hi_epu64(D0_29,a51213,a51215);
	D0_29 = _mm512_madd52lo_epu64(D0_29,a51210,a51219);
	D0_29 = _mm512_madd52lo_epu64(D0_29,a51211,a51218);
	D0_29 = _mm512_madd52lo_epu64(D0_29,a51212,a51217);
	D0_29 = _mm512_madd52lo_epu64(D0_29,a51213,a51216);
	D0_29 = _mm512_madd52lo_epu64(D0_29,a51214,a51215);
	D0_29 = _mm512_slli_epi64(D0_29,1);
	D0_29 = _mm512_madd52hi_epu64(D0_29,a51214,a51214);

	D0_30 = _mm512_madd52hi_epu64(zero,a51210,a51219);
	D0_30 = _mm512_madd52hi_epu64(D0_30,a51211,a51218);
	D0_30 = _mm512_madd52hi_epu64(D0_30,a51212,a51217);
	D0_30 = _mm512_madd52hi_epu64(D0_30,a51213,a51216);
	D0_30 = _mm512_madd52hi_epu64(D0_30,a51214,a51215);
	D0_30 = _mm512_madd52lo_epu64(D0_30,a51211,a51219);
	D0_30 = _mm512_madd52lo_epu64(D0_30,a51212,a51218);
	D0_30 = _mm512_madd52lo_epu64(D0_30,a51213,a51217);
	D0_30 = _mm512_madd52lo_epu64(D0_30,a51214,a51216);
	D0_30 = _mm512_slli_epi64(D0_30,1);
	D0_30 = _mm512_madd52lo_epu64(D0_30,a51215,a51215);

	D0_31 = _mm512_madd52hi_epu64(zero,a51211,a51219);
	D0_31 = _mm512_madd52hi_epu64(D0_31,a51212,a51218);
	D0_31 = _mm512_madd52hi_epu64(D0_31,a51213,a51217);
	D0_31 = _mm512_madd52hi_epu64(D0_31,a51214,a51216);
	D0_31 = _mm512_madd52lo_epu64(D0_31,a51212,a51219);
	D0_31 = _mm512_madd52lo_epu64(D0_31,a51213,a51218);
	D0_31 = _mm512_madd52lo_epu64(D0_31,a51214,a51217);
	D0_31 = _mm512_madd52lo_epu64(D0_31,a51215,a51216);
	D0_31 = _mm512_slli_epi64(D0_31,1);
	D0_31 = _mm512_madd52hi_epu64(D0_31,a51215,a51215);

	D0_32 = _mm512_madd52hi_epu64(zero,a51212,a51219);
	D0_32 = _mm512_madd52hi_epu64(D0_32,a51213,a51218);
	D0_32 = _mm512_madd52hi_epu64(D0_32,a51214,a51217);
	D0_32 = _mm512_madd52hi_epu64(D0_32,a51215,a51216);
	D0_32 = _mm512_madd52lo_epu64(D0_32,a51213,a51219);
	D0_32 = _mm512_madd52lo_epu64(D0_32,a51214,a51218);
	D0_32 = _mm512_madd52lo_epu64(D0_32,a51215,a51217);
	D0_32 = _mm512_slli_epi64(D0_32,1);
	D0_32 = _mm512_madd52lo_epu64(D0_32,a51216,a51216);

	D0_33 = _mm512_madd52hi_epu64(zero,a51213,a51219);
	D0_33 = _mm512_madd52hi_epu64(D0_33,a51214,a51218);
	D0_33 = _mm512_madd52hi_epu64(D0_33,a51215,a51217);
	D0_33 = _mm512_madd52lo_epu64(D0_33,a51214,a51219);
	D0_33 = _mm512_madd52lo_epu64(D0_33,a51215,a51218);
	D0_33 = _mm512_madd52lo_epu64(D0_33,a51216,a51217);
	D0_33 = _mm512_slli_epi64(D0_33,1);
	D0_33 = _mm512_madd52hi_epu64(D0_33,a51216,a51216);

	D0_34 = _mm512_madd52hi_epu64(zero,a51214,a51219);
	D0_34 = _mm512_madd52hi_epu64(D0_34,a51215,a51218);
	D0_34 = _mm512_madd52hi_epu64(D0_34,a51216,a51217);
	D0_34 = _mm512_madd52lo_epu64(D0_34,a51215,a51219);
	D0_34 = _mm512_madd52lo_epu64(D0_34,a51216,a51218);
	D0_34 = _mm512_slli_epi64(D0_34,1);
	D0_34 = _mm512_madd52lo_epu64(D0_34,a51217,a51217);

	D0_35 = _mm512_madd52hi_epu64(zero,a51215,a51219);
	D0_35 = _mm512_madd52hi_epu64(D0_35,a51216,a51218);
	D0_35 = _mm512_madd52lo_epu64(D0_35,a51216,a51219);
	D0_35 = _mm512_madd52lo_epu64(D0_35,a51217,a51218);
	D0_35 = _mm512_slli_epi64(D0_35,1);
	D0_35 = _mm512_madd52hi_epu64(D0_35,a51217,a51217);

	D0_36 = _mm512_madd52hi_epu64(zero,a51216,a51219);
	D0_36 = _mm512_madd52hi_epu64(D0_36,a51217,a51218);
	D0_36 = _mm512_madd52lo_epu64(D0_36,a51217,a51219);
	D0_36 = _mm512_slli_epi64(D0_36,1);
	D0_36 = _mm512_madd52lo_epu64(D0_36,a51218,a51218);

	D0_37 = _mm512_madd52hi_epu64(zero,a51217,a51219);
	D0_37 = _mm512_madd52lo_epu64(D0_37,a51218,a51219);
	D0_37 = _mm512_slli_epi64(D0_37,1);
	D0_37 = _mm512_madd52hi_epu64(D0_37,a51218,a51218);

	D0_38 = _mm512_madd52hi_epu64(zero,a51218,a51219);
	D0_38 = _mm512_slli_epi64(D0_38,1);
	D0_38 = _mm512_madd52lo_epu64(D0_38,a51219,a51219);

	D0_39 = _mm512_madd52hi_epu64(zero,a51219,a51219);


	// Carry management

	__m512i carry  = _mm512_srli_epi64(D0_1,52);
	D0_2 = _mm512_add_epi64(D0_2,carry);
	D0_1 = _mm512_and_si512(D0_1,mask52);
	carry  = _mm512_srli_epi64(D0_2,52);
	D0_3 = _mm512_add_epi64(D0_3,carry);
	D0_2 = _mm512_and_si512(D0_2,mask52);
	carry  = _mm512_srli_epi64(D0_3,52);
	D0_4 = _mm512_add_epi64(D0_4,carry);
	D0_3 = _mm512_and_si512(D0_3,mask52);
	carry  = _mm512_srli_epi64(D0_4,52);
	D0_5 = _mm512_add_epi64(D0_5,carry);
	D0_4 = _mm512_and_si512(D0_4,mask52);
	carry  = _mm512_srli_epi64(D0_5,52);
	D0_6 = _mm512_add_epi64(D0_6,carry);
	D0_5 = _mm512_and_si512(D0_5,mask52);
	carry  = _mm512_srli_epi64(D0_6,52);
	D0_7 = _mm512_add_epi64(D0_7,carry);
	D0_6 = _mm512_and_si512(D0_6,mask52);
	carry  = _mm512_srli_epi64(D0_7,52);
	D0_8 = _mm512_add_epi64(D0_8,carry);
	D0_7 = _mm512_and_si512(D0_7,mask52);
	carry  = _mm512_srli_epi64(D0_8,52);
	D0_9 = _mm512_add_epi64(D0_9,carry);
	D0_8 = _mm512_and_si512(D0_8,mask52);
	carry  = _mm512_srli_epi64(D0_9,52);
	D0_10 = _mm512_add_epi64(D0_10,carry);
	D0_9 = _mm512_and_si512(D0_9,mask52);
	carry  = _mm512_srli_epi64(D0_10,52);
	D0_11 = _mm512_add_epi64(D0_11,carry);
	D0_10 = _mm512_and_si512(D0_10,mask52);
	carry  = _mm512_srli_epi64(D0_11,52);
	D0_12 = _mm512_add_epi64(D0_12,carry);
	D0_11 = _mm512_and_si512(D0_11,mask52);
	carry  = _mm512_srli_epi64(D0_12,52);
	D0_13 = _mm512_add_epi64(D0_13,carry);
	D0_12 = _mm512_and_si512(D0_12,mask52);
	carry  = _mm512_srli_epi64(D0_13,52);
	D0_14 = _mm512_add_epi64(D0_14,carry);
	D0_13 = _mm512_and_si512(D0_13,mask52);
	carry  = _mm512_srli_epi64(D0_14,52);
	D0_15 = _mm512_add_epi64(D0_15,carry);
	D0_14 = _mm512_and_si512(D0_14,mask52);
	carry  = _mm512_srli_epi64(D0_15,52);
	D0_16 = _mm512_add_epi64(D0_16,carry);
	D0_15 = _mm512_and_si512(D0_15,mask52);
	carry  = _mm512_srli_epi64(D0_16,52);
	D0_17 = _mm512_add_epi64(D0_17,carry);
	D0_16 = _mm512_and_si512(D0_16,mask52);
	carry  = _mm512_srli_epi64(D0_17,52);
	D0_18 = _mm512_add_epi64(D0_18,carry);
	D0_17 = _mm512_and_si512(D0_17,mask52);
	carry  = _mm512_srli_epi64(D0_18,52);
	D0_19 = _mm512_add_epi64(D0_19,carry);
	D0_18 = _mm512_and_si512(D0_18,mask52);

	// Conversion in base 2^1039

	carry = D0_19>>51;
	D0_20 = (D0_20<<1)+carry;
	D0_19 &= mask51;
	carry = D0_20>>52;
	D0_21 = (D0_21<<1)+carry;
	D0_20 &= mask52;
	carry = D0_21>>52;
	D0_22 = (D0_22<<1)+carry;
	D0_21 &= mask52;
	carry = D0_22>>52;
	D0_23 = (D0_23<<1)+carry;
	D0_22 &= mask52;
	carry = D0_23>>52;
	D0_24 = (D0_24<<1)+carry;
	D0_23 &= mask52;
	carry = D0_24>>52;
	D0_25 = (D0_25<<1)+carry;
	D0_24 &= mask52;
	carry = D0_25>>52;
	D0_26 = (D0_26<<1)+carry;
	D0_25 &= mask52;
	carry = D0_26>>52;
	D0_27 = (D0_27<<1)+carry;
	D0_26 &= mask52;
	carry = D0_27>>52;
	D0_28 = (D0_28<<1)+carry;
	D0_27 &= mask52;
	carry = D0_28>>52;
	D0_29 = (D0_29<<1)+carry;
	D0_28 &= mask52;
	carry = D0_29>>52;
	D0_30 = (D0_30<<1)+carry;
	D0_29 &= mask52;
	carry = D0_30>>52;
	D0_31 = (D0_31<<1)+carry;
	D0_30 &= mask52;
	carry = D0_31>>52;
	D0_32 = (D0_32<<1)+carry;
	D0_31 &= mask52;
	carry = D0_32>>52;
	D0_33 = (D0_33<<1)+carry;
	D0_32 &= mask52;
	carry = D0_33>>52;
	D0_34 = (D0_34<<1)+carry;
	D0_33 &= mask52;
	carry = D0_34>>52;
	D0_35 = (D0_35<<1)+carry;
	D0_34 &= mask52;
	carry = D0_35>>52;
	D0_36 = (D0_36<<1)+carry;
	D0_35 &= mask52;
	carry = D0_36>>52;
	D0_37 = (D0_37<<1)+carry;
	D0_36 &= mask52;
	carry = D0_37>>52;
	D0_38 = (D0_38<<1)+carry;
	D0_37 &= mask52;
	carry = D0_38>>52;
	D0_39 = (D0_39<<1)+carry;
	D0_38 &= mask52;

	// D2 = Ah^2

	__m512i D2_0, D2_1;
	__m512i D2_2, D2_3;
	__m512i D2_4, D2_5;
	__m512i D2_6, D2_7;
	__m512i D2_8, D2_9;
	__m512i D2_10, D2_11;
	__m512i D2_12, D2_13;
	__m512i D2_14, D2_15;
	__m512i D2_16, D2_17;
	__m512i D2_18, D2_19;
	__m512i D2_20, D2_21;
	__m512i D2_22, D2_23;
	__m512i D2_24, D2_25;
	__m512i D2_26, D2_27;
	__m512i D2_28, D2_29;
	__m512i D2_30, D2_31;
	__m512i D2_32, D2_33;
	__m512i D2_34, D2_35;
	__m512i D2_36, D2_37;
	__m512i D2_38, D2_39;
	__m512i a51220 = _mm512_load_epi64(a512+20);
	__m512i a51221 = _mm512_load_epi64(a512+21);
	__m512i a51222 = _mm512_load_epi64(a512+22);
	__m512i a51223 = _mm512_load_epi64(a512+23);
	__m512i a51224 = _mm512_load_epi64(a512+24);
	__m512i a51225 = _mm512_load_epi64(a512+25);
	__m512i a51226 = _mm512_load_epi64(a512+26);
	__m512i a51227 = _mm512_load_epi64(a512+27);
	__m512i a51228 = _mm512_load_epi64(a512+28);
	__m512i a51229 = _mm512_load_epi64(a512+29);
	__m512i a51230 = _mm512_load_epi64(a512+30);
	__m512i a51231 = _mm512_load_epi64(a512+31);
	__m512i a51232 = _mm512_load_epi64(a512+32);
	__m512i a51233 = _mm512_load_epi64(a512+33);
	__m512i a51234 = _mm512_load_epi64(a512+34);
	__m512i a51235 = _mm512_load_epi64(a512+35);
	__m512i a51236 = _mm512_load_epi64(a512+36);
	__m512i a51237 = _mm512_load_epi64(a512+37);
	__m512i a51238 = _mm512_load_epi64(a512+38);
	__m512i a51239 = _mm512_load_epi64(a512+39);

	D2_0 = _mm512_madd52lo_epu64(zero,a51220,a51220);

	D2_1 = _mm512_madd52lo_epu64(zero,a51220,a51221);
	D2_1 = _mm512_slli_epi64(D2_1,1);
	D2_1 = _mm512_madd52hi_epu64(D2_1,a51220,a51220);

	D2_2 = _mm512_madd52hi_epu64(zero,a51220,a51221);
	D2_2 = _mm512_madd52lo_epu64(D2_2,a51220,a51222);
	D2_2 = _mm512_slli_epi64(D2_2,1);
	D2_2 = _mm512_madd52lo_epu64(D2_2,a51221,a51221);

	D2_3 = _mm512_madd52hi_epu64(zero,a51220,a51222);
	D2_3 = _mm512_madd52lo_epu64(D2_3,a51220,a51223);
	D2_3 = _mm512_madd52lo_epu64(D2_3,a51221,a51222);
	D2_3 = _mm512_slli_epi64(D2_3,1);
	D2_3 = _mm512_madd52hi_epu64(D2_3,a51221,a51221);

	D2_4 = _mm512_madd52hi_epu64(zero,a51220,a51223);
	D2_4 = _mm512_madd52hi_epu64(D2_4,a51221,a51222);
	D2_4 = _mm512_madd52lo_epu64(D2_4,a51220,a51224);
	D2_4 = _mm512_madd52lo_epu64(D2_4,a51221,a51223);
	D2_4 = _mm512_slli_epi64(D2_4,1);
	D2_4 = _mm512_madd52lo_epu64(D2_4,a51222,a51222);

	D2_5 = _mm512_madd52hi_epu64(zero,a51220,a51224);
	D2_5 = _mm512_madd52hi_epu64(D2_5,a51221,a51223);
	D2_5 = _mm512_madd52lo_epu64(D2_5,a51220,a51225);
	D2_5 = _mm512_madd52lo_epu64(D2_5,a51221,a51224);
	D2_5 = _mm512_madd52lo_epu64(D2_5,a51222,a51223);
	D2_5 = _mm512_slli_epi64(D2_5,1);
	D2_5 = _mm512_madd52hi_epu64(D2_5,a51222,a51222);

	D2_6 = _mm512_madd52hi_epu64(zero,a51220,a51225);
	D2_6 = _mm512_madd52hi_epu64(D2_6,a51221,a51224);
	D2_6 = _mm512_madd52hi_epu64(D2_6,a51222,a51223);
	D2_6 = _mm512_madd52lo_epu64(D2_6,a51220,a51226);
	D2_6 = _mm512_madd52lo_epu64(D2_6,a51221,a51225);
	D2_6 = _mm512_madd52lo_epu64(D2_6,a51222,a51224);
	D2_6 = _mm512_slli_epi64(D2_6,1);
	D2_6 = _mm512_madd52lo_epu64(D2_6,a51223,a51223);

	D2_7 = _mm512_madd52hi_epu64(zero,a51220,a51226);
	D2_7 = _mm512_madd52hi_epu64(D2_7,a51221,a51225);
	D2_7 = _mm512_madd52hi_epu64(D2_7,a51222,a51224);
	D2_7 = _mm512_madd52lo_epu64(D2_7,a51220,a51227);
	D2_7 = _mm512_madd52lo_epu64(D2_7,a51221,a51226);
	D2_7 = _mm512_madd52lo_epu64(D2_7,a51222,a51225);
	D2_7 = _mm512_madd52lo_epu64(D2_7,a51223,a51224);
	D2_7 = _mm512_slli_epi64(D2_7,1);
	D2_7 = _mm512_madd52hi_epu64(D2_7,a51223,a51223);

	D2_8 = _mm512_madd52hi_epu64(zero,a51220,a51227);
	D2_8 = _mm512_madd52hi_epu64(D2_8,a51221,a51226);
	D2_8 = _mm512_madd52hi_epu64(D2_8,a51222,a51225);
	D2_8 = _mm512_madd52hi_epu64(D2_8,a51223,a51224);
	D2_8 = _mm512_madd52lo_epu64(D2_8,a51220,a51228);
	D2_8 = _mm512_madd52lo_epu64(D2_8,a51221,a51227);
	D2_8 = _mm512_madd52lo_epu64(D2_8,a51222,a51226);
	D2_8 = _mm512_madd52lo_epu64(D2_8,a51223,a51225);
	D2_8 = _mm512_slli_epi64(D2_8,1);
	D2_8 = _mm512_madd52lo_epu64(D2_8,a51224,a51224);

	D2_9 = _mm512_madd52hi_epu64(zero,a51220,a51228);
	D2_9 = _mm512_madd52hi_epu64(D2_9,a51221,a51227);
	D2_9 = _mm512_madd52hi_epu64(D2_9,a51222,a51226);
	D2_9 = _mm512_madd52hi_epu64(D2_9,a51223,a51225);
	D2_9 = _mm512_madd52lo_epu64(D2_9,a51220,a51229);
	D2_9 = _mm512_madd52lo_epu64(D2_9,a51221,a51228);
	D2_9 = _mm512_madd52lo_epu64(D2_9,a51222,a51227);
	D2_9 = _mm512_madd52lo_epu64(D2_9,a51223,a51226);
	D2_9 = _mm512_madd52lo_epu64(D2_9,a51224,a51225);
	D2_9 = _mm512_slli_epi64(D2_9,1);
	D2_9 = _mm512_madd52hi_epu64(D2_9,a51224,a51224);

	D2_10 = _mm512_madd52hi_epu64(zero,a51220,a51229);
	D2_10 = _mm512_madd52hi_epu64(D2_10,a51221,a51228);
	D2_10 = _mm512_madd52hi_epu64(D2_10,a51222,a51227);
	D2_10 = _mm512_madd52hi_epu64(D2_10,a51223,a51226);
	D2_10 = _mm512_madd52hi_epu64(D2_10,a51224,a51225);
	D2_10 = _mm512_madd52lo_epu64(D2_10,a51220,a51230);
	D2_10 = _mm512_madd52lo_epu64(D2_10,a51221,a51229);
	D2_10 = _mm512_madd52lo_epu64(D2_10,a51222,a51228);
	D2_10 = _mm512_madd52lo_epu64(D2_10,a51223,a51227);
	D2_10 = _mm512_madd52lo_epu64(D2_10,a51224,a51226);
	D2_10 = _mm512_slli_epi64(D2_10,1);
	D2_10 = _mm512_madd52lo_epu64(D2_10,a51225,a51225);

	D2_11 = _mm512_madd52hi_epu64(zero,a51220,a51230);
	D2_11 = _mm512_madd52hi_epu64(D2_11,a51221,a51229);
	D2_11 = _mm512_madd52hi_epu64(D2_11,a51222,a51228);
	D2_11 = _mm512_madd52hi_epu64(D2_11,a51223,a51227);
	D2_11 = _mm512_madd52hi_epu64(D2_11,a51224,a51226);
	D2_11 = _mm512_madd52lo_epu64(D2_11,a51220,a51231);
	D2_11 = _mm512_madd52lo_epu64(D2_11,a51221,a51230);
	D2_11 = _mm512_madd52lo_epu64(D2_11,a51222,a51229);
	D2_11 = _mm512_madd52lo_epu64(D2_11,a51223,a51228);
	D2_11 = _mm512_madd52lo_epu64(D2_11,a51224,a51227);
	D2_11 = _mm512_madd52lo_epu64(D2_11,a51225,a51226);
	D2_11 = _mm512_slli_epi64(D2_11,1);
	D2_11 = _mm512_madd52hi_epu64(D2_11,a51225,a51225);

	D2_12 = _mm512_madd52hi_epu64(zero,a51220,a51231);
	D2_12 = _mm512_madd52hi_epu64(D2_12,a51221,a51230);
	D2_12 = _mm512_madd52hi_epu64(D2_12,a51222,a51229);
	D2_12 = _mm512_madd52hi_epu64(D2_12,a51223,a51228);
	D2_12 = _mm512_madd52hi_epu64(D2_12,a51224,a51227);
	D2_12 = _mm512_madd52hi_epu64(D2_12,a51225,a51226);
	D2_12 = _mm512_madd52lo_epu64(D2_12,a51220,a51232);
	D2_12 = _mm512_madd52lo_epu64(D2_12,a51221,a51231);
	D2_12 = _mm512_madd52lo_epu64(D2_12,a51222,a51230);
	D2_12 = _mm512_madd52lo_epu64(D2_12,a51223,a51229);
	D2_12 = _mm512_madd52lo_epu64(D2_12,a51224,a51228);
	D2_12 = _mm512_madd52lo_epu64(D2_12,a51225,a51227);
	D2_12 = _mm512_slli_epi64(D2_12,1);
	D2_12 = _mm512_madd52lo_epu64(D2_12,a51226,a51226);

	D2_13 = _mm512_madd52hi_epu64(zero,a51220,a51232);
	D2_13 = _mm512_madd52hi_epu64(D2_13,a51221,a51231);
	D2_13 = _mm512_madd52hi_epu64(D2_13,a51222,a51230);
	D2_13 = _mm512_madd52hi_epu64(D2_13,a51223,a51229);
	D2_13 = _mm512_madd52hi_epu64(D2_13,a51224,a51228);
	D2_13 = _mm512_madd52hi_epu64(D2_13,a51225,a51227);
	D2_13 = _mm512_madd52lo_epu64(D2_13,a51220,a51233);
	D2_13 = _mm512_madd52lo_epu64(D2_13,a51221,a51232);
	D2_13 = _mm512_madd52lo_epu64(D2_13,a51222,a51231);
	D2_13 = _mm512_madd52lo_epu64(D2_13,a51223,a51230);
	D2_13 = _mm512_madd52lo_epu64(D2_13,a51224,a51229);
	D2_13 = _mm512_madd52lo_epu64(D2_13,a51225,a51228);
	D2_13 = _mm512_madd52lo_epu64(D2_13,a51226,a51227);
	D2_13 = _mm512_slli_epi64(D2_13,1);
	D2_13 = _mm512_madd52hi_epu64(D2_13,a51226,a51226);

	D2_14 = _mm512_madd52hi_epu64(zero,a51220,a51233);
	D2_14 = _mm512_madd52hi_epu64(D2_14,a51221,a51232);
	D2_14 = _mm512_madd52hi_epu64(D2_14,a51222,a51231);
	D2_14 = _mm512_madd52hi_epu64(D2_14,a51223,a51230);
	D2_14 = _mm512_madd52hi_epu64(D2_14,a51224,a51229);
	D2_14 = _mm512_madd52hi_epu64(D2_14,a51225,a51228);
	D2_14 = _mm512_madd52hi_epu64(D2_14,a51226,a51227);
	D2_14 = _mm512_madd52lo_epu64(D2_14,a51220,a51234);
	D2_14 = _mm512_madd52lo_epu64(D2_14,a51221,a51233);
	D2_14 = _mm512_madd52lo_epu64(D2_14,a51222,a51232);
	D2_14 = _mm512_madd52lo_epu64(D2_14,a51223,a51231);
	D2_14 = _mm512_madd52lo_epu64(D2_14,a51224,a51230);
	D2_14 = _mm512_madd52lo_epu64(D2_14,a51225,a51229);
	D2_14 = _mm512_madd52lo_epu64(D2_14,a51226,a51228);
	D2_14 = _mm512_slli_epi64(D2_14,1);
	D2_14 = _mm512_madd52lo_epu64(D2_14,a51227,a51227);

	D2_15 = _mm512_madd52hi_epu64(zero,a51220,a51234);
	D2_15 = _mm512_madd52hi_epu64(D2_15,a51221,a51233);
	D2_15 = _mm512_madd52hi_epu64(D2_15,a51222,a51232);
	D2_15 = _mm512_madd52hi_epu64(D2_15,a51223,a51231);
	D2_15 = _mm512_madd52hi_epu64(D2_15,a51224,a51230);
	D2_15 = _mm512_madd52hi_epu64(D2_15,a51225,a51229);
	D2_15 = _mm512_madd52hi_epu64(D2_15,a51226,a51228);
	D2_15 = _mm512_madd52lo_epu64(D2_15,a51220,a51235);
	D2_15 = _mm512_madd52lo_epu64(D2_15,a51221,a51234);
	D2_15 = _mm512_madd52lo_epu64(D2_15,a51222,a51233);
	D2_15 = _mm512_madd52lo_epu64(D2_15,a51223,a51232);
	D2_15 = _mm512_madd52lo_epu64(D2_15,a51224,a51231);
	D2_15 = _mm512_madd52lo_epu64(D2_15,a51225,a51230);
	D2_15 = _mm512_madd52lo_epu64(D2_15,a51226,a51229);
	D2_15 = _mm512_madd52lo_epu64(D2_15,a51227,a51228);
	D2_15 = _mm512_slli_epi64(D2_15,1);
	D2_15 = _mm512_madd52hi_epu64(D2_15,a51227,a51227);

	D2_16 = _mm512_madd52hi_epu64(zero,a51220,a51235);
	D2_16 = _mm512_madd52hi_epu64(D2_16,a51221,a51234);
	D2_16 = _mm512_madd52hi_epu64(D2_16,a51222,a51233);
	D2_16 = _mm512_madd52hi_epu64(D2_16,a51223,a51232);
	D2_16 = _mm512_madd52hi_epu64(D2_16,a51224,a51231);
	D2_16 = _mm512_madd52hi_epu64(D2_16,a51225,a51230);
	D2_16 = _mm512_madd52hi_epu64(D2_16,a51226,a51229);
	D2_16 = _mm512_madd52hi_epu64(D2_16,a51227,a51228);
	D2_16 = _mm512_madd52lo_epu64(D2_16,a51220,a51236);
	D2_16 = _mm512_madd52lo_epu64(D2_16,a51221,a51235);
	D2_16 = _mm512_madd52lo_epu64(D2_16,a51222,a51234);
	D2_16 = _mm512_madd52lo_epu64(D2_16,a51223,a51233);
	D2_16 = _mm512_madd52lo_epu64(D2_16,a51224,a51232);
	D2_16 = _mm512_madd52lo_epu64(D2_16,a51225,a51231);
	D2_16 = _mm512_madd52lo_epu64(D2_16,a51226,a51230);
	D2_16 = _mm512_madd52lo_epu64(D2_16,a51227,a51229);
	D2_16 = _mm512_slli_epi64(D2_16,1);
	D2_16 = _mm512_madd52lo_epu64(D2_16,a51228,a51228);

	D2_17 = _mm512_madd52hi_epu64(zero,a51220,a51236);
	D2_17 = _mm512_madd52hi_epu64(D2_17,a51221,a51235);
	D2_17 = _mm512_madd52hi_epu64(D2_17,a51222,a51234);
	D2_17 = _mm512_madd52hi_epu64(D2_17,a51223,a51233);
	D2_17 = _mm512_madd52hi_epu64(D2_17,a51224,a51232);
	D2_17 = _mm512_madd52hi_epu64(D2_17,a51225,a51231);
	D2_17 = _mm512_madd52hi_epu64(D2_17,a51226,a51230);
	D2_17 = _mm512_madd52hi_epu64(D2_17,a51227,a51229);
	D2_17 = _mm512_madd52lo_epu64(D2_17,a51220,a51237);
	D2_17 = _mm512_madd52lo_epu64(D2_17,a51221,a51236);
	D2_17 = _mm512_madd52lo_epu64(D2_17,a51222,a51235);
	D2_17 = _mm512_madd52lo_epu64(D2_17,a51223,a51234);
	D2_17 = _mm512_madd52lo_epu64(D2_17,a51224,a51233);
	D2_17 = _mm512_madd52lo_epu64(D2_17,a51225,a51232);
	D2_17 = _mm512_madd52lo_epu64(D2_17,a51226,a51231);
	D2_17 = _mm512_madd52lo_epu64(D2_17,a51227,a51230);
	D2_17 = _mm512_madd52lo_epu64(D2_17,a51228,a51229);
	D2_17 = _mm512_slli_epi64(D2_17,1);
	D2_17 = _mm512_madd52hi_epu64(D2_17,a51228,a51228);

	D2_18 = _mm512_madd52hi_epu64(zero,a51220,a51237);
	D2_18 = _mm512_madd52hi_epu64(D2_18,a51221,a51236);
	D2_18 = _mm512_madd52hi_epu64(D2_18,a51222,a51235);
	D2_18 = _mm512_madd52hi_epu64(D2_18,a51223,a51234);
	D2_18 = _mm512_madd52hi_epu64(D2_18,a51224,a51233);
	D2_18 = _mm512_madd52hi_epu64(D2_18,a51225,a51232);
	D2_18 = _mm512_madd52hi_epu64(D2_18,a51226,a51231);
	D2_18 = _mm512_madd52hi_epu64(D2_18,a51227,a51230);
	D2_18 = _mm512_madd52hi_epu64(D2_18,a51228,a51229);
	D2_18 = _mm512_madd52lo_epu64(D2_18,a51220,a51238);
	D2_18 = _mm512_madd52lo_epu64(D2_18,a51221,a51237);
	D2_18 = _mm512_madd52lo_epu64(D2_18,a51222,a51236);
	D2_18 = _mm512_madd52lo_epu64(D2_18,a51223,a51235);
	D2_18 = _mm512_madd52lo_epu64(D2_18,a51224,a51234);
	D2_18 = _mm512_madd52lo_epu64(D2_18,a51225,a51233);
	D2_18 = _mm512_madd52lo_epu64(D2_18,a51226,a51232);
	D2_18 = _mm512_madd52lo_epu64(D2_18,a51227,a51231);
	D2_18 = _mm512_madd52lo_epu64(D2_18,a51228,a51230);
	D2_18 = _mm512_slli_epi64(D2_18,1);
	D2_18 = _mm512_madd52lo_epu64(D2_18,a51229,a51229);

	D2_19 = _mm512_madd52hi_epu64(zero,a51220,a51238);
	D2_19 = _mm512_madd52hi_epu64(D2_19,a51221,a51237);
	D2_19 = _mm512_madd52hi_epu64(D2_19,a51222,a51236);
	D2_19 = _mm512_madd52hi_epu64(D2_19,a51223,a51235);
	D2_19 = _mm512_madd52hi_epu64(D2_19,a51224,a51234);
	D2_19 = _mm512_madd52hi_epu64(D2_19,a51225,a51233);
	D2_19 = _mm512_madd52hi_epu64(D2_19,a51226,a51232);
	D2_19 = _mm512_madd52hi_epu64(D2_19,a51227,a51231);
	D2_19 = _mm512_madd52hi_epu64(D2_19,a51228,a51230);
	D2_19 = _mm512_madd52lo_epu64(D2_19,a51220,a51239);
	D2_19 = _mm512_madd52lo_epu64(D2_19,a51221,a51238);
	D2_19 = _mm512_madd52lo_epu64(D2_19,a51222,a51237);
	D2_19 = _mm512_madd52lo_epu64(D2_19,a51223,a51236);
	D2_19 = _mm512_madd52lo_epu64(D2_19,a51224,a51235);
	D2_19 = _mm512_madd52lo_epu64(D2_19,a51225,a51234);
	D2_19 = _mm512_madd52lo_epu64(D2_19,a51226,a51233);
	D2_19 = _mm512_madd52lo_epu64(D2_19,a51227,a51232);
	D2_19 = _mm512_madd52lo_epu64(D2_19,a51228,a51231);
	D2_19 = _mm512_madd52lo_epu64(D2_19,a51229,a51230);
	D2_19 = _mm512_slli_epi64(D2_19,1);
	D2_19 = _mm512_madd52hi_epu64(D2_19,a51229,a51229);

	D2_20 = _mm512_madd52hi_epu64(zero,a51220,a51239);
	D2_20 = _mm512_madd52hi_epu64(D2_20,a51221,a51238);
	D2_20 = _mm512_madd52hi_epu64(D2_20,a51222,a51237);
	D2_20 = _mm512_madd52hi_epu64(D2_20,a51223,a51236);
	D2_20 = _mm512_madd52hi_epu64(D2_20,a51224,a51235);
	D2_20 = _mm512_madd52hi_epu64(D2_20,a51225,a51234);
	D2_20 = _mm512_madd52hi_epu64(D2_20,a51226,a51233);
	D2_20 = _mm512_madd52hi_epu64(D2_20,a51227,a51232);
	D2_20 = _mm512_madd52hi_epu64(D2_20,a51228,a51231);
	D2_20 = _mm512_madd52hi_epu64(D2_20,a51229,a51230);
	D2_20 = _mm512_madd52lo_epu64(D2_20,a51221,a51239);
	D2_20 = _mm512_madd52lo_epu64(D2_20,a51222,a51238);
	D2_20 = _mm512_madd52lo_epu64(D2_20,a51223,a51237);
	D2_20 = _mm512_madd52lo_epu64(D2_20,a51224,a51236);
	D2_20 = _mm512_madd52lo_epu64(D2_20,a51225,a51235);
	D2_20 = _mm512_madd52lo_epu64(D2_20,a51226,a51234);
	D2_20 = _mm512_madd52lo_epu64(D2_20,a51227,a51233);
	D2_20 = _mm512_madd52lo_epu64(D2_20,a51228,a51232);
	D2_20 = _mm512_madd52lo_epu64(D2_20,a51229,a51231);
	D2_20 = _mm512_slli_epi64(D2_20,1);
	D2_20 = _mm512_madd52lo_epu64(D2_20,a51230,a51230);

	D2_21 = _mm512_madd52hi_epu64(zero,a51221,a51239);
	D2_21 = _mm512_madd52hi_epu64(D2_21,a51222,a51238);
	D2_21 = _mm512_madd52hi_epu64(D2_21,a51223,a51237);
	D2_21 = _mm512_madd52hi_epu64(D2_21,a51224,a51236);
	D2_21 = _mm512_madd52hi_epu64(D2_21,a51225,a51235);
	D2_21 = _mm512_madd52hi_epu64(D2_21,a51226,a51234);
	D2_21 = _mm512_madd52hi_epu64(D2_21,a51227,a51233);
	D2_21 = _mm512_madd52hi_epu64(D2_21,a51228,a51232);
	D2_21 = _mm512_madd52hi_epu64(D2_21,a51229,a51231);
	D2_21 = _mm512_madd52lo_epu64(D2_21,a51222,a51239);
	D2_21 = _mm512_madd52lo_epu64(D2_21,a51223,a51238);
	D2_21 = _mm512_madd52lo_epu64(D2_21,a51224,a51237);
	D2_21 = _mm512_madd52lo_epu64(D2_21,a51225,a51236);
	D2_21 = _mm512_madd52lo_epu64(D2_21,a51226,a51235);
	D2_21 = _mm512_madd52lo_epu64(D2_21,a51227,a51234);
	D2_21 = _mm512_madd52lo_epu64(D2_21,a51228,a51233);
	D2_21 = _mm512_madd52lo_epu64(D2_21,a51229,a51232);
	D2_21 = _mm512_madd52lo_epu64(D2_21,a51230,a51231);
	D2_21 = _mm512_slli_epi64(D2_21,1);
	D2_21 = _mm512_madd52hi_epu64(D2_21,a51230,a51230);

	D2_22 = _mm512_madd52hi_epu64(zero,a51222,a51239);
	D2_22 = _mm512_madd52hi_epu64(D2_22,a51223,a51238);
	D2_22 = _mm512_madd52hi_epu64(D2_22,a51224,a51237);
	D2_22 = _mm512_madd52hi_epu64(D2_22,a51225,a51236);
	D2_22 = _mm512_madd52hi_epu64(D2_22,a51226,a51235);
	D2_22 = _mm512_madd52hi_epu64(D2_22,a51227,a51234);
	D2_22 = _mm512_madd52hi_epu64(D2_22,a51228,a51233);
	D2_22 = _mm512_madd52hi_epu64(D2_22,a51229,a51232);
	D2_22 = _mm512_madd52hi_epu64(D2_22,a51230,a51231);
	D2_22 = _mm512_madd52lo_epu64(D2_22,a51223,a51239);
	D2_22 = _mm512_madd52lo_epu64(D2_22,a51224,a51238);
	D2_22 = _mm512_madd52lo_epu64(D2_22,a51225,a51237);
	D2_22 = _mm512_madd52lo_epu64(D2_22,a51226,a51236);
	D2_22 = _mm512_madd52lo_epu64(D2_22,a51227,a51235);
	D2_22 = _mm512_madd52lo_epu64(D2_22,a51228,a51234);
	D2_22 = _mm512_madd52lo_epu64(D2_22,a51229,a51233);
	D2_22 = _mm512_madd52lo_epu64(D2_22,a51230,a51232);
	D2_22 = _mm512_slli_epi64(D2_22,1);
	D2_22 = _mm512_madd52lo_epu64(D2_22,a51231,a51231);

	D2_23 = _mm512_madd52hi_epu64(zero,a51223,a51239);
	D2_23 = _mm512_madd52hi_epu64(D2_23,a51224,a51238);
	D2_23 = _mm512_madd52hi_epu64(D2_23,a51225,a51237);
	D2_23 = _mm512_madd52hi_epu64(D2_23,a51226,a51236);
	D2_23 = _mm512_madd52hi_epu64(D2_23,a51227,a51235);
	D2_23 = _mm512_madd52hi_epu64(D2_23,a51228,a51234);
	D2_23 = _mm512_madd52hi_epu64(D2_23,a51229,a51233);
	D2_23 = _mm512_madd52hi_epu64(D2_23,a51230,a51232);
	D2_23 = _mm512_madd52lo_epu64(D2_23,a51224,a51239);
	D2_23 = _mm512_madd52lo_epu64(D2_23,a51225,a51238);
	D2_23 = _mm512_madd52lo_epu64(D2_23,a51226,a51237);
	D2_23 = _mm512_madd52lo_epu64(D2_23,a51227,a51236);
	D2_23 = _mm512_madd52lo_epu64(D2_23,a51228,a51235);
	D2_23 = _mm512_madd52lo_epu64(D2_23,a51229,a51234);
	D2_23 = _mm512_madd52lo_epu64(D2_23,a51230,a51233);
	D2_23 = _mm512_madd52lo_epu64(D2_23,a51231,a51232);
	D2_23 = _mm512_slli_epi64(D2_23,1);
	D2_23 = _mm512_madd52hi_epu64(D2_23,a51231,a51231);

	D2_24 = _mm512_madd52hi_epu64(zero,a51224,a51239);
	D2_24 = _mm512_madd52hi_epu64(D2_24,a51225,a51238);
	D2_24 = _mm512_madd52hi_epu64(D2_24,a51226,a51237);
	D2_24 = _mm512_madd52hi_epu64(D2_24,a51227,a51236);
	D2_24 = _mm512_madd52hi_epu64(D2_24,a51228,a51235);
	D2_24 = _mm512_madd52hi_epu64(D2_24,a51229,a51234);
	D2_24 = _mm512_madd52hi_epu64(D2_24,a51230,a51233);
	D2_24 = _mm512_madd52hi_epu64(D2_24,a51231,a51232);
	D2_24 = _mm512_madd52lo_epu64(D2_24,a51225,a51239);
	D2_24 = _mm512_madd52lo_epu64(D2_24,a51226,a51238);
	D2_24 = _mm512_madd52lo_epu64(D2_24,a51227,a51237);
	D2_24 = _mm512_madd52lo_epu64(D2_24,a51228,a51236);
	D2_24 = _mm512_madd52lo_epu64(D2_24,a51229,a51235);
	D2_24 = _mm512_madd52lo_epu64(D2_24,a51230,a51234);
	D2_24 = _mm512_madd52lo_epu64(D2_24,a51231,a51233);
	D2_24 = _mm512_slli_epi64(D2_24,1);
	D2_24 = _mm512_madd52lo_epu64(D2_24,a51232,a51232);

	D2_25 = _mm512_madd52hi_epu64(zero,a51225,a51239);
	D2_25 = _mm512_madd52hi_epu64(D2_25,a51226,a51238);
	D2_25 = _mm512_madd52hi_epu64(D2_25,a51227,a51237);
	D2_25 = _mm512_madd52hi_epu64(D2_25,a51228,a51236);
	D2_25 = _mm512_madd52hi_epu64(D2_25,a51229,a51235);
	D2_25 = _mm512_madd52hi_epu64(D2_25,a51230,a51234);
	D2_25 = _mm512_madd52hi_epu64(D2_25,a51231,a51233);
	D2_25 = _mm512_madd52lo_epu64(D2_25,a51226,a51239);
	D2_25 = _mm512_madd52lo_epu64(D2_25,a51227,a51238);
	D2_25 = _mm512_madd52lo_epu64(D2_25,a51228,a51237);
	D2_25 = _mm512_madd52lo_epu64(D2_25,a51229,a51236);
	D2_25 = _mm512_madd52lo_epu64(D2_25,a51230,a51235);
	D2_25 = _mm512_madd52lo_epu64(D2_25,a51231,a51234);
	D2_25 = _mm512_madd52lo_epu64(D2_25,a51232,a51233);
	D2_25 = _mm512_slli_epi64(D2_25,1);
	D2_25 = _mm512_madd52hi_epu64(D2_25,a51232,a51232);

	D2_26 = _mm512_madd52hi_epu64(zero,a51226,a51239);
	D2_26 = _mm512_madd52hi_epu64(D2_26,a51227,a51238);
	D2_26 = _mm512_madd52hi_epu64(D2_26,a51228,a51237);
	D2_26 = _mm512_madd52hi_epu64(D2_26,a51229,a51236);
	D2_26 = _mm512_madd52hi_epu64(D2_26,a51230,a51235);
	D2_26 = _mm512_madd52hi_epu64(D2_26,a51231,a51234);
	D2_26 = _mm512_madd52hi_epu64(D2_26,a51232,a51233);
	D2_26 = _mm512_madd52lo_epu64(D2_26,a51227,a51239);
	D2_26 = _mm512_madd52lo_epu64(D2_26,a51228,a51238);
	D2_26 = _mm512_madd52lo_epu64(D2_26,a51229,a51237);
	D2_26 = _mm512_madd52lo_epu64(D2_26,a51230,a51236);
	D2_26 = _mm512_madd52lo_epu64(D2_26,a51231,a51235);
	D2_26 = _mm512_madd52lo_epu64(D2_26,a51232,a51234);
	D2_26 = _mm512_slli_epi64(D2_26,1);
	D2_26 = _mm512_madd52lo_epu64(D2_26,a51233,a51233);

	D2_27 = _mm512_madd52hi_epu64(zero,a51227,a51239);
	D2_27 = _mm512_madd52hi_epu64(D2_27,a51228,a51238);
	D2_27 = _mm512_madd52hi_epu64(D2_27,a51229,a51237);
	D2_27 = _mm512_madd52hi_epu64(D2_27,a51230,a51236);
	D2_27 = _mm512_madd52hi_epu64(D2_27,a51231,a51235);
	D2_27 = _mm512_madd52hi_epu64(D2_27,a51232,a51234);
	D2_27 = _mm512_madd52lo_epu64(D2_27,a51228,a51239);
	D2_27 = _mm512_madd52lo_epu64(D2_27,a51229,a51238);
	D2_27 = _mm512_madd52lo_epu64(D2_27,a51230,a51237);
	D2_27 = _mm512_madd52lo_epu64(D2_27,a51231,a51236);
	D2_27 = _mm512_madd52lo_epu64(D2_27,a51232,a51235);
	D2_27 = _mm512_madd52lo_epu64(D2_27,a51233,a51234);
	D2_27 = _mm512_slli_epi64(D2_27,1);
	D2_27 = _mm512_madd52hi_epu64(D2_27,a51233,a51233);

	D2_28 = _mm512_madd52hi_epu64(zero,a51228,a51239);
	D2_28 = _mm512_madd52hi_epu64(D2_28,a51229,a51238);
	D2_28 = _mm512_madd52hi_epu64(D2_28,a51230,a51237);
	D2_28 = _mm512_madd52hi_epu64(D2_28,a51231,a51236);
	D2_28 = _mm512_madd52hi_epu64(D2_28,a51232,a51235);
	D2_28 = _mm512_madd52hi_epu64(D2_28,a51233,a51234);
	D2_28 = _mm512_madd52lo_epu64(D2_28,a51229,a51239);
	D2_28 = _mm512_madd52lo_epu64(D2_28,a51230,a51238);
	D2_28 = _mm512_madd52lo_epu64(D2_28,a51231,a51237);
	D2_28 = _mm512_madd52lo_epu64(D2_28,a51232,a51236);
	D2_28 = _mm512_madd52lo_epu64(D2_28,a51233,a51235);
	D2_28 = _mm512_slli_epi64(D2_28,1);
	D2_28 = _mm512_madd52lo_epu64(D2_28,a51234,a51234);

	D2_29 = _mm512_madd52hi_epu64(zero,a51229,a51239);
	D2_29 = _mm512_madd52hi_epu64(D2_29,a51230,a51238);
	D2_29 = _mm512_madd52hi_epu64(D2_29,a51231,a51237);
	D2_29 = _mm512_madd52hi_epu64(D2_29,a51232,a51236);
	D2_29 = _mm512_madd52hi_epu64(D2_29,a51233,a51235);
	D2_29 = _mm512_madd52lo_epu64(D2_29,a51230,a51239);
	D2_29 = _mm512_madd52lo_epu64(D2_29,a51231,a51238);
	D2_29 = _mm512_madd52lo_epu64(D2_29,a51232,a51237);
	D2_29 = _mm512_madd52lo_epu64(D2_29,a51233,a51236);
	D2_29 = _mm512_madd52lo_epu64(D2_29,a51234,a51235);
	D2_29 = _mm512_slli_epi64(D2_29,1);
	D2_29 = _mm512_madd52hi_epu64(D2_29,a51234,a51234);

	D2_30 = _mm512_madd52hi_epu64(zero,a51230,a51239);
	D2_30 = _mm512_madd52hi_epu64(D2_30,a51231,a51238);
	D2_30 = _mm512_madd52hi_epu64(D2_30,a51232,a51237);
	D2_30 = _mm512_madd52hi_epu64(D2_30,a51233,a51236);
	D2_30 = _mm512_madd52hi_epu64(D2_30,a51234,a51235);
	D2_30 = _mm512_madd52lo_epu64(D2_30,a51231,a51239);
	D2_30 = _mm512_madd52lo_epu64(D2_30,a51232,a51238);
	D2_30 = _mm512_madd52lo_epu64(D2_30,a51233,a51237);
	D2_30 = _mm512_madd52lo_epu64(D2_30,a51234,a51236);
	D2_30 = _mm512_slli_epi64(D2_30,1);
	D2_30 = _mm512_madd52lo_epu64(D2_30,a51235,a51235);

	D2_31 = _mm512_madd52hi_epu64(zero,a51231,a51239);
	D2_31 = _mm512_madd52hi_epu64(D2_31,a51232,a51238);
	D2_31 = _mm512_madd52hi_epu64(D2_31,a51233,a51237);
	D2_31 = _mm512_madd52hi_epu64(D2_31,a51234,a51236);
	D2_31 = _mm512_madd52lo_epu64(D2_31,a51232,a51239);
	D2_31 = _mm512_madd52lo_epu64(D2_31,a51233,a51238);
	D2_31 = _mm512_madd52lo_epu64(D2_31,a51234,a51237);
	D2_31 = _mm512_madd52lo_epu64(D2_31,a51235,a51236);
	D2_31 = _mm512_slli_epi64(D2_31,1);
	D2_31 = _mm512_madd52hi_epu64(D2_31,a51235,a51235);

	D2_32 = _mm512_madd52hi_epu64(zero,a51232,a51239);
	D2_32 = _mm512_madd52hi_epu64(D2_32,a51233,a51238);
	D2_32 = _mm512_madd52hi_epu64(D2_32,a51234,a51237);
	D2_32 = _mm512_madd52hi_epu64(D2_32,a51235,a51236);
	D2_32 = _mm512_madd52lo_epu64(D2_32,a51233,a51239);
	D2_32 = _mm512_madd52lo_epu64(D2_32,a51234,a51238);
	D2_32 = _mm512_madd52lo_epu64(D2_32,a51235,a51237);
	D2_32 = _mm512_slli_epi64(D2_32,1);
	D2_32 = _mm512_madd52lo_epu64(D2_32,a51236,a51236);

	D2_33 = _mm512_madd52hi_epu64(zero,a51233,a51239);
	D2_33 = _mm512_madd52hi_epu64(D2_33,a51234,a51238);
	D2_33 = _mm512_madd52hi_epu64(D2_33,a51235,a51237);
	D2_33 = _mm512_madd52lo_epu64(D2_33,a51234,a51239);
	D2_33 = _mm512_madd52lo_epu64(D2_33,a51235,a51238);
	D2_33 = _mm512_madd52lo_epu64(D2_33,a51236,a51237);
	D2_33 = _mm512_slli_epi64(D2_33,1);
	D2_33 = _mm512_madd52hi_epu64(D2_33,a51236,a51236);

	D2_34 = _mm512_madd52hi_epu64(zero,a51234,a51239);
	D2_34 = _mm512_madd52hi_epu64(D2_34,a51235,a51238);
	D2_34 = _mm512_madd52hi_epu64(D2_34,a51236,a51237);
	D2_34 = _mm512_madd52lo_epu64(D2_34,a51235,a51239);
	D2_34 = _mm512_madd52lo_epu64(D2_34,a51236,a51238);
	D2_34 = _mm512_slli_epi64(D2_34,1);
	D2_34 = _mm512_madd52lo_epu64(D2_34,a51237,a51237);

	D2_35 = _mm512_madd52hi_epu64(zero,a51235,a51239);
	D2_35 = _mm512_madd52hi_epu64(D2_35,a51236,a51238);
	D2_35 = _mm512_madd52lo_epu64(D2_35,a51236,a51239);
	D2_35 = _mm512_madd52lo_epu64(D2_35,a51237,a51238);
	D2_35 = _mm512_slli_epi64(D2_35,1);
	D2_35 = _mm512_madd52hi_epu64(D2_35,a51237,a51237);

	D2_36 = _mm512_madd52hi_epu64(zero,a51236,a51239);
	D2_36 = _mm512_madd52hi_epu64(D2_36,a51237,a51238);
	D2_36 = _mm512_madd52lo_epu64(D2_36,a51237,a51239);
	D2_36 = _mm512_slli_epi64(D2_36,1);
	D2_36 = _mm512_madd52lo_epu64(D2_36,a51238,a51238);

	D2_37 = _mm512_madd52hi_epu64(zero,a51237,a51239);
	D2_37 = _mm512_madd52lo_epu64(D2_37,a51238,a51239);
	D2_37 = _mm512_slli_epi64(D2_37,1);
	D2_37 = _mm512_madd52hi_epu64(D2_37,a51238,a51238);

	D2_38 = _mm512_madd52hi_epu64(zero,a51238,a51239);
	D2_38 = _mm512_slli_epi64(D2_38,1);
	D2_38 = _mm512_madd52lo_epu64(D2_38,a51239,a51239);

	D2_39 = _mm512_madd52hi_epu64(zero,a51239,a51239);


	// Carry management

	carry  = _mm512_srli_epi64(D2_1,52);
	D2_2 = _mm512_add_epi64(D2_2,carry);
	D2_1 = _mm512_and_si512(D2_1,mask52);
	carry  = _mm512_srli_epi64(D2_2,52);
	D2_3 = _mm512_add_epi64(D2_3,carry);
	D2_2 = _mm512_and_si512(D2_2,mask52);
	carry  = _mm512_srli_epi64(D2_3,52);
	D2_4 = _mm512_add_epi64(D2_4,carry);
	D2_3 = _mm512_and_si512(D2_3,mask52);
	carry  = _mm512_srli_epi64(D2_4,52);
	D2_5 = _mm512_add_epi64(D2_5,carry);
	D2_4 = _mm512_and_si512(D2_4,mask52);
	carry  = _mm512_srli_epi64(D2_5,52);
	D2_6 = _mm512_add_epi64(D2_6,carry);
	D2_5 = _mm512_and_si512(D2_5,mask52);
	carry  = _mm512_srli_epi64(D2_6,52);
	D2_7 = _mm512_add_epi64(D2_7,carry);
	D2_6 = _mm512_and_si512(D2_6,mask52);
	carry  = _mm512_srli_epi64(D2_7,52);
	D2_8 = _mm512_add_epi64(D2_8,carry);
	D2_7 = _mm512_and_si512(D2_7,mask52);
	carry  = _mm512_srli_epi64(D2_8,52);
	D2_9 = _mm512_add_epi64(D2_9,carry);
	D2_8 = _mm512_and_si512(D2_8,mask52);
	carry  = _mm512_srli_epi64(D2_9,52);
	D2_10 = _mm512_add_epi64(D2_10,carry);
	D2_9 = _mm512_and_si512(D2_9,mask52);
	carry  = _mm512_srli_epi64(D2_10,52);
	D2_11 = _mm512_add_epi64(D2_11,carry);
	D2_10 = _mm512_and_si512(D2_10,mask52);
	carry  = _mm512_srli_epi64(D2_11,52);
	D2_12 = _mm512_add_epi64(D2_12,carry);
	D2_11 = _mm512_and_si512(D2_11,mask52);
	carry  = _mm512_srli_epi64(D2_12,52);
	D2_13 = _mm512_add_epi64(D2_13,carry);
	D2_12 = _mm512_and_si512(D2_12,mask52);
	carry  = _mm512_srli_epi64(D2_13,52);
	D2_14 = _mm512_add_epi64(D2_14,carry);
	D2_13 = _mm512_and_si512(D2_13,mask52);
	carry  = _mm512_srli_epi64(D2_14,52);
	D2_15 = _mm512_add_epi64(D2_15,carry);
	D2_14 = _mm512_and_si512(D2_14,mask52);
	carry  = _mm512_srli_epi64(D2_15,52);
	D2_16 = _mm512_add_epi64(D2_16,carry);
	D2_15 = _mm512_and_si512(D2_15,mask52);
	carry  = _mm512_srli_epi64(D2_16,52);
	D2_17 = _mm512_add_epi64(D2_17,carry);
	D2_16 = _mm512_and_si512(D2_16,mask52);
	carry  = _mm512_srli_epi64(D2_17,52);
	D2_18 = _mm512_add_epi64(D2_18,carry);
	D2_17 = _mm512_and_si512(D2_17,mask52);
	carry  = _mm512_srli_epi64(D2_18,52);
	D2_19 = _mm512_add_epi64(D2_19,carry);
	D2_18 = _mm512_and_si512(D2_18,mask52);

	// Conversion in base 2^1039

	carry = D2_19>>51;
	D2_20 = (D2_20<<1)+carry;
	D2_19 &= mask51;
	carry = D2_20>>52;
	D2_21 = (D2_21<<1)+carry;
	D2_20 &= mask52;
	carry = D2_21>>52;
	D2_22 = (D2_22<<1)+carry;
	D2_21 &= mask52;
	carry = D2_22>>52;
	D2_23 = (D2_23<<1)+carry;
	D2_22 &= mask52;
	carry = D2_23>>52;
	D2_24 = (D2_24<<1)+carry;
	D2_23 &= mask52;
	carry = D2_24>>52;
	D2_25 = (D2_25<<1)+carry;
	D2_24 &= mask52;
	carry = D2_25>>52;
	D2_26 = (D2_26<<1)+carry;
	D2_25 &= mask52;
	carry = D2_26>>52;
	D2_27 = (D2_27<<1)+carry;
	D2_26 &= mask52;
	carry = D2_27>>52;
	D2_28 = (D2_28<<1)+carry;
	D2_27 &= mask52;
	carry = D2_28>>52;
	D2_29 = (D2_29<<1)+carry;
	D2_28 &= mask52;
	carry = D2_29>>52;
	D2_30 = (D2_30<<1)+carry;
	D2_29 &= mask52;
	carry = D2_30>>52;
	D2_31 = (D2_31<<1)+carry;
	D2_30 &= mask52;
	carry = D2_31>>52;
	D2_32 = (D2_32<<1)+carry;
	D2_31 &= mask52;
	carry = D2_32>>52;
	D2_33 = (D2_33<<1)+carry;
	D2_32 &= mask52;
	carry = D2_33>>52;
	D2_34 = (D2_34<<1)+carry;
	D2_33 &= mask52;
	carry = D2_34>>52;
	D2_35 = (D2_35<<1)+carry;
	D2_34 &= mask52;
	carry = D2_35>>52;
	D2_36 = (D2_36<<1)+carry;
	D2_35 &= mask52;
	carry = D2_36>>52;
	D2_37 = (D2_37<<1)+carry;
	D2_36 &= mask52;
	carry = D2_37>>52;
	D2_38 = (D2_38<<1)+carry;
	D2_37 &= mask52;
	carry = D2_38>>52;
	D2_39 = (D2_39<<1)+carry;
	D2_38 &= mask52;

	// D1 = (Al+Ah)^2

	__m512i D1_0, D1_1;
	__m512i D1_2, D1_3;
	__m512i D1_4, D1_5;
	__m512i D1_6, D1_7;
	__m512i D1_8, D1_9;
	__m512i D1_10, D1_11;
	__m512i D1_12, D1_13;
	__m512i D1_14, D1_15;
	__m512i D1_16, D1_17;
	__m512i D1_18, D1_19;
	__m512i D1_20, D1_21;
	__m512i D1_22, D1_23;
	__m512i D1_24, D1_25;
	__m512i D1_26, D1_27;
	__m512i D1_28, D1_29;
	__m512i D1_30, D1_31;
	__m512i D1_32, D1_33;
	__m512i D1_34, D1_35;
	__m512i D1_36, D1_37;
	__m512i D1_38, D1_39;

	__m512i alpah0 = _mm512_add_epi64(a5120,a51220);
	carry = alpah0>>52;
	alpah0 &= mask52;
	__m512i alpah1 = _mm512_add_epi64(a5121,carry);
	alpah1 = _mm512_add_epi64(alpah1,a51221);
	carry = alpah1>>52;
	alpah1 &= mask52;
	__m512i alpah2 = _mm512_add_epi64(a5122,carry);
	alpah2 = _mm512_add_epi64(alpah2,a51222);
	carry = alpah2>>52;
	alpah2 &= mask52;
	__m512i alpah3 = _mm512_add_epi64(a5123,carry);
	alpah3 = _mm512_add_epi64(alpah3,a51223);
	carry = alpah3>>52;
	alpah3 &= mask52;
	__m512i alpah4 = _mm512_add_epi64(a5124,carry);
	alpah4 = _mm512_add_epi64(alpah4,a51224);
	carry = alpah4>>52;
	alpah4 &= mask52;
	__m512i alpah5 = _mm512_add_epi64(a5125,carry);
	alpah5 = _mm512_add_epi64(alpah5,a51225);
	carry = alpah5>>52;
	alpah5 &= mask52;
	__m512i alpah6 = _mm512_add_epi64(a5126,carry);
	alpah6 = _mm512_add_epi64(alpah6,a51226);
	carry = alpah6>>52;
	alpah6 &= mask52;
	__m512i alpah7 = _mm512_add_epi64(a5127,carry);
	alpah7 = _mm512_add_epi64(alpah7,a51227);
	carry = alpah7>>52;
	alpah7 &= mask52;
	__m512i alpah8 = _mm512_add_epi64(a5128,carry);
	alpah8 = _mm512_add_epi64(alpah8,a51228);
	carry = alpah8>>52;
	alpah8 &= mask52;
	__m512i alpah9 = _mm512_add_epi64(a5129,carry);
	alpah9 = _mm512_add_epi64(alpah9,a51229);
	carry = alpah9>>52;
	alpah9 &= mask52;
	__m512i alpah10 = _mm512_add_epi64(a51210,carry);
	alpah10 = _mm512_add_epi64(alpah10,a51230);
	carry = alpah10>>52;
	alpah10 &= mask52;
	__m512i alpah11 = _mm512_add_epi64(a51211,carry);
	alpah11 = _mm512_add_epi64(alpah11,a51231);
	carry = alpah11>>52;
	alpah11 &= mask52;
	__m512i alpah12 = _mm512_add_epi64(a51212,carry);
	alpah12 = _mm512_add_epi64(alpah12,a51232);
	carry = alpah12>>52;
	alpah12 &= mask52;
	__m512i alpah13 = _mm512_add_epi64(a51213,carry);
	alpah13 = _mm512_add_epi64(alpah13,a51233);
	carry = alpah13>>52;
	alpah13 &= mask52;
	__m512i alpah14 = _mm512_add_epi64(a51214,carry);
	alpah14 = _mm512_add_epi64(alpah14,a51234);
	carry = alpah14>>52;
	alpah14 &= mask52;
	__m512i alpah15 = _mm512_add_epi64(a51215,carry);
	alpah15 = _mm512_add_epi64(alpah15,a51235);
	carry = alpah15>>52;
	alpah15 &= mask52;
	__m512i alpah16 = _mm512_add_epi64(a51216,carry);
	alpah16 = _mm512_add_epi64(alpah16,a51236);
	carry = alpah16>>52;
	alpah16 &= mask52;
	__m512i alpah17 = _mm512_add_epi64(a51217,carry);
	alpah17 = _mm512_add_epi64(alpah17,a51237);
	carry = alpah17>>52;
	alpah17 &= mask52;
	__m512i alpah18 = _mm512_add_epi64(a51218,carry);
	alpah18 = _mm512_add_epi64(alpah18,a51238);
	carry = alpah18>>52;
	alpah18 &= mask52;
	__m512i alpah19 = _mm512_add_epi64(a51219,carry);
	alpah19 = _mm512_add_epi64(alpah19,a51239);
	carry = alpah19>>52;
	alpah19 &= mask52;

	D1_0 = _mm512_madd52lo_epu64(D0_20,alpah0,alpah0);

	D1_1 = _mm512_madd52lo_epu64(zero,alpah0,alpah1);
	D1_1 = _mm512_slli_epi64(D1_1,1);
	D1_1 = _mm512_madd52hi_epu64(D1_1,alpah0,alpah0);

	D1_2 = _mm512_madd52hi_epu64(zero,alpah0,alpah1);
	D1_2 = _mm512_madd52lo_epu64(D1_2,alpah0,alpah2);
	D1_2 = _mm512_slli_epi64(D1_2,1);
	D1_2 = _mm512_madd52lo_epu64(D1_2,alpah1,alpah1);

	D1_3 = _mm512_madd52hi_epu64(zero,alpah0,alpah2);
	D1_3 = _mm512_madd52lo_epu64(D1_3,alpah0,alpah3);
	D1_3 = _mm512_madd52lo_epu64(D1_3,alpah1,alpah2);
	D1_3 = _mm512_slli_epi64(D1_3,1);
	D1_3 = _mm512_madd52hi_epu64(D1_3,alpah1,alpah1);

	D1_4 = _mm512_madd52hi_epu64(zero,alpah0,alpah3);
	D1_4 = _mm512_madd52hi_epu64(D1_4,alpah1,alpah2);
	D1_4 = _mm512_madd52lo_epu64(D1_4,alpah0,alpah4);
	D1_4 = _mm512_madd52lo_epu64(D1_4,alpah1,alpah3);
	D1_4 = _mm512_slli_epi64(D1_4,1);
	D1_4 = _mm512_madd52lo_epu64(D1_4,alpah2,alpah2);

	D1_5 = _mm512_madd52hi_epu64(zero,alpah0,alpah4);
	D1_5 = _mm512_madd52hi_epu64(D1_5,alpah1,alpah3);
	D1_5 = _mm512_madd52lo_epu64(D1_5,alpah0,alpah5);
	D1_5 = _mm512_madd52lo_epu64(D1_5,alpah1,alpah4);
	D1_5 = _mm512_madd52lo_epu64(D1_5,alpah2,alpah3);
	D1_5 = _mm512_slli_epi64(D1_5,1);
	D1_5 = _mm512_madd52hi_epu64(D1_5,alpah2,alpah2);

	D1_6 = _mm512_madd52hi_epu64(zero,alpah0,alpah5);
	D1_6 = _mm512_madd52hi_epu64(D1_6,alpah1,alpah4);
	D1_6 = _mm512_madd52hi_epu64(D1_6,alpah2,alpah3);
	D1_6 = _mm512_madd52lo_epu64(D1_6,alpah0,alpah6);
	D1_6 = _mm512_madd52lo_epu64(D1_6,alpah1,alpah5);
	D1_6 = _mm512_madd52lo_epu64(D1_6,alpah2,alpah4);
	D1_6 = _mm512_slli_epi64(D1_6,1);
	D1_6 = _mm512_madd52lo_epu64(D1_6,alpah3,alpah3);

	D1_7 = _mm512_madd52hi_epu64(zero,alpah0,alpah6);
	D1_7 = _mm512_madd52hi_epu64(D1_7,alpah1,alpah5);
	D1_7 = _mm512_madd52hi_epu64(D1_7,alpah2,alpah4);
	D1_7 = _mm512_madd52lo_epu64(D1_7,alpah0,alpah7);
	D1_7 = _mm512_madd52lo_epu64(D1_7,alpah1,alpah6);
	D1_7 = _mm512_madd52lo_epu64(D1_7,alpah2,alpah5);
	D1_7 = _mm512_madd52lo_epu64(D1_7,alpah3,alpah4);
	D1_7 = _mm512_slli_epi64(D1_7,1);
	D1_7 = _mm512_madd52hi_epu64(D1_7,alpah3,alpah3);

	D1_8 = _mm512_madd52hi_epu64(zero,alpah0,alpah7);
	D1_8 = _mm512_madd52hi_epu64(D1_8,alpah1,alpah6);
	D1_8 = _mm512_madd52hi_epu64(D1_8,alpah2,alpah5);
	D1_8 = _mm512_madd52hi_epu64(D1_8,alpah3,alpah4);
	D1_8 = _mm512_madd52lo_epu64(D1_8,alpah0,alpah8);
	D1_8 = _mm512_madd52lo_epu64(D1_8,alpah1,alpah7);
	D1_8 = _mm512_madd52lo_epu64(D1_8,alpah2,alpah6);
	D1_8 = _mm512_madd52lo_epu64(D1_8,alpah3,alpah5);
	D1_8 = _mm512_slli_epi64(D1_8,1);
	D1_8 = _mm512_madd52lo_epu64(D1_8,alpah4,alpah4);

	D1_9 = _mm512_madd52hi_epu64(zero,alpah0,alpah8);
	D1_9 = _mm512_madd52hi_epu64(D1_9,alpah1,alpah7);
	D1_9 = _mm512_madd52hi_epu64(D1_9,alpah2,alpah6);
	D1_9 = _mm512_madd52hi_epu64(D1_9,alpah3,alpah5);
	D1_9 = _mm512_madd52lo_epu64(D1_9,alpah0,alpah9);
	D1_9 = _mm512_madd52lo_epu64(D1_9,alpah1,alpah8);
	D1_9 = _mm512_madd52lo_epu64(D1_9,alpah2,alpah7);
	D1_9 = _mm512_madd52lo_epu64(D1_9,alpah3,alpah6);
	D1_9 = _mm512_madd52lo_epu64(D1_9,alpah4,alpah5);
	D1_9 = _mm512_slli_epi64(D1_9,1);
	D1_9 = _mm512_madd52hi_epu64(D1_9,alpah4,alpah4);

	D1_10 = _mm512_madd52hi_epu64(zero,alpah0,alpah9);
	D1_10 = _mm512_madd52hi_epu64(D1_10,alpah1,alpah8);
	D1_10 = _mm512_madd52hi_epu64(D1_10,alpah2,alpah7);
	D1_10 = _mm512_madd52hi_epu64(D1_10,alpah3,alpah6);
	D1_10 = _mm512_madd52hi_epu64(D1_10,alpah4,alpah5);
	D1_10 = _mm512_madd52lo_epu64(D1_10,alpah0,alpah10);
	D1_10 = _mm512_madd52lo_epu64(D1_10,alpah1,alpah9);
	D1_10 = _mm512_madd52lo_epu64(D1_10,alpah2,alpah8);
	D1_10 = _mm512_madd52lo_epu64(D1_10,alpah3,alpah7);
	D1_10 = _mm512_madd52lo_epu64(D1_10,alpah4,alpah6);
	D1_10 = _mm512_slli_epi64(D1_10,1);
	D1_10 = _mm512_madd52lo_epu64(D1_10,alpah5,alpah5);

	D1_11 = _mm512_madd52hi_epu64(zero,alpah0,alpah10);
	D1_11 = _mm512_madd52hi_epu64(D1_11,alpah1,alpah9);
	D1_11 = _mm512_madd52hi_epu64(D1_11,alpah2,alpah8);
	D1_11 = _mm512_madd52hi_epu64(D1_11,alpah3,alpah7);
	D1_11 = _mm512_madd52hi_epu64(D1_11,alpah4,alpah6);
	D1_11 = _mm512_madd52lo_epu64(D1_11,alpah0,alpah11);
	D1_11 = _mm512_madd52lo_epu64(D1_11,alpah1,alpah10);
	D1_11 = _mm512_madd52lo_epu64(D1_11,alpah2,alpah9);
	D1_11 = _mm512_madd52lo_epu64(D1_11,alpah3,alpah8);
	D1_11 = _mm512_madd52lo_epu64(D1_11,alpah4,alpah7);
	D1_11 = _mm512_madd52lo_epu64(D1_11,alpah5,alpah6);
	D1_11 = _mm512_slli_epi64(D1_11,1);
	D1_11 = _mm512_madd52hi_epu64(D1_11,alpah5,alpah5);

	D1_12 = _mm512_madd52hi_epu64(zero,alpah0,alpah11);
	D1_12 = _mm512_madd52hi_epu64(D1_12,alpah1,alpah10);
	D1_12 = _mm512_madd52hi_epu64(D1_12,alpah2,alpah9);
	D1_12 = _mm512_madd52hi_epu64(D1_12,alpah3,alpah8);
	D1_12 = _mm512_madd52hi_epu64(D1_12,alpah4,alpah7);
	D1_12 = _mm512_madd52hi_epu64(D1_12,alpah5,alpah6);
	D1_12 = _mm512_madd52lo_epu64(D1_12,alpah0,alpah12);
	D1_12 = _mm512_madd52lo_epu64(D1_12,alpah1,alpah11);
	D1_12 = _mm512_madd52lo_epu64(D1_12,alpah2,alpah10);
	D1_12 = _mm512_madd52lo_epu64(D1_12,alpah3,alpah9);
	D1_12 = _mm512_madd52lo_epu64(D1_12,alpah4,alpah8);
	D1_12 = _mm512_madd52lo_epu64(D1_12,alpah5,alpah7);
	D1_12 = _mm512_slli_epi64(D1_12,1);
	D1_12 = _mm512_madd52lo_epu64(D1_12,alpah6,alpah6);

	D1_13 = _mm512_madd52hi_epu64(zero,alpah0,alpah12);
	D1_13 = _mm512_madd52hi_epu64(D1_13,alpah1,alpah11);
	D1_13 = _mm512_madd52hi_epu64(D1_13,alpah2,alpah10);
	D1_13 = _mm512_madd52hi_epu64(D1_13,alpah3,alpah9);
	D1_13 = _mm512_madd52hi_epu64(D1_13,alpah4,alpah8);
	D1_13 = _mm512_madd52hi_epu64(D1_13,alpah5,alpah7);
	D1_13 = _mm512_madd52lo_epu64(D1_13,alpah0,alpah13);
	D1_13 = _mm512_madd52lo_epu64(D1_13,alpah1,alpah12);
	D1_13 = _mm512_madd52lo_epu64(D1_13,alpah2,alpah11);
	D1_13 = _mm512_madd52lo_epu64(D1_13,alpah3,alpah10);
	D1_13 = _mm512_madd52lo_epu64(D1_13,alpah4,alpah9);
	D1_13 = _mm512_madd52lo_epu64(D1_13,alpah5,alpah8);
	D1_13 = _mm512_madd52lo_epu64(D1_13,alpah6,alpah7);
	D1_13 = _mm512_slli_epi64(D1_13,1);
	D1_13 = _mm512_madd52hi_epu64(D1_13,alpah6,alpah6);

	D1_14 = _mm512_madd52hi_epu64(zero,alpah0,alpah13);
	D1_14 = _mm512_madd52hi_epu64(D1_14,alpah1,alpah12);
	D1_14 = _mm512_madd52hi_epu64(D1_14,alpah2,alpah11);
	D1_14 = _mm512_madd52hi_epu64(D1_14,alpah3,alpah10);
	D1_14 = _mm512_madd52hi_epu64(D1_14,alpah4,alpah9);
	D1_14 = _mm512_madd52hi_epu64(D1_14,alpah5,alpah8);
	D1_14 = _mm512_madd52hi_epu64(D1_14,alpah6,alpah7);
	D1_14 = _mm512_madd52lo_epu64(D1_14,alpah0,alpah14);
	D1_14 = _mm512_madd52lo_epu64(D1_14,alpah1,alpah13);
	D1_14 = _mm512_madd52lo_epu64(D1_14,alpah2,alpah12);
	D1_14 = _mm512_madd52lo_epu64(D1_14,alpah3,alpah11);
	D1_14 = _mm512_madd52lo_epu64(D1_14,alpah4,alpah10);
	D1_14 = _mm512_madd52lo_epu64(D1_14,alpah5,alpah9);
	D1_14 = _mm512_madd52lo_epu64(D1_14,alpah6,alpah8);
	D1_14 = _mm512_slli_epi64(D1_14,1);
	D1_14 = _mm512_madd52lo_epu64(D1_14,alpah7,alpah7);

	D1_15 = _mm512_madd52hi_epu64(zero,alpah0,alpah14);
	D1_15 = _mm512_madd52hi_epu64(D1_15,alpah1,alpah13);
	D1_15 = _mm512_madd52hi_epu64(D1_15,alpah2,alpah12);
	D1_15 = _mm512_madd52hi_epu64(D1_15,alpah3,alpah11);
	D1_15 = _mm512_madd52hi_epu64(D1_15,alpah4,alpah10);
	D1_15 = _mm512_madd52hi_epu64(D1_15,alpah5,alpah9);
	D1_15 = _mm512_madd52hi_epu64(D1_15,alpah6,alpah8);
	D1_15 = _mm512_madd52lo_epu64(D1_15,alpah0,alpah15);
	D1_15 = _mm512_madd52lo_epu64(D1_15,alpah1,alpah14);
	D1_15 = _mm512_madd52lo_epu64(D1_15,alpah2,alpah13);
	D1_15 = _mm512_madd52lo_epu64(D1_15,alpah3,alpah12);
	D1_15 = _mm512_madd52lo_epu64(D1_15,alpah4,alpah11);
	D1_15 = _mm512_madd52lo_epu64(D1_15,alpah5,alpah10);
	D1_15 = _mm512_madd52lo_epu64(D1_15,alpah6,alpah9);
	D1_15 = _mm512_madd52lo_epu64(D1_15,alpah7,alpah8);
	D1_15 = _mm512_slli_epi64(D1_15,1);
	D1_15 = _mm512_madd52hi_epu64(D1_15,alpah7,alpah7);

	D1_16 = _mm512_madd52hi_epu64(zero,alpah0,alpah15);
	D1_16 = _mm512_madd52hi_epu64(D1_16,alpah1,alpah14);
	D1_16 = _mm512_madd52hi_epu64(D1_16,alpah2,alpah13);
	D1_16 = _mm512_madd52hi_epu64(D1_16,alpah3,alpah12);
	D1_16 = _mm512_madd52hi_epu64(D1_16,alpah4,alpah11);
	D1_16 = _mm512_madd52hi_epu64(D1_16,alpah5,alpah10);
	D1_16 = _mm512_madd52hi_epu64(D1_16,alpah6,alpah9);
	D1_16 = _mm512_madd52hi_epu64(D1_16,alpah7,alpah8);
	D1_16 = _mm512_madd52lo_epu64(D1_16,alpah0,alpah16);
	D1_16 = _mm512_madd52lo_epu64(D1_16,alpah1,alpah15);
	D1_16 = _mm512_madd52lo_epu64(D1_16,alpah2,alpah14);
	D1_16 = _mm512_madd52lo_epu64(D1_16,alpah3,alpah13);
	D1_16 = _mm512_madd52lo_epu64(D1_16,alpah4,alpah12);
	D1_16 = _mm512_madd52lo_epu64(D1_16,alpah5,alpah11);
	D1_16 = _mm512_madd52lo_epu64(D1_16,alpah6,alpah10);
	D1_16 = _mm512_madd52lo_epu64(D1_16,alpah7,alpah9);
	D1_16 = _mm512_slli_epi64(D1_16,1);
	D1_16 = _mm512_madd52lo_epu64(D1_16,alpah8,alpah8);

	D1_17 = _mm512_madd52hi_epu64(zero,alpah0,alpah16);
	D1_17 = _mm512_madd52hi_epu64(D1_17,alpah1,alpah15);
	D1_17 = _mm512_madd52hi_epu64(D1_17,alpah2,alpah14);
	D1_17 = _mm512_madd52hi_epu64(D1_17,alpah3,alpah13);
	D1_17 = _mm512_madd52hi_epu64(D1_17,alpah4,alpah12);
	D1_17 = _mm512_madd52hi_epu64(D1_17,alpah5,alpah11);
	D1_17 = _mm512_madd52hi_epu64(D1_17,alpah6,alpah10);
	D1_17 = _mm512_madd52hi_epu64(D1_17,alpah7,alpah9);
	D1_17 = _mm512_madd52lo_epu64(D1_17,alpah0,alpah17);
	D1_17 = _mm512_madd52lo_epu64(D1_17,alpah1,alpah16);
	D1_17 = _mm512_madd52lo_epu64(D1_17,alpah2,alpah15);
	D1_17 = _mm512_madd52lo_epu64(D1_17,alpah3,alpah14);
	D1_17 = _mm512_madd52lo_epu64(D1_17,alpah4,alpah13);
	D1_17 = _mm512_madd52lo_epu64(D1_17,alpah5,alpah12);
	D1_17 = _mm512_madd52lo_epu64(D1_17,alpah6,alpah11);
	D1_17 = _mm512_madd52lo_epu64(D1_17,alpah7,alpah10);
	D1_17 = _mm512_madd52lo_epu64(D1_17,alpah8,alpah9);
	D1_17 = _mm512_slli_epi64(D1_17,1);
	D1_17 = _mm512_madd52hi_epu64(D1_17,alpah8,alpah8);

	D1_18 = _mm512_madd52hi_epu64(zero,alpah0,alpah17);
	D1_18 = _mm512_madd52hi_epu64(D1_18,alpah1,alpah16);
	D1_18 = _mm512_madd52hi_epu64(D1_18,alpah2,alpah15);
	D1_18 = _mm512_madd52hi_epu64(D1_18,alpah3,alpah14);
	D1_18 = _mm512_madd52hi_epu64(D1_18,alpah4,alpah13);
	D1_18 = _mm512_madd52hi_epu64(D1_18,alpah5,alpah12);
	D1_18 = _mm512_madd52hi_epu64(D1_18,alpah6,alpah11);
	D1_18 = _mm512_madd52hi_epu64(D1_18,alpah7,alpah10);
	D1_18 = _mm512_madd52hi_epu64(D1_18,alpah8,alpah9);
	D1_18 = _mm512_madd52lo_epu64(D1_18,alpah0,alpah18);
	D1_18 = _mm512_madd52lo_epu64(D1_18,alpah1,alpah17);
	D1_18 = _mm512_madd52lo_epu64(D1_18,alpah2,alpah16);
	D1_18 = _mm512_madd52lo_epu64(D1_18,alpah3,alpah15);
	D1_18 = _mm512_madd52lo_epu64(D1_18,alpah4,alpah14);
	D1_18 = _mm512_madd52lo_epu64(D1_18,alpah5,alpah13);
	D1_18 = _mm512_madd52lo_epu64(D1_18,alpah6,alpah12);
	D1_18 = _mm512_madd52lo_epu64(D1_18,alpah7,alpah11);
	D1_18 = _mm512_madd52lo_epu64(D1_18,alpah8,alpah10);
	D1_18 = _mm512_slli_epi64(D1_18,1);
	D1_18 = _mm512_madd52lo_epu64(D1_18,alpah9,alpah9);

	D1_19 = _mm512_madd52hi_epu64(zero,alpah0,alpah18);
	D1_19 = _mm512_madd52hi_epu64(D1_19,alpah1,alpah17);
	D1_19 = _mm512_madd52hi_epu64(D1_19,alpah2,alpah16);
	D1_19 = _mm512_madd52hi_epu64(D1_19,alpah3,alpah15);
	D1_19 = _mm512_madd52hi_epu64(D1_19,alpah4,alpah14);
	D1_19 = _mm512_madd52hi_epu64(D1_19,alpah5,alpah13);
	D1_19 = _mm512_madd52hi_epu64(D1_19,alpah6,alpah12);
	D1_19 = _mm512_madd52hi_epu64(D1_19,alpah7,alpah11);
	D1_19 = _mm512_madd52hi_epu64(D1_19,alpah8,alpah10);
	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah0,alpah19);
	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah1,alpah18);
	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah2,alpah17);
	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah3,alpah16);
	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah4,alpah15);
	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah5,alpah14);
	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah6,alpah13);
	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah7,alpah12);
	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah8,alpah11);
	D1_19 = _mm512_madd52lo_epu64(D1_19,alpah9,alpah10);
	D1_19 = _mm512_slli_epi64(D1_19,1);
	D1_19 = _mm512_madd52hi_epu64(D1_19,alpah9,alpah9);

	D1_20 = _mm512_madd52hi_epu64(zero,alpah0,alpah19);
	D1_20 = _mm512_madd52hi_epu64(D1_20,alpah1,alpah18);
	D1_20 = _mm512_madd52hi_epu64(D1_20,alpah2,alpah17);
	D1_20 = _mm512_madd52hi_epu64(D1_20,alpah3,alpah16);
	D1_20 = _mm512_madd52hi_epu64(D1_20,alpah4,alpah15);
	D1_20 = _mm512_madd52hi_epu64(D1_20,alpah5,alpah14);
	D1_20 = _mm512_madd52hi_epu64(D1_20,alpah6,alpah13);
	D1_20 = _mm512_madd52hi_epu64(D1_20,alpah7,alpah12);
	D1_20 = _mm512_madd52hi_epu64(D1_20,alpah8,alpah11);
	D1_20 = _mm512_madd52hi_epu64(D1_20,alpah9,alpah10);
	D1_20 = _mm512_madd52lo_epu64(D1_20,alpah1,alpah19);
	D1_20 = _mm512_madd52lo_epu64(D1_20,alpah2,alpah18);
	D1_20 = _mm512_madd52lo_epu64(D1_20,alpah3,alpah17);
	D1_20 = _mm512_madd52lo_epu64(D1_20,alpah4,alpah16);
	D1_20 = _mm512_madd52lo_epu64(D1_20,alpah5,alpah15);
	D1_20 = _mm512_madd52lo_epu64(D1_20,alpah6,alpah14);
	D1_20 = _mm512_madd52lo_epu64(D1_20,alpah7,alpah13);
	D1_20 = _mm512_madd52lo_epu64(D1_20,alpah8,alpah12);
	D1_20 = _mm512_madd52lo_epu64(D1_20,alpah9,alpah11);
	D1_20 = _mm512_slli_epi64(D1_20,1);
	D1_20 = _mm512_madd52lo_epu64(D1_20,alpah10,alpah10);

	D1_21 = _mm512_madd52hi_epu64(zero,alpah1,alpah19);
	D1_21 = _mm512_madd52hi_epu64(D1_21,alpah2,alpah18);
	D1_21 = _mm512_madd52hi_epu64(D1_21,alpah3,alpah17);
	D1_21 = _mm512_madd52hi_epu64(D1_21,alpah4,alpah16);
	D1_21 = _mm512_madd52hi_epu64(D1_21,alpah5,alpah15);
	D1_21 = _mm512_madd52hi_epu64(D1_21,alpah6,alpah14);
	D1_21 = _mm512_madd52hi_epu64(D1_21,alpah7,alpah13);
	D1_21 = _mm512_madd52hi_epu64(D1_21,alpah8,alpah12);
	D1_21 = _mm512_madd52hi_epu64(D1_21,alpah9,alpah11);
	D1_21 = _mm512_madd52lo_epu64(D1_21,alpah2,alpah19);
	D1_21 = _mm512_madd52lo_epu64(D1_21,alpah3,alpah18);
	D1_21 = _mm512_madd52lo_epu64(D1_21,alpah4,alpah17);
	D1_21 = _mm512_madd52lo_epu64(D1_21,alpah5,alpah16);
	D1_21 = _mm512_madd52lo_epu64(D1_21,alpah6,alpah15);
	D1_21 = _mm512_madd52lo_epu64(D1_21,alpah7,alpah14);
	D1_21 = _mm512_madd52lo_epu64(D1_21,alpah8,alpah13);
	D1_21 = _mm512_madd52lo_epu64(D1_21,alpah9,alpah12);
	D1_21 = _mm512_madd52lo_epu64(D1_21,alpah10,alpah11);
	D1_21 = _mm512_slli_epi64(D1_21,1);
	D1_21 = _mm512_madd52hi_epu64(D1_21,alpah10,alpah10);

	D1_22 = _mm512_madd52hi_epu64(zero,alpah2,alpah19);
	D1_22 = _mm512_madd52hi_epu64(D1_22,alpah3,alpah18);
	D1_22 = _mm512_madd52hi_epu64(D1_22,alpah4,alpah17);
	D1_22 = _mm512_madd52hi_epu64(D1_22,alpah5,alpah16);
	D1_22 = _mm512_madd52hi_epu64(D1_22,alpah6,alpah15);
	D1_22 = _mm512_madd52hi_epu64(D1_22,alpah7,alpah14);
	D1_22 = _mm512_madd52hi_epu64(D1_22,alpah8,alpah13);
	D1_22 = _mm512_madd52hi_epu64(D1_22,alpah9,alpah12);
	D1_22 = _mm512_madd52hi_epu64(D1_22,alpah10,alpah11);
	D1_22 = _mm512_madd52lo_epu64(D1_22,alpah3,alpah19);
	D1_22 = _mm512_madd52lo_epu64(D1_22,alpah4,alpah18);
	D1_22 = _mm512_madd52lo_epu64(D1_22,alpah5,alpah17);
	D1_22 = _mm512_madd52lo_epu64(D1_22,alpah6,alpah16);
	D1_22 = _mm512_madd52lo_epu64(D1_22,alpah7,alpah15);
	D1_22 = _mm512_madd52lo_epu64(D1_22,alpah8,alpah14);
	D1_22 = _mm512_madd52lo_epu64(D1_22,alpah9,alpah13);
	D1_22 = _mm512_madd52lo_epu64(D1_22,alpah10,alpah12);
	D1_22 = _mm512_slli_epi64(D1_22,1);
	D1_22 = _mm512_madd52lo_epu64(D1_22,alpah11,alpah11);

	D1_23 = _mm512_madd52hi_epu64(zero,alpah3,alpah19);
	D1_23 = _mm512_madd52hi_epu64(D1_23,alpah4,alpah18);
	D1_23 = _mm512_madd52hi_epu64(D1_23,alpah5,alpah17);
	D1_23 = _mm512_madd52hi_epu64(D1_23,alpah6,alpah16);
	D1_23 = _mm512_madd52hi_epu64(D1_23,alpah7,alpah15);
	D1_23 = _mm512_madd52hi_epu64(D1_23,alpah8,alpah14);
	D1_23 = _mm512_madd52hi_epu64(D1_23,alpah9,alpah13);
	D1_23 = _mm512_madd52hi_epu64(D1_23,alpah10,alpah12);
	D1_23 = _mm512_madd52lo_epu64(D1_23,alpah4,alpah19);
	D1_23 = _mm512_madd52lo_epu64(D1_23,alpah5,alpah18);
	D1_23 = _mm512_madd52lo_epu64(D1_23,alpah6,alpah17);
	D1_23 = _mm512_madd52lo_epu64(D1_23,alpah7,alpah16);
	D1_23 = _mm512_madd52lo_epu64(D1_23,alpah8,alpah15);
	D1_23 = _mm512_madd52lo_epu64(D1_23,alpah9,alpah14);
	D1_23 = _mm512_madd52lo_epu64(D1_23,alpah10,alpah13);
	D1_23 = _mm512_madd52lo_epu64(D1_23,alpah11,alpah12);
	D1_23 = _mm512_slli_epi64(D1_23,1);
	D1_23 = _mm512_madd52hi_epu64(D1_23,alpah11,alpah11);

	D1_24 = _mm512_madd52hi_epu64(zero,alpah4,alpah19);
	D1_24 = _mm512_madd52hi_epu64(D1_24,alpah5,alpah18);
	D1_24 = _mm512_madd52hi_epu64(D1_24,alpah6,alpah17);
	D1_24 = _mm512_madd52hi_epu64(D1_24,alpah7,alpah16);
	D1_24 = _mm512_madd52hi_epu64(D1_24,alpah8,alpah15);
	D1_24 = _mm512_madd52hi_epu64(D1_24,alpah9,alpah14);
	D1_24 = _mm512_madd52hi_epu64(D1_24,alpah10,alpah13);
	D1_24 = _mm512_madd52hi_epu64(D1_24,alpah11,alpah12);
	D1_24 = _mm512_madd52lo_epu64(D1_24,alpah5,alpah19);
	D1_24 = _mm512_madd52lo_epu64(D1_24,alpah6,alpah18);
	D1_24 = _mm512_madd52lo_epu64(D1_24,alpah7,alpah17);
	D1_24 = _mm512_madd52lo_epu64(D1_24,alpah8,alpah16);
	D1_24 = _mm512_madd52lo_epu64(D1_24,alpah9,alpah15);
	D1_24 = _mm512_madd52lo_epu64(D1_24,alpah10,alpah14);
	D1_24 = _mm512_madd52lo_epu64(D1_24,alpah11,alpah13);
	D1_24 = _mm512_slli_epi64(D1_24,1);
	D1_24 = _mm512_madd52lo_epu64(D1_24,alpah12,alpah12);

	D1_25 = _mm512_madd52hi_epu64(zero,alpah5,alpah19);
	D1_25 = _mm512_madd52hi_epu64(D1_25,alpah6,alpah18);
	D1_25 = _mm512_madd52hi_epu64(D1_25,alpah7,alpah17);
	D1_25 = _mm512_madd52hi_epu64(D1_25,alpah8,alpah16);
	D1_25 = _mm512_madd52hi_epu64(D1_25,alpah9,alpah15);
	D1_25 = _mm512_madd52hi_epu64(D1_25,alpah10,alpah14);
	D1_25 = _mm512_madd52hi_epu64(D1_25,alpah11,alpah13);
	D1_25 = _mm512_madd52lo_epu64(D1_25,alpah6,alpah19);
	D1_25 = _mm512_madd52lo_epu64(D1_25,alpah7,alpah18);
	D1_25 = _mm512_madd52lo_epu64(D1_25,alpah8,alpah17);
	D1_25 = _mm512_madd52lo_epu64(D1_25,alpah9,alpah16);
	D1_25 = _mm512_madd52lo_epu64(D1_25,alpah10,alpah15);
	D1_25 = _mm512_madd52lo_epu64(D1_25,alpah11,alpah14);
	D1_25 = _mm512_madd52lo_epu64(D1_25,alpah12,alpah13);
	D1_25 = _mm512_slli_epi64(D1_25,1);
	D1_25 = _mm512_madd52hi_epu64(D1_25,alpah12,alpah12);

	D1_26 = _mm512_madd52hi_epu64(zero,alpah6,alpah19);
	D1_26 = _mm512_madd52hi_epu64(D1_26,alpah7,alpah18);
	D1_26 = _mm512_madd52hi_epu64(D1_26,alpah8,alpah17);
	D1_26 = _mm512_madd52hi_epu64(D1_26,alpah9,alpah16);
	D1_26 = _mm512_madd52hi_epu64(D1_26,alpah10,alpah15);
	D1_26 = _mm512_madd52hi_epu64(D1_26,alpah11,alpah14);
	D1_26 = _mm512_madd52hi_epu64(D1_26,alpah12,alpah13);
	D1_26 = _mm512_madd52lo_epu64(D1_26,alpah7,alpah19);
	D1_26 = _mm512_madd52lo_epu64(D1_26,alpah8,alpah18);
	D1_26 = _mm512_madd52lo_epu64(D1_26,alpah9,alpah17);
	D1_26 = _mm512_madd52lo_epu64(D1_26,alpah10,alpah16);
	D1_26 = _mm512_madd52lo_epu64(D1_26,alpah11,alpah15);
	D1_26 = _mm512_madd52lo_epu64(D1_26,alpah12,alpah14);
	D1_26 = _mm512_slli_epi64(D1_26,1);
	D1_26 = _mm512_madd52lo_epu64(D1_26,alpah13,alpah13);

	D1_27 = _mm512_madd52hi_epu64(zero,alpah7,alpah19);
	D1_27 = _mm512_madd52hi_epu64(D1_27,alpah8,alpah18);
	D1_27 = _mm512_madd52hi_epu64(D1_27,alpah9,alpah17);
	D1_27 = _mm512_madd52hi_epu64(D1_27,alpah10,alpah16);
	D1_27 = _mm512_madd52hi_epu64(D1_27,alpah11,alpah15);
	D1_27 = _mm512_madd52hi_epu64(D1_27,alpah12,alpah14);
	D1_27 = _mm512_madd52lo_epu64(D1_27,alpah8,alpah19);
	D1_27 = _mm512_madd52lo_epu64(D1_27,alpah9,alpah18);
	D1_27 = _mm512_madd52lo_epu64(D1_27,alpah10,alpah17);
	D1_27 = _mm512_madd52lo_epu64(D1_27,alpah11,alpah16);
	D1_27 = _mm512_madd52lo_epu64(D1_27,alpah12,alpah15);
	D1_27 = _mm512_madd52lo_epu64(D1_27,alpah13,alpah14);
	D1_27 = _mm512_slli_epi64(D1_27,1);
	D1_27 = _mm512_madd52hi_epu64(D1_27,alpah13,alpah13);

	D1_28 = _mm512_madd52hi_epu64(zero,alpah8,alpah19);
	D1_28 = _mm512_madd52hi_epu64(D1_28,alpah9,alpah18);
	D1_28 = _mm512_madd52hi_epu64(D1_28,alpah10,alpah17);
	D1_28 = _mm512_madd52hi_epu64(D1_28,alpah11,alpah16);
	D1_28 = _mm512_madd52hi_epu64(D1_28,alpah12,alpah15);
	D1_28 = _mm512_madd52hi_epu64(D1_28,alpah13,alpah14);
	D1_28 = _mm512_madd52lo_epu64(D1_28,alpah9,alpah19);
	D1_28 = _mm512_madd52lo_epu64(D1_28,alpah10,alpah18);
	D1_28 = _mm512_madd52lo_epu64(D1_28,alpah11,alpah17);
	D1_28 = _mm512_madd52lo_epu64(D1_28,alpah12,alpah16);
	D1_28 = _mm512_madd52lo_epu64(D1_28,alpah13,alpah15);
	D1_28 = _mm512_slli_epi64(D1_28,1);
	D1_28 = _mm512_madd52lo_epu64(D1_28,alpah14,alpah14);

	D1_29 = _mm512_madd52hi_epu64(zero,alpah9,alpah19);
	D1_29 = _mm512_madd52hi_epu64(D1_29,alpah10,alpah18);
	D1_29 = _mm512_madd52hi_epu64(D1_29,alpah11,alpah17);
	D1_29 = _mm512_madd52hi_epu64(D1_29,alpah12,alpah16);
	D1_29 = _mm512_madd52hi_epu64(D1_29,alpah13,alpah15);
	D1_29 = _mm512_madd52lo_epu64(D1_29,alpah10,alpah19);
	D1_29 = _mm512_madd52lo_epu64(D1_29,alpah11,alpah18);
	D1_29 = _mm512_madd52lo_epu64(D1_29,alpah12,alpah17);
	D1_29 = _mm512_madd52lo_epu64(D1_29,alpah13,alpah16);
	D1_29 = _mm512_madd52lo_epu64(D1_29,alpah14,alpah15);
	D1_29 = _mm512_slli_epi64(D1_29,1);
	D1_29 = _mm512_madd52hi_epu64(D1_29,alpah14,alpah14);

	D1_30 = _mm512_madd52hi_epu64(zero,alpah10,alpah19);
	D1_30 = _mm512_madd52hi_epu64(D1_30,alpah11,alpah18);
	D1_30 = _mm512_madd52hi_epu64(D1_30,alpah12,alpah17);
	D1_30 = _mm512_madd52hi_epu64(D1_30,alpah13,alpah16);
	D1_30 = _mm512_madd52hi_epu64(D1_30,alpah14,alpah15);
	D1_30 = _mm512_madd52lo_epu64(D1_30,alpah11,alpah19);
	D1_30 = _mm512_madd52lo_epu64(D1_30,alpah12,alpah18);
	D1_30 = _mm512_madd52lo_epu64(D1_30,alpah13,alpah17);
	D1_30 = _mm512_madd52lo_epu64(D1_30,alpah14,alpah16);
	D1_30 = _mm512_slli_epi64(D1_30,1);
	D1_30 = _mm512_madd52lo_epu64(D1_30,alpah15,alpah15);

	D1_31 = _mm512_madd52hi_epu64(zero,alpah11,alpah19);
	D1_31 = _mm512_madd52hi_epu64(D1_31,alpah12,alpah18);
	D1_31 = _mm512_madd52hi_epu64(D1_31,alpah13,alpah17);
	D1_31 = _mm512_madd52hi_epu64(D1_31,alpah14,alpah16);
	D1_31 = _mm512_madd52lo_epu64(D1_31,alpah12,alpah19);
	D1_31 = _mm512_madd52lo_epu64(D1_31,alpah13,alpah18);
	D1_31 = _mm512_madd52lo_epu64(D1_31,alpah14,alpah17);
	D1_31 = _mm512_madd52lo_epu64(D1_31,alpah15,alpah16);
	D1_31 = _mm512_slli_epi64(D1_31,1);
	D1_31 = _mm512_madd52hi_epu64(D1_31,alpah15,alpah15);

	D1_32 = _mm512_madd52hi_epu64(zero,alpah12,alpah19);
	D1_32 = _mm512_madd52hi_epu64(D1_32,alpah13,alpah18);
	D1_32 = _mm512_madd52hi_epu64(D1_32,alpah14,alpah17);
	D1_32 = _mm512_madd52hi_epu64(D1_32,alpah15,alpah16);
	D1_32 = _mm512_madd52lo_epu64(D1_32,alpah13,alpah19);
	D1_32 = _mm512_madd52lo_epu64(D1_32,alpah14,alpah18);
	D1_32 = _mm512_madd52lo_epu64(D1_32,alpah15,alpah17);
	D1_32 = _mm512_slli_epi64(D1_32,1);
	D1_32 = _mm512_madd52lo_epu64(D1_32,alpah16,alpah16);

	D1_33 = _mm512_madd52hi_epu64(zero,alpah13,alpah19);
	D1_33 = _mm512_madd52hi_epu64(D1_33,alpah14,alpah18);
	D1_33 = _mm512_madd52hi_epu64(D1_33,alpah15,alpah17);
	D1_33 = _mm512_madd52lo_epu64(D1_33,alpah14,alpah19);
	D1_33 = _mm512_madd52lo_epu64(D1_33,alpah15,alpah18);
	D1_33 = _mm512_madd52lo_epu64(D1_33,alpah16,alpah17);
	D1_33 = _mm512_slli_epi64(D1_33,1);
	D1_33 = _mm512_madd52hi_epu64(D1_33,alpah16,alpah16);

	D1_34 = _mm512_madd52hi_epu64(zero,alpah14,alpah19);
	D1_34 = _mm512_madd52hi_epu64(D1_34,alpah15,alpah18);
	D1_34 = _mm512_madd52hi_epu64(D1_34,alpah16,alpah17);
	D1_34 = _mm512_madd52lo_epu64(D1_34,alpah15,alpah19);
	D1_34 = _mm512_madd52lo_epu64(D1_34,alpah16,alpah18);
	D1_34 = _mm512_slli_epi64(D1_34,1);
	D1_34 = _mm512_madd52lo_epu64(D1_34,alpah17,alpah17);

	D1_35 = _mm512_madd52hi_epu64(zero,alpah15,alpah19);
	D1_35 = _mm512_madd52hi_epu64(D1_35,alpah16,alpah18);
	D1_35 = _mm512_madd52lo_epu64(D1_35,alpah16,alpah19);
	D1_35 = _mm512_madd52lo_epu64(D1_35,alpah17,alpah18);
	D1_35 = _mm512_slli_epi64(D1_35,1);
	D1_35 = _mm512_madd52hi_epu64(D1_35,alpah17,alpah17);

	D1_36 = _mm512_madd52hi_epu64(zero,alpah16,alpah19);
	D1_36 = _mm512_madd52hi_epu64(D1_36,alpah17,alpah18);
	D1_36 = _mm512_madd52lo_epu64(D1_36,alpah17,alpah19);
	D1_36 = _mm512_slli_epi64(D1_36,1);
	D1_36 = _mm512_madd52lo_epu64(D1_36,alpah18,alpah18);

	D1_37 = _mm512_madd52hi_epu64(zero,alpah17,alpah19);
	D1_37 = _mm512_madd52lo_epu64(D1_37,alpah18,alpah19);
	D1_37 = _mm512_slli_epi64(D1_37,1);
	D1_37 = _mm512_madd52hi_epu64(D1_37,alpah18,alpah18);

	D1_38 = _mm512_madd52hi_epu64(zero,alpah18,alpah19);
	D1_38 = _mm512_slli_epi64(D1_38,1);
	D1_38 = _mm512_madd52lo_epu64(D1_38,alpah19,alpah19);

	D1_39 = _mm512_madd52hi_epu64(carry,alpah19,alpah19);


	// Carry management

	carry  = _mm512_srli_epi64(D1_0,52);
	D1_0 = _mm512_and_si512(D1_0,mask52);
D1_1 = _mm512_add_epi64(D0_21,D1_1);	D1_1 = _mm512_add_epi64(D1_1,carry);
	carry  = _mm512_srli_epi64(D1_1,52);
D1_2 = _mm512_add_epi64(D0_22,D1_2);	D1_2 = _mm512_add_epi64(D1_2,carry);
	D1_1 = _mm512_and_si512(D1_1,mask52);
	carry  = _mm512_srli_epi64(D1_2,52);
D1_3 = _mm512_add_epi64(D0_23,D1_3);	D1_3 = _mm512_add_epi64(D1_3,carry);
	D1_2 = _mm512_and_si512(D1_2,mask52);
	carry  = _mm512_srli_epi64(D1_3,52);
D1_4 = _mm512_add_epi64(D0_24,D1_4);	D1_4 = _mm512_add_epi64(D1_4,carry);
	D1_3 = _mm512_and_si512(D1_3,mask52);
	carry  = _mm512_srli_epi64(D1_4,52);
D1_5 = _mm512_add_epi64(D0_25,D1_5);	D1_5 = _mm512_add_epi64(D1_5,carry);
	D1_4 = _mm512_and_si512(D1_4,mask52);
	carry  = _mm512_srli_epi64(D1_5,52);
D1_6 = _mm512_add_epi64(D0_26,D1_6);	D1_6 = _mm512_add_epi64(D1_6,carry);
	D1_5 = _mm512_and_si512(D1_5,mask52);
	carry  = _mm512_srli_epi64(D1_6,52);
D1_7 = _mm512_add_epi64(D0_27,D1_7);	D1_7 = _mm512_add_epi64(D1_7,carry);
	D1_6 = _mm512_and_si512(D1_6,mask52);
	carry  = _mm512_srli_epi64(D1_7,52);
D1_8 = _mm512_add_epi64(D0_28,D1_8);	D1_8 = _mm512_add_epi64(D1_8,carry);
	D1_7 = _mm512_and_si512(D1_7,mask52);
	carry  = _mm512_srli_epi64(D1_8,52);
D1_9 = _mm512_add_epi64(D0_29,D1_9);	D1_9 = _mm512_add_epi64(D1_9,carry);
	D1_8 = _mm512_and_si512(D1_8,mask52);
	carry  = _mm512_srli_epi64(D1_9,52);
D1_10 = _mm512_add_epi64(D0_30,D1_10);	D1_10 = _mm512_add_epi64(D1_10,carry);
	D1_9 = _mm512_and_si512(D1_9,mask52);
	carry  = _mm512_srli_epi64(D1_10,52);
D1_11 = _mm512_add_epi64(D0_31,D1_11);	D1_11 = _mm512_add_epi64(D1_11,carry);
	D1_10 = _mm512_and_si512(D1_10,mask52);
	carry  = _mm512_srli_epi64(D1_11,52);
D1_12 = _mm512_add_epi64(D0_32,D1_12);	D1_12 = _mm512_add_epi64(D1_12,carry);
	D1_11 = _mm512_and_si512(D1_11,mask52);
	carry  = _mm512_srli_epi64(D1_12,52);
D1_13 = _mm512_add_epi64(D0_33,D1_13);	D1_13 = _mm512_add_epi64(D1_13,carry);
	D1_12 = _mm512_and_si512(D1_12,mask52);
	carry  = _mm512_srli_epi64(D1_13,52);
D1_14 = _mm512_add_epi64(D0_34,D1_14);	D1_14 = _mm512_add_epi64(D1_14,carry);
	D1_13 = _mm512_and_si512(D1_13,mask52);
	carry  = _mm512_srli_epi64(D1_14,52);
D1_15 = _mm512_add_epi64(D0_35,D1_15);	D1_15 = _mm512_add_epi64(D1_15,carry);
	D1_14 = _mm512_and_si512(D1_14,mask52);
	carry  = _mm512_srli_epi64(D1_15,52);
D1_16 = _mm512_add_epi64(D0_36,D1_16);	D1_16 = _mm512_add_epi64(D1_16,carry);
	D1_15 = _mm512_and_si512(D1_15,mask52);
	carry  = _mm512_srli_epi64(D1_16,52);
D1_17 = _mm512_add_epi64(D0_37,D1_17);	D1_17 = _mm512_add_epi64(D1_17,carry);
	D1_16 = _mm512_and_si512(D1_16,mask52);
	carry  = _mm512_srli_epi64(D1_17,52);
D1_18 = _mm512_add_epi64(D0_38,D1_18);	D1_18 = _mm512_add_epi64(D1_18,carry);
	D1_17 = _mm512_and_si512(D1_17,mask52);
	carry  = _mm512_srli_epi64(D1_18,52);
D1_19 = _mm512_add_epi64(D0_39,D1_19);	D1_19 = _mm512_add_epi64(D1_19,carry);
	D1_18 = _mm512_and_si512(D1_18,mask52);

	// Conversion in base 2^1039

	carry = D1_19>>51;
	D1_20 = (D1_20<<1)+carry;
	D1_19 &= mask51;
	D1_20 = _mm512_add_epi64(D1_20,D2_0);	carry = D1_20>>52;
	D1_21 = (D1_21<<1)+carry;
	D1_20 &= mask52;
	D1_21 = _mm512_add_epi64(D1_21,D2_1);
	carry = D1_21>>52;
	D1_22 = (D1_22<<1)+carry;
	D1_21 &= mask52;
	D1_22 = _mm512_add_epi64(D1_22,D2_2);
	carry = D1_22>>52;
	D1_23 = (D1_23<<1)+carry;
	D1_22 &= mask52;
	D1_23 = _mm512_add_epi64(D1_23,D2_3);
	carry = D1_23>>52;
	D1_24 = (D1_24<<1)+carry;
	D1_23 &= mask52;
	D1_24 = _mm512_add_epi64(D1_24,D2_4);
	carry = D1_24>>52;
	D1_25 = (D1_25<<1)+carry;
	D1_24 &= mask52;
	D1_25 = _mm512_add_epi64(D1_25,D2_5);
	carry = D1_25>>52;
	D1_26 = (D1_26<<1)+carry;
	D1_25 &= mask52;
	D1_26 = _mm512_add_epi64(D1_26,D2_6);
	carry = D1_26>>52;
	D1_27 = (D1_27<<1)+carry;
	D1_26 &= mask52;
	D1_27 = _mm512_add_epi64(D1_27,D2_7);
	carry = D1_27>>52;
	D1_28 = (D1_28<<1)+carry;
	D1_27 &= mask52;
	D1_28 = _mm512_add_epi64(D1_28,D2_8);
	carry = D1_28>>52;
	D1_29 = (D1_29<<1)+carry;
	D1_28 &= mask52;
	D1_29 = _mm512_add_epi64(D1_29,D2_9);
	carry = D1_29>>52;
	D1_30 = (D1_30<<1)+carry;
	D1_29 &= mask52;
	D1_30 = _mm512_add_epi64(D1_30,D2_10);
	carry = D1_30>>52;
	D1_31 = (D1_31<<1)+carry;
	D1_30 &= mask52;
	D1_31 = _mm512_add_epi64(D1_31,D2_11);
	carry = D1_31>>52;
	D1_32 = (D1_32<<1)+carry;
	D1_31 &= mask52;
	D1_32 = _mm512_add_epi64(D1_32,D2_12);
	carry = D1_32>>52;
	D1_33 = (D1_33<<1)+carry;
	D1_32 &= mask52;
	D1_33 = _mm512_add_epi64(D1_33,D2_13);
	carry = D1_33>>52;
	D1_34 = (D1_34<<1)+carry;
	D1_33 &= mask52;
	D1_34 = _mm512_add_epi64(D1_34,D2_14);
	carry = D1_34>>52;
	D1_35 = (D1_35<<1)+carry;
	D1_34 &= mask52;
	D1_35 = _mm512_add_epi64(D1_35,D2_15);
	carry = D1_35>>52;
	D1_36 = (D1_36<<1)+carry;
	D1_35 &= mask52;
	D1_36 = _mm512_add_epi64(D1_36,D2_16);
	carry = D1_36>>52;
	D1_37 = (D1_37<<1)+carry;
	D1_36 &= mask52;
	D1_37 = _mm512_add_epi64(D1_37,D2_17);
	carry = D1_37>>52;
	D1_38 = (D1_38<<1)+carry;
	D1_37 &= mask52;
	D1_38 = _mm512_add_epi64(D1_38,D2_18);
	carry = D1_38>>52;
	D1_39 = (D1_39<<1)+carry;
	D1_38 &= mask52;
	D1_39 = _mm512_add_epi64(D1_39,D2_19);
	carry = D1_39>>51;
	D1_39 &= mask51;

	// Final reconstruction


	// tmp = D0+D2, radix 2^1039

	__m512i tmp0 = _mm512_add_epi64(D0_0,D2_0);
	__m512i carry_tmp  = _mm512_srli_epi64(tmp0,52);
	tmp0 = _mm512_and_si512(tmp0,mask52);

	__m512i tmp1 = _mm512_add_epi64(D0_1,carry_tmp);
	tmp1 = _mm512_add_epi64(tmp1,D2_1);
	carry_tmp  = _mm512_srli_epi64(tmp1,52);
	tmp1 = _mm512_and_si512(tmp1,mask52);

	__m512i tmp2 = _mm512_add_epi64(D0_2,carry_tmp);
	tmp2 = _mm512_add_epi64(tmp2,D2_2);
	carry_tmp  = _mm512_srli_epi64(tmp2,52);
	tmp2 = _mm512_and_si512(tmp2,mask52);

	__m512i tmp3 = _mm512_add_epi64(D0_3,carry_tmp);
	tmp3 = _mm512_add_epi64(tmp3,D2_3);
	carry_tmp  = _mm512_srli_epi64(tmp3,52);
	tmp3 = _mm512_and_si512(tmp3,mask52);

	__m512i tmp4 = _mm512_add_epi64(D0_4,carry_tmp);
	tmp4 = _mm512_add_epi64(tmp4,D2_4);
	carry_tmp  = _mm512_srli_epi64(tmp4,52);
	tmp4 = _mm512_and_si512(tmp4,mask52);

	__m512i tmp5 = _mm512_add_epi64(D0_5,carry_tmp);
	tmp5 = _mm512_add_epi64(tmp5,D2_5);
	carry_tmp  = _mm512_srli_epi64(tmp5,52);
	tmp5 = _mm512_and_si512(tmp5,mask52);

	__m512i tmp6 = _mm512_add_epi64(D0_6,carry_tmp);
	tmp6 = _mm512_add_epi64(tmp6,D2_6);
	carry_tmp  = _mm512_srli_epi64(tmp6,52);
	tmp6 = _mm512_and_si512(tmp6,mask52);

	__m512i tmp7 = _mm512_add_epi64(D0_7,carry_tmp);
	tmp7 = _mm512_add_epi64(tmp7,D2_7);
	carry_tmp  = _mm512_srli_epi64(tmp7,52);
	tmp7 = _mm512_and_si512(tmp7,mask52);

	__m512i tmp8 = _mm512_add_epi64(D0_8,carry_tmp);
	tmp8 = _mm512_add_epi64(tmp8,D2_8);
	carry_tmp  = _mm512_srli_epi64(tmp8,52);
	tmp8 = _mm512_and_si512(tmp8,mask52);

	__m512i tmp9 = _mm512_add_epi64(D0_9,carry_tmp);
	tmp9 = _mm512_add_epi64(tmp9,D2_9);
	carry_tmp  = _mm512_srli_epi64(tmp9,52);
	tmp9 = _mm512_and_si512(tmp9,mask52);

	__m512i tmp10 = _mm512_add_epi64(D0_10,carry_tmp);
	tmp10 = _mm512_add_epi64(tmp10,D2_10);
	carry_tmp  = _mm512_srli_epi64(tmp10,52);
	tmp10 = _mm512_and_si512(tmp10,mask52);

	__m512i tmp11 = _mm512_add_epi64(D0_11,carry_tmp);
	tmp11 = _mm512_add_epi64(tmp11,D2_11);
	carry_tmp  = _mm512_srli_epi64(tmp11,52);
	tmp11 = _mm512_and_si512(tmp11,mask52);

	__m512i tmp12 = _mm512_add_epi64(D0_12,carry_tmp);
	tmp12 = _mm512_add_epi64(tmp12,D2_12);
	carry_tmp  = _mm512_srli_epi64(tmp12,52);
	tmp12 = _mm512_and_si512(tmp12,mask52);

	__m512i tmp13 = _mm512_add_epi64(D0_13,carry_tmp);
	tmp13 = _mm512_add_epi64(tmp13,D2_13);
	carry_tmp  = _mm512_srli_epi64(tmp13,52);
	tmp13 = _mm512_and_si512(tmp13,mask52);

	__m512i tmp14 = _mm512_add_epi64(D0_14,carry_tmp);
	tmp14 = _mm512_add_epi64(tmp14,D2_14);
	carry_tmp  = _mm512_srli_epi64(tmp14,52);
	tmp14 = _mm512_and_si512(tmp14,mask52);

	__m512i tmp15 = _mm512_add_epi64(D0_15,carry_tmp);
	tmp15 = _mm512_add_epi64(tmp15,D2_15);
	carry_tmp  = _mm512_srli_epi64(tmp15,52);
	tmp15 = _mm512_and_si512(tmp15,mask52);

	__m512i tmp16 = _mm512_add_epi64(D0_16,carry_tmp);
	tmp16 = _mm512_add_epi64(tmp16,D2_16);
	carry_tmp  = _mm512_srli_epi64(tmp16,52);
	tmp16 = _mm512_and_si512(tmp16,mask52);

	__m512i tmp17 = _mm512_add_epi64(D0_17,carry_tmp);
	tmp17 = _mm512_add_epi64(tmp17,D2_17);
	carry_tmp  = _mm512_srli_epi64(tmp17,52);
	tmp17 = _mm512_and_si512(tmp17,mask52);

	__m512i tmp18 = _mm512_add_epi64(D0_18,carry_tmp);
	tmp18 = _mm512_add_epi64(tmp18,D2_18);
	carry_tmp  = _mm512_srli_epi64(tmp18,52);
	tmp18 = _mm512_and_si512(tmp18,mask52);

	__m512i tmp19 = _mm512_add_epi64(D0_19,carry_tmp);
	tmp19 = _mm512_add_epi64(tmp19,D2_19);
	carry_tmp  = _mm512_srli_epi64(tmp19,51);
	tmp19 = _mm512_and_si512(tmp19,mask51);// radix 2**1039

	__m512i tmp20 = _mm512_add_epi64(D0_20,carry_tmp);
	tmp20 = _mm512_add_epi64(tmp20,D2_20);
	carry_tmp  = _mm512_srli_epi64(tmp20,52);
	tmp20 = _mm512_and_si512(tmp20,mask52);

	__m512i tmp21 = _mm512_add_epi64(D0_21,carry_tmp);
	tmp21 = _mm512_add_epi64(tmp21,D2_21);
	carry_tmp  = _mm512_srli_epi64(tmp21,52);
	tmp21 = _mm512_and_si512(tmp21,mask52);

	__m512i tmp22 = _mm512_add_epi64(D0_22,carry_tmp);
	tmp22 = _mm512_add_epi64(tmp22,D2_22);
	carry_tmp  = _mm512_srli_epi64(tmp22,52);
	tmp22 = _mm512_and_si512(tmp22,mask52);

	__m512i tmp23 = _mm512_add_epi64(D0_23,carry_tmp);
	tmp23 = _mm512_add_epi64(tmp23,D2_23);
	carry_tmp  = _mm512_srli_epi64(tmp23,52);
	tmp23 = _mm512_and_si512(tmp23,mask52);

	__m512i tmp24 = _mm512_add_epi64(D0_24,carry_tmp);
	tmp24 = _mm512_add_epi64(tmp24,D2_24);
	carry_tmp  = _mm512_srli_epi64(tmp24,52);
	tmp24 = _mm512_and_si512(tmp24,mask52);

	__m512i tmp25 = _mm512_add_epi64(D0_25,carry_tmp);
	tmp25 = _mm512_add_epi64(tmp25,D2_25);
	carry_tmp  = _mm512_srli_epi64(tmp25,52);
	tmp25 = _mm512_and_si512(tmp25,mask52);

	__m512i tmp26 = _mm512_add_epi64(D0_26,carry_tmp);
	tmp26 = _mm512_add_epi64(tmp26,D2_26);
	carry_tmp  = _mm512_srli_epi64(tmp26,52);
	tmp26 = _mm512_and_si512(tmp26,mask52);

	__m512i tmp27 = _mm512_add_epi64(D0_27,carry_tmp);
	tmp27 = _mm512_add_epi64(tmp27,D2_27);
	carry_tmp  = _mm512_srli_epi64(tmp27,52);
	tmp27 = _mm512_and_si512(tmp27,mask52);

	__m512i tmp28 = _mm512_add_epi64(D0_28,carry_tmp);
	tmp28 = _mm512_add_epi64(tmp28,D2_28);
	carry_tmp  = _mm512_srli_epi64(tmp28,52);
	tmp28 = _mm512_and_si512(tmp28,mask52);

	__m512i tmp29 = _mm512_add_epi64(D0_29,carry_tmp);
	tmp29 = _mm512_add_epi64(tmp29,D2_29);
	carry_tmp  = _mm512_srli_epi64(tmp29,52);
	tmp29 = _mm512_and_si512(tmp29,mask52);

	__m512i tmp30 = _mm512_add_epi64(D0_30,carry_tmp);
	tmp30 = _mm512_add_epi64(tmp30,D2_30);
	carry_tmp  = _mm512_srli_epi64(tmp30,52);
	tmp30 = _mm512_and_si512(tmp30,mask52);

	__m512i tmp31 = _mm512_add_epi64(D0_31,carry_tmp);
	tmp31 = _mm512_add_epi64(tmp31,D2_31);
	carry_tmp  = _mm512_srli_epi64(tmp31,52);
	tmp31 = _mm512_and_si512(tmp31,mask52);

	__m512i tmp32 = _mm512_add_epi64(D0_32,carry_tmp);
	tmp32 = _mm512_add_epi64(tmp32,D2_32);
	carry_tmp  = _mm512_srli_epi64(tmp32,52);
	tmp32 = _mm512_and_si512(tmp32,mask52);

	__m512i tmp33 = _mm512_add_epi64(D0_33,carry_tmp);
	tmp33 = _mm512_add_epi64(tmp33,D2_33);
	carry_tmp  = _mm512_srli_epi64(tmp33,52);
	tmp33 = _mm512_and_si512(tmp33,mask52);

	__m512i tmp34 = _mm512_add_epi64(D0_34,carry_tmp);
	tmp34 = _mm512_add_epi64(tmp34,D2_34);
	carry_tmp  = _mm512_srli_epi64(tmp34,52);
	tmp34 = _mm512_and_si512(tmp34,mask52);

	__m512i tmp35 = _mm512_add_epi64(D0_35,carry_tmp);
	tmp35 = _mm512_add_epi64(tmp35,D2_35);
	carry_tmp  = _mm512_srli_epi64(tmp35,52);
	tmp35 = _mm512_and_si512(tmp35,mask52);

	__m512i tmp36 = _mm512_add_epi64(D0_36,carry_tmp);
	tmp36 = _mm512_add_epi64(tmp36,D2_36);
	carry_tmp  = _mm512_srli_epi64(tmp36,52);
	tmp36 = _mm512_and_si512(tmp36,mask52);

	__m512i tmp37 = _mm512_add_epi64(D0_37,carry_tmp);
	tmp37 = _mm512_add_epi64(tmp37,D2_37);
	carry_tmp  = _mm512_srli_epi64(tmp37,52);
	tmp37 = _mm512_and_si512(tmp37,mask52);

	__m512i tmp38 = _mm512_add_epi64(D0_38,carry_tmp);
	tmp38 = _mm512_add_epi64(tmp38,D2_38);
	carry_tmp  = _mm512_srli_epi64(tmp38,52);
	tmp38 = _mm512_and_si512(tmp38,mask52);

	__m512i tmp39 = _mm512_add_epi64(D0_39,carry_tmp);
	tmp39 = _mm512_add_epi64(tmp39,D2_39);
	carry_tmp  = _mm512_srli_epi64(tmp39,51);
	tmp39 = _mm512_and_si512(tmp39,mask51);// radix 2**1039




	// D1 = D1 -tmp, radix 2^1039

	D1_0 = _mm512_sub_epi64(D1_0,tmp0);
	__m512i borrow = _mm512_srli_epi64(D1_0,52)&un;
	D1_0 = _mm512_and_si512(D1_0,mask52);
	borrow = _mm512_add_epi64(tmp1,borrow);
	D1_1 = _mm512_sub_epi64(D1_1,borrow);
	borrow  = _mm512_srli_epi64(D1_1,52)&un;
	D1_1 = _mm512_and_si512(D1_1,mask52);

	borrow = _mm512_add_epi64(tmp2,borrow);
	D1_2 = _mm512_sub_epi64(D1_2,borrow);
	borrow  = _mm512_srli_epi64(D1_2,52)&un;
	D1_2 = _mm512_and_si512(D1_2,mask52);

	borrow = _mm512_add_epi64(tmp3,borrow);
	D1_3 = _mm512_sub_epi64(D1_3,borrow);
	borrow  = _mm512_srli_epi64(D1_3,52)&un;
	D1_3 = _mm512_and_si512(D1_3,mask52);

	borrow = _mm512_add_epi64(tmp4,borrow);
	D1_4 = _mm512_sub_epi64(D1_4,borrow);
	borrow  = _mm512_srli_epi64(D1_4,52)&un;
	D1_4 = _mm512_and_si512(D1_4,mask52);

	borrow = _mm512_add_epi64(tmp5,borrow);
	D1_5 = _mm512_sub_epi64(D1_5,borrow);
	borrow  = _mm512_srli_epi64(D1_5,52)&un;
	D1_5 = _mm512_and_si512(D1_5,mask52);

	borrow = _mm512_add_epi64(tmp6,borrow);
	D1_6 = _mm512_sub_epi64(D1_6,borrow);
	borrow  = _mm512_srli_epi64(D1_6,52)&un;
	D1_6 = _mm512_and_si512(D1_6,mask52);

	borrow = _mm512_add_epi64(tmp7,borrow);
	D1_7 = _mm512_sub_epi64(D1_7,borrow);
	borrow  = _mm512_srli_epi64(D1_7,52)&un;
	D1_7 = _mm512_and_si512(D1_7,mask52);

	borrow = _mm512_add_epi64(tmp8,borrow);
	D1_8 = _mm512_sub_epi64(D1_8,borrow);
	borrow  = _mm512_srli_epi64(D1_8,52)&un;
	D1_8 = _mm512_and_si512(D1_8,mask52);

	borrow = _mm512_add_epi64(tmp9,borrow);
	D1_9 = _mm512_sub_epi64(D1_9,borrow);
	borrow  = _mm512_srli_epi64(D1_9,52)&un;
	D1_9 = _mm512_and_si512(D1_9,mask52);

	borrow = _mm512_add_epi64(tmp10,borrow);
	D1_10 = _mm512_sub_epi64(D1_10,borrow);
	borrow  = _mm512_srli_epi64(D1_10,52)&un;
	D1_10 = _mm512_and_si512(D1_10,mask52);

	borrow = _mm512_add_epi64(tmp11,borrow);
	D1_11 = _mm512_sub_epi64(D1_11,borrow);
	borrow  = _mm512_srli_epi64(D1_11,52)&un;
	D1_11 = _mm512_and_si512(D1_11,mask52);

	borrow = _mm512_add_epi64(tmp12,borrow);
	D1_12 = _mm512_sub_epi64(D1_12,borrow);
	borrow  = _mm512_srli_epi64(D1_12,52)&un;
	D1_12 = _mm512_and_si512(D1_12,mask52);

	borrow = _mm512_add_epi64(tmp13,borrow);
	D1_13 = _mm512_sub_epi64(D1_13,borrow);
	borrow  = _mm512_srli_epi64(D1_13,52)&un;
	D1_13 = _mm512_and_si512(D1_13,mask52);

	borrow = _mm512_add_epi64(tmp14,borrow);
	D1_14 = _mm512_sub_epi64(D1_14,borrow);
	borrow  = _mm512_srli_epi64(D1_14,52)&un;
	D1_14 = _mm512_and_si512(D1_14,mask52);

	borrow = _mm512_add_epi64(tmp15,borrow);
	D1_15 = _mm512_sub_epi64(D1_15,borrow);
	borrow  = _mm512_srli_epi64(D1_15,52)&un;
	D1_15 = _mm512_and_si512(D1_15,mask52);

	borrow = _mm512_add_epi64(tmp16,borrow);
	D1_16 = _mm512_sub_epi64(D1_16,borrow);
	borrow  = _mm512_srli_epi64(D1_16,52)&un;
	D1_16 = _mm512_and_si512(D1_16,mask52);

	borrow = _mm512_add_epi64(tmp17,borrow);
	D1_17 = _mm512_sub_epi64(D1_17,borrow);
	borrow  = _mm512_srli_epi64(D1_17,52)&un;
	D1_17 = _mm512_and_si512(D1_17,mask52);

	borrow = _mm512_add_epi64(tmp18,borrow);
	D1_18 = _mm512_sub_epi64(D1_18,borrow);
	borrow  = _mm512_srli_epi64(D1_18,52)&un;
	D1_18 = _mm512_and_si512(D1_18,mask52);

	borrow = _mm512_add_epi64(tmp19,borrow);
	D1_19 = _mm512_sub_epi64(D1_19,borrow);
	borrow = _mm512_srli_epi64(D1_19,51)&un;
	D1_19 = _mm512_and_si512(D1_19,mask51);// radix 2**1039

	borrow = _mm512_add_epi64(tmp20,borrow);
	D1_20 = _mm512_sub_epi64(D1_20,borrow);
	borrow  = _mm512_srli_epi64(D1_20,52)&un;
	D1_20 = _mm512_and_si512(D1_20,mask52);

	borrow = _mm512_add_epi64(tmp21,borrow);
	D1_21 = _mm512_sub_epi64(D1_21,borrow);
	borrow  = _mm512_srli_epi64(D1_21,52)&un;
	D1_21 = _mm512_and_si512(D1_21,mask52);

	borrow = _mm512_add_epi64(tmp22,borrow);
	D1_22 = _mm512_sub_epi64(D1_22,borrow);
	borrow  = _mm512_srli_epi64(D1_22,52)&un;
	D1_22 = _mm512_and_si512(D1_22,mask52);

	borrow = _mm512_add_epi64(tmp23,borrow);
	D1_23 = _mm512_sub_epi64(D1_23,borrow);
	borrow  = _mm512_srli_epi64(D1_23,52)&un;
	D1_23 = _mm512_and_si512(D1_23,mask52);

	borrow = _mm512_add_epi64(tmp24,borrow);
	D1_24 = _mm512_sub_epi64(D1_24,borrow);
	borrow  = _mm512_srli_epi64(D1_24,52)&un;
	D1_24 = _mm512_and_si512(D1_24,mask52);

	borrow = _mm512_add_epi64(tmp25,borrow);
	D1_25 = _mm512_sub_epi64(D1_25,borrow);
	borrow  = _mm512_srli_epi64(D1_25,52)&un;
	D1_25 = _mm512_and_si512(D1_25,mask52);

	borrow = _mm512_add_epi64(tmp26,borrow);
	D1_26 = _mm512_sub_epi64(D1_26,borrow);
	borrow  = _mm512_srli_epi64(D1_26,52)&un;
	D1_26 = _mm512_and_si512(D1_26,mask52);

	borrow = _mm512_add_epi64(tmp27,borrow);
	D1_27 = _mm512_sub_epi64(D1_27,borrow);
	borrow  = _mm512_srli_epi64(D1_27,52)&un;
	D1_27 = _mm512_and_si512(D1_27,mask52);

	borrow = _mm512_add_epi64(tmp28,borrow);
	D1_28 = _mm512_sub_epi64(D1_28,borrow);
	borrow  = _mm512_srli_epi64(D1_28,52)&un;
	D1_28 = _mm512_and_si512(D1_28,mask52);

	borrow = _mm512_add_epi64(tmp29,borrow);
	D1_29 = _mm512_sub_epi64(D1_29,borrow);
	borrow  = _mm512_srli_epi64(D1_29,52)&un;
	D1_29 = _mm512_and_si512(D1_29,mask52);

	borrow = _mm512_add_epi64(tmp30,borrow);
	D1_30 = _mm512_sub_epi64(D1_30,borrow);
	borrow  = _mm512_srli_epi64(D1_30,52)&un;
	D1_30 = _mm512_and_si512(D1_30,mask52);

	borrow = _mm512_add_epi64(tmp31,borrow);
	D1_31 = _mm512_sub_epi64(D1_31,borrow);
	borrow  = _mm512_srli_epi64(D1_31,52)&un;
	D1_31 = _mm512_and_si512(D1_31,mask52);

	borrow = _mm512_add_epi64(tmp32,borrow);
	D1_32 = _mm512_sub_epi64(D1_32,borrow);
	borrow  = _mm512_srli_epi64(D1_32,52)&un;
	D1_32 = _mm512_and_si512(D1_32,mask52);

	borrow = _mm512_add_epi64(tmp33,borrow);
	D1_33 = _mm512_sub_epi64(D1_33,borrow);
	borrow  = _mm512_srli_epi64(D1_33,52)&un;
	D1_33 = _mm512_and_si512(D1_33,mask52);

	borrow = _mm512_add_epi64(tmp34,borrow);
	D1_34 = _mm512_sub_epi64(D1_34,borrow);
	borrow  = _mm512_srli_epi64(D1_34,52)&un;
	D1_34 = _mm512_and_si512(D1_34,mask52);

	borrow = _mm512_add_epi64(tmp35,borrow);
	D1_35 = _mm512_sub_epi64(D1_35,borrow);
	borrow  = _mm512_srli_epi64(D1_35,52)&un;
	D1_35 = _mm512_and_si512(D1_35,mask52);

	borrow = _mm512_add_epi64(tmp36,borrow);
	D1_36 = _mm512_sub_epi64(D1_36,borrow);
	borrow  = _mm512_srli_epi64(D1_36,52)&un;
	D1_36 = _mm512_and_si512(D1_36,mask52);

	borrow = _mm512_add_epi64(tmp37,borrow);
	D1_37 = _mm512_sub_epi64(D1_37,borrow);
	borrow  = _mm512_srli_epi64(D1_37,52)&un;
	D1_37 = _mm512_and_si512(D1_37,mask52);

	borrow = _mm512_add_epi64(tmp38,borrow);
	D1_38 = _mm512_sub_epi64(D1_38,borrow);
	borrow  = _mm512_srli_epi64(D1_38,52)&un;
	D1_38 = _mm512_and_si512(D1_38,mask52);

	borrow = _mm512_add_epi64(tmp39,borrow);
	D1_39 = _mm512_sub_epi64(D1_39,borrow);
	borrow = _mm512_srli_epi64(D1_39,51)&un;
	D1_39 = _mm512_and_si512(D1_39,mask51);// radix 2**1039


	// Final borrows

	borrow = _mm512_add_epi64(carry_tmp,borrow);
	D2_20 = _mm512_add_epi64(D2_20,carry);
	D2_20 = _mm512_sub_epi64(D2_20,borrow);
	borrow  = _mm512_srli_epi64(D2_20,52)&un;
	D2_20 = _mm512_and_si512(D2_20,mask52);

	D2_21 = _mm512_sub_epi64(D2_21,borrow);
	borrow  = _mm512_srli_epi64(D2_21,52)&un;
	D2_21 = _mm512_and_si512(D2_21,mask52);

	D2_22 = _mm512_sub_epi64(D2_22,borrow);
	borrow  = _mm512_srli_epi64(D2_22,52)&un;
	D2_22 = _mm512_and_si512(D2_22,mask52);

	D2_23 = _mm512_sub_epi64(D2_23,borrow);
	borrow  = _mm512_srli_epi64(D2_23,52)&un;
	D2_23 = _mm512_and_si512(D2_23,mask52);

	D2_24 = _mm512_sub_epi64(D2_24,borrow);
	borrow  = _mm512_srli_epi64(D2_24,52)&un;
	D2_24 = _mm512_and_si512(D2_24,mask52);

	D2_25 = _mm512_sub_epi64(D2_25,borrow);
	borrow  = _mm512_srli_epi64(D2_25,52)&un;
	D2_25 = _mm512_and_si512(D2_25,mask52);

	D2_26 = _mm512_sub_epi64(D2_26,borrow);
	borrow  = _mm512_srli_epi64(D2_26,52)&un;
	D2_26 = _mm512_and_si512(D2_26,mask52);

	D2_27 = _mm512_sub_epi64(D2_27,borrow);
	borrow  = _mm512_srli_epi64(D2_27,52)&un;
	D2_27 = _mm512_and_si512(D2_27,mask52);

	D2_28 = _mm512_sub_epi64(D2_28,borrow);
	borrow  = _mm512_srli_epi64(D2_28,52)&un;
	D2_28 = _mm512_and_si512(D2_28,mask52);

	D2_29 = _mm512_sub_epi64(D2_29,borrow);
	borrow  = _mm512_srli_epi64(D2_29,52)&un;
	D2_29 = _mm512_and_si512(D2_29,mask52);

	D2_30 = _mm512_sub_epi64(D2_30,borrow);
	borrow  = _mm512_srli_epi64(D2_30,52)&un;
	D2_30 = _mm512_and_si512(D2_30,mask52);

	D2_31 = _mm512_sub_epi64(D2_31,borrow);
	borrow  = _mm512_srli_epi64(D2_31,52)&un;
	D2_31 = _mm512_and_si512(D2_31,mask52);

	D2_32 = _mm512_sub_epi64(D2_32,borrow);
	borrow  = _mm512_srli_epi64(D2_32,52)&un;
	D2_32 = _mm512_and_si512(D2_32,mask52);

	D2_33 = _mm512_sub_epi64(D2_33,borrow);
	borrow  = _mm512_srli_epi64(D2_33,52)&un;
	D2_33 = _mm512_and_si512(D2_33,mask52);

	D2_34 = _mm512_sub_epi64(D2_34,borrow);
	borrow  = _mm512_srli_epi64(D2_34,52)&un;
	D2_34 = _mm512_and_si512(D2_34,mask52);

	D2_35 = _mm512_sub_epi64(D2_35,borrow);
	borrow  = _mm512_srli_epi64(D2_35,52)&un;
	D2_35 = _mm512_and_si512(D2_35,mask52);

	D2_36 = _mm512_sub_epi64(D2_36,borrow);
	borrow  = _mm512_srli_epi64(D2_36,52)&un;
	D2_36 = _mm512_and_si512(D2_36,mask52);

	D2_37 = _mm512_sub_epi64(D2_37,borrow);
	borrow  = _mm512_srli_epi64(D2_37,52)&un;
	D2_37 = _mm512_and_si512(D2_37,mask52);

	D2_38 = _mm512_sub_epi64(D2_38,borrow);
	borrow  = _mm512_srli_epi64(D2_38,52)&un;
	D2_38 = _mm512_and_si512(D2_38,mask52);

	D2_39 = _mm512_sub_epi64(D2_39,borrow);


	_mm512_store_epi64(out+0,D0_0);
	_mm512_store_epi64(out+1,D0_1);
	_mm512_store_epi64(out+2,D0_2);
	_mm512_store_epi64(out+3,D0_3);
	_mm512_store_epi64(out+4,D0_4);
	_mm512_store_epi64(out+5,D0_5);
	_mm512_store_epi64(out+6,D0_6);
	_mm512_store_epi64(out+7,D0_7);
	_mm512_store_epi64(out+8,D0_8);
	_mm512_store_epi64(out+9,D0_9);
	_mm512_store_epi64(out+10,D0_10);
	_mm512_store_epi64(out+11,D0_11);
	_mm512_store_epi64(out+12,D0_12);
	_mm512_store_epi64(out+13,D0_13);
	_mm512_store_epi64(out+14,D0_14);
	_mm512_store_epi64(out+15,D0_15);
	_mm512_store_epi64(out+16,D0_16);
	_mm512_store_epi64(out+17,D0_17);
	_mm512_store_epi64(out+18,D0_18);
	_mm512_store_epi64(out+19,D0_19);
	_mm512_store_epi64(out+20,D1_0);
	_mm512_store_epi64(out+21,D1_1);
	_mm512_store_epi64(out+22,D1_2);
	_mm512_store_epi64(out+23,D1_3);
	_mm512_store_epi64(out+24,D1_4);
	_mm512_store_epi64(out+25,D1_5);
	_mm512_store_epi64(out+26,D1_6);
	_mm512_store_epi64(out+27,D1_7);
	_mm512_store_epi64(out+28,D1_8);
	_mm512_store_epi64(out+29,D1_9);
	_mm512_store_epi64(out+30,D1_10);
	_mm512_store_epi64(out+31,D1_11);
	_mm512_store_epi64(out+32,D1_12);
	_mm512_store_epi64(out+33,D1_13);
	_mm512_store_epi64(out+34,D1_14);
	_mm512_store_epi64(out+35,D1_15);
	_mm512_store_epi64(out+36,D1_16);
	_mm512_store_epi64(out+37,D1_17);
	_mm512_store_epi64(out+38,D1_18);
	_mm512_store_epi64(out+39,D1_19);
	_mm512_store_epi64(out+40,D1_20);
	_mm512_store_epi64(out+41,D1_21);
	_mm512_store_epi64(out+42,D1_22);
	_mm512_store_epi64(out+43,D1_23);
	_mm512_store_epi64(out+44,D1_24);
	_mm512_store_epi64(out+45,D1_25);
	_mm512_store_epi64(out+46,D1_26);
	_mm512_store_epi64(out+47,D1_27);
	_mm512_store_epi64(out+48,D1_28);
	_mm512_store_epi64(out+49,D1_29);
	_mm512_store_epi64(out+50,D1_30);
	_mm512_store_epi64(out+51,D1_31);
	_mm512_store_epi64(out+52,D1_32);
	_mm512_store_epi64(out+53,D1_33);
	_mm512_store_epi64(out+54,D1_34);
	_mm512_store_epi64(out+55,D1_35);
	_mm512_store_epi64(out+56,D1_36);
	_mm512_store_epi64(out+57,D1_37);
	_mm512_store_epi64(out+58,D1_38);
	_mm512_store_epi64(out+59,D1_39);
	_mm512_store_epi64(out+60,D2_20);
	_mm512_store_epi64(out+61,D2_21);
	_mm512_store_epi64(out+62,D2_22);
	_mm512_store_epi64(out+63,D2_23);
	_mm512_store_epi64(out+64,D2_24);
	_mm512_store_epi64(out+65,D2_25);
	_mm512_store_epi64(out+66,D2_26);
	_mm512_store_epi64(out+67,D2_27);
	_mm512_store_epi64(out+68,D2_28);
	_mm512_store_epi64(out+69,D2_29);
	_mm512_store_epi64(out+70,D2_30);
	_mm512_store_epi64(out+71,D2_31);
	_mm512_store_epi64(out+72,D2_32);
	_mm512_store_epi64(out+73,D2_33);
	_mm512_store_epi64(out+74,D2_34);
	_mm512_store_epi64(out+75,D2_35);
	_mm512_store_epi64(out+76,D2_36);
	_mm512_store_epi64(out+77,D2_37);
	_mm512_store_epi64(out+78,D2_38);
	_mm512_store_epi64(out+79,D2_39);
}




#include "TruncatedK512_2078.c"


