...
/*	mpz_set_str(modul_p, "",10);
	mpz_set_str(p_prime, "",10);
	mpz_set_str(r2_gmp_512, "",10);//*/
...
int main(void){
	...
	//scratch_limbs = (mp_limb_t*) calloc (mpn_sec_mul_itch(nb_limbs,nb_limbs), sizeof(mp_limb_t));
	...
	for(int i=0;i<8;i++)
	{
		...
		displayVect((uint64_t*) am1_limbs_8[i],"am1_limbs_8(64) =",nb_limbs);
	...
	__m512i tmp[NB_COEFF*2];
	uint64_t tmp64[8][NB_COEFF64*2];
	mul512_8_2078K(tmp, a512, b512);
	fcontract_8_512_2078K_res( tmp64, tmp);
	
	for(int i=0;i<8;i++){
		mpz_mul(E8[i],A8[i],B8[i]);
		displayVect(tmp64[i],"tmp64[i]     ",NB_COEFF64*2);
		gmp_printf("TMP8[i] : %ZX\n",E8[i]);
		//displayVect((uint64_t*) am1_limbs_8[i],"am1_limbs_8(64) =",nb_limbs);
	}
	...
	mont_mul512_8_2078K(aMont512, a512, r2_512, n512, n_prime512);
	
	/*fcontract_8_512_2078K(res64, aMont512);
	for(int i=0;i<8;i++)
	{
	...
	mont_mul512_8_2078K(bMont512, b512, r2_512, n512, n_prime512);
	
	//displayVect512_NB_COEFF(aMont512,"aMont512");
	fcontract_8_512_2078K( c64, aMont512);
	...
	/*for(int i=0;i<8;i++)
	{
		...
		displayVect(c64[i],"aMont512",NB_COEFF64);
		displayVect((uint64_t*) am1_limbs_8[i],"am1_limbs_8(64) =",nb_limbs);
	}//*/
	...
	mont_mul512_8_2078K(resMont512, aMont512, bMont512, n512, n_prime512);
	
	// back to standard domain
	
	mont_mul512_8_2078K(res512, resMont512, un512, n512, n_prime512);
	...
	for(int i=0;i<NTEST;i++)
	{
		...
		mont_square512_8_2078K(resMont512, aMont512, n512, n_prime512);
	...
	*********************/
	...
	printf("\ngmp, BN, BN_MONT, mont_mul512_8_2078K, mont_square512_8_2078K\n");
	printf("-----------------------------------------------------------\n");
	...
	for(int k=0; k<NSAMPLES;k++){
		...
		for(int i=0;i<NTEST;i++)
		{
			mont_mul512_8_2078K(res512, a512, b512, n512, n_prime512);
		}
		...
		for(int i=0;i<NTEST;i++)
		{
			mont_square512_8_2078K(res512, a512, n512, n_prime512);
		}
	...
	printf("timer mont_square512_8_2078K       : %llu\n",timer6/(NSAMPLES*10));
	...
	*********************/
	...
	printf("mont_mul512_8_2078K          : %llu\n",timer2/(NSAMPLES*10));
	
	printf("mont_square512_8_2078K       : %llu\n",timer6/(NSAMPLES*10));
