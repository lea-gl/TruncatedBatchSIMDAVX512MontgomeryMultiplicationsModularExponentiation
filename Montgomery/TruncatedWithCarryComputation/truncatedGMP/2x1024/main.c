...
void MontMulGMP(uint64_t out[NB_COEFF64], uint64_t a[NB_COEFF64], uint64_t b[NB_COEFF64], uint64_t N[NB_COEFF64], uint64_t N_prime[NB_COEFF64]);

void MontMulGMP_truncated(uint64_t out[NB_COEFF64], uint64_t a[NB_COEFF64], uint64_t b[NB_COEFF64], uint64_t N[NB_COEFF64], uint64_t N_prime[NB_COEFF64]);
...
/*   Montgomery parameters !*/
	...
	//mpz_invert(,
	//mpz_set_str(p_prime, "",10);
...
int main(void){
	...
	mpz_invert(C,modul_p,UPB);
	
	mpz_neg(p_prime,C);
	mpz_mod(p_prime,p_prime,UPB);
	...
	uint64_t upb64[	NB_COEFF64], p64[NB_COEFF64], p_prime64[NB_COEFF64], r2_64[NB_COEFF64], un64[NB_COEFF64];
	...
	mpz_export(upb64, &countp, -1, sizeof(upb64[0]) ,0,0, UPB);

	mpz_export(p64, &countp, -1, sizeof(upb64[0]) ,0,0, modul_p);
	mpz_export(p_prime64, &countp, -1, sizeof(upb64[0]) ,0,0, p_prime);
	
	//mpz_export(r2_64, &countp, -1, sizeof(upb64[0]) ,0,0, r2_gmp_512);

	mpz_export(r2_64, &countp, -1, sizeof(upb64[0]) ,0,0, R2);

	copy_limbs(am1_limbs, A, nb_limbs);
	copy_limbs(bm1_limbs, B, nb_limbs);
	//~ conversion to Montgomery domain (block mont)
	MontMulGMP(am1_limbs,am1_limbs,r2_64, p64, p_prime64);
	MontMulGMP(bm1_limbs,bm1_limbs,r2_64, p64, p_prime64);
	//MontMulGMP(am1_limbs,am1_limbs,r2_limbs, p_limbs, mip_limbs);
	//MontMulGMP(bm1_limbs,bm1_limbs,r2_limbs, p_limbs, mip_limbs);
	from_limbs_to_mpz_t(G, am1_limbs, nb_limbs);
	gmp_printf("MontMulGMP : %ZX\n", G	);
	from_limbs_to_mpz_t(G, bm1_limbs, nb_limbs);
	gmp_printf("MontMulGMP : %ZX\n", G	);
	copy_limbs(am1_limbs, A, nb_limbs);
	copy_limbs(bm1_limbs, B, nb_limbs);
	mpn_mont_mul_red_n(am1_limbs, am1_limbs, r2_limbs, p_limbs, mip_limbs, nb_limbs);
	mpn_mont_mul_red_n(bm1_limbs, bm1_limbs, r2_limbs, p_limbs, mip_limbs, nb_limbs);
	from_limbs_to_mpz_t(G, am1_limbs, nb_limbs);
	gmp_printf("mpn_mont_mul_red_n : %ZX\n", G	);
	from_limbs_to_mpz_t(G, bm1_limbs, nb_limbs);
	gmp_printf("mpn_mont_mul_red_n : %ZX\n", G	);
	//getchar();
	

	MontMulGMP(am2_limbs,am1_limbs,bm1_limbs, p64, p_prime64);
	from_limbs_to_mpz_t(G, am2_limbs, nb_limbs);
	gmp_printf("MontMulGMP : %ZX\n", G	);
	
	
	mpn_mont_mul_red_n(bm2_limbs, am1_limbs, bm1_limbs, p_limbs, mip_limbs, nb_limbs);
	from_limbs_to_mpz_t(G, bm2_limbs, nb_limbs);
	gmp_printf("MontMulGMP : %ZX\n", G	);
	

	clean_limbs(scratch_limbs, 2*nb_limbs);
	for(int i=0; i<nb_limbs; i++)
		scratch_limbs[i] = bm2_limbs[i];
	mpn_redc_n (bm2_limbs, scratch_limbs, p_limbs, nb_limbs, mip_limbs);
	from_limbs_to_mpz_t(G, bm2_limbs, nb_limbs);
	gmp_printf("MontMulGMP : %ZX\n", G	);
	...
	for(int i=0; i<nb_limbs; i++)
		scratch_limbs[i] = am2_limbs[i];
	//mpn_redc_n (am2_limbs, scratch_limbs, p_limbs, nb_limbs, mip_limbs);
	MontMulGMP(am2_limbs,scratch_limbs,un64, p64, p_prime64);
	from_limbs_to_mpz_t(G, am2_limbs, nb_limbs);
	gmp_printf("MontMulGMP : %ZX\n", G	);
	
	
	//MontMulGMP(am2_limbs,scratch_limbs,un64, p_limbs, mip_limbs);
	...
	copy_limbs(bm1_limbs, B, nb_limbs);
	//~ conversion to Montgomery domain (block mont)
	//MontMulGMP_truncated(am1_limbs,am1_limbs,r2_64, p64, p_prime64);
	//MontMulGMP_truncated(bm1_limbs,bm1_limbs,r2_64, p64, p_prime64);
	
	MontMulGMP_truncated(am1_limbs,am1_limbs,r2_limbs, p_limbs, mip_limbs);
	MontMulGMP_truncated(bm1_limbs,bm1_limbs,r2_limbs, p_limbs, mip_limbs);
	
	//mpn_mont_mul_red_n(am1_limbs, am1_limbs, r2_limbs, p_limbs, mip_limbs, nb_limbs);
	//mpn_mont_mul_red_n(bm1_limbs, bm1_limbs, r2_limbs, p_limbs, mip_limbs, nb_limbs);
	

	MontMulGMP_truncated(am2_limbs,am1_limbs,bm1_limbs, p64, p_prime64);
	//mpn_mont_mul_red_n(am2_limbs, am1_limbs, bm1_limbs, p_limbs, mip_limbs, nb_limbs);

	clean_limbs(scratch_limbs, 2*nb_limbs);
	for(int i=0; i<nb_limbs; i++)
		scratch_limbs[i] = am2_limbs[i];
	//mpn_redc_n (am2_limbs, scratch_limbs, p_limbs, nb_limbs, mip_limbs);
	MontMulGMP_truncated(am2_limbs,scratch_limbs,un64, p_limbs,mip_limbs);

	from_limbs_to_mpz_t(G, am2_limbs, nb_limbs);
	gmp_printf("MontMulGMP_truncated : %ZX\n", G	);
	...
	mp_limb_t a_limbs_8[8][nb_limbs], b_limbs_8[8][nb_limbs], am1_limbs_8[8][nb_limbs], am2_limbs_8[8][nb_limbs], bm1_limbs_8[8][nb_limbs], bm2_limbs_8[8][nb_limbs], res_limbs_8[8][nb_limbs];
	...
	for(int i=0;i<8;i++)
	{
		...
		//~ conversion to Montgomery domain (block mont)
		mpn_mont_mul_red_1(am1_limbs_8[i], a_limbs_8[i], r2_limbs, p_limbs, mip0, nb_limbs);
		mpn_mont_mul_red_1(bm1_limbs_8[i], b_limbs_8[i], r2_limbs, p_limbs, mip0, nb_limbs);
		...
		//~ conversion to Montgomery domain (block mont)
		//MontMulGMP(a64[i],a64[i],r2_64, p64, p_prime64);
		//MontMulGMP(b64[i],b64[i],r2_64, p64, p_prime64);
		
		MontMulGMP(a64[i],a64[i],r2_limbs, p_limbs,mip_limbs);
		MontMulGMP(b64[i],b64[i],r2_limbs, p_limbs,mip_limbs);
	...
	for(int i=0;i<8;i++)
	{
		...
		mpn_mont_mul_red_1(res_limbs_8[i], am1_limbs_8[i], bm1_limbs_8[i], p_limbs, mip0, nb_limbs);
		clean_limbs(scratch_limbs, 2*nb_limbs);
		for(int j=0; j<nb_limbs; j++)
			scratch_limbs[j] = res_limbs_8[i][j];
		mpn_redc_1 (res_limbs_8[i], scratch_limbs, p_limbs, nb_limbs, mip0);
		from_limbs_to_mpz_t(F, res_limbs_8[i], nb_limbs);
		gmp_printf("r_mbgmp : %ZX\n", F);//*/


		//~ Montgomery modular multiplication (classic mont)
		mpn_mont_mul_red_n(res_limbs_8[i], am1_limbs_8[i], bm1_limbs_8[i], p_limbs, mip_limbs, nb_limbs);
		
		clean_limbs(scratch_limbs, 2*nb_limbs);
		for(int j=0; j<nb_limbs; j++)
			scratch_limbs[j] = res_limbs_8[i][j];
		mpn_redc_n (res_limbs_8[i], scratch_limbs, p_limbs, nb_limbs, mip_limbs);
		from_limbs_to_mpz_t(H, res_limbs_8[i], nb_limbs);
		gmp_printf("r_mcgmp : %ZX\n", H);
		
		
		//~ Montgomery low level modular multiplication
		mpn_mod_mult(res_limbs_8[i], a_limbs_8[i], b_limbs_8[i], p_limbs, nb_limbs);
		from_limbs_to_mpz_t(F, res_limbs_8[i], nb_limbs);
		gmp_printf("r_lgmp  : %ZX\n", F);

		//~ Montgomery low level modular multiplication
		MontMulGMP(c64[i],a64[i],b64[i], p_limbs,mip_limbs);
		clean_limbs(scratch_limbs, 2*nb_limbs);
		for(int j=0; j<nb_limbs; j++)
			scratch_limbs[j] = c64[i][j];
		//mpn_redc_n (am2_limbs, scratch_limbs, p_limbs, nb_limbs, mip_limbs);
		MontMulGMP(res64[i],scratch_limbs,un64, p_limbs,mip_limbs);

		//from_limbs_to_mpz_t(G, res_limbs_8[i], nb_limbs);
		//mpz_export(res64[i], &countp, -1, sizeof(b64[0][0]) ,0,0, G);

		//~ Montgomery low level modular multiplication
		/*MontMulGMP_truncated(c64[i],a64[i],b64[i], p_limbs,mip_limbs);
		clean_limbs(scratch_limbs, 2*nb_limbs);
		for(int j=0; j<nb_limbs; j++)
			scratch_limbs[j] = c64[i][j];
		//mpn_redc_n (am2_limbs, scratch_limbs, p_limbs, nb_limbs, mip_limbs);
		MontMulGMP_truncated(res64[i],scratch_limbs,un64, p_limbs,mip_limbs);//*/

		
		
		mpz_export(res_gmp, &countp, -1, sizeof(b64[0][0]) ,0,0, F);
	...
	for(int i=0;i<NTEST;i++)
	{
		...
		for(int i=0;i<8;i++)
		{
			...
			//~ Montgomery low level modular multiplication
			MontMulGMP(c64[i],a64[i],b64[i], p64, p_prime64);
			clean_limbs(scratch_limbs, 2*nb_limbs);
			for(int j=0; j<nb_limbs; j++)
				scratch_limbs[j] = c64[i][j];
			//mpn_redc_n (am2_limbs, scratch_limbs, p_limbs, nb_limbs, mip_limbs);
			MontMulGMP(res64[i],scratch_limbs,un64, p64, p_prime64);

			//from_limbs_to_mpz_t(G, res_limbs_8[i], nb_limbs);
			//mpz_export(res64[i], &countp, -1, sizeof(b64[0][0]) ,0,0, G);

			//~ Montgomery low level modular multiplication
			MontMulGMP_truncated(c64[i],a64[i],b64[i], p64, p_prime64);
			clean_limbs(scratch_limbs, 2*nb_limbs);
			for(int j=0; j<nb_limbs; j++)
				scratch_limbs[j] = c64[i][j];
			//mpn_redc_n (am2_limbs, scratch_limbs, p_limbs, nb_limbs, mip_limbs);
			MontMulGMP_truncated(res64[i],scratch_limbs,un64, p64, p_prime64);
	...
	*********************/
	...
	printf("\ngmp, BN, BN_MONT, mont_mul512_8_1040, mont_square512_8_1040\n");
	printf("-----------------------------------------------------------\n");
	...
	for(int k=0; k<NSAMPLES;k++){
		...
		for(int i=0;i<NTEST;i++)
		{
			MontMulGMP(a_limbs,am1_limbs,bm1_limbs, upb64, p_prime64);
		}
		...
		for(int i=0;i<NTEST;i++)
		{
			MontMulGMP_truncated(a_limbs,am1_limbs,bm1_limbs, p64, p_prime64);
		}
	...
	printf("timer gmp mpn_mont_mul_red_1  : %llu\n",timer/(NSAMPLES*10));	
	
	printf("timer gmp mpn_mont_mul_red_n  : %llu\n",timer1/(NSAMPLES*10));	
	
	printf("timer gmp mpn_mod_mult        : %llu\n",timer5/(NSAMPLES*10));
	
	printf("timer BN_mod_mul              : %llu\n",timer3/(NSAMPLES*10));

	printf("timer BN_mod_mul_montgomery   : %llu\n",timer4/(NSAMPLES*10));

	printf("timer MontMulGMP              : %llu\n",timer2/(NSAMPLES*10));

	printf("timer MontMulGMP_truncated    : %llu\n",timer6/(NSAMPLES*10));
	...
	*********************/
	...
	for(int k=0; k<NSAMPLES;k++){
		...
		for(int i=0;i<NTEST;i++)
		{
			MontMulGMP(a_limbs,am1_limbs,bm1_limbs, p64, p_prime64);
		}
	...
	printf("gmp mpn_mont_mul_red_1  : %llu\n",timer/(NSAMPLES*10));	
	
	printf("gmp mpn_mont_mul_red_n  : %llu\n",timer1/(NSAMPLES*10));	
	
	printf("gmp mpn_mod_mult        : %llu\n",timer5/(NSAMPLES*10));
	
	printf("BN_mod_mul              : %llu\n",timer3/(NSAMPLES*10));

	printf("BN_mod_mul_montgomery   : %llu\n",timer4/(NSAMPLES*10));

	printf("MontMulGMP              : %llu\n",timer2/(NSAMPLES*10));
	
	printf("MontMulGMP_truncatedd   : %llu\n",timer6/(NSAMPLES*10));
