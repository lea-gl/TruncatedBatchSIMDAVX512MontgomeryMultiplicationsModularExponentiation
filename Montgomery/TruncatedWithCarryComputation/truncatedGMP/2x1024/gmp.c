...
static inline void force_inline mulmodMGMP (uint64_t out[NB_COEFF64], uint64_t a[NB_COEFF64], uint64_t b[NB_COEFF64] )
{
	...
	//static const uint64_t mask = 0xffffUL;
...
static inline void force_inline fmaGMP (uint64_t out[NB_COEFF64], uint64_t a[NB_COEFF64], uint64_t b[NB_COEFF64], uint64_t c[NB_COEFF64<<1])
{

	//uint64_t t[NB_COEFF64<<1], tmp[NB_COEFF64<<1];
	...
	/*for(int i=0;i<NB_COEFF64;i++) tmp[i] = b[i];
	for(int i=NB_COEFF64;i<NB_COEFF64<<1;i++) tmp[i] = 0UL;//*/
	...
	for(int i=0;i<NB_COEFF64-1;i++)
	{
		...
		tmp = mpn_addmul_1(t+i,b,NB_COEFF64,a[i]);//addition with carry !!! see GMP doc p.55
		
		uint128_t tmp2 = (uint128_t)t[i+NB_COEFF64] + (uint128_t)tmp;
		
		t[i+NB_COEFF64] = (uint64_t) tmp2;
		
		t[i+1+NB_COEFF64] += (uint64_t) (tmp2>>64);
	...
	t[(NB_COEFF64<<1)-1] += mpn_addmul_1(t+NB_COEFF64-1,b,NB_COEFF64,a[NB_COEFF64-1]);
...
inline void MontMulGMP(uint64_t out[NB_COEFF64], uint64_t a[NB_COEFF64], uint64_t b[NB_COEFF64], uint64_t N[NB_COEFF64], uint64_t N_prime[NB_COEFF64]){
	...
	mpn_add_n(tn,t,qn,NB_COEFF64<<1);// T <- T + qN
	displayVect(tn,"tn",NB_COEFF64<<1);
	...
	for(int i=0;i<NB_COEFF64;i++) out[i] = t[i+NB_COEFF64];//(t[i+NB_COEFF64]<<(64-DEC))| (t[i+NB_COEFF64-1]>>DEC);
...
inline void MontMulGMP_truncated(uint64_t out[NB_COEFF64], uint64_t a[NB_COEFF64], uint64_t b[NB_COEFF64], uint64_t N[NB_COEFF64], uint64_t N_prime[NB_COEFF64]){
	...
	static const uint64_t mask = 0xffffffffffff0000UL;
