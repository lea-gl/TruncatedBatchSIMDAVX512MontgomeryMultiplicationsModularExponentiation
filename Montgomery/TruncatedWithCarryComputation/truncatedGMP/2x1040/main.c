...
void MontMulGMP(uint64_t out[NB_COEFF64], uint64_t a[NB_COEFF64], uint64_t b[NB_COEFF64], uint64_t r[NB_COEFF64], uint64_t N[NB_COEFF64], uint64_t N_prime[NB_COEFF64]);

void MontMulGMP_truncated(uint64_t out[NB_COEFF64], uint64_t a[NB_COEFF64], uint64_t b[NB_COEFF64], uint64_t r[NB_COEFF64], uint64_t N[NB_COEFF64], uint64_t N_prime[NB_COEFF64]);
...
int main(void){
	...
	copy_limbs(bm1_limbs, B, nb_limbs);
	//~ conversion to Montgomery domain (block mont)
	MontMulGMP(am1_limbs,am1_limbs,r2_64, upb64, p64, p_prime64);
	MontMulGMP(bm1_limbs,bm1_limbs,r2_64, upb64, p64, p_prime64);
	...
	MontMulGMP(am2_limbs,am1_limbs,bm1_limbs, upb64, p64, p_prime64);
	...
	for(int i=0; i<nb_limbs; i++)
		scratch_limbs[i] = am2_limbs[i];
	//mpn_redc_n (am2_limbs, scratch_limbs, p_limbs, nb_limbs, mip_limbs);
	MontMulGMP(am2_limbs,scratch_limbs,un64, upb64, p64, p_prime64);

	from_limbs_to_mpz_t(G, am2_limbs, nb_limbs);
	gmp_printf("MontMulGMP : %Zd\n", G	);
	//getchar();
	//goto end;
	...
	copy_limbs(bm1_limbs, B, nb_limbs);
	//~ conversion to Montgomery domain (block mont)
	MontMulGMP_truncated(am1_limbs,am1_limbs,r2_64, upb64, p64, p_prime64);
	MontMulGMP_truncated(bm1_limbs,bm1_limbs,r2_64, upb64, p64, p_prime64);
	//mpn_mont_mul_red_n(am1_limbs, am1_limbs, r2_limbs, p_limbs, mip_limbs, nb_limbs);
	//mpn_mont_mul_red_n(bm1_limbs, bm1_limbs, r2_limbs, p_limbs, mip_limbs, nb_limbs);
	

	MontMulGMP_truncated(am2_limbs,am1_limbs,bm1_limbs, upb64, p64, p_prime64);
	//mpn_mont_mul_red_n(am2_limbs, am1_limbs, bm1_limbs, p_limbs, mip_limbs, nb_limbs);
	...
	for(int i=0; i<nb_limbs; i++)
		scratch_limbs[i] = am2_limbs[i];
	//mpn_redc_n (am2_limbs, scratch_limbs, p_limbs, nb_limbs, mip_limbs);
	MontMulGMP_truncated(am2_limbs,scratch_limbs,un64, upb64, p64, p_prime64);

	from_limbs_to_mpz_t(G, am2_limbs, nb_limbs);
	gmp_printf("MontMulGMP_truncated : %Zd\n", G	);
	...
	for(int i=0;i<8;i++)
	{
		...
		//~ conversion to Montgomery domain (block mont)
		MontMulGMP(a64[i],a64[i],r2_64, upb64, p64, p_prime64);
		MontMulGMP(b64[i],b64[i],r2_64, upb64, p64, p_prime64);
	}
	...
	for(int i=0;i<8;i++)
	{
		...
		//~ Montgomery low level modular multiplication
		MontMulGMP(c64[i],a64[i],b64[i], upb64, p64, p_prime64);
		clean_limbs(scratch_limbs, 2*nb_limbs);
		for(int j=0; j<nb_limbs; j++)
			scratch_limbs[j] = c64[i][j];
		//mpn_redc_n (am2_limbs, scratch_limbs, p_limbs, nb_limbs, mip_limbs);
		MontMulGMP(res64[i],scratch_limbs,un64, upb64, p64, p_prime64);

		//from_limbs_to_mpz_t(G, res_limbs_8[i], nb_limbs);
		//mpz_export(res64[i], &countp, -1, sizeof(b64[0][0]) ,0,0, G);

		//~ Montgomery low level modular multiplication
		MontMulGMP_truncated(c64[i],a64[i],b64[i], upb64, p64, p_prime64);
		clean_limbs(scratch_limbs, 2*nb_limbs);
		for(int j=0; j<nb_limbs; j++)
			scratch_limbs[j] = c64[i][j];
		//mpn_redc_n (am2_limbs, scratch_limbs, p_limbs, nb_limbs, mip_limbs);
		MontMulGMP_truncated(res64[i],scratch_limbs,un64, upb64, p64, p_prime64);
	...
	*********************/
	...
	for(int k=0; k<NSAMPLES;k++){
		...
		for(int i=0;i<NTEST;i++)
		{
			MontMulGMP(a_limbs,am1_limbs,bm1_limbs, upb64, p64, p_prime64);
		}
		...
		for(int i=0;i<NTEST;i++)
		{
			MontMulGMP_truncated(a_limbs,am1_limbs,bm1_limbs, upb64, p64, p_prime64);
		}
